...
#[test]
fn emoji_profile_manifest_is_consistent() {
    let profile = default_profile();

    assert_eq!(profile.name, "emoji");
    assert_eq!(profile.expected_family_names, &["Noto Color Emoji"]);
    assert_eq!(profile.provided_roles, &[BundledFontRole::EmojiFallback]);
    assert_eq!(profile.common_fallback_families, &["Noto Color Emoji"]);
    assert!(profile.supports_role(BundledFontRole::EmojiFallback));
    assert_eq!(profile.faces.len(), 1);
}

#[test]
fn emoji_face_asset_identity_uses_extension_bundle() {
    ...
    let resolved = resolver
        ...
        .expect("expected emoji bundled face to resolve");
