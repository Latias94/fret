...
fn parse_stats_command(
    ...
    Ok(MigratedDiagCommand::Stats(
        crate::diag_stats::StatsCmdContext {
            ...
            stats_perf_threshold_keys_json: args.perf_threshold_keys_json,
...
fn parse_query_command(
    ...
    let (rest, query_out, warmup_frames, stats_json) = match args.command {
        ...
        QuerySubcommandArgs::IdentityWarnings(identity) => {
            ...
            if let Some(html_check_out) = identity.html_check_out {
                rest.push("--html-check-out".to_string());
                rest.push(html_check_out.display().to_string());
            }
            (
                rest,
                identity.output.out,
...
mod tests {
    ...
    fn migrated_query_identity_warnings_builds_a_real_context() {
        ...
        let args = vec![
            ...
            "target/check.identity_browser_html.json".to_string(),
            "--json".to_string(),
            "--out".to_string(),
