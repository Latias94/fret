...
pub(crate) fn cmd_meta(
    ...
    fn try_read_bundle_meta_json(path: &Path, warmup_frames: u64) -> Option<serde_json::Value> {
        let bytes = std::fs::read(path).ok()?;
        let v: serde_json::Value = serde_json::from_slice(&bytes).ok()?;
        if v.get("kind").and_then(|v| v.as_str()) != Some("bundle_meta") {
            return None;
        }
    ...
    {
        if try_read_bundle_meta_json(&src, warmup_frames).is_some() {
            (src.clone(), src.clone())
        } else {
        ...
        if direct.is_file() && try_read_bundle_meta_json(&direct, warmup_frames).is_some() {
            (direct.clone(), direct)
        } else {
