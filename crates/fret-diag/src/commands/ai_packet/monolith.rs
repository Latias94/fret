...
// Legacy monolith copy kept during the fearless refactor so we can diff behavior while splitting
// `diag ai-packet` into smaller modules. Not referenced by `commands::mod.rs`.
