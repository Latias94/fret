...
impl ParleyShaper {
    ...
    fn record_registered_font_blob_bytes_for_tests(&mut self, bytes: Vec<u8>) {
        self.font_db
            .record_registered_font_blob_bytes_for_tests(bytes);
    }

    #[cfg(test)]
    fn registered_font_blob_lengths_for_tests(&self) -> Vec<usize> {
        self.font_db.registered_font_blob_lengths_for_tests()
    }

    #[cfg(test)]
    fn registered_font_blob_total_bytes_for_tests(&self) -> usize {
        self.font_db.registered_font_blob_total_bytes_for_tests()
    }

    pub fn system_fonts_enabled(&self) -> bool {
        self.font_db.system_fonts_enabled()
    }
    ...
    fn disable_system_fonts(&mut self) {
        ...
        self.font_db.disable_system_fonts();
    }
    ...
    pub fn all_font_names(&mut self) -> Vec<String> {
        self.font_db.all_font_names(&mut self.fcx)
    }

    pub fn all_font_catalog_entries(&mut self) -> Vec<FontCatalogEntryMetadata> {
        self.font_db.all_font_catalog_entries(&mut self.fcx)
    }
    ...
    pub fn set_generic_family_ids(&mut self, generic: GenericFamily, ids: &[FamilyId]) -> bool {
        self.font_db
            .set_generic_family_ids(&mut self.fcx, generic, ids)
    }
    ...
    pub fn system_font_rescan_seed(&self) -> Option<crate::SystemFontRescanSeed> {
        self.font_db.system_font_rescan_seed()
    }
    ...
    pub fn apply_system_font_rescan_result(
        ...
        self.font_db
            .apply_system_font_rescan_result(&mut self.fcx, result)
    }

    #[cfg(test)]
    fn current_font_environment_fingerprint(&mut self) -> u64 {
        self.font_db
            .current_font_environment_fingerprint(&mut self.fcx)
    }

    fn base_line_metrics_cache_key(&self, style: &TextStyle, scale: f32) -> u64 {
        self.font_db.base_line_metrics_cache_key(
    ...
    fn base_ascent_descent_px_for_style(
        ...
        if let Some(hit) = self.font_db.base_line_metrics(key) {
            return Some(hit);
        }
        ...
        self.font_db
            .insert_base_line_metrics(key, (ascent, descent));
        Some((ascent, descent))
    }
...
pub fn run_system_font_rescan(seed: crate::SystemFontRescanSeed) -> crate::SystemFontRescanResult {
    crate::parley_font_db::run_system_font_rescan(seed)
}
