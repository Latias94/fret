...
impl<D: WinitAppDriver> WinitRunner<D> {
    pub fn new(config: WinitRunnerConfig, app: App, driver: D) -> Self {
        ...
        let mut runner = Self {
            ...
            diag_wheel_burst_inject: super::diag_wheel_burst_inject::DiagWheelBurstInject::from_env(
            ),
...
pub struct WinitAppBuilder<D> {
    ...
    asset_reload_policy: Option<crate::assets::AssetReloadPolicy>,
    asset_reload_targets: Vec<crate::assets::AssetReloadTarget>,
}
...
impl<D: super::WinitAppDriver + 'static> WinitAppBuilder<D> {
    pub fn new(app: App, driver: D) -> Self {
        Self {
            ...
            event_loop: None,
            asset_reload_policy: None,
            asset_reload_targets: Vec::new(),
        }
    }
    ...
    fn mount_asset_manifest(self, manifest_path: std::path::PathBuf) -> Result<Self, RunnerError> {
        ...
        let mut builder = self.init_app(move |app| {
            crate::assets::register_resolver(app, resolver);
        });
        builder
            .asset_reload_targets
            .push(crate::assets::AssetReloadTarget::Manifest {
                path: manifest_path,
            });
        Ok(builder)
    }
    ...
    fn mount_asset_dir(
        ...
        builder
            .asset_reload_targets
            .push(crate::assets::AssetReloadTarget::Dir { path: dir });
        Ok(builder)
    }
    ...
    pub fn run(self) -> Result<(), RunnerError> {
        ...
        let driver = HookedDriver::new(driver, on_main_window_created, on_gpu_ready);
