...
use platform_prefs::read_desktop_environment_snapshot;
use render::validate_scene_if_enabled;
use streaming_images::{StreamingImageUpdateNv12, StreamingImageUpdateRgba8, UploadedImageEntry};
use window::{
    DockTearoffFollow, MonitorRectF64, PendingFrontRequest, RectF64, TimerEntry, WindowRuntime,
...
impl<D: WinitAppDriver> WinitRunner<D> {
    ...
    fn insert_window(
        ...
        self.maybe_deliver_startup_incoming_open(id);
        Ok(id)
    }
        ...
        for _ in 0..MAX_EFFECT_DRAIN_TURNS {
            ...
            if self.config.streaming_update_ack_enabled {
                for ack in acks {
                    ...
                    match ack.kind {
                        super::super::streaming_upload::StreamingUploadAckKind::Dropped(reason) => {
                            self.deliver_window_event_now(
                                window,
                                &Event::ImageUpdateDropped {
                                    token: ack.token,
            ...
            for effect in effects {
                match effect {
                    ...
                    Effect::ImeRequestVirtualKeyboard { window: _, visible } => {
                        #[cfg(target_os = "android")]
