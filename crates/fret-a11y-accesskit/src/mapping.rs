...
pub fn tree_update_from_snapshot(snapshot: &SemanticsSnapshot, scale_factor: f64) -> TreeUpdate {
    ...
    for node in &snapshot.nodes {
        ...
        if let Some(state_description) = node.extra.state_description.as_ref() {
            out.set_state_description(state_description.clone());
        }
...
mod tests {
    ...
    fn maps_state_description() {
        ...
        let node = SemanticsNode {
            ...
            value: None,
            extra: SemanticsNodeExtra {
                state_description: Some("Suggestions below".to_string()),
                ..SemanticsNodeExtra::default()
            },
            text_selection: None,
            text_composition: None,
        ...
        assert_eq!(node.state_description(), Some("Suggestions below"));
    }
}
