...
pub(super) struct GpuPipelines {
    ...
    pub(super) vertex_color_pipeline_format: Option<wgpu::TextureFormat>,
    pub(super) vertex_color_pipeline: Option<wgpu::RenderPipeline>,
...
impl Default for GpuPipelines {
    fn default() -> Self {
        Self {
            ...
            vertex_color_pipeline_format: None,
...
impl GpuPipelines {
    ...
    pub(super) fn ensure_vertex_color_pipeline(
        &mut self,
        device: &wgpu::Device,
        globals: &GpuGlobals,
        format: wgpu::TextureFormat,
    ) {
        if self.vertex_color_pipeline_format == Some(format) && self.vertex_color_pipeline.is_some()
        {
            return;
        }
        ...
        let create_span = tracing::enabled!(tracing::Level::TRACE)
            .then(|| {
                let reason = if self.vertex_color_pipeline.is_none() {
                    "missing"
                } else {
                    "format_changed"
                };
                tracing::trace_span!(
                    "fret.renderer.pipeline.create.vertex_color",
                    format = ?format,
                    reason
                )
            })
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("fret vertex color shader"),
            source: wgpu::ShaderSource::Wgsl(super::shaders::VIEWPORT_SHADER.into()),
        });

        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("fret vertex color pipeline layout"),
            bind_group_layouts: &[Some(&globals.uniform_bind_group_layout)],
            immediate_size: 0,
        });

        let vertex_stride = std::mem::size_of::<super::ViewportVertex>() as wgpu::BufferAddress;
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("fret vertex color pipeline"),
            layout: Some(&pipeline_layout),
            vertex: wgpu::VertexState {
                module: &shader,
                entry_point: Some("vs_main"),
            ...
            fragment: Some(wgpu::FragmentState {
                module: &shader,
                entry_point: Some("fs_vertex_color"),
                targets: &[Some(wgpu::ColorTargetState {
                    format,
                    blend: Some(wgpu::BlendState::PREMULTIPLIED_ALPHA_BLENDING),
                    write_mask: wgpu::ColorWrites::ALL,
        ...
        self.vertex_color_pipeline_format = Some(format);
        self.vertex_color_pipeline = Some(pipeline);
    }

    pub(super) fn vertex_color_pipeline_ref(&self) -> Option<&wgpu::RenderPipeline> {
        self.vertex_color_pipeline.as_ref()
    }
}
...
impl Renderer {
    ...
    pub(in crate::renderer) fn ensure_vertex_color_pipeline(
        &mut self,
        device: &wgpu::Device,
        format: wgpu::TextureFormat,
    ) {
        self.pipelines
            .ensure_vertex_color_pipeline(device, &self.globals, format);
    }

    pub(in crate::renderer) fn vertex_color_pipeline_ref(&self) -> &wgpu::RenderPipeline {
        self.pipelines
            .vertex_color_pipeline_ref()
            .expect("vertex color pipeline must exist")
    }
}
