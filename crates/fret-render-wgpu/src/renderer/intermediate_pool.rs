...
impl IntermediatePool {
    ...
    pub(super) fn enforce_budget(&mut self, budget_bytes: u64) {
        ...
        if self.free_bytes <= budget_bytes {
            return;
        }
        ...
        while self.free_bytes > budget_bytes {
            ...
            let evict_span = if trace_enabled {
                tracing::trace_span!(
                    "fret.renderer.pool.evict",
                    w = victim_key.size.0,
                    h = victim_key.size.1,
                    bytes = victim_bytes,
                    format = ?victim_key.format,
                    usage_bits = victim_key.usage.bits(),
                    sample_count = victim_key.sample_count,
                )
            } else {
                tracing::Span::none()
            };
            let _evict_guard = evict_span.enter();
