...
use super::super::*;
use super::executor_lifecycle::render_plan_pass_written_target;
use super::recorders::{
    ...
    pub(super) fn record_pass(
        ...
        match planned_pass {
            ...
            RenderPlanPass::ReleaseTarget(target) => {
                self.lifecycle().release_plan_target(*target);
            }
        }
        if let Some(target) = render_plan_pass_written_target(planned_pass) {
            self.lifecycle().note_plan_target_written(target);
        }
    }
}
