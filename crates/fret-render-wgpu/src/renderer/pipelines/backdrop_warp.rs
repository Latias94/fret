...
impl Renderer {
    pub(in crate::renderer) fn ensure_backdrop_warp_pipeline(
        ...
        if self.backdrop_warp_pipeline_format == Some(format)
            && self.backdrop_warp_pipeline.is_some()
            && self.backdrop_warp_masked_pipeline.is_some()
        ...
        let create_span = tracing::enabled!(tracing::Level::TRACE)
            .then(|| {
                let reason = if self.backdrop_warp_pipeline_format != Some(format) {
                    "format_changed"
                } else {
                    "missing"
                };
