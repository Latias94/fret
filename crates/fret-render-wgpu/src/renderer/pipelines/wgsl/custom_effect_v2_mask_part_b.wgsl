@fragment
fn fs_main(@builtin(position) pos: vec4<f32>) -> @location(0) vec4<f32> {
  let dims_u = textureDimensions(src_texture);
  if (dims_u.x == 0u || dims_u.y == 0u) {
    return vec4<f32>(0.0);
  }

  let x = i32(floor(pos.x));
  let y = i32(floor(pos.y));
  let max_x = i32(dims_u.x) - 1;
  let max_y = i32(dims_u.y) - 1;
  let in_bounds = x >= 0 && y >= 0 && x <= max_x && y <= max_y;
  let x0 = clamp(x, 0, max_x);
  let y0 = clamp(y, 0, max_y);

  let tex = textureLoad(src_texture, vec2<i32>(x0, y0), 0);
  let dims = vec2<f32>(f32(dims_u.x), f32(dims_u.y));
  // Keep `pos_px` stable for out-of-bounds fragments so user WGSL can safely use derivatives.
  let pos_px = vec2<f32>(f32(x0) + 0.5, f32(y0) + 0.5);
  let uv = pos_px / dims;
  let out = fret_custom_effect(tex, uv, pos_px, params);

  let mdims_u = textureDimensions(mask_texture);
  let mdims = vec2<f32>(f32(mdims_u.x), f32(mdims_u.y));
  let local_x = (f32(x0) + 0.5) - viewport.mask_viewport_origin.x;
  let local_y = (f32(y0) + 0.5) - viewport.mask_viewport_origin.y;
  let in_mask = local_x >= 0.0 && local_y >= 0.0 &&
      local_x < viewport.mask_viewport_size.x && local_y < viewport.mask_viewport_size.y;
  let scale_x = mdims.x / max(viewport.mask_viewport_size.x, 1.0);
  let scale_y = mdims.y / max(viewport.mask_viewport_size.y, 1.0);
  let mx = clamp(i32(floor(local_x * scale_x)), 0, i32(mdims_u.x) - 1);
  let my = clamp(i32(floor(local_y * scale_y)), 0, i32(mdims_u.y) - 1);
  let mask = textureLoad(mask_texture, vec2<i32>(mx, my), 0).x * select(0.0, 1.0, in_mask);
  let a = mask * select(0.0, 1.0, in_bounds);
  return vec4<f32>(out.rgb * a, out.a * a);
}
