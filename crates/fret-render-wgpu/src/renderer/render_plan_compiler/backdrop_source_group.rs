...
impl BackdropSourceGroupDispatchState {
    ...
    pub(super) fn compile_push(
        ...
        self.compile_push_inner(plan, draw_scopes, degradations, args);
    }

    pub(super) fn compile_pop(&mut self) {
        self.compile_pop_inner();
    }
    ...
    fn compile_push_inner(
        ...
        let raw_selection =
            target_selection::choose_free_intermediate_target(draw_scopes, &self.reserved_targets);
        ...
        let can_afford_raw = had_free_target
            && can_allocate_intermediate_bytes(
                ...
                args.clip_path_mask_in_use_bytes
                    .saturating_add(self.in_use_bytes),
                args.format,
            );
        if !can_afford_raw {
            raw_target = None;
        ...
        if let Some(raw_target) = raw_target {
            ...
            self.reserved_targets.push(raw_target);
            let _ = args.quality;
        }

        self.in_use_bytes = self.in_use_bytes.saturating_add(reserved_bytes);
        self.scopes.push(BackdropSourceGroupScope {
    ...
    fn compile_pop_inner(&mut self) {
        let Some(scope) = self.scopes.pop() else {
            return;
        };

        self.in_use_bytes = self.in_use_bytes.saturating_sub(scope.reserved_bytes);

        if let Some(raw_target) = scope.raw_target {
            let popped = self.reserved_targets.pop();
