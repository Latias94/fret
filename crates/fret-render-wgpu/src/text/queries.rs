...
impl TextSystem {
    pub fn caret_x(&self, blob: TextBlobId, index: usize) -> Option<Px> {
        let blob_id = blob;
        let shape = self.shape_for_blob(blob_id)?;
    ...
    pub fn caret_stops(&self, blob: TextBlobId) -> Option<&[(usize, Px)]> {
        Some(self.shape_for_blob(blob)?.caret_stops())
    }
    ...
    pub fn caret_rect(
        ...
        let shape = self.shape_for_blob(blob)?;
        fret_render_text::caret_rect_from_lines(shape.lines(), index, affinity)
    }

    pub fn hit_test_point(&self, blob: TextBlobId, point: Point) -> Option<HitTestResult> {
        let shape = self.shape_for_blob(blob)?;
        fret_render_text::hit_test_point_from_lines(shape.lines(), point)
    }
    ...
    pub fn selection_rects(
        ...
        let shape = self.shape_for_blob(blob)?;
        fret_render_text::selection_rects_from_lines(shape.lines(), range, out);
        Some(())
    }
    ...
    pub fn selection_rects_clipped(
        ...
        let shape = self.shape_for_blob(blob)?;
        fret_render_text::selection_rects_from_lines_clipped(shape.lines(), range, clip, out);
        Some(())
    }

    pub fn first_line_metrics(&self, blob: TextBlobId) -> Option<TextLineMetrics> {
        let shape = self.shape_for_blob(blob)?;
        let line = shape.lines().first()?;
