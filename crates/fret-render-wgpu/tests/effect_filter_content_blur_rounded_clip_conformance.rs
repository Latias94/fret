...
fn read_texture_rgba8(
    ...
    let buffer = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("effect_filter_content_blur_rounded_clip_conformance readback buffer"),
        size: buffer_size,
    ...
    let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
        label: Some("effect_filter_content_blur_rounded_clip_conformance readback encoder"),
    });
...
fn render_and_readback(
    ...
    let texture = ctx.device.create_texture(&wgpu::TextureDescriptor {
        label: Some("effect_filter_content_blur_rounded_clip_conformance output"),
        size: wgpu::Extent3d {
            width: size.0,
            height: size.1,
            depth_or_array_layers: 1,
...
#[test]
fn gpu_filter_content_blur_respects_rounded_clip_stack_on_composite() {
    ...
    // Foreground marker quad, used to assert ordering around the PopEffect boundary.
    ...
    push_bounds_stripes(&mut without_effect, bounds, 10_000);
    ...
    push_bounds_stripes(&mut with_effect, bounds, 10_000);
