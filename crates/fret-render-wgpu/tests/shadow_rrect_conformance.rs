...
fn shadow_rrect_conformance_positive_spread_expands_hard_footprint() {
    ...
    for scale_factor in [1.0_f32, 2.0_f32] {
        ...
        let no_spread_pixels =
            render_scene_rgba8(&ctx, &mut renderer, &no_spread, size, scale_factor);
        let positive_spread_pixels =
            render_scene_rgba8(&ctx, &mut renderer, &positive_spread, size, scale_factor);
...
fn shadow_rrect_conformance_obeys_clip_stack() {
    ...
    for scale_factor in [1.0_f32, 2.0_f32] {
        ...
        let unclipped_pixels =
            render_scene_rgba8(&ctx, &mut renderer, &unclipped, size, scale_factor);
        let clipped_pixels = render_scene_rgba8(&ctx, &mut renderer, &clipped, size, scale_factor);
