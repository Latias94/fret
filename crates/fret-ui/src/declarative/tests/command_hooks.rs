...
#[test]
fn command_hooks_focus_request_ignores_stale_detached_node_entry() {
    ...
    let root = render_root(
        ...
        window,
        bounds,
        "command-hooks-focus-stale-node-entry",
        |cx| {
            let mut props = crate::element::ContainerProps::default();
            props.layout.size.width = Length::Fill;
            props.layout.size.height = Length::Fill;
    ...
    let target_node =
        crate::declarative::mount::node_for_element_in_window_frame(&mut app, window, target)
            .expect("target node");
    let stale_detached = ui.create_node_for_element(target, DetachedDummy);
