...
#[test]
fn touch_pan_scroll_target_resolution_uses_dispatch_snapshot_parent_when_hit_leaf_parent_is_stale()
{
    use std::cell::Cell;
    use std::rc::Rc;
    ...
    let root = render_root(
        ...
        window,
        bounds,
        "touch-pan-scroll-target-stale-parent",
        {
            let pressed_element = pressed_element.clone();
            move |cx| {
                let scroll = cx.scroll(
    ...
    assert!(
        handle.offset().y.0 > 0.01,
        "expected touch pan scroll owner resolution to use dispatch snapshot parents instead of retained parent pointers (offset={:?})",
        handle.offset()
    );
    assert!(
        !crate::elements::is_pressed_pressable(&mut app, window, pressable_element),
        "expected pressable pressed state to be cleared even when its retained parent pointer is stale"
    );
