...
impl Widget for Toolbar {
    ...
    fn layout(&mut self, cx: &mut LayoutCx<'_>) -> Size {
        ...
        let max_h = Px((cx.available.height.0 - self.gap.0.max(0.0) * 2.0).max(0.0));
        ...
        for item in &self.items {
            ...
            let h = Px((metrics.size.height.0 + self.padding_y.0 * 2.0).min(max_h.0));
            let bounds = Rect::new(Point::new(Px(x), Px(y)), Size::new(w, h));
        ...
        let total_h = Px((self.gap.0.max(0.0) * 2.0 + max_h.0).min(cx.available.height.0));

        // Ensure the toolbar paints a background strip even if it has no items.
        let _ = theme;
