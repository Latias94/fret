...
pub(super) struct FrameArenaScratch {
    ...
    pub(super) semantics_stack: Vec<(NodeId, Option<NodeId>, Transform2D, bool)>,
...
impl FrameArenaScratch {
    pub(super) fn capacity_estimate_bytes(&self) -> u64 {
        ...
        bytes = bytes.saturating_add((self.semantics_stack.capacity() as u128).saturating_mul(
            std::mem::size_of::<(NodeId, Option<NodeId>, Transform2D, bool)>() as u128,
        ));
        bytes = bytes.saturating_add(
