...
impl<H: UiHost> UiTree<H> {
    ...
    fn layout_node(
        ...
        if let Some(widget_started) = widget_started {
            ...
            // Collecting `element_path` can be expensive. Avoid doing it for every node in perf
            // gates: only compute it when the node is likely to end up in the top-N hotspots.
            let wants_exclusive = self.debug_layout_hotspots.len() < MAX_LAYOUT_HOTSPOTS
                || self
                    .debug_layout_hotspots
                    .last()
                    .is_some_and(|h| exclusive_time > h.exclusive_time);
            let wants_inclusive = self.debug_layout_inclusive_hotspots.len() < MAX_LAYOUT_HOTSPOTS
                || self
                    .debug_layout_inclusive_hotspots
                    .last()
                    .is_some_and(|h| inclusive_time > h.inclusive_time);
