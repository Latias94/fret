...
#[test]
fn focus_traversal_dispatch_uses_focus_barrier_scope_when_input_barrier_is_off() {
    let mut app = crate::test_host::TestHost::new();
    app.set_global(PlatformCapabilities::default());
    ...
    assert_eq!(
        ui.focus(),
        Some(overlay_b),
        "focus traversal should stay inside the focus barrier scope even when the layer is not part of active input roots"
    );
    ...
    assert_eq!(
        ui.focus(),
        Some(overlay_a),
        "focus traversal should wrap within the focused overlay instead of escaping to the underlay"
    );
}
