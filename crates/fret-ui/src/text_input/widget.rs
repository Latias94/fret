...
impl<H: UiHost> Widget<H> for TextInput {
    ...
    fn layout(&mut self, cx: &mut LayoutCx<'_, H>) -> Size {
        ...
        let metrics = cx.services.text().measure(
            fret_core::TextInput::plain(self.text.as_str(), &self.style),
            base_constraints,
        );
    ...
    fn paint(&mut self, cx: &mut PaintCx<'_, H>) {
        ...
        if show_placeholder && self.placeholder_blob.is_none() {
            ...
            let (blob, metrics) = cx.services.text().prepare(
                fret_core::TextInput::plain(placeholder, &self.style),
                constraints,
            );
            ...
            || self.preedit_blob.is_none()
        {
            ...
            let (prefix_blob, prefix_metrics) = cx.services.text().prepare(
                fret_core::TextInput::plain(&self.text[..self.caret], &self.style),
                constraints,
            );
            let (suffix_blob, suffix_metrics) = cx.services.text().prepare(
                fret_core::TextInput::plain(&self.text[self.caret..], &self.style),
                constraints,
            );
            let (pre_blob, pre_metrics) = cx.services.text().prepare(
                fret_core::TextInput::plain(self.preedit.as_str(), &self.style),
                constraints,
            );
