...
| Extracted helper context | `Cx: fret::app::RenderContextAccess<'a, App>` | Prefer this on new default-path helpers. Keep `UiCx` only as the compatibility raw alias while older helper families migrate. |
...
| Query invalidation on app lane | `cx.data().invalidate_query(...)` / `cx.data().invalidate_query_namespace(...)` | Prefer this when invalidation happens inside `AppUi` / extracted `RenderContextAccess<'a, App>` helpers; keep raw `with_query_client(...)` for pure app/driver code. |
