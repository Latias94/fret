...
| Concrete closure helper context | `&mut fret::app::AppRenderCx<'_>` | Prefer this when closure-local or inline helper families materially benefit from a concrete context carrier. Prefer `AppComponentCx` for app-hosted component/snippet helpers. Keep `UiCx` only as the deprecated compatibility old-name alias behind explicit import while older helper families migrate. |
