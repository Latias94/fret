...
| Extracted helper context | `Cx: fret::app::AppRenderContext<'a>` | Prefer this on new default-path helpers. `RenderContextAccess<'a, App>` remains the underlying generic capability; keep `UiCx` only as the compatibility raw alias while older helper families migrate. |
