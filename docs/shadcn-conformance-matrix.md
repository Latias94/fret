...
| Drawer | `drawer` | `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-docs-smoke.json`, `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-nested-child-drag-routing.json`, `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-snap-points-drag-settle.json` | `ecosystem/fret-ui-shadcn/tests/drawer_layout_invariants.rs`, `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome/drawer.rs` | Layout + focus restore + nested child drag routing + motion (snap-point settle) | In progress | `ui-gallery-shadcn-conformance` now includes the docs smoke plus nested non-modal child drag-routing gate; outside-press, bounds-within-window, and modal-on-modal nested edge cases remain follow-up work. |
