...
| Drawer | `drawer` | `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-docs-smoke.json`, `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-responsive-mobile-open-screenshot.json`, `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-outside-press-focus-restore.json`, `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-nested-child-drag-routing.json`, `tools/diag-scripts/ui-gallery/drawer/ui-gallery-drawer-snap-points-drag-settle.json` | `ecosystem/fret-ui-shadcn/tests/drawer_layout_invariants.rs`, `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome/drawer.rs` | Layout + focus restore + outside press + responsive mobile bounds + nested child drag routing + motion (snap-point settle) | In progress | `ui-gallery-shadcn-conformance` now includes the docs smoke, responsive mobile bounds gate, outside-press focus-restore gate, and nested non-modal child drag-routing gate; modal-on-modal nested edge cases remain follow-up work. |
...
| Typography | `typography` | `tools/diag-scripts/ui-gallery/typography/ui-gallery-typography-docs-smoke.json`, `tools/diag-scripts/ui-gallery/typography/ui-gallery-typography-inline-code-tab-scroll-range.json`, `tools/diag-scripts/ui-gallery/typography/ui-gallery-typography-demo-story-bundle.json`, `tools/diag-scripts/ui-gallery/typography/ui-gallery-typography-interactive-links-activation.json` | `ecosystem/fret-ui-shadcn/src/typography.rs`, `apps/fret-ui-gallery/tests/ui_authoring_surface_default_app.rs`, `crates/fret-ui/src/declarative/tests/view_cache.rs` | Text metrics + inline-link spans + helper click activation + gallery interactive-links activation + logical-edge layout (blockquote/list RTL) + docs-path smoke + authoring-surface structure | Aligned (with gaps) | Helper defaults and the demo's inline-link sentence now match the shadcn docs recipe; the helper layer now also has a focused click-activation test for `p_rich(...).on_activate_link(...)`, and the gallery-level activation diag now passes after fixing cached-subtree `SelectableTextState` retention in `fret-ui`. Remaining gaps are broader inline composition depth and call-site-owned page alignment details, not a generic children API hole. |
