...
| --- | --- | --- | --- |
| Viewport roots (multi-viewport docking) | In progress | Each viewport is solved as an independent root; percent/flex only resolve against definite per-viewport available space. | Conformance tests in `crates/fret-ui/src/declarative/tests/layout.rs` (viewport-root suite, including nested registration flush). |
