...
| Remaining `1e9` usage (code) | In progress | v2 paths must not approximate `Min/MaxContent` as huge definite bounds. Remaining hits should only survive in legacy/non-v2 paths or tests where explicitly justified. Current hits: `crates/fret-ui/src/overlay_placement/tests.rs` (test-only unbounded desired), and `crates/fret-ui/src/declarative/host_widget/layout/scrolling.rs` (legacy non-v2 fallback). | Run the audit commands below. |
