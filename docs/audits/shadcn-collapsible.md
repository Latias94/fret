...
- `CARGO_TARGET_DIR=target-codex-collapsible cargo run -p fretboard -- diag run tools/diag-scripts/ui-gallery-collapsible-docs-smoke.json --dir target/diag-collapsible-docs-codex --session-auto --pack --ai-packet --launch -- env CARGO_TARGET_DIR=target-codex-collapsible cargo run -p fret-ui-gallery`
