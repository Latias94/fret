...
- [x] Use the new `4 MiB` graphics-object signature to drive the next apples-to-apples check rather than collecting more generic local sweeps.
  ...
  - Post-optimization rebaseline: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-fret-vs-gpui-rerender-only-post-path-intermediate-release.md` shows that after landing lazy `path_intermediate` allocation, Fret still sits about `~+98.1 MiB` physical / `~+89.9 MiB` graphics (`empty`) and `~+113.5 MiB` physical / `~+103.3 MiB` graphics (`full`) above GPUI, while Fret's own `metal_current_allocated_size_bytes` drops by `~20.16 MiB`, `renderer_path_intermediate_bytes_estimate` reaches `0`, and the exact `4 MiB` bucket counts refresh to the same `GPUI 28` vs `Fret 50/52` signature.
...
- [ ] Keep chasing the residual `~66–81 MiB` outside `metal-current-allocated-size` with a narrower attribution path rather than adding more redundant mode rows.
  ...
  - Tooling closure: the unstable `footprint -v` refresh turned out to be a capture-order / target-exit problem. The external sampler now runs `footprint -v` before `vmmap`, and compare runners add extra target exit grace when verbose capture flags are enabled; the refreshed `r5` GPUI compare run now recaptures the exact `4 MiB` buckets successfully.
- [x] Stabilize `footprint -v` bucket capture for GPUI/Fret compare reruns so post-optimization cross-framework runs can refresh exact `4 MiB` row counts.
  - Tooling: `tools/sample_external_process_memory.py` now retries capture commands and prioritizes `footprint -v`; compare runners now extend target lifetime when heavy capture flags are enabled.
  - Preferred artifact: `target/diag/fret-vs-gpui-hello-world-rerender-only-footprint-verbose-release-20260307-r5/summary/summary.json`
  - Result: exact bucket capture is restored on the rerender-only post-optimization rebaseline, again showing `GPUI 4.0 MiB × 28` versus `Fret 4.0 MiB × 50/52`.
