...
- **Run an external same-scene active matrix, not just the raw `wgpu` control:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-active-mode-split-baseline.md` now gives a stable `present-only` / `paint-model` / `layout-model` matrix for Fret. The next high-value comparison is to reproduce the same matrix against GPUI (or another same-backend UI framework) so “how much higher than another real UI stack?” can be answered apples-to-apples.

- **Use the new GPUI same-scene matrix as a framework-level cross-check:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-fret-vs-gpui-same-scene-active-matrix.md` now captures a local debug/debug Fret-vs-GPUI matrix on the same hello-world scene. It shows the idle gap stays small (about `+14 MiB` physical / `+5 MiB` graphics), but cadence-aligned active rows (`rerender-only`, `layout-model`) still leave Fret about `~+99–118 MiB` physical / `~+90–107 MiB` graphics above GPUI. This is strong framework-level evidence, but it is still not same-backend closure because GPUI on macOS remains Blade/Metal rather than `wgpu`.
