...
- The external sampler now retries transient `footprint` / `vmmap` failures, but `footprint -v` still returns intermittent exit `66` on this machine during GPUI compares, so exact `4 MiB` bucket refreshes may still rely on the earlier successful `r1` capture until that tooling flake is closed.
