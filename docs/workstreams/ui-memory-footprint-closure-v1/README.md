...
- A cadence-matched release/release frame-latency sweep now narrows the surface/drawable branch further: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-active-present-frame-latency-sweep-release.md` shows that `desired_maximum_frame_latency=1` trims only about `~3.9 MiB` of `IOSurface` / app-visible Metal on both the control and Fret. On the most controlled `present-only empty` row, the hidden residual versus the cadence-matched control stays effectively flat at `~66.0–66.1 MiB` across `latency=1/2/3`, so frame latency is not the main explanation for the remaining active bucket.
- A new local `vmmap -sortBySize` attribution pass now closes the “many medium blocks” hypothesis: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-active-vmmap-regions-attribution.md` shows that the top dirty region for both control and Fret active `present-only` is still a single generic private `owned unmapped memory` row (`control≈115.5 MiB`, `empty≈208.5 MiB`, `full≈221.8 MiB`). The next rows are only the expected three `CAMetalLayer` drawables (`~3.92 MiB` each) plus small `MALLOC_SMALL` / `IOAccelerator` chunks. So local `vmmap` confirms the hidden bucket is real, but it does not split it further on this machine.
- A follow-up `footprint -v` pass now splits that monolithic local bucket into a much more useful pattern: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-active-footprint-verbose-owned-unmapped-attribution.md` shows that the active same-backend gap is dominated by `Owned physical footprint (unmapped) (graphics)` rows, and most of that family is a repeated `4 MiB` bucket. On the steady `6s` sample, the cadence-matched control carries `4.0 MiB × 28`, Fret `present-only empty` carries `4.0 MiB × 50`, and Fret `present-only full` carries `4.0 MiB × 52`, while visible `CAMetalLayer` drawables stay flat at `~11.8 MiB` on all three rows. That makes the remaining local question much narrower: why does Fret keep roughly `+22` to `+24` extra full-surface-sized hidden graphics objects versus the same-backend control?
- The same bounded `footprint -v` view is now also wired into the Fret/GPUI same-scene release comparison: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-fret-vs-gpui-rerender-only-footprint-verbose-release.md` shows that GPUI also carries the same repeated `4 MiB` `Owned physical footprint (unmapped) (graphics)` family, but only at `4.0 MiB × 28` while Fret lands at `×50` (`empty`) and `×52` (`full`). So the signature itself is not unique to Fret; the problem is the extra object count that Fret retains relative to both the same-backend control and GPUI.
- A source-side audit plus a `path_msaa=1` runtime rerun now separate one real optimization lead from the hidden plateau: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-path-msaa-upstream-transient-triage.md` shows that Fret eagerly allocates a full-viewport `path_intermediate` whenever path MSAA is enabled, and forcing `FRET_RENDER_WGPU_PATH_MSAA_SAMPLES=1` removes about `~20 MiB` of app-visible internal Metal. But the hidden `Owned physical footprint (unmapped) (graphics)` bucket and its repeated `4 MiB × 50/52` signature stay effectively flat, so path MSAA is a secondary visible-Metal optimization, not the main explanation for the hidden active bucket.
- The post-optimization GPUI rerender-only rebaseline now confirms the same story on the framework comparison path: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-fret-vs-gpui-rerender-only-post-path-intermediate-release.md` shows Fret `metal_current_allocated_size_bytes` drop by about `~20.16 MiB` (`38.33 -> 18.17 MiB` empty, `42.45 -> 22.30 MiB` full), while the remaining `Fret - GPUI` gap stays roughly `~+100–115 MiB` physical / `~+92–105 MiB` graphics and remains centered in `Owned physical footprint (unmapped) (graphics)` (`~+91.1 / +104.1 MiB` owned delta).
- The GPUI compare tooling root cause is now understood and mitigated: `tools/sample_external_process_memory.py` prioritizes `footprint -v` before the heavier `vmmap` captures, and the compare runners keep targets alive longer when verbose capture flags are enabled. The refreshed `r5` artifact re-captures the exact `4 MiB` buckets (`GPUI 28`, `Fret 50/52`) instead of relying only on the older `r1` run.
...
- **De-prioritize surface latency as the main active-floor lever:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-active-present-frame-latency-sweep-release.md` now shows that cadence-matched `present-only` `latency=1` only removes the expected `~4 MiB` drawable / `IOSurface` slice, while the hidden `empty` residual versus the control remains `~66 MiB`. That makes another frame-latency sweep lower value than Apple-side private/OS bucket attribution.

- **Treat the extra `4 MiB` object count as the best local clue:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-active-footprint-verbose-owned-unmapped-attribution.md` plus `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-fret-vs-gpui-rerender-only-footprint-verbose-release.md` now show that GPUI also has the same repeated `4 MiB` `Owned physical footprint (unmapped) (graphics)` family, but Fret retains about `+22` to `+24` more full-surface-sized objects on the same scene. The next high-value step is therefore narrower than before: upstream/source-side investigation should focus on why Fret's hidden full-surface object count is higher, not on whether the `4 MiB` signature exists at all.

- **Demote path MSAA from root-cause suspect to secondary optimization branch:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-path-msaa-upstream-transient-triage.md` shows that disabling Fret path MSAA cuts about `~20 MiB` of internal app-visible Metal, but leaves the hidden `Owned physical footprint (unmapped) (graphics)` plateau and the `4 MiB × 50/52` object count essentially unchanged. That means path-intermediate / transient-texture work is still worthwhile, but it no longer looks like the main closure path for the hidden bucket.
...
- **The first same-backend active VM+Metal paired run is now captured, but it still does not close attribution:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-apple-vm-metal-active-samebackend.md` records control continuous redraw plus Fret `rerender-only` / `paint-model` / `layout-model` in launch mode. All four bundles are complete enough to analyze, but `metal-io-surface-access` and `metal-command-buffer-frame-assignment` still show zero app-pid matches and remain dominated by WindowServer / GPU helpers. The visible surface/object-label volumes also stay in the same rough band between control and Fret, so this path argues against an obvious Fret-only compositor-surface explosion and points back toward driver/private or other Apple-side buckets that are still not attributed to the app here.

- **The first active launch-mode Game Memory pilot also fails to close the gap:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-apple-game-memory-active-samebackend-pilot.md` records same-backend control continuous redraw plus Fret `rerender-only` full. Both traces are structurally complete, but the launch-mode Game Memory summaries still report `metal-current-allocated-size = 0 rows`, `metal-resource-allocations = 0 rows`, and `metal-io-surface-access.pid_filter_match_count = 0` on the active app rows we care about. That means launch-mode Game Memory is not yet better than the earlier startup-inclusive attach path for active closure on this machine.

- **`Metal Resource Events` is now the strongest Apple-side active attribution path on this machine:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-apple-metal-resource-events-active-samebackend.md` now covers the same-backend control, Fret full-scene `present-only` / `rerender-only` / `paint-model` / `layout-model`, and an empty-scene `present-only` follow-up. Unlike `VM+Metal` and launch-mode `Game Memory`, this path exposes stable app-visible `metal-current-allocated-size`, `metal-resource-allocations`, and `metal-residency-set-resource-event` rows. The key closure is now much sharper: control sits near `~9–10 MiB`, `present-only empty` already reaches `38.33 MiB`, and every full-scene active row—including pure `present-only`—plateaus at `42.45 MiB`. So the large live Metal floor is already present before real rerender/layout work, while hello-world content itself adds only about `4.12 MiB` at this layer. The new label/backtrace clustering also turns the first optimization lead into something actionable: the dominant `128 KiB` staging allocations now point primarily at `fret_render_wgpu::renderer::render_scene::execute` and `UniformResources::write_viewport_uniforms_into`, with `GlyphAtlas::flush_uploads` only as a smaller full-scene add-on. That is a major step forward, but it still does not close the whole graphics bucket: roughly `~60–75 MiB` of the active graphics gap remains outside the app-visible current-allocation ledger and still looks more like driver/private/OS-side accounting than a simple Fret-only surface explosion.

- **The first bounded uniform-dedupe experiment closes the “steady uploads cause the floor” branch:** `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-present-uniform-dedupe-release.md` compares `present-only` release runs with and without `FRET_RENDER_WGPU_SKIP_REDUNDANT_UNIFORM_UPLOADS=1`. The result is now clear: steady `physical`, `graphics_total`, and internal `metal_current_allocated_size` move only at noise level (`empty`: about `+0.20 MiB` physical / `+0.07 MiB` graphics / `0 MiB` internal Metal; `full`: about `-0.60 MiB` / `-0.39 MiB` / `0 MiB`). But the paired `Metal Resource Events` pilot for `present-only empty` collapses from thousands of post-attach app-owned rows to zero-row header-only stores. So the repeated uniform/staging path is real, but it is churn rather than the main steady residency floor.
...
### Same-backend active scratch closure (results; local 2026-03-07)

See `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-renderer-perf-persistent-scratch-attribution.md`.

Key result:

- The same-backend hello-world compare flow now carries renderer-owned persistent scratch estimates in `internal.gpu.json` and the bounded summary output.
- On `present-only` hello-world rows, Fret reports `path_draw_calls=0` but still retains `path_intermediate_bytes_estimate≈19.07 MiB` (`~15.26 MiB` MSAA + `~3.81 MiB` resolved).
- Forcing `FRET_RENDER_WGPU_PATH_MSAA_SAMPLES=1` drives that estimate to `0` and drops `metal_current_allocated_size_bytes` by about `~20 MiB`, confirming that this is a real app-visible renderer cost.
- The hidden dirty plateau does not move: `Owned physical footprint (unmapped) (graphics)` stays at the same dominant `4 MiB × 50/52` bucket, so `path_intermediate` is now clearly a visible optimization target rather than the remaining hidden-bucket root cause.
- The disappearing `footprint -v` rows are a `~16.1 MiB` + `~4.0 MiB` virtual-only pair inside `owned_unmapped_graphics`, which matches the new renderer-perf estimate and further separates this slice from the still-unexplained dirty `4 MiB` family.
- The follow-up product change is now landed: `docs/workstreams/ui-memory-footprint-closure-v1/2026-03-07-path-intermediate-lazy-allocation.md` moves `path_intermediate` to plan-gated allocation/release, so path-free hello-world rows now report `path_intermediate_bytes_estimate=0` even with `path_msaa_samples_effective=4`.
- Against the previous same-backend baseline, that drops active `metal_current_allocated_size_bytes` by about `~20.16 MiB` on both `present-only` rows (`42.45 -> 22.30 MiB` for `full`, `38.33 -> 18.17 MiB` for `empty`).
- The hidden dirty plateau still does not move in a meaningful way: the dominant `Owned physical footprint (unmapped) (graphics)` bucket stays at `4 MiB × 52` (`full`) / `4 MiB × 50` (`empty`), so the visible scratch win is now out of the way for the remaining closure work.
- The compare runners now also wait briefly for `internal.gpu.json` before failing, which removes the small internal-report race that surfaced while validating this optimization.
