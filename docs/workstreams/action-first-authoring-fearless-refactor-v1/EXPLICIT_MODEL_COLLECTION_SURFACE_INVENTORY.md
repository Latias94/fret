...
| `apps/fretboard/src/scaffold/templates.rs` (`todo_template_main_rs`) | explicit collection models + selector/query/filter coordination | This template intentionally teaches a richer tracked graph: nested row models feed selector deps, filter chips, and query invalidation (`tip_nonce`) in one place. | Keep explicit for now; not a target for local-state-first simplification until that richer teaching goal changes. | Intentional advanced surface |
