...
- For dropdown-menu root / parts authoring, prefer `DropdownMenu::uncontrolled(cx).build(cx, trigger, entries)` for the common default path and `DropdownMenu::uncontrolled(cx).build_parts(cx, DropdownMenuTrigger::build(child), DropdownMenuContent::new()..., entries)` for the shadcn part surface. Keep `DropdownMenu::from_open(open)` / `new_controllable(cx, open, default_open)` as the explicit managed-open seams; treat `DropdownMenu::new(open)` as a compatibility alias only. Keep `DropdownMenuTrigger::new(child)` / `into_element_parts(...)` only for already-landed `AnyElement` triggers or compatibility call sites that still need the old trigger closure shape.
- For context-menu root / parts authoring, prefer `ContextMenu::uncontrolled(cx).build(cx, trigger, entries)` for the common default path and `ContextMenu::uncontrolled(cx).build_parts(cx, ContextMenuTrigger::build(child), ContextMenuContent::new()..., entries)` for the shadcn part surface. Keep `ContextMenu::from_open(open)` / `new_controllable(cx, open, default_open)` as the explicit managed-open seams, and keep `ContextMenuTrigger::new(child)` / `into_element_parts(...)` only for already-landed `AnyElement` triggers or compatibility call sites that still need the old trigger closure shape.
