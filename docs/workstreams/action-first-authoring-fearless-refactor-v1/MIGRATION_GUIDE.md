...
- `cargo run -p fretboard-dev -- new hello` uses this pattern (View runtime + typed unit actions):
  `apps/fretboard/src/scaffold/templates.rs` (`hello_template_main_rs`)
- `cargo run -p fretboard-dev -- new todo` extends the same pattern with selector/query hooks:
  `apps/fretboard/src/scaffold/templates.rs` (`todo_template_main_rs`)
- `cargo run -p fretboard-dev -- new simple-todo` provides the smallest View+actions baseline:
...
- For router outlets, use the single named surface `RouterOutlet::into_element_by_leaf(...)` /
  `into_element(...)`. Those helpers now accept builder-first route cards and not-found panels via
  `IntoUiElement<App>` directly, so there is no parallel outlet `*_ui(...)` compatibility track
  anymore.
...
- For popover / hover-card / tooltip root authoring, prefer the typed late-landing constructors `Popover::new(cx, trigger, content)` / `HoverCard::new(cx, trigger, content)` / `HoverCard::new_controllable(cx, open, default_open, trigger, content)` / `Tooltip::new(cx, trigger, content)` when the trigger or content is still a builder-first value. Keep `Popover::from_open(...).into_element_with(...)` / `from_open_controllable(...)` as the explicit advanced seams when the caller already owns the open model or needs anchor-aware closure composition. `PopoverContent::test_id(...)` and `Tooltip::new(cx, trigger, content)` both cross the common root boundary without forcing an early `into_element(cx)` just to attach semantics or diagnostics hooks.
- For dropdown-menu root / parts authoring, prefer `DropdownMenu::uncontrolled(cx).compose().trigger(...).content(...).entries(...)` for the common default and copyable path so extracted helpers can stay typed until the final landing seam. Keep `DropdownMenu::uncontrolled(cx).build(cx, trigger, entries)` as the compact adapter when the default content shell is enough, and keep `build_parts(...)` / `into_element_parts(...)` for lower-level part/closure adapters. `DropdownMenu::from_open(open)` / `new_controllable(cx, open, default_open)` remain the explicit managed-open seams, while `DropdownMenuTrigger::new(child)` stays for already-landed `AnyElement` triggers or compatibility call sites that still need the older trigger closure shape.
- For context-menu root / parts authoring, prefer `ContextMenu::uncontrolled(cx).compose().trigger(...).content(...).entries(...)` for the common default and copyable path so extracted helpers can stay typed until the final landing seam. Keep `ContextMenu::uncontrolled(cx).build(cx, trigger, entries)` as the compact adapter when the default content shell is enough, and keep `build_parts(...)` / `into_element_parts(...)` for lower-level part/closure adapters. `ContextMenu::from_open(open)` / `new_controllable(cx, open, default_open)` remain the explicit managed-open seams, while `ContextMenuTrigger::new(child)` stays for already-landed `AnyElement` triggers or compatibility call sites that still need the older trigger closure shape.
