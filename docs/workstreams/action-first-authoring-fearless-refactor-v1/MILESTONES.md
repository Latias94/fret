...
- Track the remaining ergonomics pressure separately:
  ...
  - Material snackbar gate update (as of 2026-03-09): `tools/gate_material3_snackbar_default_surface.py` now locks that gallery snippet to `action_id(...)`, and both pre-release entrypoints run the gate so the default snippet does not drift back to compat spellings.
