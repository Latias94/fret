...
- `ecosystem/fret-ui-shadcn/src/card.rs` (`CardHeader::build(...)` / `CardContent::build(...)` allow late child landing inside section components)
...
- `apps/fret-examples/src/query_demo.rs` (second `use_local` prototype; also the first builder-first composition experiment using `ui::*_build` sinks plus `UiElementSinkExt`, now paired with late-landing `CardHeader::build(...)` / `CardContent::build(...)`)
