...
| Coordinated `locals::<A>(...)` write-closure ceremony | Repeats in the canonical trio for `Add` / `ClearDone` flows. | Repeats in `form_basics.rs` for `Submit` / `Reset` flows. | This closure shape carries multi-slot transaction ownership. Broad sugar here could hide action identity or make writes feel implicit in a way the runtime cannot explain well. | Keep as a possible shared-surface candidate with more evidence. | Reopen only with a narrow proposal that also improves form-like flows, not just Todo flows, while preserving action identity and transaction boundaries. |
...
- coordinated `locals::<A>(...)` write closures remain a second watch-list item,
