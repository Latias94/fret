...
- `ecosystem/fret-ui-shadcn/src/card.rs` now provides `Card::build(...)` / `CardHeader::build(...)` / `CardContent::build(...)`, and that same card-builder path now powers the `fretboard` todo/simple-todo templates plus cookbook `commands_keymap_basics`, `form_basics`, and `async_inbox_basics`,
- the remaining visible landing points mostly come from other top-level composite wrappers that still collect `AnyElement` eagerly outside the current card path, plus the fact that host-specific build variants still use `refine_style(...)` / `refine_layout(...)` rather than the generic `.ui()` patch path.
