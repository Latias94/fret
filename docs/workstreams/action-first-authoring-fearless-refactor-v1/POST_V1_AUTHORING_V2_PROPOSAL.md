...
- `ecosystem/fret-ui-shadcn/src/popover.rs`, `ecosystem/fret-ui-shadcn/src/hover_card.rs`, and `ecosystem/fret-ui-shadcn/src/tooltip.rs` now also add or extend typed root constructors (`Popover::new(cx, ...)` / `Popover::new_controllable(...)` / `HoverCard::new(cx, ...)` / `HoverCard::new_controllable(...)` / `Tooltip::new(cx, ...)`) so trigger/content builders can cross the common root API boundary without an early `into_element(cx)` step; `Popover::from_open(...)` / `from_open_controllable(...)` stay as the explicit advanced seams for managed-open or anchor-aware closure paths, `PopoverContent::test_id(...)` now survives that late-landing root path directly, and `Tooltip::new(...)` also accepts `TooltipContent` directly,
- `ecosystem/fret-ui-shadcn/src/dropdown_menu.rs` now brings the first composite menu root fully onto that same path through `DropdownMenu::compose().trigger(...).content(...).entries(...)`, with `DropdownMenuTrigger::build(...)` keeping trigger children on the typed lane and `DropdownMenu::build(...)` / `DropdownMenu::build_parts(...)` remaining as narrower adapter seams when callers already want the compact or closure-based root shapes,
