...
| Handler placement for keyed collections / payload rows | workable and already closed for focused todo path, but root handler tables are still visible | slightly narrower, still explicit action identity | Medium | Re-evaluate only after productization, not before |
