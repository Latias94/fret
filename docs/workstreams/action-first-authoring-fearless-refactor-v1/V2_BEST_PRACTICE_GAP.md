...
| Handler placement for keyed collections / payload rows | a narrow local payload-row helper can now remove the repeated `host.models_mut()` / local clone boilerplate, but root handler tables are still visible | slightly narrower, still explicit action identity | Medium | Validate the narrow helper before considering any broader keyed-list sugar |
