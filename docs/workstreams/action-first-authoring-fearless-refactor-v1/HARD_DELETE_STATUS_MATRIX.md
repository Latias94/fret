...
| Command-first widget builders (`DropdownMenu*`, `ContextMenu*`, `Menubar*`, remaining app-facing command APIs) | Public alias pass landed; remaining `on_select(...)`/command-shaped usage is now mostly adoption/internal residue | Still partially visible on some internal/advanced helper surfaces | Policy direction is clear; next work is narrow adoption follow-through + gate strategy | **Lowest delete readiness, narrow remaining code pressure** | Keep migrating remaining app-facing/internal helper surfaces to `action(...)`, then gate default docs/examples where avoidable |
