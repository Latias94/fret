...
| `run_native_with_compat_driver(...)` | Quarantined behind `fret::advanced::interop::run_native_with_compat_driver(...)`; caller families still exist | Explicitly non-default advanced interop seam | Quarantine executed; capability intentionally retained | **Deferred after quarantine** | Keep the quarantined seam stable; only revisit hard delete if the caller families shrink materially |
