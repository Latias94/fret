...
- `apps/fretboard/src/scaffold/templates.rs` (`hello_template_main_rs`, `todo_template_main_rs`, `simple_todo_template_main_rs`)
  - Unit tests gate that templates use `ui::children![cx; ...]`, keep explicit `.into_element(cx)` calls low, and keep first-contact templates on `on_action_notify_models`.
...
- `apps/fret-examples/src/hello_counter_demo.rs`, `apps/fret-examples/src/embedded_viewport_demo.rs`, `apps/fret-examples/src/query_demo.rs`, `apps/fret-examples/src/query_async_tokio_demo.rs`, `ecosystem/fret/src/view.rs`
  - Keeps the default demo surfaces on `on_action_notify_models` / `on_action_notify_transient`, while `embedded_viewport_demo` now uses the app-lane local-state path (`cx.state().local_init(...)` + `layout_value(...)` + `cx.actions().local_set(...)`) only for its view-local size preset and still keeps runtime/interop effects explicit; `hello_counter_demo` now also uses the direct text-value bridge for its step input, and the `view.rs` source-policy tests keep the former single-model raw action aliases deleted while scaffold templates keep equivalent unit-test assertions in `apps/fretboard/src/scaffold/templates.rs`.
...
- `apps/fret-examples/src/custom_effect_v2_identity_web_demo.rs` (advanced/manual web harness; reset button uses `on_activate_request_redraw` while the control bag intentionally stays on explicit `Model<T>`)
- `apps/fret-examples/src/custom_effect_v2_glass_chrome_web_demo.rs` (advanced/manual web harness; reset button uses `on_activate_request_redraw` while the control bag intentionally stays on explicit `Model<T>`)
- `apps/fret-examples/src/custom_effect_v2_lut_web_demo.rs` (advanced/manual web harness; reset button uses `on_activate_request_redraw` while the control bag intentionally stays on explicit `Model<T>`)
