...
- [~] AFA-postv1-001 Investigate direct local-state ergonomics beyond `Model<T>` in `ViewCx::use_state`.
  ...
  - Inventory note (as of 2026-03-08, explicit-model collections): `docs/workstreams/action-first-authoring-fearless-refactor-v1/EXPLICIT_MODEL_COLLECTION_SURFACE_INVENTORY.md` now narrows that next evidence pass to `apps/fret-examples/src/todo_demo.rs` and the `fretboard` simple-todo scaffold path. Current conclusion: gather one more app-grade migration proof before changing the template or adding more tracked-write sugar.
