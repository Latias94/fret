...
- Helper visibility policy snapshot (as of 2026-03-16):
  ...
  - Follow-up shrink (as of 2026-03-17): the zero-proof single-model raw aliases (`on_action_notify_model_update`, `on_action_notify_model_set`, `on_action_notify_toggle_bool`) plus the raw non-notify `cx.on_action(...)` / `cx.on_payload_action(...)` hooks and raw availability hook are deleted from `AppUiRawActionExt`; advanced code now either stays on raw `on_action_notify(...)` / `on_payload_action_notify(...)` or uses the grouped `cx.actions().models::<A>(...)` / `locals::<A>(...)` / `availability::<A>(...)` surfaces.
