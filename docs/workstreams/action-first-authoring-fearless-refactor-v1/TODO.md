...
- [x] AFA-postv1-002 Investigate builder-first composition paths that reduce `ui::children!` and nested
  ...
  - Verification update (as of 2026-03-13): the `DropdownMenu` root fearlessly-refactored surface is now fully closed on first-party call sites. The default recipe path stays `DropdownMenu::uncontrolled(cx).build(...)` / `build_parts(...)`, the explicit managed-open seams stay `DropdownMenu::from_open(...)` / `new_controllable(...)`, the UI Gallery dropdown snippets/pages plus shadcn parity tests stay aligned with that split, and the temporary `DropdownMenu::new(open)` compatibility alias has now been removed. Validation: `cargo check -p fret-ui-shadcn --tests`, `cargo check -p fret-ui-gallery`, and `cargo check -p fret-ui-ai`.
