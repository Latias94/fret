...
- [~] AFA-postv1-002 Investigate builder-first composition paths that reduce `ui::children!` and nested
  ...
  - Update (as of 2026-03-06): `fret-ui-kit::ui::UiElementSinkExt` plus `ui::*_build` sinks now power builder-first `query_demo` and `query_async_tokio_demo` variants that remove `ui::children!` from their main layout sections (`status_row`, `buttons`, `detail_body`, and the page root). `ecosystem/fret-ui-shadcn/src/card.rs` now also exposes `CardHeader::build(...)` / `CardContent::build(...)` so those sections can late-land their children. Remaining gap: top-level composite wrappers such as `Card::new(...)` still collect `AnyElement` eagerly, so some `into_element(cx)` landing boundaries remain visible.
...
- Demo authoring review snapshot (as of 2026-03-06):
  ...
  - Medium demo status: `hello_counter_demo`, `query_demo`, and `query_async_tokio_demo` now use the `LocalState<T>` prototype for view-local state; both query demos also carry the first builder-first composition experiments. The remaining recurring noise classes are:
    ...
    2. eager `AnyElement` landing at top-level composite boundaries (for example `Card::new(...)` and similar section-hosting wrappers),
...
- Post-v1 design review (as of 2026-03-06):
  ...
  - Remaining pressure points:
    ...
    3. the query demos now demonstrate builder-first paths via `ui::*_build` plus late-landing `CardHeader::build(...)` / `CardContent::build(...)`, but `ui::children!` remains the dominant composition style elsewhere and some top-level composite `into_element(cx)` boundaries are still visible in medium demos.
