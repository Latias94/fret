...
- Helper visibility policy snapshot (as of 2026-03-16):
  ...
  - Advanced/reference surface: raw `cx.on_action_notify(...)`, `cx.on_payload_action_notify(...)`, availability hooks, and redraw-oriented `on_activate_request_redraw*` helpers.
  - Follow-up shrink (as of 2026-03-17): the zero-proof single-model raw aliases (`on_action_notify_model_update`, `on_action_notify_model_set`, `on_action_notify_toggle_bool`) plus the raw non-notify `cx.on_action(...)` / `cx.on_payload_action(...)` hooks are deleted from `AppUiRawActionExt`; advanced code now either stays on raw `on_action_notify(...)` / `on_payload_action_notify(...)` / `on_action_availability(...)` or uses the grouped `cx.actions().models::<A>(...)` / `locals::<A>(...)` surfaces.
