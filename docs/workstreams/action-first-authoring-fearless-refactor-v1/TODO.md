...
- Helper visibility policy snapshot (as of 2026-03-16):
  ...
  - Advanced/reference surface: raw `cx.on_action(...)` / `cx.on_action_notify(...)`, payload hooks, and redraw-oriented `on_activate_request_redraw*` helpers.
  - Follow-up shrink (as of 2026-03-17): the zero-proof single-model raw aliases (`on_action_notify_model_update`, `on_action_notify_model_set`, `on_action_notify_toggle_bool`) are deleted from `AppUiRawActionExt`; advanced code now either stays on raw `on_action_notify(...)` or uses the grouped `cx.actions().models::<A>(...)` / `locals::<A>(...)` surfaces.
