...
- [x] AFA-actions-019 Make `keyctx.*` gating observable and consistent across surfaces.
  ...
  - Evidence:
    ...
    - `ecosystem/fret/src/in_window_menubar.rs` + `crates/fret-launch/src/runner/desktop/runner/windows_menu.rs` (menu `when` uses key contexts)
  - Gates:
...
- View runtime ergonomics: reduce `on_action` handler boilerplate (`request_redraw` + `notify`) without weakening
  ...
  - Evidence:
    ...
    - `apps/fret-examples/src/custom_effect_v2_web_demo.rs` (advanced/manual web harness; uses `on_activate_request_redraw` while the control bag intentionally stays on explicit `Model<T>`)
