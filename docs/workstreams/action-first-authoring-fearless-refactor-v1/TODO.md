...
- third: re-evaluate keyed-list / payload-row handler ergonomics (`AFA-postv1-003`) only if keyed-list evidence still looks materially noisier than the root-handler path after the product pass,
...
- [ ] AFA-postv1-003 Investigate keyed-list / payload-row handler ergonomics without expanding the default helper surface prematurely.
...
- [x] AFA-postv1-021 Land the menu-family action-first alias pass for `ContextMenu*` / `Menubar*`.
  ...
  - Gate update (as of 2026-03-09): `tools/gate_menu_action_default_surfaces.py` now keeps the primary ui-gallery context-menu / menubar teaching snippets on `action(...)`, and `tools/pre_release.py` runs that narrow policy check alongside the other default-surface gates.
