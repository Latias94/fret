...
- [x] AFA-postv1-011 Inventory the remaining in-tree `App::ui*` callers against the app-entry policy draft.
  ...
  - Progress update (as of 2026-03-08): `apps/fret-examples/src/imui_hello_demo.rs` and `apps/fret-examples/src/chart_declarative_demo.rs` have already moved to `run_view::<...>()`, proving both a minimal IMUI demo and a plain declarative chart demo do not need closure-root `App::ui(...)`.
