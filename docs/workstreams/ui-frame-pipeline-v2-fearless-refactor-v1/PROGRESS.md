...
| Scene-fragment replay | Partial global, complete for row replay and node-fallback deletion | `CanvasSceneFragment<RowSceneFragmentPayload>` is boundary-owned for the code-editor row path. Boundary `PaintCacheEntry` replay metadata is owned by `ViewBoundaryState::paint_cache`; plain paint-cache entries are no longer node-owned and use the boundary-shaped side store. `PreviousFramePaintRecording` now names the remaining previous-frame scene recording carrier and owns entry replay validation, op slicing, and text side-index replay. | Decide final previous-frame recording owner and plain-entry store shape for non-code-editor surfaces. |
...
- boundary-node paint-cache entries are now owned by `ViewBoundaryState::paint_cache`, and the
  ...
  `PreviousFramePaintRecording` as the named previous-frame scene recording carrier. That carrier
  now owns entry replay validation and text side-index replay, but not its final global owner
  decision.
