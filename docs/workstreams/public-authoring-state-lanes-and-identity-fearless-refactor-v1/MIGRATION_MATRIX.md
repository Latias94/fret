...
| `ecosystem/fret/src/lib.rs` | `UiCx<'a>` is now only an explicitly deprecated compatibility alias to `AppComponentCx<'a>`, and the repo no longer treats it as neutral authoring vocabulary | Migrate | M2/M5 | `fret::app::prelude::*` keeps `AppRenderCx<'a>` as the blessed concrete helper carrier and no longer reexports `UiCx`; `fret::advanced::prelude::*` now teaches `AppComponentCx<'a>` instead. The grouped helper extension names have also moved to `AppRenderActionsExt` / `AppRenderDataExt`, the old `UiCx*Ext` names remain only as explicit deprecated compatibility aliases, and the default teaching-snippet gate now enforces `AppComponentCx<'a>` instead of `UiCx<'a>`. |
