...
| `ecosystem/fret/src/lib.rs` | `UiCx<'a>` is now only a compatibility alias to `AppRenderCx<'a>`, but the repo still needs to finish shrinking it out of the default helper story | Migrate | M2/M5 | `fret::app::prelude::*` now keeps `AppRenderCx<'a>` as the blessed concrete helper carrier and no longer reexports `UiCx`; long-term target is one explicit app-facing render-authoring lane for both `AppUi` and extracted helper functions, with `UiCx` retained only for explicit compatibility/advanced imports until the remaining migrations close. |
