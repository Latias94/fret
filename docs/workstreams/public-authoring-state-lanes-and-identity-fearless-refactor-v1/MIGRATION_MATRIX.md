...
| `ecosystem/fret/src/lib.rs` | `UiCx<'a>` is now only a compatibility alias to `AppComponentCx<'a>`, but the repo still needs to finish shrinking it out of the explicit compatibility story | Migrate | M2/M5 | `fret::app::prelude::*` keeps `AppRenderCx<'a>` as the blessed concrete helper carrier and no longer reexports `UiCx`; `fret::advanced::prelude::*` now teaches `AppComponentCx<'a>` instead. The grouped helper extension names have also moved to `AppRenderActionsExt` / `AppRenderDataExt`, with the old `UiCx*Ext` names retained only as explicit deprecated compatibility aliases. |
