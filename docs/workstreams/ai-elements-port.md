...
| `persona.tsx` | `fret-ui-ai` | `ecosystem/fret-ui-ai/src/elements/persona.rs` | Prototype | Variant-aware placeholder shell (upstream uses Rive/webgl2) with optional custom visual slot via `into_element_with_children(...)`. UI Gallery docs-aligned demo + diag gate: `tools/diag-scripts/ui-gallery-ai-persona-demo.json`. |
