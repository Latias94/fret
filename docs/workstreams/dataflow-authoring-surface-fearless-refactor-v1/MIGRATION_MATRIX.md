...
| widget `.dispatch::<A>()` / `.dispatch_payload::<A>(...)` and host-side `cx.actions().dispatch::<A>()` / `.dispatch_payload::<A>(...)` | public explicit aliases, but no meaningful first-party usage remains outside docs/history | remove once first-party docs/tests stop teaching the aliases | In progress |
| `cx.actions().listener(...)` alias | duplicate alias over `listen(...)`, with no in-tree usage | delete outright | Deleted |
| `cx.data().selector(...)` + `DepsBuilder::new(cx)` + `local_layout_rev(...)` + `layout_in(cx)` | still the default LocalState-first selector choreography in Todo/template docs | replace with one app-facing LocalState-first selector path owned by `ecosystem/fret` | Not started |
...
| Activation glue aliases | `action/action_payload/listen` plus `dispatch/dispatch_payload` aliases | keep `action/action_payload/listen`; delete redundant aliases | migrate docs and gates away from `dispatch*`, then remove the aliases | no first-party docs/examples/tests teach or depend on `dispatch*` | In progress | `ecosystem/fret/src/view.rs`, `docs/authoring-golden-path-v2.md`, `docs/crate-usage-guide.md`, `apps/fret-ui-gallery/tests/ui_authoring_surface_default_app.rs` |
| LocalState-first selector deps/reads | raw `DepsBuilder` choreography plus `layout_in(cx)` inside the compute closure | one app-facing LocalState-first selector surface that does not teach dependency-builder internals on first contact | add the shortening only in `ecosystem/fret`; keep `fret-selector` narrow and raw | Todo/template/docs no longer require `DepsBuilder::new(cx)` as the default selector story | Not started | `apps/fretboard/src/scaffold/templates.rs`, `apps/fret-examples/src/todo_demo.rs`, `docs/examples/todo-app-golden-path.md`, `ecosystem/fret/src/view.rs` |
...
| Docs/templates/examples/gates | current sources still mix target posture with pre-closeout residue | one fully aligned default story | migrate in lockstep: code, templates, docs, source gates, then hard delete | first-hour, golden-path, Todo docs, scaffold templates, cookbook, and UI Gallery gates all tell the same story | Not started | `docs/first-hour.md`, `docs/authoring-golden-path-v2.md`, `docs/examples/todo-app-golden-path.md`, `apps/fretboard/src/scaffold/templates.rs`, `apps/fret-ui-gallery/tests/ui_authoring_surface_default_app.rs` |
...
| widget `.dispatch::<A>()` / `.dispatch_payload::<A>(...)` | widget `.action(...)` / `.action_payload(...)` / `.listen(...)` | first-party docs/tests no longer teach the explicit alias path and gates keep it out of the default story | In progress |
| host-side `cx.actions().dispatch::<A>()` / `.dispatch_payload::<A>(...)` | `cx.actions().action(...)` / `.action_payload(...)` / `.listen(...)` | first-party docs/tests stop teaching it and no curated app surface needs the alias wording | In progress |
...
| first-contact `DepsBuilder::new(cx)` + `local_layout_rev(...)` + `layout_in(cx)` selector teaching | new app-facing LocalState-first selector sugar | the new selector surface is proven on generic-app and editor-grade surfaces and source gates prevent raw builder teaching on the default path | Not started |
