...
| Keyed payload row writes | `payload_local_update_if::<A>(...)` as default, with duplicate multi-local payload helpers later deleted and the lower-level `payload::<A>()` chain still quarantined | one canonical row-write happy path, no duplicate multi-local helper retained by default, lower-level payload chain kept out of first-contact docs | keep the happy path on keyed row writes, quarantine `payload::<A>()`, and delete duplicate zero-proof helpers rather than carrying them as advanced residue | templates/docs/examples stop teaching `payload::<A>()` as a co-equal choice and post-closeout cleanup removes duplicate multi-local helpers | Migrated | `apps/fret-examples/src/todo_demo.rs`, `apps/fretboard/src/scaffold/templates.rs`, `docs/examples/todo-app-golden-path.md`, `apps/fret-cookbook/examples/payload_actions_basics.rs`, `docs/workstreams/action-write-surface-fearless-refactor-v1/CLOSEOUT_AUDIT_2026-03-17.md` |
...
| first-contact `cx.actions().payload::<A>()` teaching | `payload_local_update_if::<A>(...)` for the happy path; no dedicated multi-local payload helper retained on the default path | templates/docs/examples no longer present `payload::<A>()` as a co-equal default path | Quarantined |
