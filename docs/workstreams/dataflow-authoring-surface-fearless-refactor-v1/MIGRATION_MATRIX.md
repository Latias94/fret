...
| Multi-slot LocalState transactions | `cx.actions().locals_with((...)).on::<A>(...)` | one canonical LocalState transaction story | keep the capture-explicit builder as the only default transaction family and delete the older bare helper once first-party surfaces migrate cleanly | first-party docs/templates/examples no longer teach `locals::<A>(...)`, and the remaining runtime surfaces migrate without reopening clone boilerplate | Migrated | `ecosystem/fret/src/view.rs`, `apps/fret-examples/src/todo_demo.rs`, `apps/fret-cookbook/examples/form_basics.rs`, `docs/authoring-golden-path-v2.md`, `apps/fretboard/src/scaffold/templates.rs` |
