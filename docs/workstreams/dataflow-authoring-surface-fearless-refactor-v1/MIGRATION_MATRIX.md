...
| `cx.actions().locals_with((...)).on::<A>(...)` | current canonical coordinated LocalState transaction path on Todo/form/template surfaces | baseline default coordinated write path | Migrated |
| `cx.actions().locals::<A>(...)` | former bare LocalState transaction helper | delete once the capture-explicit builder proves itself on Todo, form, template, and non-Todo runtime surfaces without narrowing capability | Deleted |
...
| widget `.action(...)` / `.action_payload(...)` / `.listen(...)` plus host-side `cx.actions().listen(...)` | current activation-bridge vocabulary after the redundant host-side typed-dispatch aliases were removed | keep widget-local typed dispatch on the explicit `AppActivateExt` bridge and keep the host-side seam to `listen(...)` only | Kept intentionally |
...
| `cx.data().selector_keyed(...)` | zero-proof keyed facade alias over the raw selector-engine keyed lane | remove it and keep keyed/looped selector ownership on `cx.keyed(...)` or direct `fret-selector` usage | Deleted |
...
| Multi-slot LocalState transactions | `cx.actions().locals_with((...)).on::<A>(...)` | one canonical LocalState transaction story | keep the capture-explicit builder as the only default transaction family and delete the older bare helper once first-party surfaces migrate cleanly | first-party docs/templates/examples no longer teach `locals::<A>(...)`, and the remaining runtime surfaces migrate without reopening clone boilerplate | Migrated | `ecosystem/fret/src/view.rs`, `apps/fret-examples/src/todo_demo.rs`, `apps/fret-cookbook/examples/form_basics.rs`, `docs/authoring-golden-path.md`, `apps/fretboard/src/scaffold/templates.rs` |
...
| host-side `cx.actions().dispatch::<A>()` / `.dispatch_payload::<A>(...)` | `cx.actions().listen(...)` plus explicit `AppActivateExt` when activation-only typed dispatch is intentional | first-party docs/tests stop teaching the host-side typed-dispatch alias lane and no curated app surface needs it | Deleted |
...
| first-contact `cx.actions().locals::<A>(...)` teaching and helper | `cx.actions().locals_with((...)).on::<A>(...)` on the default `fret` lane | once the capture-explicit builder exists, source/docs/templates adopt it, and no first-party surface still needs the bare helper | Deleted |
