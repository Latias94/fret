...
| `UiHostBoundIntoElement<H>` | stop teaching publicly; compatibility bridge only | still public at crate root; removed from curated component exports | Kept internally only (target) / public scaffolding (current) |
| `UiChildIntoElement<H>` | stop teaching publicly; app-internal/component-internal mechanism only | still public at `fret-ui-kit` root and used by child pipelines | Kept internally only (target) / public scaffolding (current) |
| `UiBuilderHostBoundIntoElementExt<H>` | hidden bridge only, then delete | still imported anonymously in preludes for method syntax | Kept internally only |
...
| Host-bound builder landing | `UiBuilderHostBoundIntoElementExt<H>` exists only to recover `.into_element(cx)` syntax for host-bound builders | method syntax provided through the unified conversion trait | move host-bound builder landing behind the new public trait and keep any extra bridging internal | app/component preludes stop importing the old extension trait | Scaffolding only | `ecosystem/fret/src/lib.rs`, `ecosystem/fret-ui-kit/src/ui_builder.rs` |
| Child pipelines | `ui::children!` and container helpers effectively depend on `UiChildIntoElement<H>` | heterogeneous child collection consumes the unified conversion trait | migrate macros/helpers first so component/app call sites can follow without churn | no curated child helper depends on `UiChildIntoElement<H>` publicly | In progress | `ecosystem/fret-ui-kit/src/lib.rs`, `ecosystem/fret-ui-kit/src/ui.rs`, `ecosystem/fret-ui-kit/src/imui.rs` |
...
| `UiBuilderHostBoundIntoElementExt<H>` as curated public bridge trait | unified trait-backed method syntax | app/component preludes stop importing the old bridge and first-party code compiles through the unified trait | Scaffolding only |
