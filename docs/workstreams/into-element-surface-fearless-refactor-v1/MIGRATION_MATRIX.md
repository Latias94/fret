...
| `UiChildIntoElement<H>` | stop teaching publicly; app-internal/component-internal mechanism only | still public under `fret_ui_kit::ui`, but no longer exported at the crate root; remains a thin child-pipeline bridge over `IntoUiElement<H>` | Kept internally only / module-level scaffolding |
...
| `UiChildIntoElement<H>` as curated public conversion vocabulary | unified trait for component code, `UiChild` for app-facing helpers | child pipelines and curated docs no longer require the old trait name | In progress |
