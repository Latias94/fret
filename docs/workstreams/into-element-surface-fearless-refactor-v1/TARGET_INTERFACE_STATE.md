...
- `fret::workspace_shell::{workspace_shell_model, workspace_shell_model_default_menu}` and
  `fret_workspace::WorkspacePaneContentFocusTarget` now also follow the typed-helper rule:
  pane renderers may stay on `IntoUiElement<H>`, while the shell keeps the final explicit landing
  seam only where it must register pane-content focus targets.
