...
- direct `App + UiTree` examples should follow the same ownership rule on their lower-level lane:
  ...
  closure body.
- low-level interop/app-element-context examples are a separate direct-root leaf lane:
  examples such as `external_texture_imports_demo`, `external_texture_imports_web_demo`,
  `external_video_imports_*_demo`, `chart_declarative_demo`, and `node_graph_demo`
  intentionally keep dedicated direct roots like `render_view(...)`, `chart_canvas_panel(...)`,
  or `node_graph_surface(...)`.
  Do not force those surfaces through `UiCx -> Ui` wrappers just for uniformity.
  If one of those examples later grows surrounding authoring chrome beyond the leaf contract, add a
  local typed wrapper around that chrome and keep the interop leaf itself direct.
