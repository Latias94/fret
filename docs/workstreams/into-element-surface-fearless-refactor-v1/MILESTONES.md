...
- the next UI Gallery app-facing snippet batch is now `chart`:
  it is currently the smallest remaining multi-snippet page-local family that still teaches
  top-level `UiCx -> AnyElement` returns.
