...
| Accessibility and diagnostics anchors | `a11y.rs` retained active-descendant semantics tests; deleted no-user `diag_anchors.rs` | declarative paint-only semantics/diagnostics modules exist; diagnostics anchors now use declarative surface `test_id`/diagnostics config instead of retained anchor widgets | Add default declarative active-descendant semantics tests before deleting retained a11y anchors. |
