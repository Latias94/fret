...
- `ecosystem/fret-node/src/ui/canvas/widget/sticky_wire_targets.rs`
  - `RBX-M2-261` moved sticky-wire non-port target handling off a direct retained `EventCx`
    signature. The helper now composes host/window, pointer-capture release, and handled-event tail
    behavior through `StickyWireTargetPickerCx`; retained `EventCx` remains isolated in
...
- `ecosystem/fret-node/src/ui/canvas/widget/sticky_wire_connect.rs`
  - `RBX-M2-262` moved the sticky-wire pointer-down and connect-target route wrappers off direct
    retained `EventCx` signatures. Both routes now compose host/window access through
    `StickyWireTargetPickerCx`, while the retained adapter remains isolated in
    `sticky_wire_targets/retained_picker_cx.rs`.
- `ecosystem/fret-node/src/ui/canvas/widget/left_click/cx.rs`
- `ecosystem/fret-node/src/ui/canvas/widget/left_click/**`
  - `RBX-M2-950` moved the left-click hit-routing subtree off direct retained `EventCx` names by
    composing the existing `MarqueeCx` and `WireCommitCx` seams. Retained `EventCx` remains
    available through the shared retained adapters, and the new `left_click_route_stays_off_retained_bridge`
    source-policy gate keeps the route explicit.
...
- `ecosystem/fret-node/src/ui/canvas/widget/keyboard_shortcuts_commands.rs`
  - `RBX-M2-960` moved modifier, Tab, arrow-nudge, and delete shortcut command dispatch behind the
    retained-agnostic `KeyboardShortcutCommandSink` seam. Retained `EventCx` adaptation remains
    isolated in `keyboard_shortcuts.rs`.
- `ecosystem/fret-node/src/ui/canvas/widget/keyboard_shortcuts_overlay.rs`
  - `RBX-M2-970` moved Escape/searcher/context-menu overlay keyboard helpers behind the
    retained-agnostic `KeyboardOverlayCx` seam. Retained event routing remains isolated in
    `keyboard_shortcuts.rs` and the top-level keyboard event entry.
- `ecosystem/fret-node/src/ui/canvas/widget/event_keyboard_route.rs`
  - `RBX-M2-980` moved the top-level keyboard route helper behind the retained-agnostic
    `KeyboardRouteCx` and `WidgetPaintInvalidationCx` seams. Retained `EventCx` adaptation remains
    isolated in `event_keyboard.rs`.
- `ecosystem/fret-node/src/ui/canvas/widget/event_router_system_input.rs`
  - `RBX-M2-990` moved keyboard input event routing behind `KeyboardInputSink`, leaving retained
    text-input focus access in `event_keyboard.rs`.
...
- `ecosystem/fret-node/src/ui/canvas/widget/insert_node_drag/internal_drop.rs`
  - `RBX-M2-1000` moved lifecycle routing behind `SystemLifecycleCx`, clipboard text handling
    behind `ClipboardTextCx`, and insert-node internal drag handling behind `InternalDragCx`.
    Retained adaptation remains in retained adapter files.
- `ecosystem/fret-node/src/ui/canvas/widget/event_router_system.rs`
  - `RBX-M2-1010` moved the top-level non-pointer route wrapper behind `SystemRouteCx`, composing
    `SystemLifecycleCx` and `KeyboardInputSink` without retained Cx names.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pan_zoom_move.rs`
  - `RBX-M2-740` split the shared pan host/bounds/paint capability into `PanZoomCx`, kept
    `PanZoomBeginCx` as the begin-only pointer-capture extension, and moved the panning move path
    behind retained-agnostic pan capabilities. Retained `EventCx` adaptation remains isolated in
    `pan_zoom_begin_retained_cx.rs`.
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/primary/surface.rs`
  - `RBX-M2-750` moved the primary surface pointer-move route behind `MarqueeCx`, reusing the
    existing pan and marquee capabilities instead of introducing a duplicate surface seam. The
    surrounding primary router plus group/node/connection branches remain retained-bound for later
    isolation slices.
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/primary/group.rs`
  - `RBX-M2-760` moved the primary group pointer-move route behind
    `PendingGroupActivationCx + GroupPreviewMoveCx`, reusing the existing pending group activation
    and group preview move seams. The surrounding primary router plus node/connection branches
    remain retained-bound for later isolation slices.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/primary/node.rs`
  - `RBX-M2-770` moved the primary node pointer-move route and pending node drag activation helper
    behind `PendingNodeDragActivationCx`, a narrow host-access plus pointer-capture-release seam.
    Retained `EventCx` adaptation remains isolated in
    `pending_node_drag_activation_retained_cx.rs`; pending node resize move remains retained-free
    without an event Cx parameter. The surrounding primary router plus connection branch remain
    retained-bound for later isolation slices.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/primary/connection.rs`
  - `RBX-M2-780` moved the primary connection pointer-move route behind `WireDragMoveCx`, a
    retained-agnostic host/bounds/paint-invalidation seam for pending and active wire-drag move
    handling. Retained `EventCx` adaptation remains isolated in `wire_drag_move_retained_cx.rs`.
    Pending and active edge-insert move helpers now use the existing `WidgetPaintInvalidationCx`
    seam because they only need paint invalidation. The surrounding primary router remains
    retained-bound for a later wrapper isolation slice.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/primary.rs`
  - `RBX-M2-790` moved the primary pointer-move wrapper behind `PrimaryPointerMoveCx`, a composed
    capability over the already-retained-agnostic surface, group, node, and connection branch
    seams. This adds no new side-effect methods; retained `EventCx` only satisfies the wrapper
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/secondary/node.rs`
  - `RBX-M2-800` moved the secondary node pointer-move route behind
    `NodeDragMoveCx + NodeResizeMoveCx`. `NodeDragMoveCx` composes the existing node drag geometry,
    preview, and move-tail seams with bounds access; `NodeResizeMoveCx` carries host access plus
    paint invalidation. Retained `EventCx` adaptation remains isolated in
    `node_drag_move_retained_cx.rs` and `node_resize_move_retained_cx.rs`.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/secondary/connection.rs`
  - `RBX-M2-810` moved the secondary connection pointer-move route behind
    `WireDragMoveCx + EdgeDragMoveCx`. Wire drag and edge-insert drag movement reuse the existing
    retained-agnostic move seams; edge reconnect drag now uses `EdgeDragMoveCx` for host access plus
    paint invalidation. Retained `EventCx` adaptation remains isolated in
    `edge_drag_move_retained_cx.rs`.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/secondary/insert.rs`
  - `RBX-M2-820` moved the secondary insert pointer-move route and pending insert-node drag move
    helper behind `InsertNodeDragMoveCx`, a retained-agnostic capability for
    pointer/window/bounds/tick/host access plus pointer-capture release and paint invalidation.
    Retained `EventCx` adaptation remains isolated in `insert_node_drag_move_retained_cx.rs`. The
    retained internal drag enter/over/drop entry moved to `insert_node_drag/internal_event.rs` and
    remains a later retained I/O migration slice.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch/secondary.rs`
  - `RBX-M2-830` moved the secondary pointer-move wrapper behind `SecondaryPointerMoveCx`, a
    composed capability over the already-retained-agnostic node, connection, and insert branch
    seams. This adds no new side-effect methods; retained `EventCx` only satisfies the wrapper
    through existing branch adapters.
- `ecosystem/fret-node/src/ui/canvas/widget/searcher.rs`
  ...
  - `RBX-M2-840` narrowed the top-level searcher pointer-move facade to
    `WidgetPaintInvalidationCx`; key/down/up searcher route capabilities are no longer required by
    overlay pointer-move dispatch.
...
- `ecosystem/fret-node/src/ui/canvas/widget/context_menu/mod.rs`
  - `RBX-M2-840` moved overlay pointer-move dispatch behind the existing
    `WidgetPaintInvalidationCx` seam. Context-menu and searcher pointer-move wrappers now expose
    only the paint invalidation capability their hover-update leaf helpers already consume.
- `ecosystem/fret-node/src/ui/canvas/widget/hover.rs`
- `ecosystem/fret-node/src/ui/canvas/widget/hover_move_cx.rs`
  - `RBX-M2-850` moved fallback edge/anchor hover pointer-move handling behind `HoverMoveCx`, a
    retained-agnostic capability for host access plus paint invalidation. Retained `EventCx`
    adaptation remains isolated in `hover_move_retained_cx.rs`.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_dispatch.rs`
  - `RBX-M2-860` moved the top-level pointer-move route wrapper behind `PointerMoveCx`, a composed
    capability over the already-retained-agnostic primary, secondary, and hover fallback seams. It
    adds no new side-effect methods; overlay pointer-move needs only the paint invalidation provided
    by `HoverMoveCx`.
...
- `ecosystem/fret-node/src/ui/canvas/widget/pointer_move_release_pan/pending_right_click.rs`
  - `RBX-M2-910` moved the pointer-move release route and pan-release helpers behind
...
- `ecosystem/fret-node/src/ui/canvas/widget/viewport_motion_cx.rs`
  - `RBX-M2-930` moved pointer-wheel and timer-motion route wrappers behind
    `PointerWheelCx` / `TimerMotionCx`, threading explicit `Platform` through the wheel route
    instead of keeping a retained platform adapter, and reusing the existing retained adapter
    seams for timer motion. Wheel zoom/pan, viewport animation, auto-pan, and timer-motion tests
    stay green.
...
| Keyboard shortcut command dispatch seam | retained canvas keyboard routing still receives retained `EventCx`, but modifier, Tab, arrow-nudge, and delete shortcut command dispatch only needs a command-dispatch + handled sink | `RBX-M2-960` moves `keyboard_shortcuts_commands.rs` onto `KeyboardShortcutCommandSink`, isolates retained `EventCx` adaptation in `keyboard_shortcuts.rs`, and source-policy gates the command helper | Continue migrating overlay/Escape keyboard wrappers and the top-level `event_keyboard_route.rs` before replacing the retained keyboard event entry. |
| Keyboard overlay/Escape seam | retained canvas keyboard routing still receives retained `EventCx`, but searcher/context-menu overlay keys and Escape cancellation only need the already-isolated searcher, context-menu, and cancel seams | `RBX-M2-970` moves `keyboard_shortcuts_overlay.rs` onto `KeyboardOverlayCx`, isolates retained overlay adaptation in `keyboard_shortcuts.rs`, and source-policy gates the overlay helper | Move `event_keyboard_route.rs` and `event_keyboard.rs` behind a composed retained-agnostic route seam before replacing the retained keyboard event entry. |
| Top-level keyboard event route seam | retained canvas keyboard event entry still receives retained `EventCx`, but `event_keyboard_route.rs` only needs overlay, command, handled, and paint-invalidation capabilities | `RBX-M2-980` moves `event_keyboard_route.rs` onto `KeyboardRouteCx` / `WidgetPaintInvalidationCx` and source-policy gates the route | Move or shrink `event_keyboard.rs` as the final retained `EventCx` adapter for keyboard events, then continue with sibling system-event routes. |
| Keyboard system input subroute seam | retained system event routing still receives retained `EventCx`, but key-down/key-up dispatch only needs text-input focus plus the already-isolated keyboard route capabilities | `RBX-M2-990` moves `event_router_system_input.rs` onto `KeyboardInputSink`, isolates retained text-input focus access in `event_keyboard.rs`, and source-policy gates the input subroute | Continue with `event_router_system.rs` / lifecycle routes, where clipboard/focus-loss/pointer-cancel/internal-drag/timer handling still uses retained event adapters. |
| System lifecycle subroute seam | retained top-level system routing still receives retained `EventCx`, but lifecycle subrouting only needs clipboard feedback, cancel, internal-drag, and timer-motion capabilities | `RBX-M2-1000` moves `event_router_system_lifecycle.rs` onto `SystemLifecycleCx`, moves clipboard text handling onto `ClipboardTextCx`, moves insert-node internal drag handling onto `InternalDragCx`, and source-policy gates the helpers | Move `event_router_system.rs` itself behind a composed system route seam now that keyboard input and lifecycle subroutes are retained-agnostic. |
| Top-level non-pointer system route seam | retained event routing still receives retained `EventCx` at the broader event-router boundary, but non-pointer system routing only composes lifecycle and keyboard input subroutes | `RBX-M2-1010` moves `event_router_system.rs` onto `SystemRouteCx` and source-policy gates the wrapper | Continue with the broader `event_router.rs` / pointer-router boundary, or shrink sibling pointer route wrappers next. |
...
| Pan-zoom move seam | retained canvas pointer-move routing still calls panning move from retained `EventCx`, but the panning move helper only needs host access, bounds, and paint invalidation | `RBX-M2-740` moves `pan_zoom.rs` and `pan_zoom_move.rs` onto `PanZoomCx`, keeps pointer capture only on `PanZoomBeginCx`, source-policy gates pan move helpers, and keeps middle-mouse panning, space-to-pan, move callbacks, and pan inertia end behavior green | Continue isolating `event_router_pointer_button/move_event.rs` and surrounding pointer-move routing before replacing the retained event widget. |
| Pointer-move primary surface seam | retained primary pointer-move routing still owns group/node/connection branches, but the surface branch only needs the existing pan and marquee capabilities | `RBX-M2-750` moves `pointer_move_dispatch/primary/surface.rs` onto `MarqueeCx`, source-policy gates the route, and keeps middle-mouse panning, space-to-pan, pending marquee, and marquee replace behavior green | Continue isolating `pointer_move_dispatch/primary.rs` after group/node/connection branches have retained-agnostic seams, or isolate the next narrow branch under `pointer_move_dispatch/primary/**`. |
| Pointer-move primary group seam | retained primary pointer-move routing still owns node/connection branches, but the group branch only needs pending group activation and group preview move capabilities | `RBX-M2-760` moves `pointer_move_dispatch/primary/group.rs` onto `PendingGroupActivationCx + GroupPreviewMoveCx`, source-policy gates the route, and keeps pending group drag activation plus group resize preview/commit behavior green | Continue isolating `pointer_move_dispatch/primary/node.rs` and `pointer_move_dispatch/primary/connection.rs`, then replace `pointer_move_dispatch/primary.rs`. |
| Pointer-move primary node seam | retained primary pointer-move routing still owns the connection branch, but the node branch only needs pending node drag activation host access plus pointer-capture release, while pending node resize move is already retained-free | `RBX-M2-770` moves `pointer_move_dispatch/primary/node.rs`, `pending_drag.rs`, and pending-node abort cleanup onto `PendingNodeDragActivationCx` / `PointerCaptureReleaseCx`, source-policy gates the route and activation helper, and keeps node drag activation/cancel, threshold, and pending node resize behavior green | Continue isolating `pointer_move_dispatch/primary/connection.rs`, then replace `pointer_move_dispatch/primary.rs`. |
| Pointer-move primary connection seam | retained primary pointer-move routing still owns the wrapper, but the connection branch only needs wire-drag move host/bounds/paint invalidation plus pending edge-insert paint invalidation | `RBX-M2-780` moves `pointer_move_dispatch/primary/connection.rs`, `pending_wire_drag.rs`, `wire_drag/move_update/**`, and edge-insert move helpers onto `WireDragMoveCx` / `WidgetPaintInvalidationCx`, source-policy gates those files, and keeps wire hover/threshold plus pending edge-insert threshold behavior green | Replace `pointer_move_dispatch/primary.rs` now that surface, group, node, and connection branches have retained-agnostic seams, or isolate secondary/overlay pointer-move branches first if preferred. |
| Pointer-move primary wrapper seam | retained pointer-move dispatch still owns secondary/overlay/fallback branches, but primary dispatch only composes already-isolated surface, group, node, and connection branch seams | `RBX-M2-790` moves `pointer_move_dispatch/primary.rs` onto `PrimaryPointerMoveCx`, source-policy gates the wrapper and composed capability, and keeps representative marquee, group, node, and connection behavior green | Continue isolating `pointer_move_dispatch.rs`, secondary pointer-move routing, overlay pointer-move routing, or hover fallback before replacing the retained event widget. |
| Pointer-move secondary node seam | retained secondary pointer-move dispatch still owns connection/insert branches, but the node branch only needs node-resize host access and node-drag geometry/preview/tail plus bounds access | `RBX-M2-800` moves `pointer_move_dispatch/secondary/node.rs`, `node_drag.rs`, and `node_resize/move_update.rs` onto `NodeDragMoveCx + NodeResizeMoveCx`, source-policy gates those files, and keeps node drag move plus node resize behavior green | Continue isolating `pointer_move_dispatch/secondary/connection.rs` and other secondary branches, then replace the secondary wrapper and top-level pointer-move dispatch. |
| Pointer-move secondary connection seam | retained secondary pointer-move dispatch still owns the insert branch and wrapper, but the connection branch only needs active wire move, active edge-insert move, and edge reconnect drag movement capabilities | `RBX-M2-810` moves `pointer_move_dispatch/secondary/connection.rs` and `edge_drag/**` move helpers onto `WireDragMoveCx + EdgeDragMoveCx`, source-policy gates those files, and keeps wire/edge-insert move plus edge reconnect threshold/cancel/radius behavior green | Continue isolating `pointer_move_dispatch/secondary/insert.rs`, then replace the secondary wrapper and top-level pointer-move dispatch. |
| Pointer-move secondary insert seam | retained secondary pointer-move dispatch still owns the wrapper, but the insert branch only needs pending insert-node drag pointer/window/bounds/tick/host access plus release/invalidation | `RBX-M2-820` moves `pointer_move_dispatch/secondary/insert.rs`, `insert_node_drag/mod.rs`, `insert_node_drag/pending.rs`, and `insert_node_drag/session.rs` onto `InsertNodeDragMoveCx`, source-policy gates those files, and keeps insert-node drag threshold/start/searcher-cleanup behavior green. The retained internal drag event entry is isolated in `insert_node_drag/internal_event.rs`. | Replace `pointer_move_dispatch/secondary.rs` now that node, connection, and insert branches have retained-agnostic seams, then continue toward top-level pointer-move dispatch. |
| Pointer-move secondary wrapper seam | retained pointer-move dispatch still owns overlay/fallback branches, but secondary dispatch only composes already-isolated node, connection, and insert branch seams | `RBX-M2-830` moves `pointer_move_dispatch/secondary.rs` onto `SecondaryPointerMoveCx`, source-policy gates the wrapper and composed capability, and keeps representative node drag, edge reconnect, and insert-node drag behavior green | Continue isolating `pointer_move_dispatch/overlay.rs` or the hover fallback, then replace top-level `pointer_move_dispatch.rs`. |
| Pointer-move overlay route seam | retained pointer-move dispatch still owns the fallback hover branch, but overlay dispatch only needs searcher/context-menu hover updates plus paint invalidation | `RBX-M2-840` moves `pointer_move_dispatch/overlay.rs`, `searcher::handle_searcher_pointer_move`, and `context_menu::handle_context_menu_pointer_move` onto `WidgetPaintInvalidationCx`, source-policy gates the overlay route, and keeps searcher/context-menu hover invalidation behavior green | Continue isolating the hover fallback, then replace top-level `pointer_move_dispatch.rs`. |
| Pointer-move hover fallback seam | retained top-level pointer-move dispatch still owns the wrapper, but fallback edge/anchor hover only needs host access plus paint invalidation | `RBX-M2-850` moves `hover.rs` onto `HoverMoveCx`, source-policy gates the fallback, and keeps real edge hover invalidation behavior green | Replace top-level `pointer_move_dispatch.rs` now that primary, secondary, overlay, and hover fallback branches have retained-agnostic seams. |
| Pointer-move top-level wrapper seam | retained pointer-move event entry still owns event parsing/tail cursor/timer release paths, but the route dispatcher only composes already-isolated branch seams | `RBX-M2-860` moves `pointer_move_dispatch.rs` onto `PointerMoveCx`, source-policy gates the wrapper and composed capability, and keeps representative primary/secondary/overlay/hover behavior green | Continue isolating `event_pointer_move_tail.rs`, `event_pointer_move.rs`, or sibling pointer-move release/timer/cursor routes before replacing the retained event widget. |
| Pointer-move cursor update seam | retained pointer-move tail still owns timer sync, but cursor updates only need host access plus cursor icon side effects | `RBX-M2-870` moves `cursor.rs` and `event_pointer_move_tail/cursor.rs` onto `CanvasCursorCx`, source-policy gates cursor update helpers, and keeps close-button pointer cursor behavior green | Continue isolating `event_pointer_move_tail/timer.rs`, then replace the pointer-move tail wrapper. |
| Pointer-move release route seam | retained pointer-move event entry still owns event parsing, but release guards now only need the existing pointer-up and pan-begin capabilities | `RBX-M2-910` moves `event_pointer_move.rs`, `event_pointer_move/release.rs`, `event_pointer_move/tail.rs`, `pointer_move_release.rs`, `pointer_move_release_pan.rs`, and `pointer_move_release_pan/**` onto `PointerMoveReleaseCx` / `PointerMoveTailCx`, source-policy gates the route, and keeps missed pan release plus right-button pan behavior green | Continue isolating the surrounding pointer-move event router/button move entry, or move on to remaining pointer-down/wheel/timer event routes before replacing the retained event widget. |
