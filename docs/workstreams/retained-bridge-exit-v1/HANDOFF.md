...
`RBX-M2-560` then moved the context menu top-level escape/key/pointer route wrappers behind the
retained-agnostic `ContextMenuCx` capability composed from the existing key-down and pointer-down
seams. `context_menu/mod.rs`, `context_menu/input.rs`, and `context_menu/pointer.rs` are now
...
`RBX-M2-580` then moved context menu action activation routing behind the retained-agnostic
...
`context_menu/activate/target.rs` are now source-policy gated as retained-bridge-free support;
retained command dispatch, group selection sync, and target-specific executor calls live in
`context_menu/activate/retained_cx.rs`. Focused tests prove command selection-before-dispatch,
non-command target delegation, and ignored-action no-op behavior.
`RBX-M2-590` then moved background insert context-menu execution behind the retained-agnostic
`BackgroundInsertMenuCx` seam for host/window access. `context_menu/background_execution.rs` plus
its activate/apply helpers are now source-policy gated as retained-bridge-free support; retained
host/window I/O lives in `context_menu/background_execution/retained_cx.rs`. Focused tests prove
missing-candidate no-op handling, non-candidate ignored behavior, and rejection-toast behavior with
recent-kind recording.
`RBX-M2-600` then moved edge context-menu execution behind the retained-agnostic
`EdgeContextActionCx` seam for host/window access plus edge insert menu opening.
`context_menu/edge_execution.rs` plus its open-insert/reroute/delete/custom helpers are now
source-policy gated as retained-bridge-free support; retained host/window/open-insert I/O lives in
`context_menu/edge_execution/retained_cx.rs`. Focused tests prove edge insert menu delegation,
delete-edge graph/selection updates, reroute insertion, custom presenter ops, and ignored
non-edge actions.
`RBX-M2-610` then moved connection insert context-menu execution behind the retained-agnostic
`ConnectionInsertMenuCx` seam for host/window access plus wire-drag resume/restore I/O.
`context_menu/connection_execution_insert.rs` plus its activate/apply/recovery helpers are now
source-policy gated as retained-bridge-free support; retained host/window/capture/recovery I/O
lives in `context_menu/connection_execution_insert/retained_cx.rs`. Focused tests prove missing
candidate no-op handling, non-candidate ignored behavior, rejected candidate toast plus restore,
successful resume, and ignore restore.
`RBX-M2-620` then moved connection conversion context-menu execution behind the retained-agnostic
`ConnectionConversionMenuCx` seam for host/window access plus wire-drag restoration I/O.
`context_menu/connection_execution_conversion.rs` plus its activate/apply helpers are now
source-policy gated as retained-bridge-free support; retained host/window/capture/recovery I/O
lives in `context_menu/connection_execution_conversion/retained_cx.rs`. Focused tests prove
...
`RBX-M2-640` then moved the right-click context-menu pointer-down/up route behind the
retained-agnostic `RightClickCx` seam, composed from the existing `ContextMenuOpeningCx` and
`PointerCaptureReleaseCx` capabilities. `right_click.rs` and `right_click/pending.rs` are now
source-policy gated as retained-bridge-free support. Focused tests prove pending pointer-up ignores
...
`RBX-M2-660` then moved pointer-up release routing and state helpers behind the retained-agnostic
`PointerUpReleaseCx` seam plus widget-tail paint invalidation. `pointer_up/release.rs`,
`pointer_up_state/release.rs`, and `pointer_up_release_cx.rs` are now source-policy gated as
retained-bridge-free release support, while retained `EventCx` adaptation is isolated in
...
behavior green.
`RBX-M2-690` then moved the left pointer-up pending release dispatch chain behind the
retained-agnostic `PendingNodeDragReleaseCx` seam. `pointer_up_left_route/dispatch/pending.rs` is
now source-policy gated as retained-bridge-free support. Focused real-route tests prove pending
group drag/resize release, pending node click-select release, and pending wire-drag release
promotion still work through `pointer_up::handle_pointer_up`.
`RBX-M2-700` then moved the left pointer-up active release dispatch chain behind
`WireCommitCx + PointerUpReleaseCx`. `pointer_up_left_route/dispatch/active.rs` is now
...
`RBX-M2-720` then moved the pointer-up event entry behind the composed
retained-agnostic `PointerUpRouteCx` capability. `event_pointer_up.rs` no longer names retained
bridge Cx types, and `PointerUpGuardCx` is widget-internal so the composed route seam does not
trigger private-bound warnings. The upper `event_router_pointer_button/up.rs` parser is still a
retained event route and remains a later isolation slice. Focused tests prove guard dispatch,
marquee release, pending node click-select release, edge reconnect release, and edge-drag left-up
cleanup remain green.
`RBX-M2-730` then moved the `PointerEvent::Up` parser/forwarder in
`event_router_pointer_button/up.rs` behind `PointerUpRouteCx`. The upper button router plus its
pointer-down and pointer-move branches remain retained-bound for later route isolation slices, but
the pointer-up leaf route no longer names retained bridge Cx types. Focused tests again prove guard
dispatch, marquee release, pending node click-select release, edge reconnect release, and edge-drag
left-up cleanup remain green.
`RBX-M2-740` then moved pan-zoom panning move behind the retained-agnostic `PanZoomCx` seam.
`PanZoomBeginCx` is now only the begin/capture extension over shared host/bounds/paint
capabilities, and retained `EventCx` adaptation remains isolated in `pan_zoom_begin_retained_cx.rs`.
Focused tests prove middle-mouse panning, space-to-pan panning, move start/end callbacks, and pan
inertia end behavior remain green.
`RBX-M2-750` then moved the primary surface pointer-move route in
`pointer_move_dispatch/primary/surface.rs` behind `MarqueeCx`, reusing the existing pan and marquee
...
`RBX-M2-790` then moved the primary pointer-move wrapper in
`pointer_move_dispatch/primary.rs` behind `PrimaryPointerMoveCx`, a composed capability over the
already-isolated surface, group, node, and connection branch seams. This adds no new side-effect
methods; retained `EventCx` only satisfies the wrapper through existing branch adapters. Focused
tests prove the wrapper source policy plus representative marquee, group, node, and connection
behavior remain green.
`RBX-M2-800` then moved the secondary node pointer-move route in
`pointer_move_dispatch/secondary/node.rs` behind `NodeDragMoveCx + NodeResizeMoveCx`.
`NodeDragMoveCx` composes the existing node drag geometry, preview, and move-tail seams with bounds
access; `NodeResizeMoveCx` carries host access plus paint invalidation. Retained `EventCx`
...
`RBX-M2-820` then moved the secondary insert pointer-move route in
`pointer_move_dispatch/secondary/insert.rs` behind `InsertNodeDragMoveCx`. Pending insert-node drag
movement now uses retained-agnostic pointer/window/bounds/tick/host access plus release and paint
invalidation seams, and retained `EventCx` adaptation is isolated in
`insert_node_drag_move_retained_cx.rs`. The still-retained internal drag enter/over/drop entry moved
...
behavior remain green.
`RBX-M2-840` then moved the overlay pointer-move route in `pointer_move_dispatch/overlay.rs` behind
the existing `WidgetPaintInvalidationCx` seam. Searcher and context-menu pointer-move facades now
require only paint invalidation, matching the hover-update leaf helpers they call, and no longer pull
the broader key/down/up route traits into overlay dispatch. Focused tests prove overlay source
policy plus searcher/context-menu hover invalidation behavior remain green.
`RBX-M2-850` then moved the fallback edge/anchor hover pointer-move path in `hover.rs` behind
...
`RBX-M2-880` then moved pointer-move auto-pan timer sync in
`event_pointer_move_tail/timer.rs` behind `AutoPanTimerCx`, a narrow host/window/bounds seam.
Retained `EventCx` adaptation is isolated in `auto_pan_timer_retained_cx.rs`, and a retained
compatibility test proves pointer-move node dragging near the viewport edge still starts a
repeating auto-pan timer.
...
- Continue M2 by shrinking the RBX-M2-080 ledger. The retained controls widget is now gone; the
  ...
  context menu opening routes now use the retained-agnostic `ContextMenuOpeningCx` seam; context
  menu action activation routing now uses retained-agnostic command/target action seams; context
  menu background execution now uses retained-agnostic host/window seams; context menu edge
  ...
  `AutoPanTimerCx`; and the pointer-up event entry and pointer-up button router now use
  `PointerUpRouteCx`.
  ...
  `primary_pointer_move_cx.rs`, `pointer_move_dispatch/primary.rs`,
  ...
  `context_menu/activate.rs`, `context_menu/activate/command.rs`,
  `context_menu/activate/target.rs`,
  `context_menu/background_execution.rs`, `context_menu/background_execution/activate.rs`,
  `context_menu/background_execution/apply.rs`,
  `context_menu/connection_execution_insert.rs`,
  `context_menu/connection_execution_insert/activate.rs`,
  `context_menu/connection_execution_insert/apply.rs`,
  `context_menu/connection_execution_insert/recovery.rs`,
  `context_menu/connection_execution_conversion.rs`,
  `context_menu/connection_execution_conversion/activate.rs`,
  `context_menu/connection_execution_conversion/apply.rs`,
  `context_menu/edge_execution.rs`, `context_menu/edge_execution/open_insert.rs`,
  `context_menu/edge_execution/reroute.rs`, `context_menu/edge_execution/delete.rs`,
  `context_menu/edge_execution/custom_action.rs`,
  ...
  `event_pointer_up.rs`, `event_pointer_up/dispatch.rs`,
  `event_router_pointer_button/up.rs`,
...
Last run on 2026-05-22 for `RBX-M2-880`:
...
Previous run on 2026-05-22 for `RBX-M2-870`:
...
Previous run on 2026-05-21 for `RBX-M2-840`:
...
Previous run on 2026-05-21 for `RBX-M2-830`:
...
Previous run on 2026-05-21 for `RBX-M2-810`:
...
Previous run on 2026-05-21 for `RBX-M2-790`:
...
Previous run on 2026-05-21 for `RBX-M2-780`:
