...
## F257: Checkbox required/disabled group gate confirms item-local action-state ownership

Context:

- The previous RadioGroup grouped-control gate covered the owner split where required semantics live
  on the group root and disabled action-state lives on individual radio items.
- Checkbox needed the complementary grouped-control path: required semantics stay on each concrete
  checkbox while the surrounding fieldset/field-group shell, labels, and disabled row chrome remain
  caller-owned composition.

What was found:

- No Checkbox recipe/runtime defect was reproduced. The current shadcn Checkbox recipe exposes the
  required, disabled, checked-state, focus, and invoke semantics needed by the grouped-control path.
- Disabled label forwarding behaved correctly: clicking the disabled Usage analytics checkbox or its
  associated `FieldLabel::for_control(...)` label does not toggle it, while enabled sibling labels
  still toggle their concrete checkbox controls.

What was changed:

- Added a Checkbox page `Required Disabled Group` example with stable concrete checkbox and label
  test ids for Backups, Usage analytics, and Beta updates.
- Added `tools/diag-scripts/ui-gallery/checkbox/ui-gallery-checkbox-required-disabled-group-action-state.json`.
- Added root redirect `tools/diag-scripts/ui-gallery-checkbox-required-disabled-group-action-state.json`.
- Promoted the script into `ui-gallery-checkbox-semantics` and into
  `ui-gallery-shadcn-runtime-evidence`.
- Refreshed `tools/diag-scripts/index.json` and added protocol script roundtrip coverage.
...
- New Gallery example:
  ...
  `apps/fret-ui-gallery/src/ui/snippets/checkbox/mod.rs`, and
...
## F258: ToggleGroup disabled-item gate found missing explicit checked_state semantics

Context:

- The previous recommendation was to continue concrete action-state coverage into a different owner
  split after Checkbox and RadioGroup grouped-control gates.
- ToggleGroup was the highest-value next target because single mode combines radio-button semantics,
  roving focus, disabled item skipping, label-to-group focus, and recipe-owned item action-state.

What was found:

- Disabled ToggleGroup item behavior and roving focus were already correct: direct click on the
  disabled Beta item does not change selection, and ArrowRight from Alpha skips Beta and focuses
  Gamma while leaving Alpha selected.
- A semantics completeness gap was confirmed: single-mode ToggleGroup items exported the legacy
  binary `checked` flag but not explicit `checked_state=true|false`. That meant diagnostics could
  not assert the structured checked-state predicate on concrete ToggleGroup radio-button items.
- A first runtime-script draft also exposed a diagnostics authoring hazard: proving an enabled
  Gamma click mutation at the end of the same disabled-item script was not part of the invariant and
  could time out under suite scheduling. The final gate keeps the scope tight around disabled action
  suppression, roving skip, and explicit checked-state export.

What was changed:

- `toggle_group_item_a11y_single(...)` in `fret-ui-kit` now stamps explicit
  `checked_state=true|false` while preserving the legacy `checked` flag.
- Added a focused shadcn ToggleGroup unit test that proves ArrowRight skips a disabled item and that
  Alpha, Beta, and Gamma all export explicit checked-state on their radio-button semantics nodes.
- Added a Toggle Group page `Disabled Item Action-State (Fret)` example with stable item and label
  test ids.
- Added `tools/diag-scripts/ui-gallery/toggle/ui-gallery-toggle-group-disabled-item-action-state.json`.
- Added root redirect `tools/diag-scripts/ui-gallery-toggle-group-disabled-item-action-state.json`.
- Promoted the script into `ui-gallery-toggle-semantics` and into
  `ui-gallery-shadcn-runtime-evidence`.
- Refreshed `tools/diag-scripts/index.json` and added protocol script roundtrip coverage.
...
- Semantics fix:
  `ecosystem/fret-ui-kit/src/primitives/toggle_group.rs` and
...
- New Gallery example:
  ...
  `apps/fret-ui-gallery/src/ui/snippets/toggle_group/mod.rs`, and
...
- Move the action-state axis into Menu/Command disabled-item coverage where active-descendant or
  roving-focus policy, collection metadata, disabled semantics, and invoke suppression can mutate
  independently.

## F259: Menubar disabled-item gate found roving collection-index and current-focus semantics defects

Context:

- The previous ToggleGroup slice moved the action-state axis into roving-focus widgets and found a
  structured checked-state gap.
- Menubar was the next high-value target because it combines flattened menu entries, separators,
  submenu triggers, disabled item policy, command dispatch, and programmatic roving focus.

What was found:

- The Menubar recipe had a real roving-focus indexing defect. Content and submenu rows compared
  `active` (a collection item index derived after filtering out labels/separators) against the
  flattened entry index. Any separator or label before an item could shift the exported focusable
  row and leave the true focused row without a semantics `focus` action.
- The core `fret-ui` Pressable semantics bridge had a mechanism-level current-focus defect. A
  Pressable with `focusable=false` correctly stayed out of default Tab traversal, but when roving
  focus moved programmatic focus onto that node, exported semantics still had `actions.focus=false`.
  That made the current roving focus target look non-focusable to diagnostics/accessibility even
  though it was focused.
- The disabled Menubar item action-state itself behaved correctly after the fixes: the disabled
  item exposes `disabled=true`, has no focus/invoke actions, direct click does not dispatch its
  command, and ArrowDown skips it.

What was changed:

- Menubar content and submenu rendering now compare active state against `collection_index` instead
  of flattened entry index.
- `fret-ui` Pressable semantics now exports `focus` for enabled pressables when either the pressable
  is a default traversal stop or it is the current focus target. This keeps roving-only items out of
  Tab order while making the current roving focus target semantically focusable.
- Added a Menubar demo command on the disabled `New Incognito Window` row so the runtime gate can
  prove disabled command dispatch suppression via the UI Gallery status bar.
- Added `tools/diag-scripts/ui-gallery/menubar/ui-gallery-menubar-disabled-item-action-state.json`.
- Added root redirect `tools/diag-scripts/ui-gallery-menubar-disabled-item-action-state.json`.
- Added dedicated suite `tools/diag-scripts/suites/ui-gallery-menubar-semantics/suite.json` and
  promoted the script into `ui-gallery-shadcn-runtime-evidence`.
- Refreshed `tools/diag-scripts/index.json` and added protocol script roundtrip coverage.
...
- Mechanism fix:
  `crates/fret-ui/src/declarative/host_widget/semantics.rs` and
...
- Gallery wiring:
  ...
  `apps/fret-ui-gallery/src/spec.rs`, and
...
- Continue the action-state axis into ContextMenu disabled command items or Command disabled items,
  where dynamic collection metadata, active-descendant/roving focus, disabled semantics, and invoke
  suppression can drift independently.
## F260: ContextMenu disabled item gate confirms action-state ownership and exposes transition-lint debt

Context:

- The previous Menubar slice found real roving-index and current-focus semantics defects in a
  menu-like widget with flattened entries and command dispatch.
- ContextMenu was the next high-value comparison point because it uses menu content, disabled command
  items, collection metadata, pointer opening, and vertical roving focus without the Menubar's top
  bar/submenu ownership split.

What was found:

- No ContextMenu recipe/runtime defect was reproduced for disabled command-item ownership. The
  disabled `Forward` row exports `disabled=true`, suppresses both `focus` and `invoke`, preserves
  collection position metadata, does not dispatch its attached command on pointer click, and is
  skipped by ArrowDown roving focus.
- A separate diagnostics lifecycle concern was reproduced in a default-lint dedicated suite: the
  script row passes, then suite lint can see duplicate `ui-gallery-context-menu-basic-content` nodes
  because transition captures may retain a previous `role=menu` snapshot long enough to collide with
  the current menu content. The focused run and row-only suite with `--no-suite-lint` pass.
- Pointer-open followed by ArrowDown from content still lacks a promoted deterministic runtime gate;
  the new script focuses `Back` explicitly before proving the disabled-row roving skip. Treat this as
  a follow-up keyboard-entry/lifecycle gap, not as part of the disabled action-state invariant.

What was changed:

- Added a focused shadcn ContextMenu unit test for disabled item action-state, collection metadata,
  roving-focus skip, and command-dispatch suppression.
- Added an authoring-surface test that keeps the Basic example's disabled `Forward` command row on a
  real `ContextMenuItem::disabled(true)` control rather than a gallery-local interactivity gate.
- Added `tools/diag-scripts/ui-gallery/context-menu/ui-gallery-context-menu-disabled-item-action-state.json`.
- Added root redirect `tools/diag-scripts/ui-gallery-context-menu-disabled-item-action-state.json`.
- Added dedicated suite `tools/diag-scripts/suites/ui-gallery-context-menu-semantics/suite.json` and
  promoted the script into the existing ContextMenu and broad shadcn runtime suites.
- Refreshed `tools/diag-scripts/index.json` and added protocol script roundtrip coverage.
...
- Dedicated row-only runtime suite:
...
- Default-lint suite probe:
  row execution passed, then suite lint failed with duplicate `ui-gallery-context-menu-basic-content`
  while a previous `role=menu` transition snapshot was still present. Artifact:
  `target/fret-diag-context-menu-semantics-suite-disabled-item-v2/sessions/1779387897940-40144/1779387958229-ui-gallery-context-menu-disabled-item-action-state/check.lint.json`.

Next slice recommendation:

- First investigate whether the ContextMenu duplicate-content transition lint and pointer-open
  ArrowDown entry behavior share a lifecycle/root cause. If not, keep the action-state axis moving by
  adding a Command disabled active-descendant mutation gate.

## F261: ContextMenu pointer-open keyboard-entry gate found stale panel focus targets and duplicate-id debt

Context:

- F260 left two narrow follow-ups: the default-lint ContextMenu suite could report a duplicate
  `ui-gallery-context-menu-basic-content` test id, and pointer-open followed by ArrowDown from the
  focused menu content did not yet have a promoted runtime gate.
- The Basic snippet was a useful stress path because the DocSection wrapper and the overlay panel
  previously shared the same test id, while the runtime keyboard path depends on the panel key
  handler remembering the first/last enabled item ids across overlay reuse.

What was found:

- The duplicate-id finding was a real diagnostics-authoring defect, not proof that transition
  captures necessarily duplicate menu content. The page content wrapper already used
  `ui-gallery-context-menu-basic-content`; the open ContextMenu panel reused the same id. Renaming
  the overlay panel to `ui-gallery-context-menu-basic-panel` makes the default-lint ContextMenu
  semantics suite pass without `--no-suite-lint`.
- The pointer-open ArrowDown failure was a recipe lifecycle defect, not a `fret-ui` key-routing
  defect. A focused `fret-ui` test proves focused non-modal, hit-test-inert overlay layers still
  receive key events. Runtime evidence showed key dispatch reached the panel key hook, but the hook
  saw empty first/last item targets.
- The cause was that top-level panel entries were rendered through an inline branch instead of the
  same `ContextMenuContentRenderEnv` path used by nested rendering. That bypassed the persistent
  first/last enabled item target models that the panel key handler reads at event time.
- The first script-level action assertion used the visible status label as an oracle. The failure
  bundle already contained `/shell/last_action = ui_gallery.context_menu.basic.reload`, while the
  status label was absent in that snapshot. The durable oracle is therefore the app snapshot field,
  not a shell status-bar visibility dependency.

What was changed:

- `apps/fret-ui-gallery/src/ui/snippets/context_menu/basic.rs` now uses
  `ui-gallery-context-menu-basic-panel` for the overlay panel.
- ContextMenu panel rendering now routes top-level entries through `ContextMenuContentRenderEnv`.
  Enabled item, checkbox item, and radio item render paths record first/last focus targets into
  persistent local models with cell fallback for cached render paths.
- Added shadcn unit tests for pointer-open ArrowDown entering the first enabled normal item and the
  first enabled checkbox/radio item.
- Added a focused `fret-ui` key-routing regression proving focused non-modal occluding overlays keep
  keyboard routing out of the underlay even when hit-test inert.
- Hardened ContextMenu Basic diagnostics scripts to use the unique panel id and app-snapshot
  last-action predicates, and promoted `ui-gallery-context-menu-basic-keyboard-nav.json` into the
  `ui-gallery-context-menu` suite.
- Added protocol roundtrip coverage for the Basic keyboard, right-click, touch long-press, and
  overlay-placement scripts.

Evidence anchors:

- Recipe fix and unit gates:
  `ecosystem/fret-ui-shadcn/src/context_menu.rs`.
- Mechanism guard:
  `crates/fret-ui/src/tree/tests/key_dispatch_barrier_root.rs`.
- Gallery selector fix and teaching-surface gate:
  `apps/fret-ui-gallery/src/ui/snippets/context_menu/basic.rs` and
...
- Scripts and suite:
  ...
  `tools/diag-scripts/suites/ui-gallery-context-menu/suite.json`, and
  `tools/diag-scripts/index.json`.
- Protocol script roundtrip:
  `crates/fret-diag-protocol/tests/script_json_roundtrip.rs`.
...
- Continue into Command disabled active-descendant mutation coverage. This is the highest-value
  remaining action-state comparison point because Command uses active-descendant rather than
  roving focus, and command filtering can independently desynchronize selection, disabled
  semantics, and invoke suppression.
