...
- UI Gallery: add a code-editor “Import local fonts…” action (file dialog → memory `AssetRequest`s
  + `Effect::TextAddFontAssets`) and a soft-wrap toggle for wrap-boundary regression checks.
