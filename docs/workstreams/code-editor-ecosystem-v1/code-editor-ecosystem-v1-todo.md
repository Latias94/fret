...
- [x] Load bundled default fonts during web renderer adoption (avoid “first frame” tofu; keep the
  user-provided import escape hatch on asset-backed runtime requests).
...
- [~] Soft wrap with stable coordinate mapping (buffer ↔ display ↔ pixels) (column-based baseline; pixel-accurate caret/selection/hit-test is migrating to renderer caret stops).
  - [x] Add wrap-boundary semantics regression gates (UI Gallery harness + fretboard-dev diag):
...
This milestone is defined in `docs/workstreams/code-editor-ecosystem-v1/code-editor-ecosystem-v1.md` (“Downstream Milestone:
Markdown Editor v0”).
