...
- Started converting shadcn recipe callsites to `Theme::snapshot()` where only token reads are needed.
  - Evidence: `ecosystem/fret-ui-shadcn/src/{alert_dialog,avatar,badge,button,button_group,combobox,command,data_table,dialog,dropdown_menu,native_select,popover,select,sheet}.rs`
