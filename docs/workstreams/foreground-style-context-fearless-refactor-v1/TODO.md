...
| Form / Button | `fret-ui-material3/button` | `ecosystem/fret-ui-material3/src/button.rs` | `fret-ui-material3` | Natural-width button labels inside shrink-wrap chrome and optional icon slots | Long button labels may still need a constrained-slot-specific shrink audit, but blindly forcing fill width would risk breaking intrinsic button sizing | P2 | `[~]` | Audit follow-up pending | Triaged: keep this as the next lower-priority follow-up after the chip family and wait for a concrete constrained-slot repro before changing the default button label contract |
