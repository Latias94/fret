...
| Navigation / Drawer | `fret-ui-material3/navigation_drawer` | `ecosystem/fret-ui-material3/src/navigation_drawer.rs` | `fret-ui-material3` | Drawer labels beside optional badges inside the item row | Long drawer labels can overflow the badge row unless the left content slot and label text opt into fill width + `min-w-0` | P1 | `[x]` | `navigation_drawer_labels_can_shrink_beside_badges` | Audited: drawer labels now shrink beside optional badges because the left content slot and label text both opt into fill width + `min_w_0()` |
