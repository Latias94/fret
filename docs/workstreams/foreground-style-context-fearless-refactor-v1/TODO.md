...
| Overlay / Listbox | `fret-ui-material3/select listbox item` | `ecosystem/fret-ui-material3/src/select.rs` | `fret-ui-material3` | One-line and two-line listbox item text inside icon-bearing option rows | Long select item labels and supporting text can overflow listbox rows unless the item body slot and text surfaces opt into shrink-friendly fill width + `min-w-0` | P1 | `[x]` | `select_menu_item_labels_can_shrink_within_listbox_rows` + `select_menu_item_supporting_text_can_shrink_within_two_line_rows` | Audited: select listbox item labels and supporting text now keep clip semantics while shrinking inside icon-bearing one-line and two-line rows via explicit fill width + `min_w_0()` on the text surfaces and body slot |
| Navigation / List | `fret-ui-material3/list` | `ecosystem/fret-ui-material3/src/list.rs` | `fret-ui-material3` | One-line item labels between leading/trailing icon slots | Long list item labels can overflow icon rows unless the label text itself opts into fill width + `min-w-0` | P1 | `[x]` | `list_item_labels_can_shrink_between_icon_slots` | Audited: list item labels now keep one-line clip semantics while shrinking between icon slots via explicit fill width + `min_w_0()` |
| Navigation / Drawer | `fret-ui-material3/navigation_drawer` | `ecosystem/fret-ui-material3/src/navigation_drawer.rs` | `fret-ui-material3` | Drawer labels and optional badge text inside the item row | Long drawer labels and even unusually long badge text can overflow the item row unless both sides opt into shrink-friendly constraints | P1 | `[x]` | `navigation_drawer_labels_can_shrink_beside_badges` + `navigation_drawer_badges_can_shrink_beside_labels` | Audited: drawer labels and optional badge text now both shrink safely within the item row via explicit fill width / `min_w_0()` on the left slot and `min_w_0()` + `flex_shrink(1.0)` on the badge text |
| Navigation / Tabs | `fret-ui-material3/tabs` | `ecosystem/fret-ui-material3/src/tabs.rs` | `fret-ui-material3` | Primary tab labels inside equal-width tab slots | Long tab labels can overflow non-scrollable tab slots unless the label text opts into fill width + `min-w-0` | P1 | `[x]` | `primary_tab_labels_can_shrink_within_equal_width_slots` | Audited: primary tab labels now keep one-line clip semantics while shrinking within equal-width tab slots via explicit fill width + `min_w_0()` |
| Overlay / Menu | `fret-ui-material3/menu` | `ecosystem/fret-ui-material3/src/menu.rs` | `fret-ui-material3` | One-line menu item labels inside fixed-height menu rows | Long menu item labels can overflow menu rows unless the label text opts into fill width + `min-w-0` | P1 | `[x]` | `menu_item_labels_can_shrink_within_menu_rows` | Audited: menu item labels now keep one-line clip semantics while shrinking within fixed-height menu rows via explicit fill width + `min_w_0()` |
| Navigation / Bottom Bar | `fret-ui-material3/navigation_bar` | `ecosystem/fret-ui-material3/src/navigation_bar.rs` | `fret-ui-material3` | Bottom-nav labels inside equal-width item slots | Long navigation bar labels can overflow item slots unless the label text opts into fill width + `min-w-0` | P1 | `[x]` | `navigation_bar_labels_can_shrink_within_item_slots` | Audited: navigation bar labels now keep one-line clip semantics while shrinking within equal-width bottom-nav item slots via explicit fill width + `min_w_0()` |
| Navigation / Rail | `fret-ui-material3/navigation_rail` | `ecosystem/fret-ui-material3/src/navigation_rail.rs` | `fret-ui-material3` | Rail labels inside fixed-width item slots | Long navigation rail labels can overflow item slots unless the label text opts into fill width + `min-w-0` | P1 | `[x]` | `navigation_rail_labels_can_shrink_within_item_slots` | Audited: navigation rail labels now keep one-line clip semantics while shrinking within fixed-width rail item slots via explicit fill width + `min_w_0()` |
| Overlay / Calendar | `fret-ui-material3/date_picker` | `ecosystem/fret-ui-material3/src/date_picker.rs` | `fret-ui-material3` | Month title between previous/next navigation buttons plus the modal panel headline inside fixed dialog width | Month titles and modal panel headlines can overflow unless the title text opts into fill width + `min-w-0` inside the constrained header/dialog slot | P1 | `[x]` | `date_picker_month_titles_can_truncate_between_nav_buttons` + `date_picker_modal_titles_can_truncate_within_dialog_width` | Audited: date picker month titles and modal panel headlines now keep ellipsis semantics within their constrained horizontal slots via explicit fill width + `min_w_0()` |
| Overlay / Time | `fret-ui-material3/time_picker` | `ecosystem/fret-ui-material3/src/time_picker.rs` | `fret-ui-material3` | Dialog title beside the mode toggle button | Time picker titles can overflow the header row unless the title text opts into fill width + `min-w-0` beside the mode toggle | P1 | `[x]` | `time_picker_titles_can_truncate_beside_mode_toggle` | Audited: time picker titles now keep ellipsis semantics beside the mode toggle via explicit fill width + `min_w_0()` |
| Form / Segmented | `fret-ui-material3/segmented_button` | `ecosystem/fret-ui-material3/src/segmented_button.rs` | `fret-ui-material3` | Equal-width segment labels beside optional icon slots | Long segment labels can overflow segment slots unless the label text opts into fill width + `min-w-0` | P1 | `[x]` | `segmented_button_labels_can_shrink_within_equal_width_segments` | Audited: segmented button labels now keep centered one-line clip semantics within equal-width segment slots via explicit fill width + `min_w_0()` |
| Form / Chip | `fret-ui-material3/chip` | `ecosystem/fret-ui-material3/src/chip.rs` | `fret-ui-material3` | Assist-chip label beside the optional leading icon inside natural-width chip chrome | Long assist-chip labels can still overflow unless the label text opts into `min-w-0` while preserving shrink-wrap sizing | P1 | `[x]` | `assist_chip_labels_can_shrink_beside_leading_icon` | Audited: assist-chip labels now keep one-line ellipsis semantics while shrinking beside the leading icon via explicit `min_w_0()` + `flex_shrink(1.0)` without forcing fill width |
| Form / Chip | `fret-ui-material3/suggestion_chip` | `ecosystem/fret-ui-material3/src/suggestion_chip.rs` | `fret-ui-material3` | Suggestion-chip label beside the optional leading icon inside natural-width chip chrome | Long suggestion-chip labels can still overflow unless the label text opts into `min-w-0` while preserving shrink-wrap sizing | P1 | `[x]` | `suggestion_chip_labels_can_shrink_beside_leading_icon` | Audited: suggestion-chip labels now keep one-line ellipsis semantics while shrinking beside the leading icon via explicit `min_w_0()` + `flex_shrink(1.0)` without forcing fill width |
| Form / Chip | `fret-ui-material3/filter_chip` | `ecosystem/fret-ui-material3/src/filter_chip.rs` | `fret-ui-material3` | Filter-chip label between leading/trailing icon slots inside natural-width chip chrome | Long filter-chip labels can still overflow icon rows unless the label text opts into `min-w-0` while preserving shrink-wrap sizing | P1 | `[x]` | `filter_chip_labels_can_shrink_between_icon_slots` | Audited: filter-chip labels now keep one-line ellipsis semantics while shrinking between icon slots via explicit `min_w_0()` + `flex_shrink(1.0)` without forcing fill width |
| Form / Chip | `fret-ui-material3/input_chip` | `ecosystem/fret-ui-material3/src/input_chip.rs` | `fret-ui-material3` | Input-chip label between leading/trailing icon slots inside natural-width chip chrome | Long input-chip labels can still overflow icon rows unless the label text opts into `min-w-0` while preserving shrink-wrap sizing | P1 | `[x]` | `input_chip_labels_can_shrink_between_icon_slots` | Audited: input-chip labels now keep one-line ellipsis semantics while shrinking between icon slots via explicit `min_w_0()` + `flex_shrink(1.0)` without forcing fill width |
| Form / Button | `fret-ui-material3/button` | `ecosystem/fret-ui-material3/src/button.rs` | `fret-ui-material3` | Natural-width button labels inside shrink-wrap chrome and optional icon slots | Long button labels can still overflow icon-bearing buttons unless the label text opts into `min-w-0` while preserving intrinsic button sizing | P1 | `[x]` | `button_labels_can_shrink_between_icon_slots` | Audited: button labels now keep one-line clip semantics while shrinking between icon slots via explicit `min_w_0()` + `flex_shrink(1.0)` without forcing fill width on the button label |
| Form / FAB | `fret-ui-material3/fab` | `ecosystem/fret-ui-material3/src/fab.rs` | `fret-ui-material3` | Extended FAB labels beside the leading icon inside natural-width floating-action chrome | Long extended FAB labels can still overflow unless the label text opts into `min-w-0` while preserving intrinsic extended-FAB sizing | P1 | `[x]` | `extended_fab_labels_can_shrink_beside_icon` | Audited: extended FAB labels now keep one-line clip semantics while shrinking beside the leading icon via explicit `min_w_0()` + `flex_shrink(1.0)` without forcing fill width |
