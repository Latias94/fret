...
| Overlay / Calendar | `fret-ui-material3/date_picker` | `ecosystem/fret-ui-material3/src/date_picker.rs` | `fret-ui-material3` | Month title between previous/next navigation buttons | Month titles can overflow the nav header unless the title text opts into fill width + `min-w-0` inside the header row | P1 | `[x]` | `date_picker_month_titles_can_truncate_between_nav_buttons` | Audited: date picker month titles now keep centered ellipsis semantics between nav buttons via explicit fill width + `min_w_0()` |
