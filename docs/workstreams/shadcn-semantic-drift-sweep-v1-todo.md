...
| NavigationMenu `md:*` behavior | `ecosystem/fret-ui-shadcn/src/navigation_menu.rs` | `repo-ref/ui/apps/v4/registry/bases/radix/ui/navigation-menu.tsx` (`md:absolute`, `md:w-[var(--radix-…)]`) | Mixed (upstream viewport; Fret currently container) | TODO: decide (parity-first viewport vs editor-first container) and likely expose a query-source knob | TODO: add an invariant test once decision lands |
