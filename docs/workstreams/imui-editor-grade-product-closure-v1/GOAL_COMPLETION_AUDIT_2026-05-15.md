...
The 2026-05-17 M19 Wayland acceptance-open guard keeps the same verdict: local source and
policy-skip gates remain useful evidence, but they are not a real Wayland compositor acceptance
run.
...
| Wayland source/admission posture is current | `M15_LOCAL_WAYLAND_BOUNDARY_REFRESH_2026-05-14.md`, `M16_SOURCE_DRIFT_GUARD_2026-05-14.md`, `M17_LOCAL_WAYLAND_POLICY_SKIP_GATE_2026-05-15.md`, `M18_LOCAL_WAYLAND_POLICY_SKIP_MATRIX_2026-05-16.md`, and `M19_WAYLAND_ACCEPTANCE_OPEN_GUARD_2026-05-17.md` cover local source policy, manifest/script drift, the first non-Wayland `skipped_policy` gate, the Windows plus Linux/X11 sidecar matrix, and the guard that this local evidence does not close Wayland compositor acceptance. | Met for local policy gates |
