...
completion/history wrappers, popup-open snapshot readback, expanded-state projection,
open-on-focus behavior, close-when-empty/exact-match behavior, or public facade behavior:

- IMUI text-picker open-policy owner split - 2026-06-09.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/open_policy.rs` keeps the open-policy module
  hub and re-export surface only.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/open_policy/snapshot.rs` owns popup-open
  model readback and popup panel-id snapshot projection.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/open_policy/expanded.rs` owns
  picker-expanded boolean projection.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/open_policy/apply.rs` owns open/close policy
  input and the popup close/open-at mutations.
- No public API or runtime behavior changed; callers still use the existing private
  `open_policy::{TextPickerOpenPolicyInput, apply_text_picker_open_policy,
  read_text_picker_popup_snapshot, text_picker_expanded}` surface.
- The source gate now freezes popup snapshot readback, expanded-state projection, and open/close
  mutation across separate owners while rejecting input-root, popup render, keyboard, response,
  selectable, and facade logic from drifting across open-policy boundaries.
- Evidence anchor: `text_picker_controls/open_policy.rs` declares `mod apply;`, `mod expanded;`,
  and `mod snapshot;`, then re-exports the private open-policy surface.
- Evidence anchor: `open_policy/snapshot.rs` contains only popup-open model and panel-id snapshot
  readback.
- Evidence anchor: `open_policy/expanded.rs` contains only picker-expanded projection.
- Evidence anchor: `open_policy/apply.rs` contains only open-policy input and close/open-at
  mutations.
- Evidence anchor: `tools/gate_imui_workstream_source.py` checks the open-policy hub, snapshot
  owner, expanded owner, apply owner, source inventory, and this workstream evidence boundary.
...
completion/history wrappers, keyboard navigation, enter-to-pick behavior, active-descendant
semantics, popup close-after-keyboard-pick behavior, or public facade behavior:

- IMUI text-picker keyboard owner split - 2026-06-09.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/keyboard.rs` keeps the keyboard module hub
  and re-export surface only.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/keyboard/types.rs` owns keyboard pick,
  state, and snapshot data carriers at the text-picker-only visibility boundary.
- `ecosystem/fret-ui-kit/src/imui/text_picker_controls/keyboard/reconcile.rs` owns keyboard state
  cleanup, stale active-source pruning, pending-pick extraction, and snapshot projection.
- No public API or runtime behavior changed; callers still use the existing
  `keyboard::InputTextPickerKeyboard*` and `keyboard::reconcile_picker_keyboard_state(...)`
  private module surface.
- The source gate now freezes text-picker keyboard data carriers and reconciliation out of the
  keyboard hub while rejecting keydown routing, popup rendering, selectable rows, model commits, and
  facade logic from drifting across keyboard owner boundaries.
- Evidence anchor: `text_picker_controls/keyboard.rs` declares `mod reconcile;` and `mod types;`,
  then re-exports the private keyboard handler, reconcile function, and data carriers.
- Evidence anchor: `keyboard/types.rs` contains only keyboard pick/state/snapshot data carriers.
- Evidence anchor: `keyboard/reconcile.rs` contains only keyboard state reconciliation and snapshot
  projection.
- Evidence anchor: `tools/gate_imui_workstream_source.py` checks the keyboard hub, types owner,
  reconcile owner, source inventory, and this workstream evidence boundary.
...
pressable install, pointer-click modifier capture, response population, or public facade behavior:

- IMUI item behavior options owner split - 2026-06-09.
- `ecosystem/fret-ui-kit/src/imui/item_behavior.rs` keeps shared behavior state shapes,
  install/response re-exports, and `PressableItemResponseInput`.
- `ecosystem/fret-ui-kit/src/imui/item_behavior/options.rs` owns the private
  `PressableItemBehaviorOptions` carrier.
- No public API or runtime behavior changed; the existing
  `item_behavior::PressableItemBehaviorOptions` private call surface remains re-exported through
  the item behavior module for selectable, image-item, and debug-draw callers.
- The source gate now freezes `PressableItemBehaviorOptions` out of the item behavior root and
  rejects install, pointer-hook, model, response, and facade logic from drifting into the options
  owner.
- Evidence anchor: `item_behavior.rs` declares `mod options;` and re-exports
  `options::PressableItemBehaviorOptions` while keeping install/response owner re-exports.
- Evidence anchor: `item_behavior/options.rs` contains only the `report_pointer_click` flag.
- Evidence anchor: `tools/gate_imui_workstream_source.py` checks the item behavior root, options
  owner, source inventory, and this workstream evidence boundary.
...
- `cargo check -p fret-ui-kit --features imui` - passed.
- `cargo nextest run -p fret-ui-kit --features imui selectable --no-fail-fast` - passed, 7/7.
- `cargo nextest run -p fret-ui-kit --features imui debug_draw --no-fail-fast` - passed, 42/42.
- `cargo nextest run -p fret-imui selectable --no-fail-fast` - passed, 6/6.
- `cargo nextest run -p fret-imui image_item --no-fail-fast` - passed, 2/2.
- `python -m py_compile tools\gate_imui_workstream_source.py` - passed.
- `python tools\gate_imui_workstream_source.py` - passed.
- `python -m json.tool docs\workstreams\imui-imgui-gap-closure-v1\WORKSTREAM.json > $null` -
  passed.
...
- IMUI active teaching direct-crate drift guard - 2026-06-03.
- `tools/gate_imui_facade_teaching_source.py` now defines `ACTIVE_FACADE_TEACHING_PATHS` and
  `ACTIVE_FACADE_DIRECT_CRATE_FORBIDDEN`.
- Active cookbook lessons, workbench/proof surfaces, examples docs, cookbook docs,
  `ecosystem/fret` README, and root README reject direct `fret_imui::` and
  `fret_ui_kit::imui::` teaching imports.
- The editor cookbook proof rejects direct `fret_ui_editor` imports and stays on
  `fret::imui::editor`.
- The proof-demo helper checks now validate the current `proof_helpers.rs` owner.
...
- `python -m py_compile tools\gate_imui_facade_teaching_source.py` - passed.
...
- Evidence anchor: IMUI debug draw linear command payload owner split - 2026-06-03.
- Evidence anchor: ecosystem/fret-ui-kit/src/imui/debug_draw_controls/commands/types/command/linear.rs.
...
- Public `ImUiDebugDrawList` line/poly/rect/quad/triangle methods, command summaries,
  point/vertex/index/triangle counts, path paint dispatch, residual multi-color rect paint, media
  dispatch filtering, and debug-draw response APIs remain unchanged.
- `tools/gate_imui_workstream_source.py` now source-checks the split so linear command payload
  variants cannot drift back into the root command enum owner.
...
burying capture/publication inside hover/drop resolution:

- `ecosystem/fret-docking/src/dock/declarative/drag_resolve/diagnostics.rs` owns graph
  stats/signature capture, `DockDropResolveDiagnostics` construction, hover update/clear side
  effects needed for diagnostics, and `WindowInteractionDiagnosticsStore` publication.
- `ecosystem/fret-docking/src/dock/declarative/drag_resolve.rs` keeps hover/drop target
  resolution, drop intent effect projection, tab-bar drag auto-scroll, tear-off handoff, debug
  tracing, and panel/tabs drag allow checks.
- Evidence anchor: Docking declarative drag resolve diagnostics owner split - 2026-06-03.
- Evidence anchor: ecosystem/fret-docking/src/dock/declarative/drag_resolve/diagnostics.rs.
- Evidence anchor: drag hover/drop diagnostics capture and publication.
- Evidence anchor: drag_resolve.rs keeps hover/drop resolution and debug tracing.
- Hover-change detection, drop-hover clearing, candidate rect diagnostics, graph stats/signature
  payloads, diagnostics store publication, and public docking APIs remain unchanged.
- `tools/gate_imui_workstream_source.py` now source-checks the split so diagnostics publication
  cannot drift back into the hover/drop resolve owner.
...
- `ecosystem/fret-docking/src/dock/drop_resolve/intent.rs` owns panel/tab drop intent projection,
  in-window float intent projection, tear-off request gating, effect emission, invalidate-layout
  toggling, and debug intent labels.
- `ecosystem/fret-docking/src/dock/drop_resolve/target.rs` remains the drop target resolution
  owner.
- `ecosystem/fret-docking/src/dock/drop_resolve.rs` keeps diagnostics orchestration.
- Evidence anchor: Docking drop resolve intent owner split - 2026-06-03.
- Evidence anchor: ecosystem/fret-docking/src/dock/drop_resolve/intent.rs.
- Evidence anchor: drop intent projection and effect emission.
- Evidence anchor: Drop resolve root keeps diagnostics orchestration.
- Public docking APIs, panel/tab drop intents, effect projection, invalidation behavior, debug
  intent labels, target resolution, and diagnostics payloads remain unchanged.
- `tools/gate_imui_workstream_source.py` now source-checks the split so drop intent/effect helpers
