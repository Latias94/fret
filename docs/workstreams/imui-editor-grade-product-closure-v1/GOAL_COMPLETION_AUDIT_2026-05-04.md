...
| Workstream tracking exists | `docs/workstreams/README.md` validates with 315 dedicated directories after the text-model splits, color edit follow-ons, color-edit model/popup splits, and debug-draw follow-ons. | Met |
...
| Editor `ColorEdit` depth is materially usable and reviewable | Popup presets, alpha policy/preview, AlphaBar, HSV picker, numeric readout, editable RGB/HSV numeric input, per-control popup defaults, and the internal model/popup splits are tracked by closed 2026-05-04/2026-05-05 follow-ons. | Met for this subsystem |
...
- Editor `ColorEdit` is no longer a popup stub: the current sequence covers presets,
  ...
  parsing, formatting, HSV/RGB conversion, coordinate math, popup UI composition, and popup-local
  pointer helpers out of the public control wiring file.
