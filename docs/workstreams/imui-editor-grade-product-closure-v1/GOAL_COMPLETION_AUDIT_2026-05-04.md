...
| Workstream tracking exists | `docs/workstreams/README.md` validates with 314 dedicated directories after the text-model splits, color edit follow-ons, color-edit model split, and debug-draw follow-ons. | Met |
...
| Editor `ColorEdit` depth is materially usable and reviewable | Popup presets, alpha policy/preview, AlphaBar, HSV picker, numeric readout, editable RGB/HSV numeric input, per-control popup defaults, and the internal model split are tracked by closed 2026-05-04/2026-05-05 follow-ons. | Met for this subsystem |
...
- Editor `ColorEdit` is no longer a popup stub: the current sequence covers presets,
  alpha-preserving RGB edits, checkerboard alpha preview, AlphaBar, HSV picker affordances,
  numeric readout, editable RGB/HSV numeric entry, and per-control popup defaults without moving
  editor policy into `fret-imui` or the runtime. The follow-on model split now keeps parsing,
  formatting, HSV/RGB conversion, coordinate math, sanitization, and a11y helper text out of the UI
  composition file.
