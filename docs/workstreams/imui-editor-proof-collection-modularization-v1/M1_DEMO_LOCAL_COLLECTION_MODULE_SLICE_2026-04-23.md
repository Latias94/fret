...
2. The host file now routes collection rendering through `collection::render_collection_first_asset_browser_proof(ui)`; the demo-local `authoring_parity.rs` owner consumes `collection::authoring_parity_collection_assets()` for the drag-chip seed set.
