# WGPU Clip Conformance Harness Follow-on v1 — Milestones
...
- `clip_path_conformance.rs` uses `support::{pixel_rgba, render_scene_rgba8}` for final readback.
- `affine_clip_conformance.rs` uses `support::{pixel_rgba, render_scene_rgba8}` for final readback.
