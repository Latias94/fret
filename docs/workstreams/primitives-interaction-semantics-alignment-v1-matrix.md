...
| Combobox | Base UI | `ecosystem/fret-ui-kit/src/primitives/combobox.rs` | `ecosystem/fret-ui-shadcn/src/combobox.rs` | M | - | M | M | I | - | M | M | M | I | Audit sheet: `docs/workstreams/primitives-interaction-semantics-alignment-v1-combobox.md`. State-machine helpers + unit tests are now in `fret-ui-kit`; next: move more recipe-owned semantics down (Trigger/Listbox/Commit split). |
