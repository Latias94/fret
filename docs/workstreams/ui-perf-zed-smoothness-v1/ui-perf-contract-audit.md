...
| Checked-in baselines actually contain `p50`. | Scan on 2026-05-11 after the Windows resize/code-editor v2 promotions, autoscroll v4 promotion, and view-cache toggle v1 promotion: 73 baseline files, 309 max-bearing perf rows, 13 rows with `measured_p50`. | Partially covered. `ui-resize-probes.windows-rtx4090.v2.json`, `ui-code-editor-resize-probes.windows-rtx4090.v2.json`, `ui-gallery-code-editor-torture-autoscroll-steady.windows-rtx4090.v4.json`, and `ui-gallery-view-cache-toggle-perf-steady.windows-rtx4090.v1.json` are covered; other checked-in baselines need intentional re-seeding if p50 must be checked in. |
...
- Baseline p50 coverage scan:
  - `BASELINE_FILES=73`
  - `TOTAL_ROWS=309`
  - `TOTAL_ROWS_WITH_P50=13`
  - `TOTAL_ROWS_WITH_P90=158`
  - `TOTAL_ROWS_WITH_P95=158`
  - `TOTAL_ROWS_WITH_MAX=309`
...
1. Split `ui-gallery-steady` into narrower steady-contract groups, or mark the broad suite as evidence-only until the
   ...
   cross-script drift, not by a single threshold tweak. `ui-gallery-view-cache-toggle-perf-steady` is now split out;
   remaining broad-only members are hover layout, Material tabs, menubar keyboard nav, and virtual-list torture.
...
payload-aware v4 baseline, and `ui-gallery-view-cache-toggle-perf-steady` is now a dedicated Windows v1 contract.
The next work should keep splitting or explicitly deferring the remaining broad-only `ui-gallery-steady` members, or
add the stricter editor paint stressor before any `WindowedRowsSurface` display-list rewrite.
