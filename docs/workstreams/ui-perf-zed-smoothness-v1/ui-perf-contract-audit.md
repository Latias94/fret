...
| Wrapped text remains layout/paint correct during resize. | ADR 0251 is now aligned in `docs/adr/IMPLEMENTATION_ALIGNMENT.md`; mechanism evidence lives in `crates/fret-ui/src/layout/engine/flow.rs`, `crates/fret-ui/src/tree/paint_cache.rs`, and focused regression tests in `crates/fret-ui/src/tree/tests/interactive_resize_flow_rebuild.rs` plus `crates/fret-ui/src/tree/tests/paint_cache.rs`. The UI Gallery soft-wrap editing script now captures a final layout sidecar, and local evidence from `target/fret-diag/code-editor-soft-wrap-resize-fix-2026-05-16b/sessions/1778917336764-71196` shows the final bundle/screenshot/layout sidecar after the narrow resize. | Covered for the current `fret-ui` mechanism fix; keep broader perf baselines unchanged. |
...
| Decide whether the next refactor lane is Canvas/paint replay, glyph/text-index/atlas residency, or no code change. | Decision based on verified target-machine baseline and attribution artifacts; the artifact verifier now projects the needed per-probe `decision_inputs` from `diag stats --json`. | macOS M4 evidence points away from Canvas wrapper and row replay and toward generic host-widget paint aggregate, but target-machine attribution is still absent. | Deferred with the Windows RTX4090 TODO. |
...
`python3 tools/perf/test_diag_editor_paint_contract_verify_artifacts.py` (10 tests), and
...
7. The soft-wrap resize overlap report is closed as a `fret-ui` correctness fix, not as a baseline update. The flow
   builder now honors ADR 0251 for `TextWrap::Word` / `Balance` min-content probes, and retained paint-cache replay is
   keyed by subtree geometry so stable parent bounds cannot replay stale child text layout. The diagnostic script now
   captures a final layout sidecar, and the local reproduction artifact is
   `target/fret-diag/code-editor-soft-wrap-resize-fix-2026-05-16b/sessions/1778917336764-71196`.
## Audit Conclusion
