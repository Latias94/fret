...
| Contract stabilization promotes or validates the final thresholds on the target machine profile. | Current checked-in Windows contracts exist for resize, editor resize, autoscroll steady, autoscroll typical, and complex wheel; `audit_perf_baselines.py --strict` passes for the matrix. The target-machine runner is now `tools/perf/diag_editor_paint_contract_validate.py`; synced artifacts can be rechecked with `tools/perf/diag_editor_paint_contract_verify_artifacts.py`; the local closeout gate is `tools/perf/diag_editor_paint_contract_closeout.py`. The validation runner rejects existing non-empty non-dry-run output directories by default, the verifier requires non-empty `date_tag` fields, rejects drift from the required Windows command shape, rejects paint-perf env in the baseline-validation directory, requires paint-perf coverage in the attribution directory, and the closeout CLI has unit coverage for both verifier-fail and verifier-pass non-dry-run paths. | Not complete. Needs deliberate Windows RTX4090 overlay-disabled validation with `check.perf_thresholds.json` `failures=[]`; only a threshold re-seed path additionally needs `selection-summary.json` plus the no-silent-threshold-loosening evidence. |
| P1.5 Editor Canvas paint replay can be closed. | Current audit, runbook, TODO, artifact verifier, and closeout gate now identify the remaining missing artifact, the target-machine runner command, the post-sync verification step, and the non-dry-run closeout behavior. | Not complete until the target-machine re-seed/validation is recorded, verified, and the matrix/TODO are updated. |
