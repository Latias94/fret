...
| Resize probes | `ui-resize-probes` | `docs/workstreams/perf-baselines/ui-resize-probes.windows-rtx4090.v1.json`; `docs/workstreams/perf-baselines/ui-resize-probes.macos-m4.v3.json` | `python tools/perf/diag_resize_probes_gate.py --suite ui-resize-probes --attempts 3` | Perf log entries through `2026-05-09 19:19`; Windows v2 re-seed is deferred because repeat=7 validation still finds UI/layout failures even after renderer thresholds are removed | GPUI resize coalescing and bounded layout roots; egui bounded extra work |
...
- Old baseline JSON files may not contain `measured_p50`. That is expected until each baseline is intentionally
  re-seeded.
- Windows `ui-resize-probes` v2 was attempted with `threshold_surface=ui`, but is not checked in because repeat=7
  validation remained red at both 20% and 40% headroom. Treat that as a layout/resize variability investigation, not a
  renderer threshold problem.
