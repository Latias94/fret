...
| Code editor torture autoscroll | `tools/diag-scripts/ui-gallery/code-editor/ui-gallery-code-editor-torture-autoscroll-steady.json` | `docs/workstreams/perf-baselines/ui-gallery-code-editor-torture-autoscroll-steady.windows-rtx4090.v2.json` | `cargo run -p fretboard -- diag perf tools/diag-scripts/ui-gallery/code-editor/ui-gallery-code-editor-torture-autoscroll-steady.json --repeat 7 --warmup-frames 5 --reuse-launch --perf-baseline <baseline> --env FRET_DIAG_SCRIPT_AUTO_DUMP=0 --env FRET_DIAG_SEMANTICS=0 --env FRET_UI_GALLERY_VIEW_CACHE=1 --env FRET_UI_GALLERY_VIEW_CACHE_SHELL=1 --env FRET_UI_GALLERY_VLIST_KNOWN_HEIGHTS=1 --launch -- target/release/fret-ui-gallery.exe` | `2026-05-11`: renderer-aware baseline v2 seeded via `docs/workstreams/perf-baselines/policies/ui-gallery-code-editor-torture-autoscroll-steady.v1.json`; fresh low-overhead hint probe `target/fret-diag/perf-code-editor-hotspot-hint-probe-v1/check.perf_hints.json` flags `paint.widget_heavy` on `ElementHostWidget::Canvas` plus `renderer.upload_churn`, while `target/fret-diag/perf-code-editor-paint-detail-probe-v1/1778455020350/bundle.schema2.json` still replays `288/289` visible rows and stores only 1 new row | GPUI row/layer fragment replay with stable keys; egui frame-cache discipline for expensive derived state; current Fret limiter is Canvas paint/upload churn, not more RowGeomKey / RowSceneKey splitting |
