...
| AI / plan | `PlanDescription` | `ecosystem/fret-ui-ai/src/elements/plan.rs` | Static path delegates to `CardDescription`; streaming path still owns explicit `Shimmer` text style | Keep static path on shared description helper and migrate streaming shimmer once that surface can consume inherited refinement without losing overlay metrics; follow-up tracked in `docs/workstreams/shimmer-text-style-source-fearless-refactor-v1/` | Likely same policy as card/description family, but `Shimmer` is still a visual-text exception | P2 | `[~]` | `ecosystem/fret-ui-ai/src/elements/plan.rs:354`, `docs/workstreams/shimmer-text-style-source-fearless-refactor-v1/DESIGN.md` |
