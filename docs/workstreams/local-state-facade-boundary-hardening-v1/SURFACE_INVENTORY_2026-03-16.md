...
| `use_local*`, `LocalState::watch(...)`, `AppUiState::watch(...)`, `TrackedStateExt`, `cx.actions().locals_with((...)).on::<A>(...)` | Public and already taught in docs/templates/examples | Default app lane | This is the shipped default local-state contract | Keep as the only default story; keep `AppUi::watch_local(...)` crate-private as implementation substrate |
