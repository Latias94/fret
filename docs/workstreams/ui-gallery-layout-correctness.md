...
| L10 | P1 | `data_table` | Virtualized table rows could shift horizontally when a row became “active” (the active indicator used a left border that participated in layout), causing subtle header/body misalignment under overflow. | (unit test) | Fixed (pending merge) | codex | Fix: render the 2px active indicator as an absolutely-positioned overlay (no layout participation). Gate: `ecosystem/fret-ui-kit/src/declarative/table.rs` `table_virtualized_alignment_gate_header_matches_rows_under_overflow_and_variable_height`. |
