...
| `fitView`, `fitViewOptions` (`padding`, `duration`, `ease`, `interpolate`, `nodes`) | `frame_nodes_in_view(...)` (animated “frame selection/all”) + `NodeGraphViewQueue` (`FrameNodes`) | Partial (has `nodes` + `includeHiddenNodes` + per-call `minZoom`/`maxZoom`; missing initial `fitView` on mount) | `ecosystem/fret-node/src/core/model.rs`, `ecosystem/fret-node/src/io/mod.rs`, `ecosystem/fret-node/src/ui/view_queue.rs`, `ecosystem/fret-node/src/ui/canvas/widget/view_state.rs`, `ecosystem/fret-node/src/ui/canvas/widget/tests/fit_view_options_conformance.rs` |
| Viewport animation helpers (`duration`, `ease`, `interpolate`) | Timer-driven viewport animation (duration + interpolate + optional ease override) | Partial (missing helper APIs) | `ecosystem/fret-node/src/ui/canvas/state.rs`, `ecosystem/fret-node/src/ui/canvas/widget/event_timer.rs`, `ecosystem/fret-node/src/ui/canvas/widget/viewport_timers.rs` |
...
1) **Fit-view parity**: support initial `fitView` on mount and expose a dedicated helper API (B-layer friendly), matching XyFlow's `fitView`.
