...
| `fitView`, `fitViewOptions` (`padding`, `duration`, `ease`, `interpolate`, `nodes`) | `frame_nodes_in_view(...)` (animated “frame selection/all”) + `NodeGraphViewQueue` (`FrameNodes`) | Partial (has padding/duration/ease/interpolate/nodes; missing includeHiddenNodes + per-call min/max zoom) | `ecosystem/fret-node/src/io/mod.rs`, `ecosystem/fret-node/src/ui/view_queue.rs`, `ecosystem/fret-node/src/ui/canvas/widget/view_state.rs`, `ecosystem/fret-node/src/ui/canvas/widget/tests/fit_view_nodes_conformance.rs` |
...
1) **Fit-view option parity**: add `includeHiddenNodes` and per-call `minZoom`/`maxZoom` overrides (currently global via `NodeGraphStyle`), matching XyFlow's `fitViewOptions`.
