...
- [x] SE-010 Repro and evidence for “scroll probe stall” in UI Gallery.
  ...
  - Tracker: `docs/workstreams/standalone/ui-gallery-perf-scroll-measure.md`
- [x] SE-020 Record a stable before/after table for dt_ms + layout_time_us for the repro script.
  ...
  - Method (2026-03-02, macOS aarch64, debug build):
    ...
    - `layout_time_us` is taken from `fretboard-dev diag stats ... --json` (`max.layout_time_us`) for the captured
      `...-second` bundle.
  - Baseline (no post-layout extents):
    - Command: `fretboard-dev diag perf tools/diag-scripts/ui-gallery/navigation/ui-gallery-nav-card-click-latency.json --repeat 3 --warmup-frames 0 --env RUST_LOG=error ...`
...
- [x] SE-100 Define the scroll-extent contract: how extents are derived from layout geometry.
  ...
  - Draft: `docs/workstreams/scroll-extents-dom-parity/scroll-extents-dom-parity.md` (Target Contract / SE-100)
- [~] SE-110 Identify current “available size clamping” behaviors that prevent overflow.
  ...
  - Initial inventory: `docs/workstreams/scroll-extents-dom-parity/scroll-extents-dom-parity.md` (SE-110)
...
- [x] SE-111 Decide and implement the mechanism contract for “fill vs fit” along the scroll axis.
  ...
  - Evidence: `docs/workstreams/scroll-extents-dom-parity/scroll-extents-dom-parity.md` (SE-110, clamp policy blockers).
...
- [x] SE-213 Scrollbar + overlay anchoring parity (needs harness).
    - [x] SE-213a Add `fretboard-dev diag query overlay-placement-trace` (reads `script.result.json` evidence).
