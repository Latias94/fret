...
- `docs/workstreams/fret-node-paint-root-cache-plan-adapter-v1/` — first n/a, latest n/a, 5 markdown docs (active narrow follow-on for splitting node graph paint-root cache-plan host/bounds/scale-factor route inputs behind a named adapter seam; includes `WORKSTREAM.json`)
