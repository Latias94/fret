...
| --- | --- | --- |
| `fret-ui-editor` | Has explicit `editor.*` token keys and an opt-in preset patch entrypoint. The latest preset cleanup now seeds editor-owned text-field chrome via `editor.text_field.*` while keeping legacy `component.text_field.*` reads as a compatibility fallback. It still mutates some shared semantic palette keys for proof/demo use. | `ecosystem/fret-ui-editor/src/primitives/tokens.rs`, `ecosystem/fret-ui-editor/src/theme.rs`, `ecosystem/fret-ui-editor/src/primitives/chrome.rs` |
...
| --- | --- | --- |
| Canonical editor-owned keys already modeled as constants | `editor.density.*`, `editor.text_field.*`, `editor.numeric.*`, `editor.property.*`, `editor.popup.*`, `editor.checkbox.*`, `editor.enum_select.*`, `editor.axis.*`, `editor.vec.*`, `editor.color.*`, `editor.slider.*` | This is the clearest and healthiest namespace posture in the current editor ecosystem. The latest baseline cleanup also split `editor.property.group_border` from the outer panel frame and `editor.property.panel_header_*` from repeated section headers so nested inspector hierarchy can be tuned without one shared chrome weight for every level, now also promotes shared popup surface background/border/shadow intent into `editor.popup.*`, moves editor-owned text-field chrome defaults into `editor.text_field.*` so proof presets do not need to patch `component.text_field.*` just to keep editor controls stable, and now gives checkbox its own `editor.checkbox.bg` / `editor.checkbox.checked_bg` / `editor.checkbox.checked_fg` / `editor.checkbox.ring` colors and slider its own `editor.slider.track_bg` / `editor.slider.fill_bg` / `editor.slider.thumb_bg` / `editor.slider.thumb_border` family alongside the existing size/radius metrics while centralizing composite-level panel/header/popup fallback reads in shared editor color helpers instead of leaving each surface to re-specify the ladder. |
...
| Seeding / preset entrypoint | `apply_editor_theme_preset`, `EditorThemePreset::{Default, ImguiLikeDense}` | Today this is the only editor ecosystem crate with an explicit preset patch path. |

Important observation:

- `fret-ui-editor` already owns real `editor.*` keys,
- and the latest preset slice now routes editor text-field chrome through `editor.text_field.*`
  instead of patching `component.text_field.*`.
- The remaining preset drift is now mostly about generic palette keys like `card`, `muted`,
...
- The popup-surface cleanup now moves one more visible seam out of that host-palette dependency:
  shared popup shells can read `editor.popup.*` first instead of inheriting `popover` / `muted`.
...
| Editor preset still mutates shared palette keys | `fret-ui-editor/src/theme.rs` now writes editor-owned `editor.text_field.*`, `editor.checkbox.*`, and `editor.slider.*` keys for field/control chrome, but still patches generic palette keys like `card`, `muted`, `border`, `foreground`, `muted-foreground`, `accent`, and `ring`. | An editor preset can still bleed into non-editor widgets in the same app. | Keep this path for proof/demo use, but continue moving visible editor seams onto editor-owned families before calling the seeding surface stable. |
...
| Current shadcn shell seeding stops short of `workspace.tab.*` | `fret-ui-shadcn` now seeds `workspace.frame.*`, `workspace.top_bar.*`, `workspace.status_bar.*`, and `workspace.tabstrip.*`, but not `workspace.tab.*`. `WorkspaceTabStripTheme` now also prefers `workspace.tab.hover_bg` before generic `accent`, so owner overrides remain meaningful even without adapter seeding. | This keeps the current closure small while preserving owner-token precedence for apps that do opt in. | Keep `workspace.tab.*` out of v1 seeding unless a promoted proof surface shows a real need. |
...
| `editor.text_field.*` | `fret-ui-editor` | Implemented | Editor text-input/text-area chrome now reads these keys first via shared editor chrome helpers, while keeping `component.text_field.*` as a legacy fallback for app compatibility. Seeded in `fret-ui-editor/src/theme.rs`. | Preferred v1 landing zone for editor-owned field chrome. |
...
| `editor.popup.*` | `fret-ui-editor` | Implemented | Shared popup surface background/border plus radius/shadow color and metrics now read via `EditorTokenKeys` and are seeded in the preset patch | Stable for v1 use. This is the current landing zone for assist/select/color popup chrome that should not depend on a host theme's `popover` / `muted` tone or require per-control geometry tuning. |
| `editor.checkbox.*` | `fret-ui-editor` | Implemented | Read via `EditorTokenKeys`; size/radius plus `editor.checkbox.bg` / `editor.checkbox.checked_bg` / `editor.checkbox.checked_fg` / `editor.checkbox.ring` are seeded in the preset patch | Stable for v1 use. |
...
| `editor.slider.*` | `fret-ui-editor` | Implemented | Read via `EditorTokenKeys`; track/thumb metrics plus `editor.slider.track_bg` / `editor.slider.fill_bg` / `editor.slider.thumb_bg` / `editor.slider.thumb_border` are seeded in the preset patch | Acceptable for v1. |
...
- This intentionally does not seed `workspace.tab.*` yet, and that is now the explicit v1
  decision rather than a temporary omission.
...
1. Decide whether `fret-workspace` should keep only its small resolver surface or also gain an
   opt-in local preset helper for proof/demo use.
2. Continue auditing the editor proof preset so more of its visual intent lands in editor-owned
   namespaces and less in shared palette mutation.
3. Keep shell/docking visual alignment on the adapter side instead of widening owner-crate token
   seeding prematurely.
