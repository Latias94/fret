...
| Semantics snapshot profiling | Opt-in `fret.ui.semantics.*` spans for element-id map lookup, window-frame child graph lookup, traversal, and relation normalization share `fret_perf::measure_span` with the existing `FRET_SEMANTICS_PROFILE` log fields. | `crates/fret-ui/src/tree/ui_tree_semantics.rs` | Covered for current opt-in semantics profile phases |
...
| Window runtime snapshots | `fret_perf::measure_span` timers for focus/IME repair, post-dispatch pointer-move input context refresh, full-publish input context refresh, command availability, command registry collection, command availability evaluation, and shortcut overlay refresh. Debug stats buckets and trace spans now share one boundary for the snapshot work that runs after dispatch or imperative same-frame mutations. | `crates/fret-ui/src/tree/commands.rs` | Covered |
