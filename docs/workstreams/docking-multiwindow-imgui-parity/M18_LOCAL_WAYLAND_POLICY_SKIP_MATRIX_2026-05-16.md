...
admission path. The goal stays narrow: non-Wayland platform-capabilities sidecars must stop
...
2. Confident: local non-Wayland maintenance should prove admission behavior, not acceptance.
...
3. Likely: the right local regression surface is a policy-skip matrix, not another source marker.
   ...
   the remaining local risk is admission behavior across distinct non-Wayland sidecars. If wrong,
   this note can fold back into source-only checks.
...
- Expanded `tools/diag_gate_docking_wayland_policy_skip.py` into a two-case matrix.
- The gate now probes:
...
- Both probes still publish `capabilities.json` with `diag.script_v2`, still stop at campaign
...
- `python tools/diag_gate_docking_wayland_policy_skip.py --reuse-built` passed and produced one
  `check.environment.json` with `environment.platform_capabilities.platform_ne` and one with
  `environment.platform_capabilities.ui_window_tear_off_ne`.
...
This is still a local policy-skip proof, not platform acceptance. It blocks non-Wayland hosts from
