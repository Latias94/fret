...
- [~] DW-P1-linux-003 Wayland-safe degradation policy for follow-mode.
  ...
  - Progress:
    ...
    - [x] 2026-06-04 runner driver effects owner split keeps viewport-input and Dock driver
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner streaming effects owner split keeps streaming upload lifecycle out of
      the general effect loop:
      - `docs/workstreams/docking-multiwindow-imgui-parity/M85_RUNNER_STREAMING_EFFECTS_OWNER_SPLIT_2026-06-04.md`
      - `crates/fret-launch/src/runner/desktop/runner/streaming_effects.rs` owns upload queue
        preprocessing, dropped-update ack delivery, `StreamingUploadPerfSnapshot` publication,
        `FRET_STREAMING_DEBUG` logging, and pending streaming redraw requests.
      - `crates/fret-launch/src/runner/desktop/runner/effects.rs` keeps the effect queue loop and
    ...
    - [x] 2026-06-04 runner wgpu adapter diagnostics owner split keeps adapter-selection logging
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner renderer bootstrap owner split keeps renderer installation out of the
      general application handler:
      - `docs/workstreams/docking-multiwindow-imgui-parity/M92_RUNNER_RENDERER_BOOTSTRAP_OWNER_SPLIT_2026-06-04.md`
      - `crates/fret-launch/src/runner/desktop/runner/renderer_bootstrap.rs` owns renderer
        creation, renderer capability publication, budget configuration, startup font environment
        initialization, context/renderer installation, `gpu_ready` ordering, and startup async font
        rescan gating.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps winit
    ...
    - [x] 2026-06-04 runner surface lifecycle hook owner split keeps destroy/resume/suspend
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner about-to-wait control-flow owner split keeps event-loop scheduling
      out of the general application handler:
      - `docs/workstreams/docking-multiwindow-imgui-parity/M97_RUNNER_ABOUT_TO_WAIT_CONTROL_FLOW_OWNER_SPLIT_2026-06-04.md`
      - `crates/fret-launch/src/runner/desktop/runner/event_loop.rs` owns pending-front request
        processing, timer/dispatcher/pending-front/hotpatch deadline merging, dock drag/follow
        polling pressure, RAF deadline scheduling, RAF flush behavior, and final ControlFlow
        selection.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps the
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner about-to-wait internal drag poll owner split keeps pre-turn drag
      polling out of the general application handler:
      - `docs/workstreams/docking-multiwindow-imgui-parity/M98_RUNNER_ABOUT_TO_WAIT_INTERNAL_DRAG_POLL_OWNER_SPLIT_2026-06-04.md`
      - `crates/fret-launch/src/runner/desktop/runner/device_events.rs` owns scripted cursor screen
        position override polling, event-loop turn polling, internal drag hover routing,
        diagnostic mouse-button override polling, release-to-drop routing,
        `saw_left_mouse_release_this_turn` updates, dock drag follow updates, and conditional effect
        draining.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps the
        `ApplicationHandler::about_to_wait` trait hook and delegates the pre-turn internal drag
    ...
    - [x] 2026-06-04 runner about-to-wait dock follow stop owner split keeps idle follow-stop
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner about-to-wait dock released-outside fallback owner split keeps
      platform fallback scheduling out of the general application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/docking/poll_up.rs` owns
        macOS released-outside polling, Windows poll-up routing, diagnostics pointer-input
        isolation behavior, cursor-based drop routing, follow cleanup after Windows poll-up, and
        fallback-triggered effect draining.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps the
        `ApplicationHandler::about_to_wait` trait hook and delegates platform fallback scheduling
    ...
    - [x] 2026-06-04 runner about-to-wait window turn accessibility owner split keeps per-window
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner about-to-wait mobile surface recreation owner split keeps Android/iOS
      missing-surface gating out of the general application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/surface_lifecycle.rs` owns the
        `can_create_surfaces` lifecycle gate, context-present check, missing-surface scan,
        `try_create_missing_surfaces` call, and post-bootstrap effect drain.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps the
        `ApplicationHandler::about_to_wait` trait hook and delegates Android/iOS surface
    ...
    - [x] 2026-06-04 runner about-to-wait dev-state observation owner split keeps desktop
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner about-to-wait preamble owner split keeps desktop pre-render drain,
      suspended wait fast path, and monitor topology refresh out of the general application
      handler:
      - `docs/workstreams/docking-multiwindow-imgui-parity/M106_RUNNER_ABOUT_TO_WAIT_PREAMBLE_OWNER_SPLIT_2026-06-04.md`
      - `crates/fret-launch/src/runner/desktop/runner/event_loop.rs` owns the pre-render drain,
        suspended wait fast path, and monitor topology refresh before the later about-to-wait hook
        sequence.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps the
        `ApplicationHandler::about_to_wait` trait hook and delegates the preamble before
    ...
    - [x] 2026-06-04 runner window pointer move owner split keeps dock-drag move rerouting out of
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window pointer button owner split keeps left-release drag cleanup out
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_pointer_button.rs` owns
        ...
        cursor-screen fallback sampling, left mouse down/up tracking, cursor-based internal drag
        drop delivery, DockFloating follow stop, cross-window drag cancellation, dock-source
        Up/Down rerouting, mapped event delivery, and effect draining.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps only
    ...
    - [x] 2026-06-04 runner window redraw WGPU hub report owner split keeps hub count publication
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window redraw WGPU allocator report owner split keeps allocator sample
      publication out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_redraw_wgpu_allocator_report.rs`
        owns `maybe_record_window_redraw_wgpu_allocator_report`, including
      ...
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps only redraw-time WGPU
        allocator report dispatch after hub report diagnostics and before command-buffer
        submission.
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window redraw text diagnostics owner split keeps renderer text
      diagnostics publication out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_redraw_text_diagnostics.rs` owns
        ...
        `begin_window_redraw_text_diagnostics_frame`, and
        `publish_window_redraw_text_diagnostics`, including `FRET_RENDER_TEXT_DEBUG`,
        `FRET_DIAG_DIR`, SVG text bridge diagnostics, renderer text diagnostics snapshots, and the
    ...
    - [x] 2026-06-04 runner window redraw pending-wheel owner split keeps frame-boundary wheel
      ...
      - Focused runner compile, wheel coalescing regression, Linux capability posture regression,
        source gate, JSON shape, catalog, and diff checks passed locally without recording Wayland
        compositor acceptance.
    - [x] 2026-06-04 runner window redraw surface-resize owner split keeps pending surface resize
      fallback out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_redraw_surface_resize.rs` owns
        `handle_window_redraw_pending_surface_resize`, including `pending_surface_resize`
        draining, fallback `resize_surface`, logical-size quantization,
        `last_delivered_window_resized` deduplication, `Event::WindowResized`, and
        `Event::WindowScaleFactorChanged` delivery.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps only redraw-time
        surface-resize dispatch before platform frame preparation.
    ...
    - [x] 2026-06-04 runner window redraw record owner split keeps engine-frame recording out of
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window redraw target-updates owner split keeps render-target update
      application out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps only redraw-time
        target-updates owner dispatch and continues to own present/render-scene orchestration.
    ...
    - [x] 2026-06-04 runner window redraw render-scene owner split keeps UI command recording out
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window redraw present-submit owner split keeps queue submission and
      frame presentation out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_redraw_present_submit.rs` owns
        `submit_window_redraw_present_frame`, `WindowRedrawPresentSubmitInput`,
        `input.context.queue.submit(input.command_buffers)`, and `frame.present()`.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps diagnostics,
        screenshots, scheduling presented-frame commit, engine keepalive drop, and recovery.
    ...
    - [x] 2026-06-04 runner window redraw RenderDoc capture owner split keeps redraw-time capture
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window redraw clear-color owner split keeps transparent-window clear
      selection out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_redraw_clear_color.rs` owns
        `resolve_window_redraw_clear_color`, the `RunnerWindowStyleDiagnosticsStore` lookup,
        `effective_snapshot(app_window)`, `visual_transparent`, transparent `ClearColor`, and the
        configured clear-color fallback.
      - `crates/fret-launch/src/runner/desktop/runner/app_handler.rs` keeps only clear-color owner
        dispatch before render-scene recording.
    ...
    - [x] 2026-06-04 runner window redraw webviews owner split keeps redraw-time webview snapshot
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-04 runner window redraw post-render diagnostics owner split keeps diagnostics
      dispatch out of the application handler:
      ...
      - `crates/fret-launch/src/runner/desktop/runner/window_redraw_post_render_diagnostics.rs`
        owns `WindowRedrawPostRenderDiagnosticsInput` and
        `publish_window_redraw_post_render_diagnostics`.
      - The owner dispatches text diagnostics, renderer perf samples, WGPU hub reports, and WGPU
        allocator reports while leaving env gates and sampling logic in existing diagnostics
        modules.
      - Evidence marker: WGPU allocator reports.
    ...
    - [x] 2026-06-05 runner window redraw present-capture command owner split keeps command-buffer
      ...
      - Focused runner compile, Linux capability posture regression, source gate, JSON shape,
        catalog, and diff checks passed locally without recording Wayland compositor acceptance.
    - [x] 2026-06-05 runner window redraw present owner split keeps present-phase orchestration out
      of the application handler:
      ...
      - The owner sequences surface acquire/target prepare, clear-color dispatch, render-scene
        recording, post-render diagnostics, present capture commands, submit/present, successful
        present finish, and `RedrawPhase::Present` timing.
      - `app_handler.rs` keeps winit redraw scheduling, RenderDoc capture end, present error
        recovery, and hitch-summary orchestration.
      - Marker: redraw-time present-phase orchestration after render-target updates and before
