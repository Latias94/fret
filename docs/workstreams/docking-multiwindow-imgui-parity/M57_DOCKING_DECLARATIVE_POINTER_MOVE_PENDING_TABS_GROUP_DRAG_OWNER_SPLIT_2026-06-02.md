# M57 Docking Declarative PointerMove Pending Tabs-Group Drag Owner Split - 2026-06-02
...
Declarative docking `PointerMove` pending tabs-group drag activation now lives in
`ecosystem/fret-docking/src/dock/declarative/events/pointer_move/pending_tabs_group_drag.rs`,
while `events/pointer_move.rs` remains the event-phase owner for viewport capture forwarding,
divider drag resize handling, floating title-bar movement, pending panel drag activation, and final
hover/cursor projection. The split does not change pending tabs-group drag lookup, left-button
release cleanup, activation threshold handling, tabs-group drag startup, hover clearing, capture
release, redraw requests, propagation stopping, or the Wayland acceptance boundary.

## Source Shape

- `ecosystem/fret-docking/src/dock/declarative/events/pointer_move.rs` declares
  `mod pending_tabs_group_drag;` and delegates the tabs-group activation phase to
  `pending_tabs_group_drag::handle_pointer_move_pending_tabs_group_drag(...)`.
- `ecosystem/fret-docking/src/dock/declarative/events/pointer_move/pending_tabs_group_drag.rs`
  owns:
  - pending tabs-group drag lookup through `service.pending_dock_tabs_drag(window, pointer_id)`,
  - left-button release cleanup through `service.take_pending_dock_tabs_drag(window, pointer_id)`,
  - distance activation through `fret_dnd::ActivationConstraint::Distance`,
  - tabs-group drag startup through `begin_declarative_tabs_group_drag(...)`,
  - hover clearing through `DockManager::default` and `dock.hover = None`,
  - capture release, redraw, and propagation stop for handled pending tabs-group moves.
- `events/pointer_move.rs` now acts as the ordered stage router and keeps `PointerMove` routing out
  of the generic `events.rs` router.
...
This keeps declarative pointer-move pending tabs-group drag activation source-auditable without
