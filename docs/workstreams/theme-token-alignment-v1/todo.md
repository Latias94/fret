...
| charts/plot | Chart chrome + legend contrast | (ecosystem ports) | seed `chart.palette.*` from shadcn `chart-*` + gate demo colors | `chart.palette.*` (alias of `chart-*`) | diag screenshot (zinc/dark) | [~] | `ecosystem/fret-chart/src/retained/style.rs`, `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs`, `tools/diag-scripts/ui-gallery/chart/ui-gallery-chart-demo-area-screenshot-zinc-dark.json` |
| docking | Docking chrome (drag affordances) | (in-tree) | tokenize drop overlay + tab insert markers (primary-derived alphas) | `component.docking.drop_overlay.*`, `component.docking.tab_insert.*` | diag screenshot (zinc/dark) | [~] | `ecosystem/fret-docking/src/dock/paint.rs`, `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs` |
