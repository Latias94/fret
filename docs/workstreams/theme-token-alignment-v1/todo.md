...
| ecosystem | Markdown (inline code + links) | (ecosystem) | align theme key namespace + keep compat (`component.*` → `fret.*` → legacy) | `component.markdown.*` (compat: `fret.markdown.*`, `markdown.*`) | diag screenshot (zinc/dark) | [x] | `ecosystem/fret-markdown/src/theme.rs`, `ecosystem/fret-markdown/src/lib.rs`, `ecosystem/fret-markdown/src/tests.rs`, `tools/diag-scripts/ui-gallery/markdown-editor/ui-gallery-markdown-link-gate-activation-zinc-dark.json` |
...
| ecosystem | CodeView / Syntax (tokens contrast) | (ecosystem / vscode-theme) | centralize `color.syntax.*` resolution + seed shadcn defaults + audit contrast | `color.syntax.*` (+ semantic fallbacks) | diag screenshot (zinc/dark) + Rust seeding asserts | [x] | `crates/fret-ui/src/theme/mod.rs`, `ecosystem/fret-code-view/src/syntax.rs`, `ecosystem/fret-code-editor/src/editor/paint/mod.rs`, `ecosystem/fret-vscode-theme/src/lib.rs`, `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs`, `tools/diag-scripts/ui-gallery/code-view/ui-gallery-code-view-syntax-screenshot-zinc-dark.json` |
| material3 | Overlay scrim + surface contrast | (spec / Material Web v30 tokens) | use `md.sys.color.scrim` + per-component `md.comp.*.scrim.opacity` (no literal black/white) | `md.sys.color.scrim`, `md.comp.dialog.scrim.opacity`, `md.comp.sheet.bottom.docked.modal.scrim.opacity`, `md.comp.date-picker.modal.scrim.opacity`, `md.comp.time-picker.scrim.opacity` | Rust token seeding asserts | [x] | `ecosystem/fret-ui-material3/src/dialog.rs`, `ecosystem/fret-ui-material3/src/bottom_sheet.rs`, `ecosystem/fret-ui-material3/src/date_picker.rs`, `ecosystem/fret-ui-material3/src/time_picker.rs`, `ecosystem/fret-ui-material3/src/tokens/v30.rs` |
| charts/plot | Chart chrome + legend contrast | (ecosystem ports) | seed `chart.palette.*` + `plot.palette.*` from shadcn `chart-*` + gate demo colors | `chart.palette.*` + `plot.palette.*` (+ `fret.plot.palette.*`) | diag screenshot (zinc/dark) + Rust seeding asserts | [x] | `ecosystem/fret-chart/src/retained/style.rs`, `ecosystem/fret-plot/src/theme_tokens.rs`, `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs`, `tools/diag-scripts/ui-gallery/chart/ui-gallery-chart-demo-area-screenshot-zinc-dark.json` |
| docking | Docking chrome (drag affordances) | (in-tree) | tokenize drop overlay + tab insert markers (primary-derived alphas) | `component.docking.drop_overlay.*`, `component.docking.tab_insert.*` | diag screenshot (drag preview) | [x] | `ecosystem/fret-docking/src/dock/paint.rs`, `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs`, `tools/diag-scripts/workspace/shell-demo/workspace-shell-demo-tab-drag-to-split-right-drop-preview-screenshot.json`, `tools/diag-scripts/docking/arbitration/docking-arbitration-demo-tab-bar-drop-end-insert-index-screenshot.json` |
| canvas/node | Node graph selection highlights | (in-tree) | seed viewport selection tokens from shadcn `ring` (avoid default blue) | `color.viewport.selection.*`, `color.viewport.marker` (derived from `ring`) | diag screenshot (zinc/dark) | [x] | `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs`, `ecosystem/fret-node/src/ui/style.rs`, `tools/diag-scripts/ui-gallery/node-graph/ui-gallery-node-graph-marquee-selection-screenshot-zinc-dark.json` |
...
| ui-assets | Image placeholders + skeletons | `repo-ref/ui` skeleton | `bg-accent` + `animate-pulse` | `component.skeleton.bg` (+ pulse policy) | diag screenshot (zinc/dark) | [x] | `ecosystem/fret-ui-shadcn/src/skeleton.rs`, `ecosystem/fret-ui-shadcn/src/shadcn_themes.rs`, `tools/diag-scripts/ui-gallery/skeleton/ui-gallery-skeleton-demo-screenshot-zinc-dark.json`, `apps/fret-ui-gallery/src/ui/pages/skeleton.rs` |
