...
- [x] Choose the next 2?3 high-ROI seam extractions for landable follow-up PRs:
  ...
  - [ ] check planning/execution,
    ...
    - fifteenth landing: `diag_suite` now routes passed-script post-run preparation through `SuiteScriptPostRunPreparationRequest`, so bundle waits, bundle doctor application, and post-run default-check planning reuse one seam without forcing the remaining success-tail extraction to land in the same patch
    - audit note: the remaining `diag_suite` holdout is now the success-only lint/finalize tail around row emission, post-run apply, and stop-demo teardown; it is still a valid high-ROI follow-up seam rather than a parked area
    - next focus: finish the remaining `diag_suite` success-tail/finalize seam before deciding whether the next higher-ROI extraction moves back to `diag_run` or `diag_campaign`
