...
| Regression first-open/share evidence projection | Shared projection of `bundle_artifact`, `triage_artifact`, `script_result`, `screenshots_manifest`, `share_artifact`, and `packed_report` paths from `regression.summary.json` into CLI-adjacent consumers. | `cargo nextest run -p fret-diag regression_summary_drilldown_projects_perf_evidence --no-fail-fast`; `cargo nextest run -p fret-devtools load_regression_summary_drilldown_collects_perf_evidence --no-fail-fast`; `cargo nextest run -p fret-devtools-mcp build_regression_dashboard_result_limits_top_rows_and_builds_human_summary --no-fail-fast` | Covered | Keep DevTools/MCP on `regression_summary_drilldown` instead of adding local summary-evidence parsers. |
| Campaign failing evidence bundle handoff | `diag campaign share` and automatic campaign/batch failure-share outputs expose `bundle_artifact` in `share.manifest.json` and include present bundle artifacts in `share/combined-failures.zip`. | `cargo nextest run -p fret-diag build_campaign_combined_failure_item_zip_entries_orders_existing_item_artifacts resolve_campaign_share_manifest_item_bundle_artifact_prefers_item_path_then_bundle_dir build_campaign_share_manifest_item_run_entry_uses_artifact_snapshot campaign_share_writes_manifest_and_ai_zip_for_failed_items --no-fail-fast` | Covered | Keep bundle artifacts as additive handoff evidence; do not reinterpret Layer A `bundle_json` chunk indexes. |
| DevTools regression drill-down evidence selection | GUI handling of selected summary path, bundle dirs, first-open/share evidence paths, and capability-check artifacts. | `cargo nextest run -p fret-devtools load_regression_summary_drilldown_collects_perf_evidence --no-fail-fast` | Covered | Add another gate only if selection behavior grows beyond evidence-path presentation. |
...
| `diag run` post-run doctor/check orchestration seam | Transport result normalization, bundle-doctor, and post-run check handling in `diag_run`. | `cargo nextest run -p fret-diag maybe_run_bundle_doctor_and_checks_requires_bundle_when_checks_are_requested prepare_cmd_run_setup_plans_bundle_artifact_for_post_run_checks_without_pack --no-fail-fast` | Covered | Keep future `diag run` post-run changes tied to command-setup planning plus bundle-doctor/check helper coverage. |
| `diag suite` default post-run/gate planning seam | Default post-run check planning and lint/doctor orchestration in `diag_suite`. | `cargo test -p fret-diag build_suite_core_default_post_run_checks_sets_small_scroll_vlist_defaults`; `cargo test -p fret-diag finalize_suite_script_success_tail_records_row_when_lint_and_post_run_skip`; default smoke command `cargo run -p fretboard-dev -- diag suite ui-gallery --launch -- cargo run -p fret-ui-gallery --release` | Covered | Keep future suite-seam changes tied to one named helper-level test plus one promoted suite command. |
| Campaign aggregate artifact handoff seam | Shared summary/index/report/share/dashboard path shaping across single-run and batch flows in `diag_campaign`. | `cargo nextest run -p fret-diag campaign_result_aggregate_json_uses_summary_artifact_outputs build_campaign_batch_artifacts_preserves_plan_paths_and_summary_outputs build_campaign_execution_finalize_plan_uses_failure_count_and_summary_setup write_campaign_dashboard_text_uses_shared_dashboard_projection --no-fail-fast`; smoke command `cargo run -p fretboard-dev -- diag campaign run --lane smoke --tag ui-gallery --platform native --launch -- cargo run -p fret-ui-gallery --release` | Covered | Keep handoff-path changes anchored to result/finalize helper tests plus one campaign-run smoke path. |
| Artifact doctor/lint compatibility seam | Artifact normalization, doctor, lint, and bounded-share repair flow. | `cargo test -p fret-diag doctor_report_includes_normalized_capabilities_from_shared_loader`; default commands `cargo run -p fretboard-dev -- diag doctor --check <bundle_dir> --warmup-frames 4` and `cargo run -p fretboard-dev -- diag artifact lint <run_dir|out_dir>` | Covered | Keep doctor/lint compatibility changes tied to one helper test and one maintainer command set. |
