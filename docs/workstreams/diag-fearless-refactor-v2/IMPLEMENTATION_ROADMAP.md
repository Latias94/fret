...
2. extract the next high-ROI orchestration seams: `artifact resolution/materialization`, `run planning/context`, and `suite/campaign resolution`,
   ...
   - next recommended landing: continue inside `commands/resolve` only if a very small remaining normalization seam still pays; otherwise let `resolve` cool down and revisit the higher-ROI holdout in `diag_campaign` or another artifact/materialization command
