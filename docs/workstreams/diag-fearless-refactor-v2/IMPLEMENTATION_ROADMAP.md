...
2. extract the next high-ROI orchestration seams: `artifact resolution/materialization`, `run planning/context`, and `suite/campaign resolution`,
   ...
   - fifty-second landing: `diag_suite` now routes passed-script post-run preparation through `SuiteScriptPostRunPreparationRequest`, so bundle waits, bundle doctor application, and default-check planning no longer stay inline immediately before `apply_post_run_checks`
   - next recommended landing: finish the remaining `diag_suite` success-only lint/finalize tail if it can stay narrowly scoped; otherwise revisit the next `diag_campaign` summary-finalize execution seam or any newly exposed artifact/materialization holdout
   - remaining known holdouts: `diag_suite` still keeps a success-only lint/finalize tail plus a few one-time setup and session-root-adjacent helpers, `diag_run::cmd_run` is mostly resolved-path setup plus exclusive transport dispatch, `diag_campaign` share-manifest handoff is substantially sliced, `commands/resolve` entry shaping plus session/script-result/bundle normalization are substantially de-risked, and the larger artifact/evidence orchestration surface may now pay better in `diag_campaign` or any newly exposed artifact/materialization holdout than in further tiny `resolve` slices
