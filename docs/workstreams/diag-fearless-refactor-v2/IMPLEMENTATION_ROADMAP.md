...
2. extract the next high-ROI orchestration seams: `artifact resolution/materialization`, `run planning/context`, and `suite/campaign resolution`,
   ...
   - latest artifact-resolution landing: `commands/resolve` now routes `diag resolve latest` through dedicated option-parse plus JSON/text render helpers, the deeper session path now routes target session-id selection plus existing-session directory validation through dedicated helpers, and `resolve_script_result_json_path_or_latest` now routes latest-vs-src search-start selection through dedicated helpers, so top-level command parsing/output shaping plus session/script-result search normalization no longer stay intertwined inline and the output contract keeps direct helper-level regression coverage
   ...
   - next recommended landing: continue inside `commands/resolve` by extracting one of the last bundle-ref/input normalization seams (for example the shared root-dir + artifact-path derivation used by `resolve_bundle_ref` / `resolve_bundle_input_or_latest`), unless a newly exposed artifact/materialization holdout proves more painful than the remaining resolve flow
   - remaining known holdouts: `diag_suite` now mainly keeps a few one-time setup and session-root-adjacent helpers that intentionally do not require bundle materialization, `diag_run::cmd_run` is mostly resolved-path setup plus exclusive transport dispatch, `diag_campaign` share-manifest handoff is substantially sliced, `commands/resolve` entry shaping plus session/script-result search selection are substantially de-risked, and the larger artifact/evidence orchestration surface now clusters around the remaining bundle-ref/input normalization seams that still justify extraction
