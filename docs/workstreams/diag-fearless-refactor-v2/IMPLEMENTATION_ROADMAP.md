...
2. extract the next high-ROI orchestration seams: `artifact resolution/materialization`, `run planning/context`, and `suite/campaign resolution`,
   ...
   - latest artifact-resolution landing: `commands/resolve` now routes `diag resolve latest` through dedicated option-parse plus JSON/text render helpers, so top-level command parsing/output shaping no longer stays inline and the output contract gains direct helper-level regression coverage
   ...
   - next recommended landing: return to `commands/resolve` for the next deeper session/bundle resolution helper, unless a newly exposed artifact/materialization holdout proves more painful than the remaining resolve flow
   - remaining known holdouts: `diag_suite` now mainly keeps a few one-time setup and session-root-adjacent helpers that intentionally do not require bundle materialization, `diag_run::cmd_run` is mostly resolved-path setup plus exclusive transport dispatch, `diag_campaign` share-manifest handoff is substantially sliced, `commands/resolve` entry shaping is substantially de-risked, and the larger artifact/evidence orchestration surface now clusters around the deeper session/bundle resolution seams that still justify extraction
