# WGPU Materials Conformance Harness Follow-on v1 — Milestones
...
- `materials_conformance.rs` uses `support::{pixel_rgba, render_scene_rgba8}` for final readback.
- `materials_sampled_conformance.rs` uses `support::{pixel_rgba, render_scene_rgba8}` for final
  readback.
...
- Material conformance tests pass.
