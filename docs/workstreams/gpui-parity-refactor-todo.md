...
- [~] GPUI-MVP3-virt-002 VirtualList: reduce rerender cost during scroll via incremental range reuse (GPUI-component parity).
  ...
  - Perf snapshot (release, `--warmup-frames 5`, `--sort time`, `--repeat 7`; nearest-rank p50/p95; updated after per-row view-cache roots):
    ...
    - Smooth wheel (`tools/diag-scripts/ui-gallery-virtual-list-smooth-scroll.json`):
      ...
      - Note: the smooth-wheel script currently mixes “enter view + scroll” in a short capture window, so the max-frame metric (`--sort time`) is sensitive to transient mount/settle spikes. If we want a steadier scroll-focused metric, introduce a diagnostics “reset” step before capturing (or use a longer scroll segment to push mount frames out of the ring buffer).
  - Commands:
