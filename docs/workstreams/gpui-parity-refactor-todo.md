...
- [~] GPUI-MVP3-rec-001 Define the minimal interaction stream vocabulary for replay.
  ...
  - Evidence: `crates/fret-ui/src/tree/hit_test.rs` (`hit_test_layers_cached`, `try_hit_test_along_cached_path`),
    ...
    `crates/fret-ui/src/tree/prepaint.rs` (`InteractionRecord` focus flags), `crates/fret-ui/src/tree/tests/focus_traversal_prepaint_cache.rs`,
    `crates/fret-ui/src/tree/mod.rs` (`UiDebugLayerInfo` outside-press fields), `ecosystem/fret-bootstrap/src/ui_diagnostics.rs` (`UiLayerInfoV1` export).
