...
| `RowPagination.ts` | `pagination.rs` | Partial | Pagination option gates + `pageCount`/`rowCount` + controlled-state hook parity is gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_pagination_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/pagination.json`). Remaining: `autoResetPageIndex` / `autoResetAll` behaviors + deeper `paginateExpandedRows=false` edge cases. |
...
| `ColumnPinning.ts` | `column_pinning.rs` | Aligned | Option gates + hook/state transition parity is gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_column_pinning_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/column_pinning.json`). |
