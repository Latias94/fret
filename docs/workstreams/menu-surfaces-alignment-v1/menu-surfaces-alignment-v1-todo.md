...
- `docs/workstreams/menu-surfaces-alignment-v1/menu-surfaces-alignment-v1.md`

Related:

- OS menubar tracker: `docs/workstreams/standalone/os-menubar.md`
...
- [x] MENU-MVP0-gating-001 Use `WindowCommandGatingSnapshot` for in-window menu enable/disable.
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (`command_item`, `menubar_from_runtime_with_focus_handle`)
- [x] MENU-MVP0-sanitize-002 Sanitize in-window menu entries (drop duplicate/leading/trailing separators; drop empty submenus).
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (`sanitize_entries` + unit tests)
- [x] MENU-MVP0-parity-003 Make in-window menubar overlay modal by default (prevent click-through).
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (`dismissible_menu_request_with_modal_and_dismiss_handler(..., modal=true)`)
- [x] MENU-MVP0-kbd-004 Keyboard-open initial focus prefers first enabled item, otherwise content root.
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (first-item focus tracking)
...
- [x] MENU-MVP0-sanitize-010 Centralize menu normalization so OS and in-window share the same sanitize rules.
  ...
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (normalizes before building in-window entries)
...
- [x] MENU-MVP1-kbd-021 Implement mnemonic strategy (underlined letter + Alt+Key) or explicitly defer with rationale.
  ...
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (wires `open_on_alt_mnemonic` / `open_on_mnemonic_when_active` into the in-window menubar bridge)
...
- [x] MENU-MVP1-kbd-026 Render mnemonic underlines for in-window menubar triggers (presentation).
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (`attributed_title_with_mnemonic_underline`, trigger label rendering)
...
- [x] MENU-MVP1-kbd-022 Ensure Escape closes submenu, then menu, then returns focus to trigger (predictable unwind).
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (test `escape_unwinds_submenu_then_menu_and_restores_focus`)
...
- [x] MENU-MVP1-parity-025 Implement submenu “grace intent” (diagonal pointer travel tolerance); cover via `MENU-MVP4-test-052`.
  ...
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (installs submenu pointer-move handler for menubar overlays)
...
- [x] MENU-MVP2-sem-032 Add a minimal in-window rendering for checked/radio indicators (no OS mapping required at first).
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (checkbox/radio semantics roles + checkmark indicator)
...
- [x] MENU-MVP3-dyn-040 Define a stable “dynamic submenu” contract (IDs, update frequency, ordering rules).
  ...
  - Evidence: `ecosystem/fret/src/in_window_menubar.rs` (in-window bridge for `Label`)
