# ImUi Debug Draw Path Bezier Builder v1 Evidence And Gates
...
- `repo-ref/imgui/imgui.h`: `ImDrawList::PathBezierCubicCurveTo` and
  `PathBezierQuadraticCurveTo`.
- `repo-ref/imgui/imgui_draw.cpp`: Dear ImGui appends sampled Bezier points to the current path and
  then finishes through `PathStroke` / `PathFillConvex`.
...
python -m json.tool docs/workstreams/imui-debug-draw-path-bezier-builder-v1/WORKSTREAM.json
