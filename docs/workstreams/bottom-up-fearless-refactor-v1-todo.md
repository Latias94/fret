...
- [ ] BU-FR-guard-004 Convert “huge Rust conformance sources” into data-driven harnesses where possible.
  ...
  - Evidence (wiring, split modules are exercised by the integration test root):
    - `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout.rs` (`mod chart`, `mod layout_chart_scaffold_fixtures`, `mod dashboard`, `mod layout_form_fixtures`, `mod layout_field_fixtures`, `mod layout_input_fixtures`, `mod layout_scroll_fixtures`, `mod layout_typography_fixtures`, `mod kbd`, `mod pagination`, `mod skeleton`, `mod sonner`)
