...
| App prelude vocabulary | named overlap-heavy trait/helper exports shared with the component lane | app nouns stay named; overlap-heavy trait helpers become anonymous imports or move off the app lane entirely | first hide trait-name noise with `as _`, then remove raw helper names that compete with the grouped app action story, then move style/icon/env/state nouns plus zero-usage theme/key/command-gating traits off the default lane before revisiting the remaining helper overlap | `fret::app::prelude::*` no longer named-exports component-centric trait/helper families, style/icon/env/state nouns are explicit through `fret::style` / `fret::icons::{icon, IconId}` / `fret::env` / `fret::selector` / `fret::query`, app-facing semantic roles are explicit through `fret::semantics::SemanticsRole`, reusable-component command identity is explicit through `fret::actions::CommandId`, and `Px` is documented as an intentional shared primitive rather than unresolved overlap debt; the status docs match the shipped source/tests | In progress | `ecosystem/fret/src/lib.rs`, `ecosystem/fret/src/actions.rs`, `apps/fret-cookbook/examples/hello_counter.rs`, `apps/fret-examples/src/hello_counter_demo.rs`, `apps/fretboard/src/scaffold/templates.rs`, `docs/crate-usage-guide.md`, `TARGET_INTERFACE_STATE.md`, `TODO.md` |
...
| `ecosystem/fret-ui-shadcn` | recipe crate with some broad imports and optional app integration | recipe/taxonomy crate on top of component surface + explicit `shadcn::app` seam, with advanced service hooks kept off the curated facade | keep optional app integration visible without turning shadcn root into a second app-runtime namespace; expose `fret::shadcn` as a curated facade with explicit `themes` / `raw` seams, teach direct-crate advanced hooks through `fret_ui_shadcn::advanced::*` instead of the raw crate root, keep first-party raw usage limited to documented escape-hatch lanes, and keep gallery/page snippets off the flat root/module paths | default examples/docs use `shadcn::app::*` for setup, `shadcn::themes::*` for presets, keep advanced hooks explicit, and keep fully raw access explicit through `shadcn::raw::*`; remaining direct-crate cleanup is now bounded to internal gate strings and any future drift rather than active first-party code paths | Migrated | `ecosystem/fret-ui-shadcn/src/lib.rs`, `ecosystem/fret-ui-shadcn/src/advanced.rs`, `ecosystem/fret/src/lib.rs`, `ecosystem/fret-ui-ai/src/elements/model_selector.rs`, `ecosystem/fret-ui-ai/tests/shadcn_import_surface.rs`, `tools/gate_fret_ui_ai_curated_shadcn_surfaces.py`, `tools/pre_release.py`, `ecosystem/fret-bootstrap/src/ui_app_driver.rs`, `apps/fret-cookbook/src/lib.rs`, `apps/fret-examples/src/lib.rs`, `apps/fret-ui-gallery/src/ui/doc_layout.rs`, `apps/fret-ui-gallery/tests/ui_authoring_surface_import_policies.rs`, `docs/crate-usage-guide.md`, `docs/shadcn-declarative-progress.md` |
