...
| App prelude vocabulary | named overlap-heavy trait/helper exports shared with the component lane | app nouns stay named; overlap-heavy trait helpers become anonymous imports or move off the app lane entirely | first hide trait-name noise with `as _`, then remove raw helper names that compete with the grouped app action story, then move style/icon/env/state nouns plus zero-usage theme/key/command-gating traits off the default lane before revisiting the remaining helper overlap | `fret::app::prelude::*` no longer named-exports component-centric trait/helper families, style/icon/env/state nouns are explicit through `fret::style` / `fret::icons::{icon, IconId}` / `fret::env` / `fret::selector` / `fret::query`, sink-style child collection is explicit through `fret::children::UiElementSinkExt as _`, command-availability reads are explicit through `fret::actions::ElementCommandGatingExt as _`, app-facing semantic roles are explicit through `fret::semantics::SemanticsRole`, reusable-component command identity is explicit through `fret::actions::CommandId`, and `Px` is documented as an intentional shared primitive rather than unresolved overlap debt; the status docs match the shipped source/tests | In progress | `ecosystem/fret/src/lib.rs`, `ecosystem/fret/src/actions.rs`, `apps/fret-cookbook/examples/commands_keymap_basics.rs`, `apps/fret-cookbook/examples/form_basics.rs`, `apps/fret-cookbook/examples/overlay_basics.rs`, `apps/fret-cookbook/examples/virtual_list_basics.rs`, `docs/crate-usage-guide.md`, `TARGET_INTERFACE_STATE.md`, `TODO.md` |
| Component prelude | mixed import posture via `fret-ui-kit` / recipe crates | `fret::component::prelude::*` plus explicit secondary lanes such as `fret::activate::{...}` for raw activation-helper glue | replace blanket `fret-ui-kit::prelude::*` forwarding with curated component-author exports, move raw `on_activate*` helpers off the default component lane, and migrate first-party helper surfaces | first-party reusable crates no longer need app prelude, raw activation helpers stay explicit, and component docs/examples stay on the curated surface | In progress | `ecosystem/fret/src/lib.rs`, `docs/crate-usage-guide.md`, `docs/component-author-guide.md`, `apps/fret-cookbook/src/scaffold.rs`, `TARGET_INTERFACE_STATE.md` |
