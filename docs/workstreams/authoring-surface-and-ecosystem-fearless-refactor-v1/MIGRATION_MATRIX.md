...
| App prelude vocabulary | named overlap-heavy trait/helper exports shared with the component lane | app nouns stay named; overlap-heavy trait helpers become anonymous imports or move off the app lane entirely | first hide trait-name noise with `as _`, then remove raw helper names that compete with the grouped app action story, then revisit remaining high-frequency type overlap explicitly | `fret::app::prelude::*` no longer named-exports component-centric trait/helper families and the status docs match the shipped source/tests | In progress | `ecosystem/fret/src/lib.rs`, `ecosystem/fret/tests/uicx_data_surface.rs`, `TARGET_INTERFACE_STATE.md`, `TODO.md` |
