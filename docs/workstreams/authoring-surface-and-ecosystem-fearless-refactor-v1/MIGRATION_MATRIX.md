...
| App prelude vocabulary | named overlap-heavy trait/helper exports shared with the component lane | app nouns stay named; overlap-heavy trait helpers become anonymous imports or move off the app lane entirely | first hide trait-name noise with `as _`, then remove raw helper names that compete with the grouped app action story, then move style/icon/env/state nouns plus zero-usage theme/key/command-gating traits off the default lane before revisiting the remaining helper overlap | `fret::app::prelude::*` no longer named-exports component-centric trait/helper families, style/icon/env/state nouns are explicit through `fret::style` / `fret::icons` / `fret::env` / `fret::selector` / `fret::query`, zero-usage theme/key/command-gating traits are gone from the default lane, and the status docs match the shipped source/tests | In progress | `ecosystem/fret/src/lib.rs`, `apps/fret-cookbook/examples/hello_counter.rs`, `apps/fret-examples/src/todo_demo.rs`, `apps/fretboard/src/scaffold/templates.rs`, `docs/crate-usage-guide.md`, `TARGET_INTERFACE_STATE.md`, `TODO.md` |
