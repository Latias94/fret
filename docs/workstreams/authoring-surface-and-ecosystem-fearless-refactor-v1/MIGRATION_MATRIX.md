...
| Component prelude | mixed import posture via `fret-ui-kit` / recipe crates | `fret::component::prelude::*` plus explicit secondary lanes such as `fret::activate::{...}` for raw activation-helper glue and `fret::overlay::*` for lower-level overlay anchoring/introspection nouns | replace blanket `fret-ui-kit::prelude::*` forwarding with curated component-author exports, move raw `on_activate*` helpers off the default component lane, keep only high-frequency overlay builder nouns on the prelude, and migrate first-party helper surfaces | first-party reusable crates no longer need app prelude, raw activation helpers stay explicit, overlay stack/introspection names stay off wildcard imports, and component docs/examples stay on the curated surface | In progress | `ecosystem/fret/src/lib.rs`, `docs/crate-usage-guide.md`, `docs/component-author-guide.md`, `apps/fret-cookbook/src/scaffold.rs`, `TARGET_INTERFACE_STATE.md` |
...
| Default builder setup seam | mixed `install_app` / `install` / `init_app` naming on the default `fret` path | `FretApp::setup(...)` for named installers/tuples/bundles and `UiAppBuilder::setup_with(...)` for explicit one-off closures | migrate docs/examples/tests on the `fret` facade, keep `.setup(...)` grep-friendly, and keep raw bootstrap names explicit | default docs/templates/examples no longer teach legacy setup names or `.setup(|app| ...)` on the `fret` facade | In progress | `ecosystem/fret/src/app_entry.rs`, `ecosystem/fret/src/lib.rs`, `apps/fret-examples/src/lib.rs`, `docs/workstreams/app-entry-builder-v1/DESIGN.md` |
