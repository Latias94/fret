...
| `ecosystem/fret-router` | app-level ecosystem with its own teaching posture | grouped app/advanced extension surface | keep `fret-router-ui` as a thin app-owned adoption layer instead of a parallel runtime | official routing examples use the new grouped posture | Not started | `ecosystem/fret-router/src/lib.rs`, `ecosystem/fret-router-ui/src/lib.rs`, `DESIGN.md` |
