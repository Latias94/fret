...
- [ ] Move overlay/menu/toolbar policy to the right ecosystem surfaces where that boundary is
  ...
  - Progress: retained action-panel pointer state now also routes through the private
    ...
    activate-on-matching-up authority instead of each re-embedding that pointer-state policy
    inline.
  ...
  - Progress: searcher session take/restore now also routes through the private
  ...
  - Progress: command-driven conversion-picker menu replacement now also routes through the
  ...
  - Progress: retained toolbar anchor layout lifecycle now also routes through the private
