...
The main text pipeline tracker remains: `docs/workstreams/standalone/text-system-v2-parley.md`.
Conformance gates for fallback policy changes live in: `docs/workstreams/standalone/font-fallback-conformance-v1.md`.
Audit follow-ups and refactor milestones are tracked in: `docs/workstreams/standalone/font-system-audit-zed-parley-xilem-v1.md`.
...
- M2 (implemented, partial): picker metadata (axes + monospace hint) is available via `FontCatalogMetadata`.
  - Next: settings UI adoption + explicit refresh/invalidation policy and caching (tracked in
    `docs/workstreams/standalone/font-catalog-refresh-policy-v1.md` and ADR 0258).
