...
Primary text pipeline tracker remains: `docs/workstreams/standalone/text-system-v2-parley.md`.
...
- Document when runners refresh the catalog (startup/adopt + `TextAddFontAssets`).
...
- The renderer retains injected font bytes (from `Effect::TextAddFontAssets`) to re-register them
  during a system rescan.
