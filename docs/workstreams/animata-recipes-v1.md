...
| AR-TOAST-001 | Toast stack shift + interrupt | (Animata: use `stack.shift` intent) | `ecosystem/fret-ui-shadcn/src/sonner.rs` | `stack.shift` | `tools/diag-scripts/ui-gallery-sonner-interrupt-fixed-frame-delta.json` + `tools/diag-scripts/ui-gallery-sonner-stack-shift-stagger-fixed-frame-delta.json` | Landed (baseline + staggered reflow) |
