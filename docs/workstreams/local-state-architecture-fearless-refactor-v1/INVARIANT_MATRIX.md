...
| Lower portable crates must not depend on app-facing `LocalState<T>` | `fret-selector` / `fret-query` / lower mechanism crates must stay portable and layering-safe. | `docs/workstreams/authoring-density-reduction-fearless-refactor-v1/SELECTOR_QUERY_DIRECTION_2026-03-16.md`; `ecosystem/fret/src/view.rs` (`LocalSelectorLayoutInputs` and the internal selector-deps bridge substrate) | Reject options that solve app-facing ergonomics by inverting crate layering. |
