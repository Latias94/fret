...
- `docs/workstreams/control-chrome-normalization-audit-v1/control-chrome-normalization-audit-v1-milestones.md`
...
- [ ] Add `fretboard-dev diag` scenarios that exercise stretch-sensitive chrome:
  ...
  - [x] Material3 Autocomplete option: chrome fills pressable: `tools/diag-scripts/ui-gallery-material3-autocomplete-option-chrome-fill.json`
  - [x] Material3 TimePicker: chrome fills + ids derived: `tools/diag-scripts/ui-gallery-material3-time-picker-chrome-fill.json`
...
| `ecosystem/fret-ui-shadcn/src/navigation_menu.rs` | `NavigationMenuLink` | `ControlChrome` | Yes | Yes | OK | N/A | `navigation_menu_link_uses_control_chrome_with_derived_chrome_id` | Uses `control_chrome_pressable_with_id_props`; chrome id is derived from the link test id. |
...
| `ecosystem/fret-ui-material3/src/autocomplete.rs` | Autocomplete options | `ManualFill` | Yes | Yes | OK | Optional: derive chrome test ids consistently | `tools/diag-scripts/ui-gallery-material3-autocomplete-option-chrome-fill.json` | Adds `<id>.chrome` semantics to support diag gates. |
| `ecosystem/fret-ui-material3/src/time_picker.rs` | TimePicker scrim/dial/fields | `ManualFill` | Yes | Yes | OK | Optional: derive chrome test ids consistently | `tools/diag-scripts/ui-gallery-material3-time-picker-chrome-fill.json` | Adds `<id>.chrome` semantics to support diag gates. |
...
| `ecosystem/fret/src/in_window_menubar.rs` | Menubar trigger | `ControlChrome` | Possible (caller-dependent) | Yes | OK | N/A | Uses helper | Migrated to `control_chrome_pressable_with_id_props`. |
| `ecosystem/fret/src/in_window_menubar.rs` | Menubar item | `ControlChrome` | Yes (`w = Fill`) | Yes | OK | N/A | Uses helper | Migrated to `control_chrome_pressable_with_id_props`. |
...
| `ecosystem/fret-workspace/src/tab_strip.rs` | Workspace TabStrip tabs | `ControlChrome` | Yes | Yes | OK | N/A | `ecosystem/fret-workspace/src/tab_strip/mod.rs` unit test | Migrated to `control_chrome_pressable_with_id_props`; chrome test id is derived from the tab pressable semantics. |
