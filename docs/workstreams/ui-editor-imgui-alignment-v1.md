...
| Target | Layer | Milestone | ImGui reference (non-normative) | Precision reference (non-normative) | Notes / gaps |
| --- | --- | --- | --- | --- | --- |
...
| `VecNEdit` | B | M3 | `DragFloat2/3/4` | `repo-ref/fret-ui-precision/src/components/showcase/demos/MatrixInputDemo.tsx` | `editor.numeric.*`, axis color tokens | `axis_label_slot`, `axis_reset_hook` | Axis labels + axis color tokens; per-axis reset hooks. |
| `TransformEdit` | C | M3 | common editor composite | `repo-ref/fret-ui-precision/src/components/showcase/demos/MaterialEditorDemo.tsx` | `editor.density.*` | `layout_variant`, per-section slots | Composite of VecN + Angle; layout variants (row/column). |
