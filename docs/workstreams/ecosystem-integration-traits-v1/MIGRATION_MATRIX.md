...
| --- | --- | --- | --- | --- | --- | --- |
| App installer composition | mixed `install_app`, `install`, and ad-hoc bundle patterns | canonical installer functions plus `InstallIntoApp` for composition | add a thin trait adapter over existing installer functions | official examples use one consistent app wiring story | In progress | `ecosystem/fret/src/integration.rs`, `ecosystem/fret/src/app_entry.rs`, `ecosystem/fret/src/lib.rs`, `ecosystem/fret/README.md`, `DESIGN.md` |
...
| Command palette/catalog extension | shadcn recipe code currently owns most command palette composition | `CommandCatalog` in a reusable policy layer | extract catalog contracts from shadcn UI recipe code | recipe crates consume, rather than own, the catalog contract | Not started | `ecosystem/fret-ui-shadcn/src/command.rs`, `DESIGN.md` |
| Typed route integration | typed routes are possible, but there is no shared codec seam yet | `RouteCodec` | add a shared route encode/decode contract next to router core | official router examples stop teaching stringly route construction as the primary path | Not started | `ecosystem/fret-router/src/location.rs`, `docs/workstreams/router-v1.md`, `DESIGN.md` |
| Reusable dock panel contributions | app-owned registry service exists, but contribution-level factory is not standardized | `DockPanelFactory` aggregated into the dock registry story | define a contribution unit that feeds the existing registry service | reusable panel packs no longer need bespoke registration shapes | Not started | `ecosystem/fret-docking/src/dock/panel_registry.rs`, `docs/adr/0016-plugin-and-panel-boundaries.md`, `DESIGN.md` |
...
| `fret-docking` | registry service exists, but reusable contribution shape is app-specific | explicit panel-factory contribution model feeding the registry | keep app-owned aggregation and stable `PanelKind` identity | reusable panel packs use the shared contribution seam | Not started | `ecosystem/fret-docking/src/dock/panel_registry.rs`, `docs/adr/0013-docking-ops-and-persistence.md` |
| `fret-router` | strong core/location/history contract; typed route seam not standardized | typed route integration via `RouteCodec` | keep history/canonicalization router-owned | official route examples use typed codec-driven patterns | Not started | `ecosystem/fret-router/src/lib.rs`, `docs/workstreams/router-v1.md`, `docs/workstreams/router-ui-v1.md` |
...
| ad-hoc panel registration logic per app | `DockPanelFactory` plus registry aggregation | first reusable panel pack migrates | Not started |
| stringly route construction in official routing guidance | typed route codec + canonical location helpers | first official router examples migrate | Not started |
