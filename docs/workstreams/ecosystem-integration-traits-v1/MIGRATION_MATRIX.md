...
| ad-hoc panel registration logic per app | `DockPanelFactory` plus registry aggregation | reusable panel packs and baseline first-party demos migrate; remaining dynamic-kind demos are refactored to stable `PanelKind + instance` identity | In progress |
