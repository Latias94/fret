...
| `fret-ui-assets` / `fret-icons-*` / `fret-node` app helpers | direct app-install helpers used to be re-exported from the crate root with mixed `install_app` / `install` naming | explicit `crate::app::*` submodules for default app wiring, plus explicitly named advanced UI-services helpers | move first-party callers/docs/templates to `crate::app::install(...)`, rename UI-services-boundary wrappers so they stop competing with the default app path, and add surface-policy tests per crate | official docs/templates/examples no longer teach root-level app helper re-exports | Migrated | `ecosystem/fret-ui-assets/src/lib.rs`, `ecosystem/fret-icons-lucide/src/lib.rs`, `ecosystem/fret-icons-radix/src/lib.rs`, `ecosystem/fret-node/src/lib.rs`, `ecosystem/fret-bootstrap/src/lib.rs`, `apps/fretboard/src/scaffold/templates.rs`, `docs/crate-usage-guide.md`, `docs/examples/todo-app-golden-path.md` |
