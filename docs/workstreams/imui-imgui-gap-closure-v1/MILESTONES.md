...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/collection_module.rs` owns the collection module
routing assertion bundle for the collection entrypoint, module declarations, re-exports/imports,
child-owner orchestration calls, status readout mounting, and header mounting. No app runtime code,
public APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface chrome owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/chrome.rs` owns the collection chrome/readout
assertion bundle for `proof_collection_readout_text(...)`, `render_collection_header(...)`, section
label mounting, header/prose copy, and root negative routing checks. No app runtime code, public
APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface derived state owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/derived_state.rs` owns the collection derived
visible-state assertion bundle for `ProofCollectionDerivedState`, visible-order asset/key
projection, active-id projection, rename-ready projection, and root negative routing checks. No app
runtime code, public APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.
...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/lifecycle.rs` owns the collection stale rename
cleanup assertion bundle for runtime model/snapshot inputs, missing-asset detection,
rename-session/focus reset updates, and root negative routing checks. No app runtime code, public
APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.
...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/status_readouts.rs` owns the collection
status/readouts assertion bundle for status mounting, readouts hub/re-exports, status formatting,
and root negative routing checks. No app runtime code, public APIs, `fret-imui`,
`fret-ui-kit::imui`, docking, or runner contract changed.
...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/assets.rs` owns the collection asset fixture
assertion bundle for asset fixture defaults. No app runtime code, public APIs, `fret-imui`,
`fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface selection commands owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/selection_commands.rs` owns the selection command
hub re-export assertion bundle. No app runtime code, public APIs, `fret-imui`,
`fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface models owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/models.rs` owns the collection model state slot
registration assertion bundle. No app runtime code, public APIs, `fret-imui`,
`fret-ui-kit::imui`, docking, or runner contract changed.
...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/selection_duplicate_naming.rs` owns the selection
duplicate naming assertion bundle for copy-suffix registry helpers, naming behavior tests, naming
fixtures, and duplicate-flow drift checks. No app runtime code, public APIs, `fret-imui`,
`fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface selection duplicate owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/selection_duplicate.rs` owns the selection duplicate
command assertion bundle for duplicate shortcut/facade exports, shortcut tests, shortcut-test
exclusion, and naming/selection-repair child-owner delegation boundaries. No app runtime code,
public APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface selection delete owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/selection_delete.rs` owns the selection delete
command assertion bundle for delete/refocus transitions, delete behavior tests, selection fixtures,
and negative drift boundaries. No app runtime code, public APIs, `fret-imui`, `fret-ui-kit::imui`,
...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/selection_keyboard.rs` owns the selection keyboard
assertion bundle for arrow/range/Escape policy, keyboard selection tests/fixtures, navigation
helpers, navigation tests/fixtures, and negative drift boundaries. No app runtime code, public APIs,
`fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface selection owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/selection.rs` owns the selection assertion bundle
for selection hub exports, visible-order projection, selected-assets projection, and active-id
fallback boundaries. No app runtime code, public APIs, `fret-imui`, `fret-ui-kit::imui`, docking,
or runner contract changed.

2026-06-10 Fret Examples collection modularization surface rename owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/rename.rs` owns the rename assertion bundle for
rename hub, tests, fixtures, commit, focus, and negative drift boundaries. No app runtime code,
public APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface keyboard owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/keyboard.rs` owns the keyboard assertion bundle for
keyboard dispatch, actions mutation, and negative drift boundaries. No app runtime code, public
APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.
...
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/drag_drop.rs` owns the drag/drop assertion bundle for
payload projection, behavior-test, fixture, and negative drift boundaries. No app runtime code,
public APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner contract changed.

2026-06-10 Fret Examples collection modularization surface context-menu owner-split result:
`apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` now keeps the single
source-surface test entry point and include bundle, while
`imui_editor_collection_modularization_surface/context_menu.rs` owns the context-menu assertion
bundle for popup workflow, action writeback, menu chrome/options/test IDs, and negative drift
boundaries. No app runtime code, public APIs, `fret-imui`, `fret-ui-kit::imui`, docking, or runner
contract changed.
...
`disabled_options/closable.rs` owns `closable=false` proof, `disabled_options/movable.rs` owns
`movable=false` proof, `disabled_options/resizable.rs` owns `resizable=false` proof, and
`disabled_options/collapsible.rs` owns `collapsible=false` proof. `resize.rs` owns corner and
left-edge resize gesture proof, and `collapse.rs` owns title-bar double-click collapse/expand
lifecycle proof including stable floating area id and resize-handle visibility while collapsed. No
`fret-ui-kit::imui` floating-window runtime implementation, `fret-imui` public facade, or floating
option API changed.
...
owners, `selection/changed.rs` owns changed-on-pick plus selected model projection proof,
`selection/lifecycle.rs` owns lifecycle edit plus deactivated-after-edit proof, `popup.rs` owns
popup Escape close/focus restore and popup scope/test-id override proof, `shortcuts.rs` routes
combo-model shortcut proof owners, `shortcuts/focus_scope.rs` owns focused-trigger shortcut scoping
proof, and `shortcuts/repeat.rs` owns `shortcut_repeat` opt-in proof. No `fret-ui-kit::imui`
combo-model runtime implementation, `fret-imui` public facade, or combo-model option API changed.
...
the direct combo proof surface. `combo_direct/lifecycle.rs` routes popup Escape/focus restore and
open-session edge reporting child owners, `shortcuts.rs` routes direct-combo shortcut proof owners,
`shortcuts/focus_scope.rs` owns focused-trigger shortcut scoping proof, `shortcuts/repeat.rs` owns
`shortcut_repeat` opt-in proof, and `selection.rs` routes selectable-row commit, selected
preview/model projection, and close-after-pick child owners. No `fret-ui-kit::imui` combo runtime implementation,
`fret-imui` public facade, or combo option API changed.

2026-06-10 Fret-ImUi combo-direct lifecycle proof child-owner split result:
`ecosystem/fret-imui/src/tests/models_combo/combo_direct/lifecycle.rs` now keeps only child-owner
routing for direct combo lifecycle proofs. `lifecycle/escape_focus.rs` owns popup Escape close,
option disappearance, and trigger focus restore proof, while `lifecycle/session_edges.rs` owns
open-session edge reporting through `activated`, `deactivated`, `edited`,
`deactivated_after_edit`, and `open` response reads. No combo runtime implementation,
`fret-ui-kit::imui` combo policy, `fret-imui` public facade, combo option API, or test semantics
changed.

2026-06-10 Fret-ImUi combo-direct selection proof child-owner split result:
`ecosystem/fret-imui/src/tests/models_combo/combo_direct/selection.rs` now keeps only child-owner
routing for direct combo selection proofs. `selection/commit.rs` owns the scenario-level
trigger/open/pick/close assertions, while `selection/harness.rs` owns selected-model fixture setup,
repeated selectable combo rendering, selectable row test IDs, selected model writeback, popup close,
...
property group: field visibility matching, opacity DragValue outcome writeback, reset handlers,
roughness/metallic sliders, base color ColorEdit, asset ref rows, shading EnumSelect, alpha clip,
cast shadows, and the Material no-matches row. `imui_editor_proof_demo.rs` now only declares/imports
the material owner and passes `EditorMaterialModels` while keeping route/workbench/render
...
2026-06-07 collection command-buttons chrome owner split result:
`apps/fret-examples/src/imui_editor_proof_demo/collection/command_buttons.rs` now keeps
Duplicate/Rename/Delete button mounting, clicked-response routing, duplicate/delete state
transitions, inline-rename startup routing, app model writes, and command-status publication while
routing button labels, enabled-state option construction, and button test IDs through the
demo-local `collection/command_buttons/chrome.rs` child owner. Public crate APIs, button labels,
...
active-tile update projection, and `#[cfg(test)] mod tests;` while routing context-menu behavior coverage through the demo-local
`collection/selection/context_menu/tests.rs` test owner. Right-click selection replacement,
selected-range preservation, active-tile updates, asset-grid call imports, public crate APIs, and
the app-owned no-helper-widening boundary remain unchanged, while the collection source gate,
workstream source gate, manifest, and surface tests now freeze the context-menu selection tests
owner boundary. The context-menu selection tests owner boundary is now tracked as its own
source-gated proof slice.

2026-06-09 collection context-menu selection fixture owner split result:
`apps/fret-examples/src/imui_editor_proof_demo/collection/selection/context_menu/tests.rs` now
keeps the context-menu selection behavior tests and imports fixture helpers through `mod fixtures;`
while routing selection fixture construction/projection through the demo-local
`collection/selection/context_menu/tests/fixtures.rs` fixture owner. Right-click selection
...
selection fixture owner boundary. The context-menu selection fixture owner boundary is now tracked
...
command/context-menu/keyboard/select-all re-exports while routing arrow-key movement, Shift-range
extension, Escape clear behavior, and primary-modifier shortcut bypass through the demo-local
`collection/selection/keyboard.rs` child owner. Public crate APIs, keyboard handler imports, and
the app-owned no-helper-widening boundary remain unchanged, while the collection source gate,
...
`collection/selection/commands/duplicate/naming/tests.rs` test owner, which now imports asset setup
through `mod fixtures;`. Copy suffix text, duplicate command flow, selection repair,
keyboard/button/context-menu command behavior, public crate APIs, and the app-owned
no-helper-widening boundary remain unchanged, while the collection source gate, workstream source
gate, manifest, and surface tests now freeze the duplicate naming tests owner boundary.

2026-06-09 collection duplicate naming fixture owner split result:
`apps/fret-examples/src/imui_editor_proof_demo/collection/selection/commands/duplicate/naming/tests.rs`
now keeps duplicate naming registry behavior assertions and routes asset construction through the
demo-local `collection/selection/commands/duplicate/naming/tests/fixtures.rs` fixture owner.
`naming/tests/fixtures.rs` owns `pub(super) fn asset(...)`, `ProofCollectionAsset` construction,
the default `"Texture"` kind, and `size_kib: 256` without taking registry behavior coverage,
duplicate command flow, keyboard/button/context-menu command behavior, public crate APIs, or the
app-owned no-helper-widening boundary. The collection source gate, workstream source gate, manifest,
and surface tests now freeze the duplicate naming fixture owner boundary.
...
selection publication, active-id publication, and click-clear runtime while routing pure
left-button box-select pointer session transitions through the demo-local
`collection/browser_scope/input_runtime/box_select/session.rs` child owner. Left-button background
arming, pressable-origin suppression after focus request, baseline selection capture, append-mode
semantics, drag-threshold handling, pointer-id mismatch restoration, pointer cancel behavior,
...
2026-06-08 collection duplicate selection tests owner split result:
`apps/fret-examples/src/imui_editor_proof_demo/collection/selection/commands/duplicate/selection.rs`
now keeps selected-visible filtering, duplicate insertion, visible-order duplicate projection,
selection/anchor repair, active copied-tile repair, and `#[cfg(test)] mod tests;` while routing
selection fixtures and duplicate selection behavior coverage through the demo-local
`collection/selection/commands/duplicate/selection/tests.rs` test owner. Copy suffix text,
duplicate insertion order, visible-order copy reselect behavior, active copied-tile preservation,
keyboard/button/context-menu command behavior, public crate APIs, and the app-owned
no-helper-widening boundary remain unchanged, while the collection source gate, workstream source
...
and `#[cfg(test)] mod tests;` while routing select-all behavior coverage through the
...
2026-06-09 collection select-all fixture owner split result:
`apps/fret-examples/src/imui_editor_proof_demo/collection/selection/select_all/tests.rs` now keeps
select-all behavior tests, `ProofCollectionKeyboardState`, and `ImUiMultiSelectState::default()`
while importing fixture helpers through `mod fixtures;` and routing selection fixture
construction/projection through the demo-local
`collection/selection/select_all/tests/fixtures.rs` fixture owner. `Primary+A` matching,
visible-order full selection, anchor preservation/fallback, active-tile preservation/fallback,
keyboard dispatch imports, public crate APIs, and the app-owned no-helper-widening boundary remain
unchanged, while the collection source gate, workstream source gate, manifest, and surface tests
now freeze the select-all fixture owner boundary. The select-all fixture owner boundary is now
tracked as its own source-gated proof slice.
