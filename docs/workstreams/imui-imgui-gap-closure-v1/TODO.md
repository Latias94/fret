...
- [x] Split `VirtualListOptions` field/default/debug ownership out of
      `ecosystem/fret-ui-kit/src/imui/options/collections/virtual_list.rs` without changing the
      public option type, public fields, default values, `Debug` formatting, virtual-list runtime
      behavior, `fret-imui` facade shape, or crate boundaries.
      Result: `virtual_list.rs` is now the public hub, `virtual_list/options.rs` owns the
      `VirtualListOptions` field contract, `virtual_list/debug.rs` owns `Debug`, and
      `virtual_list/defaults.rs` owns `Default`. The IMUI workstream source gate now freezes that
      field/debug/default split.
...
- [x] Split `TextAreaOptions` default policy out of
      `ecosystem/fret-ui-kit/src/imui/options/controls/text/textarea/options.rs` without changing
      the public option type, public fields, default values, multiline command behavior,
      `fret-imui` facade shape, or crate boundaries.
      Result: `textarea.rs` is the public hub, `textarea/options.rs` owns the `TextAreaOptions`
      field contract, and `textarea/defaults.rs` owns the `Default` implementation. The IMUI
      workstream source gate now freezes that field/default split.
...
- [x] Split the `DragResponse` core record out of
      `ecosystem/fret-ui-kit/src/imui/response/drag.rs` without changing public response names,
      drag source/drop target response ownership, drag/drop behavior, `fret-imui` facade shape, or
      crate boundaries.
      Result: `response/drag.rs` is now a small re-export hub, while
      `response/drag/core.rs` owns the `DragResponse` fields, private mutation helpers, edge merge
      policy, and public accessors. The IMUI workstream source gate now freezes the hub/core split.
...
- [x] Split the collection module-routing assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod collection_module;` and delegates to
      `collection_module::assert_collection_module_routing(...)`, while
      `imui_editor_collection_modularization_surface/collection_module.rs` owns collection
      entrypoint, module declaration, re-export/import, child-owner call, status readout, and header
      mounting checks. The IMUI workstream source gate freezes the test-owner split.
...
- [x] Split the collection derived visible-state assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod derived_state;` and delegates to
      `derived_state::assert_derived_state_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/derived_state.rs` owns visible-order asset/key
      projection, active-id projection, rename-ready projection, and collection-root negative
      routing checks. The IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Runtime State Owner Split - 2026-06-10

- [x] Split the collection runtime model/snapshot assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod runtime_state;` and delegates to
      `runtime_state::assert_runtime_state_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/runtime_state.rs` owns runtime structs, model
      handle lookups, selector snapshots, layout projection, and collection-root negative routing
      checks. The IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Child Models Owner Split - 2026-06-10

- [x] Split the collection child model bundle assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod child_models;` and delegates to
      `child_models::assert_child_models_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/child_models.rs` owns
      `ProofCollectionChildModels`, child DTO assembly, cloned model handles, and collection-root
      negative routing checks. The IMUI workstream source gate freezes the test-owner split.
...
- [x] Split the collection reverse-order button assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod order_toggle;` and delegates to
      `order_toggle::assert_order_toggle_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/order_toggle.rs` owns reverse-order button
      labels, order-toggle test ID, model update, returned state, and collection-root negative
      routing checks. The IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Status Readouts Owner Split - 2026-06-10

- [x] Split the collection status/readouts assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod status_readouts;` and delegates to
      `status_readouts::assert_status_readouts_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/status_readouts.rs` owns status mounting,
      readouts hub/re-export checks, readout status formatter checks, and collection-root negative
      routing checks. The IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Import Target Owner Split - 2026-06-10

- [x] Split the collection import target/drop-status UI assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod import_target;` and delegates to
      `import_target::assert_import_target_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/import_target.rs` owns import target routing
      and root negative drift checks. The IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Assets Owner Split - 2026-06-10

- [x] Split the collection asset fixture assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod assets;` and delegates to
      `assets::assert_assets_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/assets.rs` owns asset fixture defaults. The
      IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Selection Commands Owner Split - 2026-06-10

- [x] Split the selection command hub re-export assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod selection_commands;` and delegates to
      `selection_commands::assert_selection_commands_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/selection_commands.rs` owns the `delete` /
      `duplicate` sub-owner declaration and re-export checks. The IMUI workstream source gate
      freezes the test-owner split.
...
- [x] Split the selection duplicate insertion/reselect-repair assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod selection_duplicate_selection;` and
      delegates to
      `selection_duplicate_selection::assert_selection_duplicate_selection_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/selection_duplicate_selection.rs` owns
      duplicate insertion checks, reselect repair checks, duplicate selection behavior coverage,
      selection fixtures, and policy negative drift checks. The IMUI workstream source gate freezes
...
- [x] Split the selection delete command assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod selection_delete;` and delegates to
      `selection_delete::assert_selection_delete_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/selection_delete.rs` owns delete/refocus
      transitions, delete behavior tests, selection fixtures, and negative drift boundary checks.
...
- [x] Split the selection context-menu assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod selection_context_menu;` and delegates to
      `selection_context_menu::assert_selection_context_menu_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/selection_context_menu.rs` owns right-click
      selection replacement/preservation policy, behavior tests, fixtures, and negative drift
      boundary checks. The IMUI workstream source gate freezes the test-owner split.
...
- [x] Split the selection hub/projection assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod selection;` and delegates to
      `selection::assert_selection_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/selection.rs` owns selection hub exports,
      visible-order projection, selected-assets projection, active-id fallback, and boundary checks.
      The IMUI workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Rename Owner Split - 2026-06-10

- [x] Split the rename assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod rename;` and delegates to
      `rename::assert_rename_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/rename.rs` owns rename hub, tests, fixtures,
      commit, focus, and negative drift boundary checks. The IMUI workstream source gate freezes the
      test-owner split.

## Fret Examples Collection Modularization Surface Keyboard Owner Split - 2026-06-10

- [x] Split the keyboard assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod keyboard;` and delegates to
      `keyboard::assert_keyboard_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/keyboard.rs` owns keyboard dispatch, actions
      mutation, and negative drift boundary checks. The IMUI workstream source gate freezes the
      test-owner split.

## Fret Examples Collection Modularization Surface Geometry Owner Split - 2026-06-10

- [x] Split the geometry/zoom assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod geometry;` and delegates to
      `geometry::assert_geometry_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/geometry.rs` owns base geometry, geometry tests,
      zoom owner, zoom tests, zoom fixtures, and negative drift boundary checks. The IMUI workstream
      source gate freezes the test-owner split.
...
- [x] Split the context-menu assertion bundle out of
      ...
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod context_menu;` and delegates to
      `context_menu::assert_context_menu_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/context_menu.rs` owns popup workflow, action
      writeback, menu chrome/options/test ID, and negative drift boundary checks. The IMUI
      workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Command Buttons Owner Split - 2026-06-10

- [x] Split the command-buttons assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod command_buttons;` and delegates to
      `command_buttons::assert_command_buttons_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/command_buttons.rs` owns root routing, action
      writeback, button chrome/options/test ID, and negative drift boundary checks. The IMUI
      workstream source gate freezes the test-owner split.

## Fret Examples Collection Modularization Surface Box-Select Owner Split - 2026-06-10

- [x] Split the box-select assertion bundle out of
      `apps/fret-examples/tests/imui_editor_collection_modularization_surface.rs` without changing
      the source-surface test entry point, coverage intent, runtime code, public APIs, or layer
      boundaries.
      Result: the root test declares a path-qualified `mod box_select;` and delegates to
      `box_select::assert_box_select_owner_split(...)`, while
      `imui_editor_collection_modularization_surface/box_select.rs` owns implementation,
      behavior-test, fixture, and negative drift boundary checks. The IMUI workstream source gate
      freezes the test-owner split.
...
- [x] Split collection context-menu selection behavior tests out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/selection/context_menu.rs` into
      the demo-local `collection/selection/context_menu/tests.rs` test owner without changing
      right-click selection replacement, selected-range preservation, active-tile updates,
      asset-grid call imports, public crate APIs, or the app-owned no-helper-widening boundary.
...
- [x] Split collection browser input box-select pointer runtime out of
      ...
      without changing left-button background arming, pressable-origin suppression after focus
      request, baseline selection capture, append-mode semantics, drag-threshold handling,
      selection/active-id updates, click-clear behavior, pointer capture/release, pointer cancel,
      background right-click context-menu priority on pointer-up, public crate APIs, or the
      app-owned no-helper-widening boundary. Result: `input_runtime.rs` keeps pointer props,
      keyboard handler installation, Primary+Wheel zoom installation, context-menu anchor priority,
      and child runtime installation while `input_runtime/box_select.rs` owns pointer
      down/move/up/cancel box-select session runtime, capture/release, selection publication,
...
- [x] Split collection drag/drop payload behavior tests out of
      ...
      `collection/drag_drop/tests.rs` imports fixtures and owns the selected-payload and
      unselected-dragged-asset behavior coverage. The collection source gate, workstream source
      gate, manifest, and surface tests now include the drag/drop tests child owner.
- [x] Split collection drag/drop test fixtures out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/drag_drop/tests.rs` into the
      demo-local `collection/drag_drop/tests/fixtures.rs` fixture owner without changing
      selected-set payload formation, dragged-only fallback, preview title/subtitle projection,
      drop-status text, public crate APIs, or the app-owned no-helper-widening boundary. Result:
      `collection/drag_drop/tests.rs` owns `mod fixtures;`, imports helpers through
      `use fixtures::selection_state;`, and keeps the selected-payload and
      unselected-dragged-asset behavior tests while `collection/drag_drop/tests/fixtures.rs` owns
      `selection_state(...)`. The collection source gate, workstream source gate, manifest, and
      surface tests now include the drag/drop fixture child owner.
...
- [x] Split collection browser input box-select pointer session tests out of
      ...
      helpers plus `#[cfg(test)] mod tests;` while `session/tests.rs` owns the focused
      box-select session behavior tests and imports fixture helpers. The collection source
      gate, workstream source gate, manifest, and surface tests now include the box-select session
      tests child owner.
- [x] Split collection browser input box-select pointer session test fixtures out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/browser_scope/input_runtime/box_select/session/tests.rs`
      into the demo-local
      `collection/browser_scope/input_runtime/box_select/session/tests/fixtures.rs` fixture owner
      without changing left-button background arming, pressable-origin suppression, append-mode
      capture, drag-threshold detection, pointer-id mismatch restoration, pointer cancel behavior,
      runtime event wiring, model publication, public crate APIs, or the app-owned
      no-helper-widening boundary. Result: `session/tests.rs` keeps the focused down/move/up/
      cancel behavior tests while `session/tests/fixtures.rs` owns pointer down/move/up/cancel,
      point, and session construction helpers. The collection source gate, workstream source gate,
      manifest, and surface tests now include the box-select session fixture child owner.
...
- [x] Split collection duplicate selection behavior tests out of
      ...
      `duplicate/selection/tests.rs` imports selection fixtures and owns the duplicate selection
      behavior tests. The collection source gate, workstream source gate, manifest, and surface tests now
      include the duplicate selection tests child owner.
- [x] Split collection duplicate selection test fixtures out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/selection/commands/duplicate/selection/tests.rs`
      into the demo-local
      `collection/selection/commands/duplicate/selection/tests/fixtures.rs` fixture owner without
      changing `Primary+D` matching, duplicate result DTO fields, copy id/label/path suffix
      generation, duplicate insertion order, visible-order copy reselect behavior, active copied-tile
      preservation, keyboard/button/context-menu command call sites, public crate APIs, or the
      app-owned no-helper-widening boundary. Result: `duplicate/selection/tests.rs` keeps the
      duplicate selection behavior tests and imports helpers through `mod fixtures;` while
      `duplicate/selection/tests/fixtures.rs` owns `selection_state(...)`, `selected_ids(...)`,
      `anchor_id(...)`, and `ImUiMultiSelectState::new(...)`. The collection source gate,
      workstream source gate, manifest, and surface tests now include the duplicate selection test
      fixture owner.
- [x] Split collection delete command behavior tests out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/selection/commands/delete.rs`
      into the demo-local `collection/selection/commands/delete/tests.rs` test owner without
      changing Delete/Backspace matching, delete result DTO fields, deletion/refocus behavior,
...
- [x] Split collection select-all behavior tests out of
      ...
      select-all behavior coverage and imports selection fixtures. The collection source gate,
      workstream source
      gate, manifest, and surface tests now include the select-all tests child owner.
- [x] Split collection select-all test fixtures out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/selection/select_all/tests.rs`
      into the demo-local `collection/selection/select_all/tests/fixtures.rs` fixture owner without
      changing `Primary+A` matching, visible-order full selection, anchor preservation/fallback,
      active-tile preservation/fallback, keyboard dispatch imports, public crate APIs, or the
      app-owned no-helper-widening boundary. Result: `selection/select_all/tests.rs` keeps
      `ProofCollectionKeyboardState`, `ImUiMultiSelectState::default()`, and the three behavior
      tests while importing helpers through `mod fixtures;`; `selection/select_all/tests/fixtures.rs`
      owns `selection_state(...)`, `anchor_id(...)`, and `ImUiMultiSelectState::new(...)`. The
      collection source gate, workstream source gate, manifest, and surface tests now include the
      select-all fixture child owner.
...
- [x] Split collection inline rename commit tests out of
      ...
      owns commit success and empty-trim rejection coverage and imports setup through
      `mod fixtures;`. The collection source gate, workstream source gate, manifest, and surface
      tests now include the rename commit tests child owner.
- [x] Split collection inline rename commit test fixtures out of
      `apps/fret-examples/src/imui_editor_proof_demo/collection/rename/commit/tests.rs` into the
      demo-local `collection/rename/commit/tests/fixtures.rs` fixture owner without changing
      empty-label rejection, label trim semantics, asset order/id preservation, renamed-asset
      projection, commit facade imports, public crate APIs, or the app-owned no-helper-widening
      boundary. Result: `rename/commit/tests.rs` keeps commit behavior assertions and imports
      `stored_assets()` and `rename_session()` through `mod fixtures;`, while
      `rename/commit/tests/fixtures.rs` owns stored asset setup and the canonical
      `ProofCollectionRenameSession` fixture. The collection source gate, workstream source gate,
      manifest, and surface tests now include the rename commit fixture owner.
