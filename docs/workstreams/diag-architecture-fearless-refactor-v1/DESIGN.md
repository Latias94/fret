...
- Initial scaffolding landed:
  ...
  - `crates/fret-diag/src/post_run_checks.rs` begins routing post-run gates via `CheckRegistry` (starting with `gc_sweep_liveness`, `notify_hotspot_file_max`, and `triage_hint_absent_codes`).
