...
| AI / plan | `PlanDescription` streaming path | `ecosystem/fret-ui-ai/src/elements/plan.rs` | Manual `card_description_text_style(...)` + `Shimmer::text_style(...)` | Main remaining text-style cascade gap for semantic supporting copy | P0 | `[ ]` | `ecosystem/fret-ui-ai/src/elements/plan.rs:360` |
...
- [ ] SHIMMER-contract-001 Decide the minimum mechanism-level outcome for custom visual-text recipes.
  - Minimum outcome:
...
- [ ] SHIMMER-contract-002 Decide whether the mechanism returns:
  - a resolved snapshot,
  - a resolver callback,
  - or another runtime-owned bridge.
- [ ] SHIMMER-contract-003 Decide whether inherited foreground is part of the same bridge or a
  sibling contract.

## C. `Shimmer` bridge

- [ ] SHIMMER-impl-010 Keep `.text_style(TextStyle)` as a compatibility path.
- [ ] SHIMMER-impl-011 Add a subtree-resolved style source mode for `Shimmer`.
- [ ] SHIMMER-impl-012 Ensure base text and overlay painter share one resolved snapshot.
...
- [ ] SHIMMER-migrate-021 Migrate `PlanDescription` streaming path to the subtree-resolved mode.
...
- [ ] SHIMMER-gates-030 Add a test proving subtree-resolved shimmer style matches passive text
  resolution under the same inherited scope.
- [ ] SHIMMER-gates-031 Add a test proving explicit `.text_style(...)` still wins over inherited
  subtree style.
- [ ] SHIMMER-gates-032 Add a focused plan streaming gate (unit test or diag script) proving the
...
- [ ] SHIMMER-docs-042 Update `docs/adr/IMPLEMENTATION_ALIGNMENT.md` once the mechanism lands.
- [ ] SHIMMER-docs-043 Update the text-style cascade workstream when `PlanDescription` moves from
