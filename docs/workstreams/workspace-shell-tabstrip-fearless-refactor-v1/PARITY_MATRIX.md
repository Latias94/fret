...
| Overflow dropdown/list | Partial (scroll-first, plus UX around scroll) | Yes | Yes (`menu(true)`) | Yes | Yes |
| Overflow membership detection | Yes | Yes (`OverflowObserver`) | Partial | Yes (unit) | Yes |
...
| Cross-pane move (drag to other pane) | Yes | Yes | Yes | Yes (gate) | Yes |
| Drag-to-split (edge targets) | Yes | Yes | Partial | Yes (gate) | Yes |
| Pinned tabs | Yes | No | No | Yes (unit + gate) | Yes |
...
| MRU tab switch (Ctrl+Tab) | Yes | No | No | Yes (unit + gate via close MRU fallback) | Yes |
| Stable automation hooks (`test_id`/selectors) | Yes | Partial (DOM selectors) | Partial | Yes (gated) | Yes |
