# Fret Examples Build Latency v1 - M35 IMUI Collection Rename Workstream Source Gate - 2026-05-01

Status: complete

## Decision

Move the source-only IMUI collection rename workstream freeze check out of the monolithic
...
- the `imui-collection-rename-v1` design, baseline, landed-slice, and closeout markers,
...
The `proof_collection_begin_rename_session_*`, `proof_collection_rename_shortcut_*`, and
`proof_collection_commit_rename_*` tests are real Rust unit behavior checks and remain in Rust.
...
The Rust `#[test]` count in `apps/fret-examples/src/lib.rs` dropped from 47 to 46, and the
`include_str!` count dropped from 228 to 223.
