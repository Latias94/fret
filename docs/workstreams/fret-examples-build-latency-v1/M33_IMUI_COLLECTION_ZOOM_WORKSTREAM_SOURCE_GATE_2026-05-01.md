# Fret Examples Build Latency v1 - M33 IMUI Collection Zoom Workstream Source Gate - 2026-05-01

Status: complete

## Decision

Move the source-only IMUI collection zoom workstream freeze check out of the monolithic
...
- the `imui-collection-zoom-v1` design, baseline, landed-slice, and closeout markers,
...
The `proof_collection_layout_metrics_*` and `proof_collection_zoom_request_*` tests are real Rust
...
The Rust `#[test]` count in `apps/fret-examples/src/lib.rs` dropped from 49 to 48, and the
`include_str!` count dropped from 238 to 233.
