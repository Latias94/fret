# Fret Examples Build Latency v1 - M44 IMUI Editor Notes Draft Actions Workstream Source Gate - 2026-05-01

Status: complete

## Decision

Move the source-only IMUI editor-notes draft actions closeout check out of the monolithic
...
- the `imui-editor-notes-draft-actions-v1` design, TODO, milestones, evidence, M1 landed slice, and
  closeout markers,
- the `editor_notes_demo.rs` source markers that keep the `Draft actions` row visible,
...
The Rust `#[test]` count in `apps/fret-examples/src/lib.rs` dropped from 38 to 37, and the
`include_str!` count dropped from 177 to 170.
