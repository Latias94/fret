...
| `combobox` | many parts (Base UI-rooted) | `Combobox` (Popover + Command recipe) + `ComboboxOption/OptionGroup` data model | Missing v4 parts surface (`ComboboxInput/Content/List/Item/...`) | High | Data model rename landed (`src/combobox_data.rs`) + placement knobs; in-tree call sites use `options(...)` + `combobox_option(...)`. Next: add thin v4 part adapters + an `into_element_parts(...)` surface while reserving upstream part names. | unit tests in `ecosystem/fret-ui-shadcn/src/combobox.rs` | P0 | In progress |
