...
| `combobox` | many parts (Base UI-rooted) | `Combobox` (Popover + Command recipe) + `ComboboxItem/Group` data types | **Name conflict**: upstream `ComboboxItem` is an element part, but Fret's `ComboboxItem` is the data model; missing v4 parts surface (`ComboboxInput/Content/List/...`) | High | Stage 1 landed: `ComboboxOption` + migration helpers + placement knobs; UI Gallery call sites now use `options(...)` + `combobox_option(...)`. Next: staged rename of data models to free upstream part names, then add thin part adapters + an `into_element_parts(...)` surface. | unit tests in `ecosystem/fret-ui-shadcn/src/combobox.rs` | P0 | In progress |
