...
| `dropdown-menu` | `DropdownMenu, Trigger, Portal, Content, Group, Item, CheckboxItem, RadioGroup, RadioItem, Label, Separator, Shortcut, Sub, SubTrigger, SubContent` | `DropdownMenu` + item/label/group/shortcut/etc | missing **major** part split (`Trigger/Portal/Content/Sub*`) | Medium | Add part structs mirroring upstream; map to `fret-ui-kit` menu primitives; keep old facade for transition | diag script (open/close, roving, submenus) | P0 | Not started |
| `menubar` | `Menubar, Menu, Trigger, Portal, Content, Group, Item, CheckboxItem, RadioGroup, RadioItem, Label, Separator, Shortcut, Sub, SubTrigger, SubContent` | `Menubar` + `MenubarMenu*` + item/label/group/shortcut/etc (submenu is bundled) | missing `Trigger/Portal/Content/Separator/Sub*` explicit parts | Medium | Expose missing parts; refactor `MenubarSubmenu` into `Sub/SubTrigger/SubContent` or add adapters | diag script (keyboard nav, open on pointer/keyboard) | P0 | Not started |
