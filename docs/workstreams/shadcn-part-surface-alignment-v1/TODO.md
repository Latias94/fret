...
| `combobox` | many parts (Base UI-rooted) | `Combobox` (Popover + Command recipe) + `ComboboxOption/OptionGroup` data model + v4 parts adapter (`into_element_parts`) | Missing full v4 surface (notably chips parts + Base UI render-prop ergonomics) | High | Data model rename landed (`src/combobox_data.rs`) + placement knobs + v4-named parts (`ComboboxInput/Content/Empty/List/Item/Group/...`) with an `into_element_parts(...)` adapter. Next: add chips part surface parity and tighten gates around copy/paste docs usage. | unit tests in `ecosystem/fret-ui-shadcn/src/combobox.rs` | P0 | In progress |
