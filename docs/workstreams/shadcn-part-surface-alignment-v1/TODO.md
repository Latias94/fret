...
| `empty` | `Empty, EmptyHeader, EmptyTitle, EmptyDescription, EmptyContent, EmptyMedia` | `Empty*` parts | No explicit gates in this workstream | Low | Keep surface; add a simple unit gate if we rely on container query padding behavior | tbd | P2 | In progress |
