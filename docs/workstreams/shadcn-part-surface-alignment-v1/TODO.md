...
| `combobox` | many parts (Base UI-rooted) | `Combobox` (Popover + Command recipe) + `ComboboxItem/Group` data types | **Name conflict**: upstream `ComboboxItem` is an element part, but Fret's `ComboboxItem` is the data model; missing v4 parts surface (`ComboboxInput/Content/List/...`) | High | Add `ComboboxOption` alias to start migration; add content placement knobs; later fearless refactor: rename data model + introduce part types + `into_element_parts` adapter | unit tests in `ecosystem/fret-ui-shadcn/src/combobox.rs` | P0 | In progress |
