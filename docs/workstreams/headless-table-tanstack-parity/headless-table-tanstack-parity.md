...
- `docs/workstreams/headless-table-tanstack-parity/headless-table-tanstack-parity-capability.md`
...
Pinning parity coverage:

- `row.pin(position, includeLeafRows, includeParentRows)` is parity-gated on both flat and tree data.
  - Fixtures: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/pinning.json`,
    `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/pinning_tree.json`
  - Gates: `ecosystem/fret-ui-headless/tests/tanstack_v8_pinning_parity.rs`,
    `ecosystem/fret-ui-headless/tests/tanstack_v8_pinning_tree_parity.rs`
- Pinning grouped rows by id (e.g. `role:1`) is parity-gated, including ordering outcomes under
  grouping + pagination.
...
- TanStack core types (`headers`, `cells`, nested column trees / header groups) are not yet fully
  represented 1:1 in `fret-ui-headless` (core inventories are still incomplete; tracked under
  `HTP-core-040`).
- Filtering supports typed filter values (via `serde_json::Value`) and parity-gated built-in
  ...
  custom `addMeta`-like callbacks). Global filtering (value + eligibility + controlled hook) is
  parity-gated by fixtures, but we still need to keep the capability inventory honest as we expand
  consumer surfaces (`HTP-cap-010` / `HTP-base-004`).
...
| --- | --- | --- | --- |
| `ColumnFiltering.ts` | `filtering.rs` | Aligned | Built-in filterFns + `resolveFilterValue` + `autoRemove` are parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_filtering_fns_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/filtering_fns.json`). `filterFromLeafRows` + `maxLeafRowFilterDepth` recursion semantics and per-row pass-map tracking are covered by unit gates in `ecosystem/fret-ui-headless/src/table/filtering.rs` (`root_filter_depth_zero_preserves_unfiltered_subtree`, `leaf_filter_depth_gate_controls_descendant_bubbling`, `evaluate_row_filter_state_tracks_per_row_pass_map`). Custom filter `addMeta`-like callback semantics are parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_filtering_meta_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/filtering_meta.json`). Column capability (`column.getCanFilter`-equivalent via `Table::column_can_filter`) is fixture-gated by `tanstack_v8_filtering_fns_parity.rs`. |
| `GlobalFiltering.ts` | `filtering.rs` | Aligned | Global filter row-model behavior (default `globalFilterFn: 'auto'` => includesString), `getColumnCanGlobalFilter` eligibility, and controlled hook parity (`onGlobalFilterChange` modes) are fixture-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_filtering_fns_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/filtering_fns.json`, snapshot: `filtering_fns_global_filter_default_excludes_bool`). The TanStack-compatible `globalFilter` value surface (via `serde_json::Value`) is exercised by the same fixture suite. |
...
| `RowSorting.ts` | `sorting.rs` | Partial | sortingFn resolution (`auto` + built-ins + named registry) is parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_sorting_fns_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/sorting_fns.json`). `manualSorting` + `getSortedRowModel` override semantics are parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_sorting_manual_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/sorting_manual.json`). `getAutoSortDir` first-toggle inference, `invertSorting`, `sortDescFirst`, multi-sort gates/transitions, and `sortUndefined` semantics are parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_parity.rs` + `ecosystem/fret-ui-headless/tests/tanstack_v8_sort_undefined_parity.rs` (fixtures under `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/`). Remaining: broaden edge-case coverage for mixed/nullish values and grouped sorting interactions. |
...
| `RowSelection.ts` | `row_selection.rs` | Aligned | Parity-gated selected row models + toggle semantics for flat + grouped rows: `ecosystem/fret-ui-headless/tests/tanstack_v8_selection_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/selection.json`). Nested sub-row selection + `isSomeSelected`/`isAllSubRowsSelected` semantics are gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_selection_tree_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/selection_tree.json`). Per-row function options (`enableRowSelection` / `enableSubRowSelection` / `enableMultiRowSelection` as functions) are exercised by dedicated snapshots in the same fixture sets. |
| `RowExpanding.ts` | `row_expanding.rs` | Aligned | Parity-gated expanded row model + pagination interactions (including `paginateExpandedRows` true/false, page bounds, and TanStack `flatRows` duplication) plus option gates and controlled hook semantics: `ecosystem/fret-ui-headless/tests/tanstack_v8_expanding_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/expanding.json`). Row capability hooks (`getRowCanExpand` / `getIsRowExpanded`) are exercised by the same fixture suite. |
| `ColumnGrouping.ts` | `grouping.rs`, `grouped_sorting.rs`, `grouped_aggregation.rs` | Partial | groupedColumnMode ordering parity is gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_headers_cells_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/headers_cells.json`, snapshots: `headers_cells_grouped_column_mode_*`). aggregationFns + renderFallbackValue parity is gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_grouping_aggregation_fns_parity.rs` + `ecosystem/fret-ui-headless/tests/tanstack_v8_render_fallback_parity.rs` (fixtures: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/grouping_aggregation_fns.json`, `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/render_fallback.json`). grouped multi-sort precedence + aggregation-any sort-key behavior are parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_grouping_sorting_precedence_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/grouping_sorting_precedence.json`). grouped row-model pipeline composition (`pre_sorted` -> `sorted` -> `expanded`) is parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_grouping_row_model_pipeline_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/grouping_row_model_pipeline.json`). grouped/placeholder/aggregated cell inventory flags are parity-gated by `ecosystem/fret-ui-headless/tests/tanstack_v8_grouping_cells_parity.rs` (fixture: `ecosystem/fret-ui-headless/tests/fixtures/tanstack/v8/grouping_cells.json`). Remaining: broaden non-option API inventory coverage (`HTP-base-004`, `HTP-cap-010`). |
...
- `docs/workstreams/headless-table-tanstack-parity/headless-table-tanstack-parity-todo.md`
...
- Use the TODO tracker: `docs/workstreams/headless-table-tanstack-parity/headless-table-tanstack-parity-todo.md`
