...
| Slider widgets | `egui::Slider` (clamping, step, log, vertical, show_value) | `SliderFloat*`, `VSliderFloat*` | `Slider<T>` exists (horizontal, clamp+step, value+typing) | Missing: vertical/log variants, unit formatting helpers, range labeling | `fret-ui-editor` (policy) | P1 |
...
| Docking UX polish | ImGui docking demo parity | `DockSpace`, docking flags | Tab labels are painted and more robust against theme padding misconfig | Still needs: close/overflow SVG icons, better hit targets, overflow menu polish, and tab chrome parity | `fret-docking` (policy) | P0/P1 |
