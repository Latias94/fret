# ADR 0235: Controlled Materials Registry and Procedural Paints (v1)
...
extensibility without leaking backend handles or arbitrary shaders into component code (ADR 0123).
...
- solid colors,
- or even gradients alone (ADR 0233).
...
- and it becomes difficult to budget, degrade deterministically, and diagnose cost (ADR 0118 / ADR 0036).

If we expose arbitrary user-provided WGSL or `wgpu` handles:

- portability drift and security risks increase (ADR 0123 trust model),
...
This keeps v1 aligned with ADR 0123’s trust boundary.
...
Capabilities (ADR 0122) may restrict which materials are available on a given backend:

- registration of unsupported materials must fail deterministically,
- callers must fall back to a portable look (see D6).

### D5 — Scene integration: materials are referenced via `Paint` (follow-up to ADR 0233)
...
- a future revision of `Paint` (ADR 0233 follow-up) will gain:
...
- and any capability-driven degradations (ADR 0122 / ADR 0118).

This is required so “ecosystem makes things pretty” cannot silently destroy performance ceilings.

## Non-goals (v1)

- Untrusted/user-provided shader code, plugin ABIs, or general shader graphs (ADR 0123 future work).
...
2) Allow arbitrary WGSL in ecosystem crates
   - Rejected: violates ADR 0123 trust model and portability goals.
...
- Extensibility tiers + trust model: `docs/adr/0123-renderer-extensibility-materials-effects-and-sandboxing-v1.md`
...
- Paint vocabulary (gradients): `docs/adr/0233-paint-primitives-brushes-and-gradients-v1.md`
- Determinism (seeds + explicit time): `docs/adr/0244-procedural-material-determinism-seeds-and-time-inputs-v1.md`
