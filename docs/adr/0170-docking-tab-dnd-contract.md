---
title: "ADR 0170: Docking Tab Drag-and-Drop Contract (Headless DnD Alignment)"
---

# ADR 0170: Docking Tab Drag-and-Drop Contract (Headless DnD Alignment)
...
- ADR 0164: `docs/adr/0164-headless-dnd-toolbox-and-ui-integration.md`
...
Fret already has an internal, cross-window drag session mechanism owned by the runner (ADR 0041). Separately, we introduced a headless DnD policy toolbox and a UI integration registry (ADR 0164) to avoid re-implementing sensors/collision/modifiers per subsystem.
...
- Aligns the policy boundary: activation belongs to ecosystem policy (ADR 0164), while cross-window routing belongs to the runner (ADR 0041).
