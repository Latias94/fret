# ADR 0185: Code Editor Ecosystem v1 (Buffer/View/Surface Contracts)
...
- View caching + dirty view gating (`ViewCache`, `notify`) and multi-stream recording direction (`prepaint`) (ADR 0213 / ADR 0165 / ADR 0167).
- “Prepaint-windowed virtual surfaces” for scroll/viewport-driven content (ADR 0175), and the longer-term “retained windowed hosts” direction (ADR 0177).
- Attributed spans (shaping vs paint split) for rich text inputs that must wrap and remain cache-friendly under theme changes (ADR 0142 / ADR 0146).
...
1. `ecosystem/fret-code-editor-buffer`
   ...
   - Undo integration hooks (ADR 0127; app-owned policy).
...
3. `ecosystem/fret-code-editor` (UI surface crate)
   ...
   - Implements the editor as a **windowed virtual surface** (ADR 0175):
...
- Reserve the path to “composable rows” via ADR 0177 (retained windowed surface hosts) when needed for:
...
- dirty views + cache roots gate re-execution (ADR 0165 / ADR 0213),
- window membership changes are an “ephemeral prepaint update” (ADR 0175),
- interaction/semantics ranges must remain correct under cache reuse (ADR 0167).
...
- The editor caches prepared row layouts (`TextBlobId` + metrics) behind a bounded LRU keyed by:
  ...
  - paint/shaping fingerprints split per ADR 0131/0146 (theme-only changes should avoid reshaping).

This aligns with ADR 0046 (“large-document editors are a separate layer (virtualization)”) and the renderer’s cache boundary direction (ADR 0143).
...
- Apps own undo history policy and routing (ADR 0127); the default reusable history can be `ecosystem/fret-undo`.
...
- Adopt ADR 0177 retained windowed hosts for composable per-row subtrees (when needed).
...
3) Performance baseline:
   - windowed virtual surface first (ADR 0175),
...
4) Input/IME: reuse the runtime contracts (`Event::Ime` / `Event::TextInput`, ADR 0012/0071) and keep
   the web IME bridge runner-owned (ADR 0180).
...
- Attributed spans: ADR 0142 / ADR 0146.
- Cache roots / dirty views / prepaint streams: ADR 0213 / ADR 0165 / ADR 0167.
- Windowed virtual surfaces: ADR 0175 / ADR 0177.
