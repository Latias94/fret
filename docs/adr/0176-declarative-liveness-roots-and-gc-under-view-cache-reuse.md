...
# ADR 0176: Declarative Liveness Roots and GC Under ViewCache Reuse

## Context

Fret's declarative element tree is rebuilt per-frame, while the `UiTree` is retained (ADR 0005, ADR 0028).
Element-local state is keyed by stable identity (`(GlobalElementId, TypeId)`) and is retained across frames with a small lag window (ADR 0028, ADR 0212).

The ViewCache experiment allows clean cache roots to skip re-running the declarative child render closure while reusing the mounted subtree (ADR 0213) and reusing prepaint/paint ranges (ADR 0055, ADR 0167).
...
- and keep the system extensible toward prepaint-driven ephemeral items (ADR 0175).
...
- Keep future refactors incremental: this contract must continue to hold as more work moves to
  prepaint (ADR 0167) and prepaint-windowed surfaces (ADR 0175).
...
- **Liveness roots**: the root set used for reachability under GC.
  ...
  - **Retained keep-alive roots**: runtime-owned keep-alive buckets that must remain live even when
    ...
    ADR 0177).
- **ViewCache root**: an element/node that defines a reuse boundary and a cache key boundary (ADR 0213).
...
- **Ownership root**: `NodeEntry.root` (`crates/fret-ui/src/elements/runtime.rs`); cross-root overwrites must not be “repaired” implicitly (ADR 0176).
...
- **ViewCache reuse root**: a `GlobalElementId` for which the runtime decides "reuse mounted subtree" for this frame (ADR 0213).
...
3. **All retained keep-alive roots** (e.g. retained VirtualList keep-alive buckets; ADR 0177).
...
- The runtime MUST keep the subtree's liveness bookkeeping consistent without re-running render:
  - state-key touches (ADR 0213),
  - model/global observation inheritance (ADR 0165),
...
in cached-subtree systems, and to keep future work (ADR 0175 / ADR 0167) incremental.
...
- **Prepaint-driven ephemeral items** (ADR 0175/ADR 0167): the liveness root set must remain authoritative even as more output shifts from declarative render into prepaint/paint phases.
...
- ADR 0212: identity debug paths + staged state: `docs/adr/0212-element-identity-debug-paths-and-frame-staged-element-state.md`
- ADR 0213: ViewCache subtree reuse + state retention: `docs/adr/0224-view-cache-subtree-reuse-and-state-retention.md`
- Workstream tracker: `docs/workstreams/gpui-parity-refactor/gpui-parity-refactor-todo.md` (MVP2-cache-005)
