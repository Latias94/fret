# ADR 0224: View Cache Subtree Reuse and Element State Retention
...
Note: the authoritative lifetime/GC contract under reuse is defined by `docs/adr/0176-declarative-liveness-roots-and-gc-under-view-cache-reuse.md`. In particular, cache-hit frames must preserve liveness deterministically via explicit liveness roots and subtree membership lists; "seen this frame" is not a sufficient semantic signal by itself.
