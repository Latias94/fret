# ADR 0149: Effect Recipes and Tier Selection (User-Facing Postprocessing v1)
...
small and portable (ADR 0118 / ADR 0119). They unlock a wide range of "cool UI" looks (glass, pixel filters, subtle color
adjustments) while keeping ordering deterministic (ADR 0002 / ADR 0009) and enforcing budgets with deterministic
degradation (ADR 0120).
...
2) **Confusion about "custom shaders"**:
   ...
   - Exposing raw `wgpu` handles to component code is explicitly discouraged (ADR 0125), but we need a clear,
...
This ADR adopts ADR 0125’s two-tier model and makes it explicit in user-facing docs:
...
- Component-/recipe-level APIs must remain `wgpu`-free (ADR 0093 / ADR 0125).
...
- A future extensibility surface (trusted registry, and later untrusted DSL/node graph) remains a follow-up and must
  respect budgets, portability, and determinism (ADR 0125 / ADR 0124 / ADR 0120).
...
- Renderer evolution stays additive and budgetable (ADR 0118 / ADR 0120).
...
- Extensibility tiers: `docs/adr/0125-renderer-extensibility-materials-effects-and-sandboxing-v1.md`
- Video/streaming surfaces: `docs/adr/0121-streaming-images-and-video-surfaces.md`
