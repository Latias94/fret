# ADR 0177: fret-canvas Modules and Feature Strategy (Portable Canvas Substrate)
...
- ADR 0176 (Canvas cache policy and hosted resource caches)
...
layer (ADR 0137): the runtime provides mechanisms and safe defaults; ecosystem/app crates own domain
...
4. Support both retained and declarative authoring:
   ...
   - declarative `Canvas` remains a runtime mechanism (ADR 0156).
...
- `cache`: retained-friendly hosted-resource caches (ADR 0176 alignment)
  - `TextCache` (existing)
  - `PathCache`
  - `SvgCache`
...
- `kurbo`: optional geometry backend for hit-testing/path operations (ADR 0167)
- `rstar`: optional spatial index backend
- `declarative`: reserved for a future declarative canvas element surface (ADR 0156/0137 direction)
...
- Declarative canvas contract: `docs/adr/0156-declarative-canvas-element-and-painter.md`
- Cache policy vocabulary: `docs/adr/0176-canvas-cache-policy-and-hosted-resource-caches.md`
