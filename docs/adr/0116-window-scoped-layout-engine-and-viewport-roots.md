# ADR 0116: Window-Scoped Layout Engine and Viewport Roots
...
3. Layout uses a strict two-phase protocol:
   ...
   - **Compute/apply**: run the solver for each root and write **definite** bounds back into the
     retained UI tree.
4. Intrinsic measurement is non-reentrant and constraint-correct per ADR 0115:
...
7. Measurement cycle policy:
   ...
   - Release builds must not crash; they use a safe fallback policy (exact fallback defined in ADR
     0115 and implementation docs) plus rate-limited diagnostics.
...
1. Land ADR 0115 mechanism changes (`AvailableSpace`, `measure_in`, non-reentrant measurement).
