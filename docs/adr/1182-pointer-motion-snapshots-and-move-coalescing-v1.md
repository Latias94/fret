# ADR 1182: Pointer Motion Snapshots and Move Coalescing (v1)
...
- they must remain correct under `RenderTransform` (ADR 0083) and the coordinate rules in ADR 1177.
...
- RenderTransform coordinate semantics: `docs/adr/0083-render-transform-hit-testing.md`
- Pointer coordinate spaces: `docs/adr/1177-pointer-coordinate-spaces-and-element-local-mapping-v1.md`
- Scheduling / animation frames: `docs/adr/0034-timers-animation-and-redraw-scheduling.md`
- Frame clock: `docs/adr/1179-frame-clock-and-reduced-motion-gates-v1.md`
...
- If a widget wants time-driven pointer-follow animation, it must request animation frames (ADR
  0034) and advance its own state using the frame clock (ADR 1179).
...
space, using the same inverse-transform traversal rules as event dispatch (ADR 0083 / ADR 1177):
