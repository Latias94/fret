# ADR 1173: Imported Render Targets and External Texture Imports (v1)
...
- Web: WebCodecs `VideoFrame` → WebGPU “external texture” (capability-gated; ADR 0124).
...
- leaking backend handles into `fret-ui` (ADR 0093 / ADR 0125),
- breaking queue ownership rules (ADR 0038),
- or forcing the app to always fall back to CPU byte uploads (ADR 0121).
...
1) **Contract path (must run and be verifiable in-repo)**
   ...
   - The UI displays the target via `ViewportSurface` with stable resizing/object-fit behavior
     (ADR 1170 + `ViewportSurfaceProps.fit`).
...
2) **True external import (capability-gated, platform-specific, optional)**
   ...
   - The ingestion path is selected by a renderer capability snapshot (ADR 0124) and must have a
     deterministic fallback (ADR 0121).
...
- be capability-gated where needed (ADR 0124),
- have deterministic fallbacks (ADR 0121),
- and remain outside `fret-core` scene types if they require backend-specific handles (ADR 0093).
...
- Add diagnostics/perf baselines that make the cost visible:
  - draw calls, upload bytes, intermediate usage where relevant (ADR 0096 / ADR 0120).
...
- Taking ownership of decoders, codecs, audio, or streaming stacks (ADR 0121).
...
- Streaming frames: `docs/adr/0121-streaming-images-and-video-surfaces.md`
- Optional zero-copy imports: `docs/adr/0124-renderer-capabilities-and-optional-zero-copy-imports.md`
- Color/compositing: `docs/adr/0040-color-management-and-compositing-contracts.md`
- Foreign UI embedding (isolated surfaces): `docs/adr/0189-foreign-ui-embedding-isolated-surfaces.md`
