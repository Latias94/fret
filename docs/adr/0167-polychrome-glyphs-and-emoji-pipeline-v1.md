# ADR 0167: Polychrome Glyphs (Color Emoji) and the Text Rendering Pipeline (v1)
...
- A text system v2 direction based on Parley shaping and attributed spans (ADR 0157).
- A font bootstrap + invalidation contract with `TextFontStackKey` (ADR 0162).
...
Paint-only span attributes (foreground color, underline color, etc.) must **not** participate in
 shaping/layout keys (ADR 0157), but may participate in paint caching where applicable.
...
(ADR 0162). No widget may bypass this by injecting hidden fallback stacks.
...
- Text system v2 (Parley + spans): `docs/adr/0157-text-system-v2-parley-attributed-spans-and-quality-baseline.md`
- Font bootstrap + invalidation: `docs/adr/0162-font-stack-bootstrap-and-textfontstackkey-v1.md`
