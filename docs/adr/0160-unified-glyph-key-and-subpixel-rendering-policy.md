# ADR 0160: Unified Glyph Key and Subpixel Rendering Policy
...
ADR 0157 locks the direction for text shaping/layout (Parley + attributed spans) and explicitly states that the mainline text system must not depend on backend feature gates. ADR 0158 then locks the cache boundary between:
...
Shader-side reconstruction and gamma/contrast correction are part of the v2 quality baseline (tracked separately; see ADR 0157 “Quality baseline”).
...
2) Store UV/page back into layout outputs.
   - Rejected by ADR 0158 (prevents eviction independence and breaks scene encoding cache correctness).
