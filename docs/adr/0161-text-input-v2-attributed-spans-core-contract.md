...
ADR 0157 locks the *direction* for the renderer text system v2:
...
This ADR refines the `TextInput`/`TextSpan` sketch in ADR 0157 into an explicit `fret-core` API and migration plan.
...
- Decoration layout: underline/strikethrough placement and thickness rules (align with ADR 0103).
