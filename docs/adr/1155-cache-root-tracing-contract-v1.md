# ADR 1155: Cache Root Tracing Contract (ViewCache v1)

Status: Proposed

## Context

Fret's GPUI-style view caching work (ADR 1152) introduces explicit cache-root boundaries (`ElementKind::ViewCache`)
...
- `label`: best-effort debug label (see ADR 1151)
...
- per-cache-root hit/miss counters are not part of this contract (tracked by ADR 1152 follow-ups).
...
- ADR 1152: Cache roots: `docs/adr/1152-cache-roots-and-cached-subtree-semantics-v1.md`
- ADR 1151: Element identity debug paths: `docs/adr/1151-element-identity-debug-paths-and-frame-staged-element-state.md`
