# ADR 0141: Declarative Canvas Element and Painter (Resource-Hosted Custom Draw)
...
- `render_transform` maps paint + hit-testing + pointer coordinates consistently (`docs/adr/0082-render-transform-hit-testing.md`),
...
3. Keep interaction policy out of the runtime: input maps, tool modes, snapping, etc. remain
   ecosystem/app-owned (ADR 0128 direction).
...
- `Overflow::Clip` clipping conventions (ADR 0087 / ADR 0063),
- effect layers and opacity stacks (ADR 0117),
- ancestor `render_transform` behavior (ADR 0082).
...
This ADR locks a layering split aligned with ADR 0128:
...
(ADR 0007) plus explicit `ViewportInput` forwarding (ADR 0132 / ADR 0025 direction).
...
- Canvas guidance and layering direction: `docs/adr/0128-canvas-widgets-and-interactive-surfaces.md`
- Render transforms (paint + hit test + event coords): `docs/adr/0082-render-transform-hit-testing.md`
...
- Rounded clipping and overflow conventions: `docs/adr/0063-rounded-clipping-and-soft-clip-masks.md`,
  `docs/adr/0087-overflow-and-clipping-conventions.md`
- Effect layers: `docs/adr/0117-effect-layers-and-backdrop-filters-scene-semantics-v1.md`
