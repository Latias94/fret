...
- `ecosystem/fret-query` — async resource state (TanStack Query-like) adapted to ADR 0190.
...
3. **Async resources (read-only state machine + cache):**
   ...
   - background fetch runs through `Dispatcher` + inbox drainers (ADR 0190),
...
All background work must respect the kernel’s execution model (ADR 0190):
...
- **Portable concurrency:** only depends on `Dispatcher.exec_capabilities()` (ADR 0190).
