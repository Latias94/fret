...
2) **The kernel should not become a giant component/effect library**:
   ...
   - we must preserve budgeted, deterministic degradation and diagnostics (ADR 0118 / ADR 0036).
...
- maps cleanly onto Tier A vs Tier B (ADR 0123),
- and stays consistent across ecosystem crates (shadcn, magicui, material3, editor kits).

Related ADRs:

- Effect recipes and tier selection: `docs/adr/0134-effect-recipes-and-tier-selection-v1.md`
- Paint primitives: `docs/adr/0233-paint-primitives-brushes-and-gradients-v1.md`
- Controlled materials: `docs/adr/0235-controlled-materials-registry-and-procedural-paints-v1.md`
- Sampled materials (v2): `docs/adr/0242-sampled-materials-and-fixed-binding-shapes-v2.md`
- Masks: `docs/adr/0239-mask-layers-and-alpha-masks-v1.md`
- Compositing groups: `docs/adr/0247-compositing-groups-and-blend-modes-v1.md`
- Frame clock and reduced motion: `docs/adr/0240-frame-clock-and-reduced-motion-gates-v1.md`
...
- Capabilities + budgets: `docs/adr/0122-renderer-capabilities-and-optional-zero-copy-imports.md`,
...
- Flutter/Skia: “effects and blends live in a paint-like object at group boundaries” (`saveLayer`
  ...
  portable and budgeted (ADR 0118) and routes authoring through recipes instead of exposing a raw
  shader surface.
...
- `PaintRecipe` (solid/gradients/materials, per ADR 0233/0235)
- `MaskRecipe` (gradient alpha masks, per ADR 0239)
- `EffectRecipe` (effect chains, per ADR 0134/0117/0236)
- `CompositeRecipe` (blend groups, per ADR 0241)
- `MotionRecipe` (time gating + reduced motion, per ADR 0240)
...
This keeps the Tier A vs Tier B decision rule stable and user-visible (ADR 0123 / ADR 0134).
...
- `PushMask/PopMask` for alpha masks (ADR 0239).
- `PushCompositeGroup/PopCompositeGroup` for blend groups (ADR 0241).
- `Paint` / `MaterialId` for fills and strokes (ADR 0233 / ADR 0235).
...
- Avoid calling `rand::random()` (or equivalent) directly inside view building / paint without an
  explicit seed.
- Prefer explicit seeds in params for procedural looks (ADR 0244).
...
- Treat wall-clock time as forbidden input for visuals; use the frame clock snapshot (ADR 0240)
