...
- engine-pass (depth-tested) rendering as the primary topology (ADR 0139),
- explicit viewport input units and mapping (ADR 0147),
- explicit pick primitives and overlap policy (ADR 0148),
...
3. Preserve the engine-pass vs UI overlay separation (ADR 0139):
...
### 5) Rendering topology stays consistent with ADR 0139
...
UI layer above the viewport surface (ADR 0139). A future extension may add an optional
...
Similarly, the pick/math layer may adopt an internal f64 path (ADR 0148) without changing the public
plugin signatures.
...
2. **Encode 3D gizmo geometry as UI `SceneOp` primitives**
   - Rejected by ADR 0139 (breaks depth-tested rendering quality and engine pipeline integration).
...
- ADR 0136: Undo/redo infrastructure boundary
