...
intermediate texture pool (ADR 0118). Public effect semantics (effect layers + backdrop filters) are expressed
in the display list as ordered `SceneOp`s (ADR 0119), and must preserve strict `Scene.ops` ordering (ADR 0002 / ADR 0009).
...
4) If still over budget, disable the entire effect group (ADR 0119 rules):
