...
- A **View** is defined at cache boundary granularity: a `ViewCache` root (ADR 1152).
...
Nested cache roots must compose correctly (ADR 1152):
...
3) Add diagnostics counters and inspector exposure (ADR 0174): list dirty views + last notify source (debug-only).
