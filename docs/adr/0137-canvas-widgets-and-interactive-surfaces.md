# ADR 0137: Canvas Widgets and Interactive Surfaces (2D, Plots, Node Graphs)
...
- `Widget::render_transform` maps paint + hit-testing + event coordinates consistently (ADR 0083).
...
4. Ensure canvas widgets remain compatible with:
   ...
   - performance work (paint caching constraints under transforms) (ADR 0083 / ADR 0055).
...
- Prefer implementing **view transforms** (pan/zoom) via `render_transform` (ADR 0083) when the
...
1. **Window overlays (preferred when the overlay must escape clipping)**:
   ...
   - Anchor using window-space geometry queries that include render transforms (ADR 0083).
...
- When using `render_transform`, be aware that paint caching may be disabled or constrained (ADR
  0083 / ADR 0055). Canvases should treat caching as “best effort” and still be correct without it.
...
- `render_transform` semantics (ADR 0083),
...
- data models (graph/plot/chart state) and undo/redo policy (ADR 0136).
...
- **Geometry queries for anchoring**: ability to anchor overlays to what the user sees (visual
  bounds that include render transforms), not just layout bounds (ADR 0083).
...
- **Undo hooks, not policy**: begin/update/commit/cancel semantics should be expressible in app
  code without requiring `fret-ui` to own a history stack (ADR 0136).
...
- Undo/redo boundary (hooks): `docs/adr/0136-undo-redo-infrastructure-boundary.md`
- Streaming surfaces (raster/video): `docs/adr/0121-streaming-images-and-video-surfaces.md`
- Streaming budgets/backpressure: `docs/adr/0123-streaming-upload-budgets-and-backpressure-v1.md`
