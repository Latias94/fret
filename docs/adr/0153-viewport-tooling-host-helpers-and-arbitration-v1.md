# ADR 0153: Viewport Tooling Host Helpers and Arbitration (v1)
...
- Pointer/wheel input is forwarded as `Effect::ViewportInput(ViewportInputEvent)` with explicit units (ADR 0132).
- Engine-pass 3D overlays (gizmos, debug draw, selection outlines) are recorded through a runner hook service
  (`ViewportOverlay3dHooksService`), consistent with the “engine owns the pass topology” boundary (ADR 0038 / ADR 0130).
...
- A tool-specific plugin boundary in `fret-gizmo` (ADR 0140) and a host property read boundary (ADR 0152).
...
- gizmo plugins read host values via `GizmoPropertySource` (ADR 0152) and emit edits via `GizmoCustomEdit`.
...
- Explicit-units viewport input: `docs/adr/0132-viewport-input-forwarding-explicit-units.md`
- Engine-pass gizmo boundary: `docs/adr/0130-viewport-gizmos-engine-pass-and-ui-overlay-boundary.md`
- Engine render hook substrate: `docs/adr/0038-engine-render-hook-and-submission-coordinator.md`
- Gizmo plugin contract: `docs/adr/0140-custom-gizmo-plugins-and-handle-contract.md`
- Gizmo host properties (read-only): `docs/adr/0164-gizmo-host-property-source-readonly-v1.md`
