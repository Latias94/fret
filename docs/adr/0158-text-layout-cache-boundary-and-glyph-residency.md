# ADR 0158: Text Layout Cache Boundary and Glyph Residency
...
- v2 shaping direction via Parley + attributed spans (ADR 0157).
...
backend-specific variants once v2 fully converges on Parley (ADR 0157).
...
5) Remove legacy coupling (drop per-blob pinning once frame residency is correct) and converge
   fully on Parley for shaping/rasterization keys (ADR 0157).
