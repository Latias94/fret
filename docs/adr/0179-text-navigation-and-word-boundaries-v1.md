...
- Read-only selection as a first-class primitive (`SelectableText`) (ADR 0137).
- Click count and double-click semantics in pointer events (ADR 0136).
...
- pointer double-click selection (ADR 0136),
- and future code editor ecosystem surfaces (ADR 0185).
...
- Cross-element text selection (still out of scope; see ADR 0137).
...
- Allow a focused widget to override via a widget hook (e.g. `Widget::text_boundary_mode(...) -> Option<TextBoundaryMode>`)
  that participates in dispatch-path queries (ADR 0218). This is intentionally an override, not the primary policy path.
...
- If the focused surface is a code-editor-grade widget (e.g. the future `fret-code-editor` surface, ADR 0185),
...
When a text surface receives pointer activation with a click count (ADR 0136):
...
- Double/triple click selection correctness under scroll offsets and transforms (ADR 0217 / ADR 0082).
...
- ADR 0185: Code editor ecosystem v1 (ecosystem layering)
