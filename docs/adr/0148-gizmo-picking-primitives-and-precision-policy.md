# ADR 0148: Gizmo Picking Primitives and Precision Policy
...
rather than an immediate-mode UI overlay (ADR 0139). Input forwarding at the viewport boundary is
explicit-units and includes mapping geometry + scale factor (ADR 0147).
...
- Replacing engine-pass rendering with a 2D overlay-only approach (ADR 0139 remains the boundary).
...
- Pick radii/thickness are expressed in **screen pixels** and scaled via
  `ViewportInputEvent.geometry.pixels_per_point` (ADR 0147).
...
- ADR 0139: viewport gizmos engine-pass and UI overlay boundary
- ADR 0147: viewport input forwarding explicit units
- ADR 0049: viewport tools input capture and overlays
- ADR 0136 / ADR 0024: undo/redo and edit transaction direction (gizmo phases)
