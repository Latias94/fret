...
(`docs/adr/0110-golden-path-ui-app-driver-and-pipelines.md`), but today those patterns are mostly
...
- Hiding the Event / Command / Effect pipeline model (ADR 0110).
...
- composes cleanly with the Event / Command / Effect pipelines (ADR 0110),
- is friendly to hot reload reset boundaries (ADR 0105).
...
- **User-facing/UI-affecting timers** SHOULD flow through effects (e.g. `Effect::SetTimer`) so they
  remain visible in the Effect pipeline model (ADR 0110) and are owned by the runner.
...
- Golden path pipelines and async patterns: `docs/adr/0110-golden-path-ui-app-driver-and-pipelines.md`
