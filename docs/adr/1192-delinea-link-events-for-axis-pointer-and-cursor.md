# ADR 1192: `delinea` Link Events for AxisPointer/Cursor (P0)
...
- multi-grid routing contracts (ADR 1191) where any grid-tied output must be unambiguous (`GridId` in output).
...
- ADR 1191: multi-grid viewport/layout and routing invariants (`GridId` must be carried for routing).
