# ADR 1175: Effect Steps — Color Matrix and Alpha Threshold (v1)

Status: Proposed

## Context

Fret’s effect layer contract (ADR 0119) provides a small, portable `EffectChain` surface intended
for common UI postprocessing (blur, pixelate, simple color adjustment) while keeping ordering,
budgets, and determinism intact (ADR 0002 / ADR 0120).
...
- and does not expand into a full general shader authoring model (ADR 0125).
...
Under intermediate budgets (ADR 0120), deterministic degradation rules apply:
...
- Arbitrary user-provided color matrices/effects from untrusted plugins without a sandbox (ADR 0125).
...
- a “blur + alpha threshold” chain produces stable results and respects clip/effect bounds (ADR 0119),
...
- Extensibility trust boundary: `docs/adr/0125-renderer-extensibility-materials-effects-and-sandboxing-v1.md`
