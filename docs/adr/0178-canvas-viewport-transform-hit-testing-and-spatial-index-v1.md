# ADR 0178: Canvas Viewport, Transforms, Hit Testing, and Spatial Index (Toolkit v1)
...
- Related:
  ...
  - ADR 0083 (Render transform hit testing)
  ...
  - ADR 0177 (fret-canvas modules and feature strategy)
...
ADR 0137 defines canvas terminology and high-level guidance. This ADR locks a **minimal, reusable toolkit contract**
...
3. Support both:
   ...
   - declarative `Canvas` as a runtime mechanism (ADR 0156).
...
Locked terminology aligns with ADR 0137:
...
- When the entire surface transforms together, use `Widget::render_transform` (ADR 0083) and keep
...
Clipping semantics are owned by the scene/runtime (ADR 0078/0088/0063). Canvas toolkits:

- must not bypass core clip semantics,
- may provide helpers to compute clip rects for a mapped canvas region,
- should encourage overlay escape patterns (ADR 0137 + ADR 0064).
...
- geometry helpers remain feature-gated (ADR 0167):
...
Backend strategy (aligned with ADR 0177):
...
- If the toolkit becomes too large, `fret-canvas` risks becoming a “misc utils” bucket (ADR 0177).
...
4. Add instrumentation:
   ...
   - integrate with diagnostics bundles (ADR 0174) without coupling to policy.
...
- View transform and input mapping building blocks:
  ...
  - `ecosystem/fret-ui` `Widget::render_transform` contract (ADR 0083)
...
2. Do we want a dedicated diagnostics registry for canvas “interaction perf” (candidates/refines)
   analogous to the cache stats registry added in ADR 0176?
3. Should the budget helper integrate with renderer capability tiers (ADR 0120/0123) or remain
