...
3. Keep kernel boundaries intact (ADR 0093), and align with the golden-path story (ADR 0111/0112/0113).
...
Hard rule: `crates/*` must not depend on `ecosystem/*` (ADR 0093).
...
- Keeping it as a function preserves hotpatch-friendly behavior (ADR 0107 / ADR 0112).
...
- Public behaviors that can be invoked via menus/palette/shortcuts must be expressed as a `CommandId`
  (ADR 0023 / ADR 0020).
- Command IDs must be namespaced: `domain.scope.action` (ADR 0113).
...
All component styling should be token/key driven (ADR 0032 / ADR 0102 / ADR 0050).
...
consistent with the explicit-units viewport input contract (ADR 0147).
...
Ecosystem crates may integrate with UI render-asset caches (ADR 0004, ADR 0111/0113) but must not
...
Rejected as the default recommendation, because it conflicts with ADR 0111’s explicit goal to avoid a mixed
“everything crate” story. A meta crate may still exist as an optional distribution later, but must not be the
primary recommended path unless we supersede ADR 0111 with a new decision.
...
- User-facing crate surfaces: `docs/adr/0111-user-facing-crate-surfaces-and-golden-path.md`
...
- DPI + units: `docs/adr/0017-multi-window-display-and-dpi.md`, `docs/adr/0147-viewport-input-forwarding-explicit-units.md`
- Tokens/theme: `docs/adr/0032-style-tokens-and-theme-resolution.md`, `docs/adr/0102-semantic-theme-keys-and-extensible-token-registry.md`
