# ADR 0206: `delinea` Brush Selection -> Row Selection Fast Path (P0)
...
ADR 0205 defines brush selection as a headless **selection output** (not a view-window write), exposed as a
...
- ADR 0202: dataset storage and index identity.
- ADR 0205: brush selection output contract.
...
- Those features are deferred and must be introduced as explicit, budget-aware contracts (ADR 0199 follow-ups).

The authoritative selection rectangle remains available via `ChartOutput.brush_selection_2d` (ADR 0205), so a UI
