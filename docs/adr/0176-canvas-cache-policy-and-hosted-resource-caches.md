# ADR 0176: Canvas Cache Policy and Hosted Resource Caches (Smooth-By-Default)
...
- declarative custom draw via the `Canvas` leaf element (ADR 0156),
...
3. Keep the framework vs ecosystem boundary aligned with ADR 0137/0156:
...
- All cached resources must be released deterministically on teardown:
  - declarative: via the element removal cleanup hook (ADR 0156),
  - retained: via `Widget::cleanup_resources` (ADR 0004).

### 2) Runtime: Canvas hosted caches become smooth-by-default and configurable

`crates/fret-ui`’s hosted canvas caches (ADR 0156) adopt the cache policy vocabulary:
...
- The runtime remains policy-light: it does not attempt to infer domain identity; callers still
  provide stable keys (ADR 0156).
- The runtime does not infer zoom from transforms; callers still supply `scale_factor = dpi * zoom`
  when higher-resolution preparation is desired (ADR 0156 / ADR 0159).
...
- `ecosystem/fret-canvas` UI integration (feature-gated) and higher layers:
  ...
  - owns input mapping policy and tool arbitration (ADR 0159).
...
- Canvas guidance and layering: `docs/adr/0137-canvas-widgets-and-interactive-surfaces.md`
- Declarative Canvas element and hosted painter: `docs/adr/0156-declarative-canvas-element-and-painter.md`
- Canvas pan/zoom policy placement: `docs/adr/0159-canvas-pan-zoom-input-mapping-v1.md`
