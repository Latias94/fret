# ADR 0164: Gizmo Host Property Source (Read-only) v1

Status: Proposed

## Context

Fret's gizmo plugin surface (ADR 0140) allows tools to contribute custom handles and emit domain
...
- Undo/redo and transaction policy remains **host/app-owned** (ADR 0024 / ADR 0127 direction).
- Viewport tools remain **engine-pass** (ADR 0130) and input is forwarded explicitly (ADR 0132).
...
- Implementing 3D picking acceleration / BVH (tracked separately; see ADR 0140 gaps).
...
This keeps the core `Gizmo` surface unchanged and scopes the contract to plugins (ADR 0140).
...
3. Add a dedicated follow-up ADR for 3D picking primitives + acceleration (Godot-style collision
   registry + BVH), building on ADR 0140.
