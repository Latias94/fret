# ADR 0110: Golden-Path UI App Driver and Pipelines

Status: Accepted

## Context

Fret’s kernel/backends/apps boundaries are locked (ADR 0092) and we already have an ecosystem bootstrap story (ADR 0106).
...
- dev-only hotpatch toggles and safe hot reload hooks (ADR 0105),
...
- remains compatible with conservative Subsecond-style hotpatching (ADR 0105),
...
- **Backend glue stays out of the kernel** (ADR 0092): this is ecosystem-level and may depend on `fret-launch`.
- The authoring surface should be **hotpatch-friendly** by construction:
  ...
  - keep long-lived callback registries disposable on hot reload (ADR 0105).
...
When users enable hotpatch (ADR 0105):
...
- **Reset boundary**: on patch applied, the runner performs a conservative reset:
  ...
  - keep safety-first default behavior for window state drop (ADR 0105).
...
- providing a unified facade (non-binding recommendation from ADR 0106).
...
- Bootstrap/tools story: `docs/adr/0106-ecosystem-bootstrap-ui-assets-and-dev-tools.md`
- Dev hotpatch boundaries: `docs/adr/0105-dev-hotpatch-subsecond-and-hot-reload-safety.md`
- Execution and concurrency surface: `docs/adr/0184-execution-and-concurrency-surface-v1.md`
