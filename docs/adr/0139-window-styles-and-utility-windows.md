...
2. Keep a strict **runner/backends own OS window implementation** boundary (ADR 0090).
3. Make all features **capability-gated and degradable at runtime** (ADR 0054, ADR 0083).
...
solver (ADR 0064 / ADR 0103). They must not require creating additional OS windows.
...
- Utility windows should not accidentally inherit “main window” close/exit semantics (ADR 0093 and
...
### 5) Single-window platforms (ADR 0083)
...
- GPUI exposes a richer “window kind” concept (`WindowKind::{Normal, PopUp, Floating, Dialog, ...}`),
  ...
  relying on OS popup windows, to keep single-window platforms viable (ADR 0083).
...
- Enables “desktop pet / HUD / tool window” classes without violating crate layering (ADR 0090).
...
- Rejected: violates backend/portable layering (ADR 0090) and forces platform forks in ecosystem code.
...
- Platform backends and layering: `docs/adr/0090-platform-backends-native-web.md`
...
- Anchored overlay placement: `docs/adr/0064-overlay-placement-contract.md`, `docs/adr/0103-layout-driven-anchored-overlays.md`
- Window close semantics: `docs/adr/0093-window-close-and-web-runner-destroy.md`
