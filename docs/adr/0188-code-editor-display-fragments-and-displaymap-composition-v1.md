# ADR 0188: Code Editor Display Fragments and DisplayMap Composition v1

- Status: Proposed
- Date: 2026-02-08
- Related:
  - ADR 0185 (Code editor ecosystem v1)
  ...
  - ADR 0175 (Prepaint-windowed virtual surfaces)
  - ADR 0177 (Retained windowed surface hosts)
...
- hit-testing stable under windowing (ADR 0175).
...
- v1 policy and staging are locked by diagnostics:
  ...
  - ADR 0188 staging opt-ins (decorations present under inline preedit in specific baselines):
...
- [x] Provide a view-owned way to materialize composed display text for windowed export ranges.
  ...
  - Constraint: windowed-only outputs (ADR 0175); avoid full-document composed strings.
  - Evidence:
...
- [x] Update a11y export to consume the composed display value (and composition range) from the view layer (staged).
  - Status: when `compose_inline_preedit=true`, `SemanticsRole::TextField` exports a windowed composed `value`
    (ADR 0175) plus selection/composition ranges within that value (ADR 0071).
