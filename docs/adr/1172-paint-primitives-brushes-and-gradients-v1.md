...
- Renderer evolution (ADR 0118 / ADR 0125) is blocked because there is no stable contract for
  “brush-like” draw-time evaluation.

We need a minimal, portable, budgetable paint surface that:

- preserves strict `Scene.ops` ordering semantics (ADR 0002 / ADR 0082),
- remains `wgpu`-free at the component level (ADR 0125),
...
- This creates a natural path to a controlled `MaterialId` registry (ADR 0125) without exposing
...
- Controlled `MaterialId` registry for procedural patterns (noise, beams, sparkles) with budgets
  and deterministic degradation (ADR 0125 + ADR 0120).
