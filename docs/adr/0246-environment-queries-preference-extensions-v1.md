# ADR 0246: Environment Queries — Preference Extensions (v1)

Status: Proposed

## Context

ADR 0232 defines a mechanism for reading a **committed per-window environment snapshot** during
...
Extend the committed per-window environment snapshot (ADR 0232) with additional **optional**
...
`debug.environment` (best-effort), alongside existing ADR 0232 fields.
