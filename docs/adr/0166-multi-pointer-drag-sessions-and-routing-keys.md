---
title: "ADR 0166: Multi-Pointer Drag Sessions, Extensible Drag Kinds, and Internal-Drag Routing Keys"
---

# ADR 0166: Multi-Pointer Drag Sessions, Extensible Drag Kinds, and Internal-Drag Routing Keys
...
- we add true multi-pointer interaction (ADR 0165),
...
3. Align internal-drag routing and arbitration with existing contracts (ADR 0041 / ADR 0072 / ADR 0165).
4. Preserve determinism: event routing and drag session selection must be stable and testable.

## Non-Goals

- Defining higher-level DnD policy (sensors/collision/modifiers) which belongs to the ecosystem (ADR 0164).
...
1. Land ADR 0165 changes: `PointerId` in input events and per-pointer capture semantics.
