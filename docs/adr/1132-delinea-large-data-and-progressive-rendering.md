# ADR 1132: `delinea` Large Data + Progressive Rendering Strategy (P0/P1 Baseline)
...
`delinea` already supports incremental work via `WorkBudget` (ADR 1128), but we need to lock down
baseline policies so future features do not force a large rewrite.

## Relationship to Other ADRs

- ADR 1128: headless engine + `WorkBudget`.
- ADR 1129: transform pipeline and dataZoom semantics (filtering vs windowing).
