...
- ad-hoc handling for `RenderTransform` (ADR 0082),
...
- Render transforms + event coordinates: `docs/adr/0082-render-transform-hit-testing.md`
- Viewport input forwarding: `docs/adr/0025-viewport-input-forwarding.md`
- Window metrics (scale factor): `docs/adr/0017-multi-window-display-and-dpi.md`
...
- For pointer-position-bearing events, the `position` field MUST be expressed in that widget's
  **untransformed layout space** as defined by ADR 0082.
...
- For `PointerEvent::Wheel.delta`, the delta MUST be mapped as a vector through the same inverse
  transform traversal used for `position` (ADR 0082).
...
- Subsequent pointer events for that pointer MUST still be mapped into the **capturing widget's**
  untransformed layout space (ADR 0082), even when the pointer is outside the widget's bounds.
