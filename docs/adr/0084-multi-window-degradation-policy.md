...
- Docking UI (`crates/fret-components-docking`) should consult capabilities and/or settings to avoid
...
- virtual window surfaces must be implemented as policy-layer UI composition (likely in
  `fret-components-docking`), including:
  - z-ordering,
...
- Rendering model: treat each additional `DockLayoutWindow` root as an in-window “floating dock host”
  whose chrome and interaction policy live in `fret-components-docking`.
- Testing strategy:
