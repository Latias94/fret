# ADR 1157: Input Dispatch Phases, `prevent_default`, and Action Availability (v2)
...
- There is also a command-level enable/disable override seam for OS menu gating:
  - `WindowCommandEnabledService` (ADR 0188).
...
- **Per-command enabled overrides** (`WindowCommandEnabledService`): a data-only, window-scoped
  override intended for OS menu validation and shortcut consistency (ADR 0188).
...
- OS menu command gating seam: `docs/adr/0188-window-command-enabled-service.md`
