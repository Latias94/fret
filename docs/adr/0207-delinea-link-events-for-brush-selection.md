# ADR 0207: `delinea` Link Events for Brush Selection (P0)
...
- ADR 0205: brush selection as a headless output.
- ADR 0206: derived X-only row range fast path.
...
Brush selection remains available as stable output fields (ADR 0205/0145). Link events are:
