# ADR 0185: Menu Roles, System Menus, and OS Actions

Status: Accepted

## Context

ADR 0183 defines the platform integration seam for OS menubars via `Effect::SetMenuBar`, and ADR
0023 defines a minimal, data-only `MenuBar` model derived from commands.
...
- Shortcut labels MUST use the stable display policy (ADR 0183 / MVP 2).
- Enable/disable gating MUST be best-effort and driven by `when` + the window-scoped
  `InputContext` snapshots (ADR 0183).
