# ADR 0249: `delinea` Link Events for AxisPointer/Cursor (P0)
...
- optional cross-chart propagation via `LinkConfig.group` + `ChartOutput.link_events` (ADR 0207),
- multi-grid routing contracts (ADR 0248) where any grid-tied output must be unambiguous (`GridId` in output).
...
- ADR 0207: link events for brush selection (change-based emission).
- ADR 0248: multi-grid viewport/layout and routing invariants (`GridId` must be carried for routing).
- ADR 0209: tooltip formatting contract (cursor/axisPointer affects tooltip sampling).
