# ADR 0182: Prepaint Interaction Stream + Range Reuse (GPUI-Aligned)
...
- diagnostics and scripted tests need deterministic “what was interactive this frame” data (ADR 0174).
...
When a cache root is clean (ADR 1152) and caching is enabled:
...
- The runtime gains a clean place to implement hover/focus pseudoclass tracking without forcing layout churn (ADR 0181).
...
- Builds on cache-root attribution and nested cache root correctness rules from ADR 1152.
