...
`ElementRuntime`. View-level caching (ADR 0213) and multi-stream recording (ADR 0055, ADR 0167) aim to make “steady
state” frames near-zero cost by reusing recorded ranges when views are clean (ADR 0165).
...
- View-cache reuse remains gated by dirty views and explicit cache keys (ADR 0165 / ADR 0213).
...
- A window shift MAY be applied during `prepaint` by reconciling retained children inside an explicit retained-host
  boundary (ADR 0177), without forcing a dirty-view rerender of the parent cache root.
...
- Nested cache roots inside barrier-driven windowed surfaces (virtual lists, scroll content, etc.) MUST remain safe under
  ...
  can be desynchronized (e.g. bounds expressed in unscrolled content space). See ADR 0213 section 5.
...
- ADR 0165: remains the primary gate for view-cache reuse (“dirty views block reuse”).
- ADR 0167: defines multi-stream recording and prepaint responsibilities; this ADR specifies a virtualization/windowing
...
- ADR 0213: cache-root semantics and subtree reuse must remain correct under windowed updates.
