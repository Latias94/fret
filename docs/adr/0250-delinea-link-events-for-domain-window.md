# ADR 0250: `delinea` Link Events for Domain Windows (P0)
...
- ADR 0191: dataZoom X semantics and filter-mode behavior.
- ADR 0198: Y + 2D view semantics (mapping-first, constraints).
- ADR 0248: multi-grid routing invariants.
- ADR 0207: link events emission must be change-based and non-spammy.
...
Any cross-grid linking policy must be explicitly enabled (ADR 0248 section 5).
