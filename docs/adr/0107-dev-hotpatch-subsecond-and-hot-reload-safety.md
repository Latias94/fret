...
- Subsecond patches take effect only for **function-pointer based entry points** that are called through the Subsecond
  jump table (ADR 0107). In this workspace, the recommended hotpatch-ready path is the `FnDriver`/`UiAppDriver` based
...
This is tooling-only; no kernel crates depend on it (ADR 0093 / ADR 0108).
