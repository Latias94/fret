...
- **In flight (worktree branches; not merged):**
  - `refactor/render-plan-effects`:
    ...
    - Streaming image v1 (RGBA8 only): runner holds uploaded textures and applies `Effect::ImageUpdateRgba8` via dirty-rect `queue.write_texture` writes (desktop + web), with deterministic latest-wins coalescing + cross-frame queueing + per-window upload/staging budgets (ADR 0123). Optional counters are exposed via `fret_core::StreamingUploadPerfSnapshot` when enabled (`WinitRunnerConfig.streaming_perf_snapshot_enabled`). Visual smoke demo: `cargo run -p fret-demo --bin streaming_image_demo`.
