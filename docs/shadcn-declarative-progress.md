...
| sheet | `sheet` | Present | In review | Default copyable root path is `Sheet::new_controllable(cx, None, false).compose().trigger(...).content_with(...)`; `SheetContent::build(...)` now covers the typed content side of that same lane, default close affordance lives in `SheetContent` and aligns with upstream `showCloseButton` / `show_close_button(false)`, while `Parts` stays a focused follow-up for explicit root-part ownership; gallery order now mirrors shadcn docs before Fret-specific parts docs; audit: `docs/audits/shadcn-sheet.md`; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs` |
...
| tooltip | `tooltip` | Present | In review | `Tooltip::new(cx, trigger, content)` remains the right recipe-level bridge (no extra generic `compose()` needed); `TooltipTrigger::build(...)` plus `TooltipContent::build(cx, ...)` now define the typed compound-parts lane on first-party snippets; gallery order mirrors shadcn docs before Fret-specific long-content / keyboard-focus parity sections; rendered via overlay root (not clipped); audit: `docs/audits/shadcn-tooltip.md` |
