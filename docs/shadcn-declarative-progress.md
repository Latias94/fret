...
| button | `button` | Present | In review | Audit: `docs/audits/shadcn-button.md`; gallery order now mirrors shadcn docs with explicit `Usage` / `Cursor` / `Default` / `Outline` / `Secondary` / `Ghost` / `Destructive` / `Link` / `API Reference` sections; the shared semantic-link follow-up is titled `As Link / As Child (Semantic)` and maps both the Base UI `As Link` docs path and the Radix `asChild` link example onto `ButtonRender::Link`, logical `data-icon` child compositions now map to `leading_child(...)` / `trailing_child(...)` (with plural helpers for multi-node landed content), the full-row override stays explicit via `child(...)` / `children(...)`, no extra generic root `asChild` / composable children API is warranted, and button docs sections now expose stable page-scoped ids for page-level diag gates; cursor ownership stays runtime/pressable-owned; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_control_chrome.rs` (`button-demo`) |
| button-group | `button_group` | Present | In review | Audit: `docs/audits/shadcn-button-group.md`; gallery order now mirrors shadcn docs with dedicated `Usage` / `Accessibility` / `ButtonGroup vs ToggleGroup` / `API Reference` sections; embedded `InputGroup` / `Select` examples now also follow those child families' default lanes instead of reintroducing parts adapters, `ButtonGroupText` keeps an explicit `new_children(...)` / `.children(...)` surface and now teaches the upstream `asChild` label lane via `Label::for_control(...)`, and `Flex-1 items` stays a Fret-only caller-owned layout example |
...
| dialog | `dialog` | Present | In review | Default copyable root path is `Dialog::new_controllable(cx, None, false).children([DialogPart::trigger(...), DialogPart::content_with(...)])`; `DialogPart` is now on the curated facade so the default import lane stays copyable, `DialogContent::with_children(...)` plus `DialogHeader::with_children(...)` / `DialogFooter::with_children(...)` now cover the default nested content side of that lane, `DialogContent::build(...)` remains the secondary builder-first surface, `DialogClose::from_scope().build(cx, button)` keeps footer/custom close actions aligned with upstream `<DialogClose asChild>`, and `compose()` plus `Parts` stay focused follow-ups for explicit builder-style or root-part ownership; `DialogHandle` is now landed as the narrow Base UI-style detached-trigger follow-up lane, so opener/content split ownership and focus restore are covered without widening the default docs path or adding a broader root children API; the current parity call is that typed `DialogPart` is already the warranted composable root API, so no broader untyped JSX-style root children API is justified at this time; the remaining drift was on the first-party docs/public surface rather than `fret-ui` mechanism, and the docs-path snippets now keep `Save changes` as a normal action button while aligning `Demo` / `Custom Close Button` / `RTL` widths and copy more closely with upstream examples; gallery order now mirrors shadcn docs through `API Reference` before Fret-specific follow-ups; audit: `docs/audits/shadcn-dialog.md`; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs`, `apps/fret-ui-gallery/tests/dialog_docs_surface.rs`, `tools/diag-scripts/ui-gallery/overlay/ui-gallery-dialog-docs-order-smoke.json`, `tools/diag-scripts/ui-gallery/overlay/ui-gallery-dialog-detached-trigger-focus-restore.json` |
...
| field | `field` | Present | In review | Audit: `docs/audits/shadcn-field.md`; gallery now keeps the full base/radix Field docs path intact through `API Reference`, adds explicit source-axis notes (shadcn base/radix docs + registry sources, Base UI field parts, and the lack of a standalone Radix Field primitive), and moves `Composable Children` to a single focused Fret follow-up after the docs path; `Usage` and `Anatomy` remain snippet-backed instead of page-local Rust strings, `Field::build(...)` is the intentional field-local control-association lane for `Select`, `Input`, and `Textarea`, while `Field::new([...])` stays the plain eager-children wrapper; no new mechanism gap found, no generic root `children(...)` / `compose()` / `asChild` widening is warranted, and width ownership stays explicit (`FieldDescription` recipe-owned full-width, plain `FieldLabel` / `FieldTitle` intrinsic by default); gates: `apps/fret-ui-gallery/tests/field_docs_surface.rs`, `apps/fret-ui-gallery/tests/ui_authoring_surface_default_app.rs`, `tools/diag-scripts/ui-gallery/field/ui-gallery-field-docs-smoke.json`, `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout.rs` (`field-*`) |
...
| item | `item` | Present | In review | Audit: `docs/audits/shadcn-item.md`; gallery now mirrors the base Item docs path first with explicit `Usage` / `Item vs Field` / `Examples` / `API Reference` sections, while `Gallery` and `Link (render)` stay as focused Fret follow-ups; `Item::new([...])` remains the composable root-children lane, `ItemTitle::new_children(...)` / `ItemDescription::new_children(...)` keep rich slot composition available without widening the root API, avatar rows stay slot-composed instead of adding a recipe-only `avatar` variant, and `ItemGroup` keeps only the list-container semantics while per-row `listitem` semantics stay caller-owned when a row also needs link/button semantics; intrinsic item chrome, slot spacing, and size presets remain recipe-owned, and existing item web layout gates continue to cover `item-demo` / `item-size` / `item-avatar` / `item-link` |
| kbd | `kbd` | Present | In review | Audit: `docs/audits/shadcn-kbd.md`; gallery now mirrors the base Kbd docs with a snippet-backed single-key `Usage` section, docs-aligned textual/glyph examples (`⌘`, `⇧`, `⌥`, `⌃`, `⏎`) on the primary teaching lane, and stable section-scoped `ui-gallery-kbd-*` ids for docs diagnostics, with the docs smoke now asserting every docs-path section anchor (`Demo`, `Usage`, `Group`, `Button`, `Tooltip`, `Input Group`, `RTL`, `API Reference`); `Kbd::from_children([...])` remains the explicit Fret escape hatch for icon-only caps rather than a broader generic children API; fixed-height keycap chrome stays recipe-owned and button / tooltip / input-group placement remains caller-owned composition; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout/kbd.rs`, `ecosystem/fret-ui-shadcn/tests/fret_kbd_tooltip_slot.rs`, `tools/diag-scripts/ui-gallery/kbd/ui-gallery-kbd-docs-smoke.json` |
...
| separator | `separator` | Present | In review | Audit: `docs/audits/shadcn-separator.md`; vertical recipe default now maps upstream `data-vertical:self-stretch` to auto cross-axis sizing plus `align-self: stretch`, the gallery `Demo` section is back to the single horizontal top preview, and `API Reference` now documents the Base UI vs Radix source-axis split behind `.decorative(...)`; gates: `ecosystem/fret-ui-shadcn/src/separator.rs`, `ecosystem/fret-ui-shadcn/tests/web_vs_fret_{control_chrome,layout}/separator.rs`, `apps/fret-ui-gallery/tests/separator_docs_surface.rs`, `tools/diag-scripts/ui-gallery/separator/ui-gallery-separator-docs-smoke.json` |
...
| slider | `slider` | Present | In review | Audit: `docs/audits/shadcn-slider.md`; gallery now mirrors the shadcn/Base UI docs path first with explicit `Demo` / `Usage` / `Range` / `Multiple Thumbs` / `Vertical` / `Controlled` / `Disabled` / `RTL` / `API Reference` sections, while `Label Association`, `Extras`, and `Notes` stay as focused Fret follow-ups; the docs-path snippets now mirror the upstream example shapes more closely (`[25, 50] step(5)`, `[10, 20, 70] step(10)`, two vertical sliders, controlled label/readout association), slider semantics now live on thumbs rather than the root for better Radix/Base UI alignment, the existing `slider(model)` / `new_controllable(...)` surface is sufficient, and no generic children API is warranted on the shadcn lane while Base UI's compound parts stay a headless reference for future `fret-ui-kit` work; the diagnostics `set_slider_value` path now resolves descendant slider semantics before falling back to drag synthesis, so the long-standing UI Gallery slider set-value gate is green again; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_control_chrome.rs` (`slider-demo`), `apps/fret-ui-gallery/tests/slider_docs_surface.rs`, `tools/diag-scripts/ui-gallery/slider/ui-gallery-slider-set-value.json`, `tools/diag-scripts/ui-gallery/slider/ui-gallery-slider-range-drag-stability.json`, `tools/diag-scripts/ui-gallery/slider/ui-gallery-slider-label-click-focus.json`, `tools/diag-scripts/ui-gallery/slider/ui-gallery-slider-docs-screenshots.json` |
| sonner | `sonner` | Present | In review | Audit: `docs/audits/shadcn-sonner.md`; gallery now mirrors the shadcn Sonner docs path first with explicit `Demo` / `About` / `Usage` / `Examples` / `Types` / `Description` / `Position` / `API Reference` sections, code tabs use standalone docs sources that keep `Toaster` mounting inline for copyability, the current shadcn lane remains intentionally message-oriented instead of widening into a generic `children([...])` API, and any fully custom toast-body surface should land as a future lower-level `fret-ui-kit` toast primitive; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_placement/sonner.rs`, `apps/fret-ui-gallery/tests/sonner_docs_surface.rs`, `tools/diag-scripts/ui-gallery/sonner/ui-gallery-sonner-docs-screenshots.json`, `tools/diag-scripts/ui-gallery/sonner/ui-gallery-sonner-demo-layout-collapse-screenshots.json` |
| spinner | `spinner` | Present | In review | Audit: `docs/audits/shadcn-spinner.md`; gallery now mirrors the radix Spinner docs path first with explicit `Demo` / `Usage` / `Customization` / `Size` / `Button` / `Badge` / `Input Group` / `Empty` / `RTL` sections, while `Extras` and `API Reference` stay as focused Fret follow-ups; default icon/size/spin/current-color behavior remain recipe-owned, explicit icon/color/size refinements remain caller-owned, spinner semantics now align to upstream status/live-region expectations instead of a numeric progress role, neither `repo-ref/primitives` nor `repo-ref/base-ui` defines a dedicated spinner primitive so this remains a leaf recipe/docs-parity surface rather than a missing mechanism contract, Button/Badge/InputGroup slot surfaces remain sufficient so no generic children API is needed on the spinner leaf, and docs-surface gates now include `apps/fret-ui-gallery/tests/spinner_docs_surface.rs` plus `tools/diag-scripts/ui-gallery/spinner/ui-gallery-spinner-docs-screenshots.json`; existing unit/layout gates continue to cover spinner timing plus web geometry in `ecosystem/fret-ui-shadcn/src/spinner.rs` and `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout/spinner.rs` |
...
| textarea | `textarea` | Present | In review | Audit: `docs/audits/shadcn-textarea.md`; gallery now mirrors the base Textarea docs path with explicit source-axis notes and `API Reference`, the docs-path `Field` example again matches the upstream `Message` + helper-text-before-control composition, `RTL` keeps the translated feedback/four-row example, and `With Text` / `Label Association` stay as focused Fret follow-ups; `Textarea::control_id(...)` also inherits the field-local association inside `Field::build(...)`, so builder-composed fields get label/description/focus forwarding without changing the default explicit-id teaching lane; root `w-full min-w-0`, control chrome, min height, and resize behavior remain recipe-owned, and shadcn-web chrome gates continue to cover `textarea-demo` / invalid / focus states |
| toggle | `toggle` | Present | In review | Audit: `docs/audits/shadcn-toggle.md`; gallery now mirrors the base Toggle docs path first with explicit `Demo` / `Usage` / `Outline` / `With Text` / `Size` / `Disabled` / `RTL` sections, then keeps `Children (Fret)`, `Label Association`, and `API Reference` as focused Fret follow-ups; the builder-preserving `toggle(...)` / `toggle_uncontrolled(...)` helpers remain the source-shaped composable-children lane while `Toggle::children([...])` stays the landed-content follow-up, toggle chrome/size presets/pressed-state colors remain recipe-owned, docs-surface gates now include `apps/fret-ui-gallery/tests/toggle_docs_surface.rs` plus `tools/diag-scripts/ui-gallery/toggle/ui-gallery-toggle-docs-smoke.json`, and existing shadcn-web chrome gates continue to cover `toggle-demo` and focus states |
