...
| checkbox | `checkbox` | Present | In review | Audit: `docs/audits/shadcn-checkbox.md`; gallery order now mirrors shadcn docs with a dedicated `API Reference` section, while `Label Association` / `With Title` stay as Fret-specific composition follow-ups; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_control_chrome.rs` (`checkbox-demo`) |
