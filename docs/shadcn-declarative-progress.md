...
| form | `form` | Present | Pass | Audit: `docs/audits/shadcn-form.md`; gallery keeps the Form docs/follow-up path intact through `Form Demo`, `Usage`, `Submit Validation`, `Disabled Field`, `Demo`, `Input`, `Textarea`, `Checkbox + Switch`, `Fieldset`, `RTL`, and `Notes`; `FormControl` stays slot-like for one child and falls back to a zero-gap column for multi-child content, while `FormField` owns field-level `required` / invalid decoration and control-level `disabled` / `read_only` remain caller-owned; docs-order/raw-free gate: `apps/fret-ui-gallery/tests/form_docs_surface.rs`; matrix packet: `docs/workstreams/shadcn-component-parity-matrix-v1/artifacts/form_agent_packet_p0_v1.json` |
