...
| alert-dialog | `alert_dialog` | Present | In review | Default copyable root path is `AlertDialog::new_controllable(cx, None, false).children([AlertDialogPart::trigger(...), AlertDialogPart::content(...)])`; `children([...])` + `from_scope(...)` align the common shadcn call site more closely, `AlertDialogPart` is now on the curated facade so the default import lane stays copyable, and `AlertDialogCancel::variant(...)` covers the destructive ghost-cancel pairing; `compose()`, `Parts`, `Detached Trigger`, and `Rich Content` stay as focused follow-ups for explicit ownership or advanced seams; root surface is still recipe-layer part collection rather than full JSX nesting; audit: `docs/audits/shadcn-alert-dialog.md`; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs` |
...
| input | `input` | Present | In review | Audit: `docs/audits/shadcn-input.md`; gallery now mirrors the base Input docs with explicit `Usage` and `API Reference` sections, while `Label Association` stays as a focused Fret follow-up after the upstream path; root width/height defaults remain recipe-owned, file input stays composed `Input + Browse`, and shadcn-web chrome gates continue to cover `input-demo` / invalid / focus states |
