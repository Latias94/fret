...
| alert-dialog | `alert_dialog` | Present | In review | Default copyable root path is `AlertDialog::new_controllable(cx, None, false).compose().trigger(...).content_with(...)`; `compose()` + `from_scope(...)` align the common shadcn call site, while `Parts`, `Detached Trigger`, and `Rich Content` stay as focused Fret follow-ups; root surface still uses a closure/compose bridge instead of fully nested children; audit: `docs/audits/shadcn-alert-dialog.md`; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs` |
