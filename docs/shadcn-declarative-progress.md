...
| hover-card | `hover_card` | Present | In review | `HoverCard::new(trigger, content)` remains the right recipe-level bridge (no extra generic `compose()` needed); gallery order and usage now align directly with shadcn docs; supports custom anchor via `HoverCard::anchor_element(...)`; audit: `docs/audits/shadcn-hover-card.md` |
...
| native-select | `native_select` | Defer | In review | Audit: `docs/audits/shadcn-native-select.md`; current shadcn-facing authoring surface is present, but true parity depends on backend-native select widgets; prefer `select` today and revisit when platform-native plumbing is active |
...
| popover | `popover` | Present | Pass | Anchored placement + click-through outside press dismissal (ADR 0069); anchor-aware `into_element_with_anchor(...)` keeps closure-root authoring intentional instead of adding a generic `compose()`; usage/gallery now align directly with shadcn docs; audit: `docs/audits/shadcn-popover.md` |
