...
| calendar | `calendar` | Present | In review | Audit: `docs/audits/shadcn-calendar.md`; headless month grid lives in `fret-ui-kit` (`headless::calendar`); gallery now includes upstream doc-only `About` / `Date Picker` / `Selected Date (With TimeZone)` / `API Reference` sections before Fret-only follow-ups; caller owns `rounded/border` and page width, recipe owns inner chrome, `caption_layout(...)` now stays shared across `Calendar` and single-month `CalendarRange`, and `CalendarDayButton::supporting_text_by(...)` covers the copyable custom-days slot across both typed surfaces without adding a generic root children API |
...
| checkbox | `checkbox` | Present | In review | Audit: `docs/audits/shadcn-checkbox.md`; gallery now mirrors the base Checkbox docs path first with explicit `API Reference`, restores the upstream `Description` / `Group` framing, and teaches a derived mixed-state `Table` select-all example on the same action-first snapshot path while `Label Association` and `With Title` stay as focused Fret follow-ups; `Checkbox::from_checked(...)` / `from_checked_state(...)` remain the narrow source-aligned snapshot path, no generic children API is warranted, and checkbox chrome/focus ring/indicator visuals remain recipe-owned while surrounding field layout stays caller-owned |
...
| dialog | `dialog` | Present | In review | Default copyable root path is `Dialog::new_controllable(cx, None, false).children([DialogPart::trigger(...), DialogPart::content(...)])`; `DialogPart` is now on the curated facade so the default import lane stays copyable, `DialogClose::from_scope().build(cx, button)` keeps footer/custom close actions aligned with upstream `<DialogClose asChild>`, and `compose()` plus `Parts` stay focused follow-ups for explicit builder-style or root-part ownership; gallery order now mirrors shadcn docs through `API Reference` before Fret-specific follow-ups; audit: `docs/audits/shadcn-dialog.md`; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs` |
| drawer | `drawer` | Present | In review | `direction(...)` now aliases the upstream Vaul/shadcn placement prop, `disable_pointer_dismissal(...)` covers the Base UI-style root dismissal alias, `modal(false)` / `modal_mode(DrawerModalMode::{NonModal,TrapFocus})` now cover the Base UI non-modal follow-up, and `DrawerClose::from_scope().build(...)` covers the common `asChild` close path; default copyable root path is `Drawer::new_controllable(cx, None, false).children([DrawerPart::trigger(...), DrawerPart::content_with(...)])`, while `compose()` stays the builder-first alternative and Vaul-specific `snap_points(...)` stays a focused follow-up on that same root lane; gallery order now mirrors shadcn docs before the Fret-specific snap-points recipe; remaining larger gaps are controlled snap points, nested-drawer coordination, and Base UI's popup-root autofocus nuance for the non-modal follow-up, which should stay in `fret-ui-kit` / recipe policy rather than `fret-ui`; audit: `docs/audits/shadcn-drawer.md` |
...
| input | `input` | Present | In review | Audit: `docs/audits/shadcn-input.md`; gallery now mirrors the base Input docs with a dedicated `Usage` snippet source plus stable page-scoped `ui-gallery-input-*` section ids, while `Label Association`, `API Reference`, and `Notes` stay as focused Fret follow-ups after the upstream path; root width/height defaults remain recipe-owned, file input stays composed `Input + Browse`, page-level gallery gates now target the real docs structure, and shadcn-web chrome gates continue to cover `input-demo` / invalid / focus states |
| input-group | `input_group` | Present | In review | Audit: `docs/audits/shadcn-input-group.md`; gallery order now mirrors the base docs with explicit `Usage` / `Align` / `API Reference` sections, while `Tooltip` / `Label Association` / `Button Group` stay as Fret follow-ups; `InputGroup::new(model)` is now the first-party ergonomic lane, explicit addon/control parts remain the docs-parity lane, root `w-full min-w-0` remains recipe-owned, and caller override is gated in `ecosystem/fret-ui-shadcn/src/input_group.rs`; shadcn-web layout gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout.rs` (`input-group-dropdown` and related `input-group-*` cases); gallery docs-order gate: `apps/fret-ui-gallery/src/driver/render_flow.rs` |
| input-otp | `input_otp` | Present | In review | Classified as a direct recipe root/bridge family: first-party snippets now prefer `InputOTP::new(model)` plus `length(...)` and optional `group_size(...)`, while `InputOTPGroup` / `InputOTPSlot` / `InputOTPSeparator` plus `into_element_parts(...)` remain the upstream-shaped bridge; default slot chrome ownership aligns with upstream and separator semantics + `control_id` form association are landed; audit: `docs/audits/shadcn-input-otp.md`; gates: `ecosystem/fret-ui-shadcn/src/input_otp.rs`, `apps/fret-ui-gallery/src/ui/pages/input_otp.rs` |
...
| kbd | `kbd` | Present | In review | Audit: `docs/audits/shadcn-kbd.md`; gallery now mirrors the base Kbd docs with explicit `Usage` and `API Reference` sections, while fixed-height keycap chrome stays recipe-owned and button / tooltip / input-group placement remains caller-owned composition; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_control_chrome.rs` (`kbd-demo`) |
...
| navigation-menu | `navigation_menu` | Present | In review | Classified as a dual-lane family: `navigation_menu(cx, model, |cx| ..)` is the compact typed lane, while `NavigationMenuRoot/List/Item/Trigger/Content/Link/Viewport/Indicator` remain the upstream-shaped lane; mobile `viewport` mode + anchored placement; top-level link parity now uses contentless `NavigationMenuItem` `href/target/rel` while trigger chrome stays recipe-owned; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_placement.rs` (navigation-menu-demo.* open variants), `ecosystem/fret-ui-shadcn/src/navigation_menu.rs` |
| pagination | `pagination` | Present | In review | Classified as a dual-lane family: first-party app authoring prefers the `pagination(...)` / `pagination_content(...)` / `pagination_item(...)` / `pagination_link(...)` wrapper lane, while the explicit `Pagination` / `PaginationContent` / `PaginationItem` / `PaginationLink` parts surface remains the upstream-shaped lane; no extra generic `compose()` is needed; gallery order now mirrors shadcn docs with dedicated `Simple` / `Icons Only` examples; audit: `docs/audits/shadcn-pagination.md`; gates: `ecosystem/fret-ui-shadcn/src/pagination.rs` |
| popover | `popover` | Present | Pass | Default recipe path now uses `Popover::new(cx, trigger, content)` while `from_open(...).into_element_with(...)` / `into_element_with_anchor(...)` stay as explicit advanced seams for managed-open and anchor-aware sizing flows; anchored placement + click-through outside press dismissal (ADR 0069); usage/gallery now align directly with shadcn docs; audit: `docs/audits/shadcn-popover.md` |
...
| sheet | `sheet` | Present | In review | Default copyable root path is `Sheet::new_controllable(cx, None, false).compose().trigger(...).content_with(...)`; default close affordance lives in `SheetContent` and aligns with upstream `showCloseButton` / `show_close_button(false)`, while `Parts` stays a focused follow-up for explicit root-part ownership; gallery order now mirrors shadcn docs before Fret-specific parts docs; audit: `docs/audits/shadcn-sheet.md`; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs` |
...
| textarea | `textarea` | Present | In review | Audit: `docs/audits/shadcn-textarea.md`; gallery now mirrors the base Textarea docs with explicit `API Reference`, while `With Text` and `Label Association` stay as focused Fret follow-ups; root `w-full min-w-0`, control chrome, min height, and resize behavior remain recipe-owned, and shadcn-web chrome gates continue to cover `textarea-demo` / invalid / focus states |
| toggle | `toggle` | Present | In review | Audit: `docs/audits/shadcn-toggle.md`; gallery now mirrors the base Toggle docs path first with explicit `Usage` and `API Reference` sections, while `children([...])` remains the source-aligned content surface and `Label Association` stays as a focused Fret follow-up; toggle chrome, size presets, and pressed-state colors remain recipe-owned, and existing shadcn-web chrome gates continue to cover `toggle-demo` and focus states |
| toggle-group | `toggle_group` | Present | In review | Audit: `docs/audits/shadcn-toggle-group.md`; gallery now mirrors the base Toggle Group docs path first with explicit `Size` / `Custom` / `API Reference` sections, while `Single`, `Small`, `Large`, `Label Association`, and layout regression slices stay as focused Fret follow-ups; `ToggleGroupItem::refine_layout(...)` and `refine_style(...)` now cover upstream custom item-root sizing/rounding, selection semantics and segmented chrome remain recipe-owned, and existing web chrome/layout gates continue to cover `toggle-group-demo` and size/spacing variants |
| tooltip | `tooltip` | Present | In review | `Tooltip::new(cx, trigger, content)` remains the right recipe-level bridge (no extra generic `compose()` needed); gallery order now mirrors shadcn docs before Fret-specific long-content / keyboard-focus parity sections; rendered via overlay root (not clipped); audit: `docs/audits/shadcn-tooltip.md` |
