...
| input-group | `input_group` | Present | In review | Audit: `docs/audits/shadcn-input-group.md`; gallery order now mirrors the base docs with explicit `Usage` / `Align` / `API Reference` sections, while `Tooltip` / `Label Association` / `Button Group` stay as Fret follow-ups; `InputGroup::new(model)` is now the first-party ergonomic lane, explicit addon/control parts remain the docs-parity lane, narrow `custom_input(...)` / `custom_textarea(...)` entry points now cover the upstream `Custom Input` docs path without widening the recipe to generic root `children(...)`, root `w-full min-w-0` remains recipe-owned, and caller override is gated in `ecosystem/fret-ui-shadcn/src/input_group.rs`; shadcn-web layout gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout.rs` (`input-group-dropdown`, `input-group-custom`, and related `input-group-*` cases); gallery docs-order gate: `apps/fret-ui-gallery/src/driver/render_flow.rs` |
