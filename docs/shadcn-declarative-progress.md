...
| select | `select` | Present | In review | Anchored placement supports `side`/`align` + offsets; roving navigation supports `loop` default; shadcn-web placement gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_placement.rs` (`web_vs_fret_select_demo_overlay_placement_matches`); audit: `docs/audits/shadcn-select.md` |
