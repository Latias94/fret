...
| Input OTP | `input_otp` | P0 | Text engine + slot metrics + focus choreography | shadcn docs/examples + Base UI | `fret-ui-shadcn` + runtime text surfaces | Recipe-heavy | Focus/input invariant test + control-id semantics test + gallery form evidence | Reviewed 2026-03: default gap/slot chrome ownership already matched upstream, while separator semantics and `control_id` form association needed explicit alignment |
...
| context-menu | `context_menu` | Present | In review | Right click + (macOS) ctrl-click + Shift+F10; anchors to click position for web/Radix parity; default copyable root path is `ContextMenu::uncontrolled(cx).build_parts(...)` while managed-open seams stay explicit on `ContextMenu::from_open(...)` / `new_controllable(...)`; trigger surface stays caller-owned while explicit panel width overrides live on `ContextMenuContent::min_width(...)`; parts bridges remain `ContextMenu::build_parts(...)` / `into_element_parts(...)`; audit: `docs/audits/shadcn-context-menu.md`; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_placement.rs`, `ecosystem/fret-ui-shadcn/tests/radix_web_overlay_geometry.rs` |
...
| dropdown-menu | `dropdown_menu` | Present | In review | Menu navigation + typeahead + dismissible popover infra (ADR 0074); now includes `Label`/`Group`/`Shortcut` + destructive items; default copyable root path is `DropdownMenu::uncontrolled(cx).build_parts(...)` while managed-open seams stay explicit on `DropdownMenu::from_open(...)` / `new_controllable(...)`; trigger sizing stays caller-owned while explicit panel width overrides live on `DropdownMenuContent::min_width(...)`; parts bridge remains `DropdownMenu::into_element_parts(...)`; audit: `docs/audits/shadcn-dropdown-menu.md`; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_placement.rs` |
...
| input-group | `input_group` | Present | In review | Audit: `docs/audits/shadcn-input-group.md`; gallery order now mirrors the base docs with explicit `Usage` / `Align` / `API Reference` sections, while `Tooltip` / `Label Association` / `Button Group` stay as Fret follow-ups; root `w-full min-w-0` remains recipe-owned, with caller override gated in `ecosystem/fret-ui-shadcn/src/input_group.rs`; shadcn-web layout gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_layout.rs` (`input-group-dropdown` and related `input-group-*` cases); gallery docs-order gate: `apps/fret-ui-gallery/src/driver/render_flow.rs` |
| input-otp | `input_otp` | Present | In review | Default slot chrome ownership aligns with upstream; separator semantics + `control_id` form association landed; audit: `docs/audits/shadcn-input-otp.md`; gates: `ecosystem/fret-ui-shadcn/src/input_otp.rs`, `apps/fret-ui-gallery/src/ui/pages/input_otp.rs` |
...
| menubar | `menubar` | Present | In review | Click-to-open; hover switching; Escape close focus is gated against radix timelines; `MenubarContent::{min_width, submenu_min_width}` now cover shadcn-style content width overrides while root width stays caller-owned; `entries_parts(...)` remains the parts bridge (no extra generic children API); audit: `docs/audits/shadcn-menubar.md`; gates: `ecosystem/fret-ui-shadcn/tests/{radix_web_primitives_state,web_vs_fret_overlay_placement}.rs` |
...
| navigation-menu | `navigation_menu` | Present | In review | Mobile `viewport` mode + anchored placement; top-level link parity now uses contentless `NavigationMenuItem` `href/target/rel` while trigger chrome stays recipe-owned; gates: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_placement.rs` (navigation-menu-demo.* open variants), `ecosystem/fret-ui-shadcn/src/navigation_menu.rs` |
| pagination | `pagination` | Present | In review | Explicit parts surface already matches upstream (no extra generic `compose()` needed); gallery order now mirrors shadcn docs with dedicated `Simple` / `Icons Only` examples; audit: `docs/audits/shadcn-pagination.md`; gates: `ecosystem/fret-ui-shadcn/src/pagination.rs` |
...
| sheet | `sheet` | Present | In review | `compose()` + parts bridge are present; default close affordance lives in `SheetContent` and aligns with upstream `showCloseButton` / `show_close_button(false)`; gallery order now mirrors shadcn docs before Fret-specific parts docs; audit: `docs/audits/shadcn-sheet.md`; shadcn-web chrome gate: `ecosystem/fret-ui-shadcn/tests/web_vs_fret_overlay_chrome.rs` |
