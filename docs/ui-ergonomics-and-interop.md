...
2. **Policy / authoring surface** (`ecosystem/fret-ui-kit`, `ecosystem/fret-ui-shadcn`, `fret`):
   ...
   graphs. Treat raw `on_action_notify`, raw `on_payload_action_notify`, the lower-level
   `cx.actions().payload::<A>().models(...)` seam, and low-level `.on_activate(cx.actions()....)`
   helpers as cookbook/reference-only advanced seams.
