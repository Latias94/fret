...
2. **Policy / authoring surface** (`ecosystem/fret-ui-kit`, `ecosystem/fret-ui-shadcn`, `fret`):
   ...
   For the `fret` golden path, keep the default first-contact handler surface on `cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)` (shared graphs), `cx.actions().transient::<A>(...)`, and widget-local `.on_activate(cx.actions().dispatch::<A>())` / `.dispatch_payload::<A>(...)` / `.listener(...)`; treat raw `on_action_notify` and low-level `on_activate*` helpers as cookbook/reference-only host-side glue.
...
- Whether a simple todo app can stay on `cx.actions().locals::<A>(...)`, `cx.actions().transient::<A>(...)`, and widget-local `.on_activate(cx.actions().dispatch::<A>())` / `.listener(...)` without reaching for raw `on_action_notify` or shared-model coordination.
