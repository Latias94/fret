...
- Whether a simple todo app can stay on `cx.actions().locals::<A>(...)`, `cx.actions().transient::<A>(...)`, and widget-local `.dispatch::<A>()` / `.listen(...)` without reaching for raw `on_action_notify` or shared-model coordination.
