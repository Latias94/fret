...
2. **Policy / authoring surface** (`ecosystem/fret-ui-kit`, `ecosystem/fret-ui-shadcn`, `fret`):
   ...
   For the `fret` golden path, keep the default first-contact handler surface on `cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)` (shared graphs), `cx.actions().transient::<A>(...)`, and local `on_activate*`; treat raw `on_action_notify` as cookbook/reference-only host-side glue.
...
- Whether a simple todo app can stay on `cx.actions().locals::<A>(...)`, `cx.actions().transient::<A>(...)`, and local `on_activate*` without reaching for raw `on_action_notify` or shared-model coordination.
