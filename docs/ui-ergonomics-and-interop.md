...
2. **Policy / authoring surface** (`ecosystem/fret-ui-kit`, `ecosystem/fret-ui-shadcn`, `fret`):
   ...
   For the `fret` golden path, keep the default first-contact handler surface on `cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)` (shared graphs), `cx.actions().transient::<A>(...)`, widget `.action(...)` / `.action_payload(...)` when a stable action slot exists, and the same action-first vocabulary (`.action(...)` / `.action_payload(...)` / `.listen(...)`) for activation-only surfaces after an explicit `use fret::app::AppActivateExt as _;`; `.dispatch::<A>()` / `.dispatch_payload::<A>(...)` stay available as explicit aliases. Treat raw `on_action_notify` and low-level `.on_activate(cx.actions()....)` helpers as cookbook/reference-only host-side glue.
