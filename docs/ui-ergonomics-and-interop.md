...
2. **Policy / authoring surface** (`ecosystem/fret-ui-kit`, `ecosystem/fret-ui-shadcn`, `fret`):
   ...
   For the `fret` golden path, keep the default first-contact handler surface on `cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)` (shared graphs), `cx.actions().transient::<A>(...)`, widget `.action(...)` / `.action_payload(...)` when a stable action slot exists, and widget-local `.dispatch::<A>(cx)` / `.dispatch_payload::<A>(cx, ...)` / `.listen(cx, ...)` for activation-only surfaces; treat raw `on_action_notify` and low-level `.on_activate(cx.actions()....)` helpers as cookbook/reference-only host-side glue.
...
- Whether a simple todo app can stay on `cx.actions().locals::<A>(...)`, `cx.actions().transient::<A>(...)`, and widget-local `.dispatch::<A>(cx)` / `.listen(cx, ...)` without reaching for raw `on_action_notify` or shared-model coordination.
