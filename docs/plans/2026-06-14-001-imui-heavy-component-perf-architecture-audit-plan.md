...
- A view-cache observation-collapse experiment split two different ideas: a pre-scan fast path was rejected because it still rebuilt the whole observation index and did not reduce the hot frames, while incremental relocation of descendant observations to their nearest cache root reduced collapse work without changing invalidation semantics.
- The latest accepted view-cache collapse direction is a shared mechanism optimization: keep existing root and non-cache observation entries in place, move only descendant entries, and union masks when root and descendant observe the same model/global.
...
- 2026-06-14: the first view-cache observation-collapse fast path was rejected after perf comparison: worst total stayed around `12578us` and `layout_collapse_layout_observations_time_us` stayed around `1872us`.
- 2026-06-14: replacing full observation-index rebuild with incremental descendant relocation passed the same `dev-fast` combobox gate with `failures=[]` and reduced worst-frame observation collapse from about `1783us` to `386us` for layout and from about `511us` to `137us` for paint. Worst total moved from the baseline gate's `12666us` to `11575us`; this is a real shared-mechanism win but still not a formal 120Hz release contract.
