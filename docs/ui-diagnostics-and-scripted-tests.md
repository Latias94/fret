...
we plan to add explicit gating flags (e.g. `--check-view-cache-reuse-min N`) that assert a minimum
number of cache-root reuse events in the exported `bundle.json`.

Current workaround:

- run the script with view-cache enabled (e.g. `--env FRET_UI_GALLERY_VIEW_CACHE=1` for the UI gallery),
- then use `fretboard diag stats <bundle_dir|bundle.json> --warmup-frames <n>` and confirm reuse counters
  (e.g. `cache_roots_reused`) are non-zero in snapshots after warmup.
...
the synthesis seam", we plan to export synthesis events into `bundle.json` (e.g. `debug.overlay_synthesis`)
and add a corresponding gating flag (e.g. `--check-overlay-synthesis-min N`).

### Matrix runner (uncached vs cached)

We plan to add a `diag matrix` runner that captures uncached vs cached bundles for a suite and compares
them automatically (semantics + optional scene fingerprint). Track progress in:
`docs/workstreams/overlay-arbitration-polish-todo.md`.

### Bundle comparison (cached vs uncached)

We plan to add a `diag compare` command that focuses on stable `debug.semantics.nodes[].test_id` anchors
and can optionally compare `scene_fingerprint` (paint output fingerprint). Track progress in:
`docs/workstreams/overlay-arbitration-polish-todo.md`.
