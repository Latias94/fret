...
- Last verified: U9 consumption profile gate hardening passed the new Python helper tests, actual consumption-profile gate with default/batteries `fret` checks, formatting, layering/surface gates, and `git diff --check`.
...
- Latest done: U9 consumption profile gate hardening.
...
- Next action: split the `ecosystem/fret/src/view.rs` runtime cluster into a private `view/runtime.rs` module while preserving root/prelude aliases and authoring-surface tests.
