...
- Last verified: U9 view context split passed focused `fret` authoring-surface tests, backend-free/default/batteries `fret` checks, `fret-framework` manual assembly check, formatting, layering/surface/consumption gates, and `git diff --check`.
...
- Latest done: U9 view context split.
- In progress: U9 consumption-profile gate hardening remains open; U8 target-machine native text budget runs and web/wasm runtime bundle evidence remain open.
- Blocked: no blocking issue.
- Next action: add default `fret` and `--features batteries` compile checks to `tools/check_consumption_profiles.py`, include the gate in `tools/pre_release.py`, and add a small helper test for banned dependency-tree detection.
