...
- Last verified: U9 view activation bridge split passed focused activation/action/facade nextest gates, the consumption profile gate, formatting, layering/surface gates, and `git diff --check`.
...
- Latest done: U9 view activation bridge split.
...
- Next action: split the remaining grouped action helper carriers and `AppRenderActionsExt` implementation into `view/actions.rs`.
