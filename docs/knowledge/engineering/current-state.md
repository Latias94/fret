...
- Last verified: Phase 3 U3 hit-test topology slice passed focused hit-test tests, full `fret-ui`
  nextest, layering, surface, formatting, and whitespace gates on 2026-07-03.
...
- Latest done: Phase 3 U3 sixth slice migrated hit-test bounds-tree candidate reachability and
  hit-test path-cache publication from retained `Node.parent` to layer-root child-edge paths.
- In progress: Phase 3 U3 remaining normal-query retained parent audit.
- Blocked: none known after the boundary store migration.
- Next action: continue U3 by classifying the remaining parent uses: migrate semantics snapshot
  parent assignment, widget coordinate mapping, propagation depth, bounds-tree prepaint parent
  reconstruction, and remaining invalidation/debug-only parent reads where they are normal query
  paths; keep retained storage mutation and U5 parent repair/dirty-count work separate.
