...
- Last verified: Phase 3 U3 input ancestry topology slice passed focused input tests, full
...
- Latest done: Phase 3 U3 fifth slice migrated shortcut key-context ancestry and pointer-down
  default-focus ancestor resolution from retained `Node.parent` to layer-forest child-edge topology.
- In progress: Phase 3 U3 remaining normal-query retained parent audit.
- Blocked: none known after the boundary store migration.
- Next action: continue U3 by classifying the remaining parent uses: migrate hit-test path cache,
  semantics snapshot parent assignment, widget coordinate mapping, propagation depth, and remaining
