...
- Last verified: U9 view runtime split passed focused runtime/facade nextest gates, backend-free/default/batteries `fret` checks through the consumption profile gate, formatting, layering/surface gates, and `git diff --check`.
...
- Latest done: U9 view runtime split.
...
- Next action: choose the next U9 split boundary: AppUi actions/data/effects helper groups, or component/recipe adapters currently mixed into `view/local_state.rs`.
