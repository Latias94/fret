...
- Last verified: Phase 2 U11 passed focused `fret` facade tests, mutation cookbook source-policy
  tests, the mutation toast feedback example check, `fret-mutation` handle-clone coverage,
  formatting, surface policy, consumption profiles, and execution-surface on 2026-07-02.
...
- Latest done: Phase 2 U11 lets public cookbook/starter code submit/retry mutations, project
  terminal mutation state, and emit Sonner feedback through `AppUi` wrappers without raw
  `ModelStore` or `UiActionHostAdapter` imports.
- In progress: continue to U12 mutation-workbench starter generation and diagnostics.
