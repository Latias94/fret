...
- Last verified: U9 view bridge impls split passed focused bridge/facade nextest gates, the consumption profile gate, formatting, layering/surface gates, and `git diff --check`.
...
- Latest done: U9 view bridge impls split.
...
- Next action: split remaining `AppUi` shell helpers into focused scheduling/layout-query/lane-barrier modules, or stop U9 once the facade file is small enough for the current plan checkpoint.
