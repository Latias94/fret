...
- Last verified: U9 view state/effects split passed focused state/source-shape/facade nextest gates, the consumption profile gate, formatting, layering/surface gates, and `git diff --check`.
...
- Latest done: U9 view state and effects split.
...
- Next action: split the AppUi action helper group into `view/actions.rs` while preserving `AppRenderActionsExt` and the explicit `AppActivateExt` bridge surface.
