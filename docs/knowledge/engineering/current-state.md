...
- Last verified: U9 view action helpers split passed focused action/facade nextest gates, the consumption profile gate, formatting, layering/surface gates, and `git diff --check`.
...
- Latest done: U9 view action helpers split.
...
- Next action: split grouped data/query/mutation helper carriers into `view/data.rs` while preserving `AppUiData` / `AppRenderData` public names and source-shape gates.
