...
- Last verified: Phase 3 U13 internal-harness classification passed source-policy tests/gate,
  consumption-profile, execution-surface, layering, formatting, and whitespace gates on 2026-07-03.
...
- Latest done: Phase 3 U13 eleventh slice makes `internal_harness` a real source-policy category
  and moves `docking_arbitration_demo.rs` plus `plot_stress_demo.rs` out of `advanced_manual`
  retirement records.
...
- Next action: continue U13 by shrinking exact cookbook/example records through code migration;
  ...
  migrating one of the now-explicit `fret-examples` raw local-state helper clusters to app-facing
  helpers, or moving more true stress/conformance-only demos to `internal_harness` when they are
  not copyable public examples.
