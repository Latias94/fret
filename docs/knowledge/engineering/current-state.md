...
- Last verified: Phase 3 U3 dirty/remove topology slice passed `cargo check -p fret-ui`, focused
  dirty/remove/view-cache/window snapshot tests, non-stack full `fret-ui` nextest, formatting,
  whitespace, layering, surface, consumption-profile, execution-surface, ADR-number,
  workstream-catalog, and wiki-memory gates on 2026-07-03. Full `fret-ui` nextest still has a local
...
- Latest done: Phase 3 U3 dirty/remove topology slice migrated semantics dirty propagation, layout
  dirty propagation and underflow repair, subtree removal unlink/delta propagation, layout
  profile/debug ancestry, and view-cache membership ancestor walks away from normal retained-parent
  reads where covered.
- In progress: Phase 3 U3 remaining normal-query retained parent audit, with the
  ...
  and [dirty/remove topology slice](progress/2026-07-03-phase3-u3-dirty-remove-topology.md)
  leaving mount repair and retained subtree membership collection as the next retained bridges.
- Blocked: none known after the boundary store migration.
- Next action: finish U3 by classifying remaining retained parent reads; migrate any remaining
  normal live-topology queries; then use U4/U5 to move cache-hit membership, GC liveness, and parent
  repair off normal paths rather than preserving them as hidden shims.
