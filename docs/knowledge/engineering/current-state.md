...
- Last verified: Phase 3 U13 examples root internal-harness classification passed source-policy
  tests/gate, consumption-profile, execution-surface, layering, formatting, engineering-memory,
...
- Latest done: Phase 3 U13 fourteenth slice moves `apps/fret-examples/src/lib.rs` from
  `advanced_manual` quarantine to `internal_harness` because it is crate-private runner/helper
  infrastructure rather than copyable app authoring material.
...
- Next action: continue U13 by shrinking exact cookbook/example records through code migration;
  ...
  migrating `virtual_list_basics.rs` through an explicit virtual-list facade lane, or migrating one
