---
type: Work Progress
title: Plot stacked bars surface policy gate
timestamp: 2026-07-07T17:03:15Z
tags:
  - fret-examples
  - surface-policy
  - plot
...
Added a dedicated source-policy boundary for `apps/fret-examples/src/stacked_bars_demo.rs`.
The demo remains an advanced manual runner because it owns the `FnDriver` and `UiTree` lifecycle,
but category bar authoring, panel props, and query output reads must stay routed through
`BarsPlotPanelBinding`.

# Truth

- `stacked_bars_demo.rs` is no longer covered only by the generic manual chart owner.
- A retained/manual plot-state regression is rejected by the Python surface policy fixture.
- The current declarative binding authoring shape remains allowed.
...
- `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tools.test_check_surface_policy.SurfacePolicyTests.test_plot_stacked_bars_legacy_retained_authoring_is_rejected tools.test_check_surface_policy.SurfacePolicyTests.test_plot_stacked_bars_declarative_binding_surface_is_allowed`
- `PYTHONDONTWRITEBYTECODE=1 python3 -m unittest tools.test_check_surface_policy`
- `PYTHONDONTWRITEBYTECODE=1 python3 tools/check_surface_policy.py`
- `cargo nextest run -p fret-examples stacked_bars_demo_uses_manual_harness_declarative_bars_plot_panel --no-fail-fast`
- `cargo fmt --all --check`
