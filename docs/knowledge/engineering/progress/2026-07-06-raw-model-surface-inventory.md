...
- Custom effect demos such as `custom_effect_v2_*`. They expose effect parameter models and reset
  ...
  - Follow-up: `custom_effect_v2_web_demo.rs` now keeps direct reset/toggle model writes behind a
    local `CustomEffectV2WebModelOwner`. Keep model allocation in the function-driver setup path
    for now; apply the owner-boundary pattern to the remaining V2 web variants before designing a
...
- Remaining custom-effect V2 web variants (`identity`, `lut`, `glass-chrome`) where raw model-store
  writes still appear outside a dedicated owner helper.
