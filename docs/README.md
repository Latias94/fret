...
- widget-local `.dispatch::<A>()` / `.dispatch_payload::<A>(...)` / `.listen(...)` - activation-only glue on the default app lane; raw `on_activate*` helper families stay component/advanced-oriented.
