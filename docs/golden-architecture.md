...
- `ecosystem/fret-docking/src/facade.rs` + `src/facade/{types,query,viewport,host,driver}.rs` (DockSurface app-facing facade, DTO/query helpers, typed viewport lifecycle, ordinary host callbacks, driver-tier callbacks)
- `ecosystem/fret-docking/src/dock/declarative.rs` (declarative docking host)
- `ecosystem/fret-docking/src/dock/drop_resolve/{target,intent,transaction,diagnostics}.rs` (drop target resolution, release transaction validation, diagnostics)
- `ecosystem/fret-docking/src/runtime/{tear_off,before_close,window_created,layout_invalidation}.rs` (docking-owned OS-window lifecycle, explicit close targets, transient-state cleanup, and imported-window invalidation)
