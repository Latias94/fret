...
- If advanced code intentionally wants the raw model-backed hook, import `use fret::advanced::raw::AppUiRawModelExt;` and call `cx.raw_model::<T>()` explicitly instead of treating it as part of the default `AppUi` surface.
