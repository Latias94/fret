...
- Handle actions via `cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)` (shared graphs), `cx.actions().transient::<A>(...)`, plus widget-local `.action(...)` / `.action_payload(...)` / `.listen(...)` for activation-only surfaces after explicitly importing `use fret::app::AppActivateExt as _;`; keep `.dispatch::<A>()` / `.dispatch_payload::<A>(payload)` as explicit aliases and keep raw `AppUi::on_action_notify*` and low-level `on_activate*` helpers for cookbook/reference host-side cases.
