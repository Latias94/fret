...
- Handle actions via `cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)` (shared graphs), `cx.actions().transient::<A>(...)`, plus widget-local `.dispatch::<A>()` / `.dispatch_payload::<A>(payload)` / `.listen(...)` by default; keep raw `AppUi::on_action_notify*` and low-level `on_activate*` helpers for cookbook/reference host-side cases.
...
- typed action handlers (`cx.actions().locals::<A>(...)`, `cx.actions().models::<A>(...)`, `cx.actions().transient::<A>(...)`, widget-local `.dispatch::<A>()` / `.dispatch_payload::<A>(payload)` / `.listen(...)`, and rare cookbook/reference `AppUi::on_action_notify*`) plus action IDs
