...
- P0: Tighten the **framework vs components** boundary so Tailwind/shadcn sizing/variants can converge without fighting runtime widgets:
  ...
  - remove hard-coded control heights/spacing from runtime primitives (notably `TextInput`) so `Size` (ADR 0056) stays component-owned. (MVP 48; in progress, `TextInput` done and tooltip/toast moved)
...
- P1: Anchored overlays (popover-style) for component primitives.
  - Note: a prototype exists in `fret-ui` (notably popover/dialog/menu shells), but the long-term plan is to ship these surfaces from `fret-components-ui` (MVP 48) so sizing/variants stay component-owned.
