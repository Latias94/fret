...
- ImUi Dear ImGui gap-closure lane (current source-backed audit for which IMUI gaps are still real,
...
- ImUi color edit alpha preview options follow-on (closed narrow P1 lane adding per-control
  checkerboard, opaque, no-background, and half-alpha preview modes to editor `ColorEdit`, matching
  Dear ImGui's ColorButton preview policy axis without global `SetColorEditOptions()` state):
...
- ImUi color edit reference preview follow-on (closed narrow P1 lane adding Dear ImGui-style
  current/original reference previews to editor `ColorEdit` popups, including original restore
  rules that copy RGB only for no-alpha targets and RGBA for alpha-visible targets):
...
- ImUi color edit vertical AlphaBar follow-on (closed narrow P1 lane inlining a Dear ImGui-style
  vertical AlphaBar into editor `ColorEdit`'s `HsvHueBar` picker while preserving the picker-hidden
  standalone alpha path):
  `docs/workstreams/imui-color-edit-vertical-alpha-bar-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-vertical-alpha-bar-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-color-edit-vertical-alpha-bar-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi color edit HueWheel picker follow-on (closed narrow P1 lane adding an opt-in Dear
  ImGui-style `PickerHueWheel` surface to editor `ColorEdit`, including hue ring angle mapping,
  rotated SV triangle mapping, Canvas rendering, and optional vertical AlphaBar composition):
  `docs/workstreams/imui-color-edit-hue-wheel-picker-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-hue-wheel-picker-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-color-edit-hue-wheel-picker-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi color edit picker options popup follow-on (closed narrow P1 lane adding a popup-local
  options surface for switching `HsvHueBar` / `HsvHueWheel` and toggling AlphaBar visibility
  without global `SetColorEditOptions()` state):
...
- ImUi color edit editable palette slots follow-on (closed narrow P1 lane making editor
  `ColorEdit` popup palette entries RGB drag sources and app-owned editable drop targets without
  adding framework-owned palette storage):
  `docs/workstreams/imui-color-edit-editable-palette-slots-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-editable-palette-slots-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-color-edit-editable-palette-slots-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi color edit history swatches follow-on (closed narrow P1 lane adding app-owned recent color
  swatches to editor `ColorEdit` popups while keeping history recording policy outside the
  framework):
  `docs/workstreams/imui-color-edit-history-swatches-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-history-swatches-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-color-edit-history-swatches-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi color edit tooltip preview follow-on (closed narrow P1 lane adding Dear ImGui-style
  `ColorTooltip()` hover previews to editor `ColorEdit` root swatches while keeping tooltip policy
  per-control and editor-owned):
  `docs/workstreams/imui-color-edit-tooltip-preview-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-tooltip-preview-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-color-edit-tooltip-preview-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi color edit copy-as context menu follow-on (closed narrow P1 lane adding Dear ImGui-style
  `ColorEditOptionsPopup()` copy payloads to editor `ColorEdit` root swatches while keeping
  clipboard writes effect-driven and editor-owned):
  `docs/workstreams/imui-color-edit-copy-as-context-menu-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-copy-as-context-menu-v1/EVIDENCE_AND_GATES.md`, and
...
- ImUi color edit popup split follow-on (closed narrow P1 refactor lane splitting popup overlay
  composition, picker UI helpers, numeric rows, alpha/preset preview helpers, and popup-local pointer
  handlers out of the editor `ColorEdit` public control file without behavior changes):
  `docs/workstreams/imui-color-edit-popup-split-v1/DESIGN.md`,
  `docs/workstreams/imui-color-edit-popup-split-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-color-edit-popup-split-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi color edit popup numeric split follow-on (closed narrow P1 refactor lane splitting editable
  RGB/HSV numeric row composition, validation error display, placeholders, and Enter/Escape commit
  handling into `popup/numeric.rs` without behavior changes):
...
- ImUi color edit popup preview split follow-on (closed narrow P1 refactor lane splitting shared
  checkerboard, fill-preview layout, and color preview stack helpers into `popup/preview.rs` without
...
- ImUi debug draw quad primitives follow-on (closed narrow P1 lane adding `AddQuad`- and
  `AddQuadFilled`-style commands to the canvas-backed helper without adding tessellation or
  hit-testing contracts):
  `docs/workstreams/imui-debug-draw-quad-primitives-v1/DESIGN.md`,
  `docs/workstreams/imui-debug-draw-quad-primitives-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-debug-draw-quad-primitives-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi debug draw ngon primitives follow-on (closed narrow P1 lane adding `AddNgon`- and
  `AddNgonFilled`-style commands to the canvas-backed helper without adding tessellation or
  hit-testing contracts):
...
- ImUi debug draw path Bezier builder follow-on (closed narrow P1 lane adding scoped Dear
  ImGui-style `PathBezierQuadraticCurveTo` / `PathBezierCubicCurveTo` helpers to the path builder
  through stable sampled points):
...
- ImUi debug draw path elliptical arc builder follow-on (closed narrow P1 lane adding a scoped Dear
  ImGui-style `PathEllipticalArcTo` rotated elliptical arc helper to the path builder through stable
  sampled points):
  `docs/workstreams/imui-debug-draw-path-elliptical-arc-builder-v1/DESIGN.md`,
  `docs/workstreams/imui-debug-draw-path-elliptical-arc-builder-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-debug-draw-path-elliptical-arc-builder-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi debug draw path rect builder follow-on (closed narrow P1 lane adding scoped Dear ImGui-style
  `PathRect` square and rounded rectangle helpers to the path builder through typed corner flags):
...
- ImUi debug draw vertex quad follow-on (closed narrow P1 lane adding true Dear ImGui-style
  `AddRectFilledMultiColor` and `AddImageQuad` semantics through portable scene primitives and WGPU
...
- ImUi debug draw clip metadata follow-on (closed narrow P1 lane adding source-level Dear ImGui
  `ImDrawCmd::ClipRect`-style effective clip rect/depth metadata to command summaries):
  `docs/workstreams/imui-debug-draw-clip-metadata-v1/DESIGN.md`,
  `docs/workstreams/imui-debug-draw-clip-metadata-v1/EVIDENCE_AND_GATES.md`, and
  `docs/workstreams/imui-debug-draw-clip-metadata-v1/CLOSEOUT_AUDIT_2026-05-05.md`.
- ImUi debug draw cookbook proof follow-on (closed narrow P1 lane adding a runnable public
  `fret::imui::kit` cookbook proof for debug-draw channel ordering, clip metadata, triangle
  meshes, image triangle meshes, and metadata summaries):
...
- ImUi debug draw response surface follow-on (closed narrow P1 lane returning post-merge
  source-level summaries and opt-in canvas-level `ResponseExt` interaction for the public
  debug-draw helper without adding renderer callbacks or raw buffers):
  `docs/workstreams/imui-debug-draw-response-surface-v1/DESIGN.md`,
  `docs/workstreams/imui-debug-draw-response-surface-v1/EVIDENCE_AND_GATES.md`, and
...
- ImUi interaction inspector follow-on (closed product-facing slice that makes the cleaned IMUI
  response vocabulary visible in `imui_interaction_showcase_demo` while keeping
  `imui_response_signals_demo` as the proof/contract surface and avoiding public API widening):
...
- ImUi editor-notes inspector command closeout record (closed narrow P1 lane that deepened the
  existing shell-mounted `editor_notes_demo.rs` proof with one app-owned inspector command/status
  loop rather than reopening generic command, clipboard, inspector, or IMUI helper APIs):
...
- ImUi next-gap audit closeout record (closed narrow P1 audit lane that ranks the next locally
  testable IMUI follow-on after the editor-notes dirty-status proof, recommends
  `imui-editor-notes-draft-actions-v1`, and keeps public IMUI/helper APIs plus macOS-only
  multi-window work out of scope):
...
- ImUi edit lifecycle hardening closeout record (closed narrow P1 lane for slider, drag-value,
  ...
  and `docs/workstreams/imui-edit-lifecycle-hardening-v1/CLOSEOUT_AUDIT_2026-04-25.md`.
...
- ImUi control geometry stability closeout record (closed narrow lane after the text-control chrome
  fix that locally locks base IMUI control geometry so hover/focus/pressed/active/disabled state
  changes do not alter compact editor control geometry, without Linux compositor acceptance):
...
- ImUi label identity ergonomics closeout record (closed narrow lane for Dear ImGui-style `##` /
...
- ImUi table sortable header closeout record (closed narrow follow-on for app-owned sorting via
  sortable header responses and current-direction indicators, without adding a row sorting engine,
  multi-sort policy, resize, or persistence):
...
- ImUi table sortable diagnostics gate follow-on (closed narrow lane that promotes the runnable
  sortable inspector table proof into a passing `fretboard diag` script without reopening row
  sorting engines or table policy):
  ...
  and `docs/workstreams/imui-table-sortable-diag-gate-v1/CLOSEOUT_AUDIT_2026-04-29.md`.
- ImUi table column resize follow-on closeout record (closed narrow lane for resizable IMUI table header
  boundaries and response reporting while sizing state, persistence, row sorting, multi-sort, and
  runtime table semantics remain outside `fret-ui-kit::imui`):
...
- ImUi ID stack browser follow-on (closed narrow lane for browser-style triage over captured
  ...
  and `docs/workstreams/imui-id-stack-browser-v1/CLOSEOUT_AUDIT_2026-04-28.md`.
- ImUi identity browser HTML follow-on (closed narrow lane for a self-contained offline HTML
  artifact over captured identity warnings, built on the closed browser model/query lane without
  adding live devtools or public runtime identity APIs):
  ...
  and `docs/workstreams/imui-identity-browser-html-v1/CLOSEOUT_AUDIT_2026-04-28.md`.
- ImUi identity browser visual gate follow-on (closed narrow lane for deterministic structural
  smoke checks over the offline HTML identity browser artifact without adding browser screenshots,
  dashboard integration, or live devtools):
  ...
  and `docs/workstreams/imui-identity-browser-visual-gate-v1/CLOSEOUT_AUDIT_2026-04-28.md`.
- ImUi identity browser fixture follow-on (closed narrow lane for a committed schema2 sample bundle
  that can drive grouped JSON and offline HTML/check sidecars without running a demo):
  ...
  and `docs/workstreams/imui-identity-browser-fixture-v1/CLOSEOUT_AUDIT_2026-04-28.md`.
...
- Outer-shell editor-rail mobile downgrade closeout record (closed narrow follow-on that freezes
  mobile downgrade as an outer-shell concern, keeps `Sidebar` out of the editor-rail story, and
  declines to extract a shared downgrade helper without repeated editor-shell evidence):
  ...
  and `docs/workstreams/outer-shell-editor-rail-mobile-downgrade-v1/CLOSEOUT_AUDIT_2026-04-11.md`.
...
- Docking multi-window parity (ImGui-style tear-off): `docs/workstreams/docking-multiwindow-imgui-parity/WORKSTREAM.json` (overview: `docs/workstreams/docking-multiwindow-imgui-parity/docking-multiwindow-imgui-parity.md`; mixed-DPI acceptance: `docs/workstreams/docking-multiwindow-imgui-parity/M7_MIXED_DPI_REAL_HOST_ACCEPTANCE_2026-04-26.md`; local non-Linux continuation boundary: `docs/workstreams/docking-multiwindow-imgui-parity/M11_LOCAL_NON_LINUX_CONTINUATION_BOUNDARY_2026-04-29.md`; latest local Wayland-boundary refresh: `docs/workstreams/docking-multiwindow-imgui-parity/M15_LOCAL_WAYLAND_BOUNDARY_REFRESH_2026-05-14.md`; latest source-drift guard: `docs/workstreams/docking-multiwindow-imgui-parity/M16_SOURCE_DRIFT_GUARD_2026-05-14.md` with a 2026-05-15 Wayland admission follow-up; latest local Wayland policy-skip matrix: `docs/workstreams/docking-multiwindow-imgui-parity/M18_LOCAL_WAYLAND_POLICY_SKIP_MATRIX_2026-05-16.md`; launched bounded-campaign repair: `docs/workstreams/docking-multiwindow-imgui-parity/M14_LAUNCHED_BOUNDED_CAMPAIGN_REPAIR_2026-05-13.md`; current TODOs: `docs/workstreams/docking-multiwindow-imgui-parity/docking-multiwindow-imgui-parity-todo.md`).
...
- Public authoring state lanes + identity contract (closed closeout lane for LocalState-first default teaching, explicit raw-model naming, kernel/facade substrate convergence, full example migration, and the default app-facing render-authoring closure; the later `UiCx*` compatibility cleanup is now also closed on a delete verdict): `docs/workstreams/public-authoring-state-lanes-and-identity-fearless-refactor-v1/DESIGN.md`, `docs/workstreams/public-authoring-state-lanes-and-identity-fearless-refactor-v1/TODO.md`, `docs/workstreams/public-authoring-state-lanes-and-identity-fearless-refactor-v1/MILESTONES.md`, `docs/workstreams/public-authoring-state-lanes-and-identity-fearless-refactor-v1/MIGRATION_MATRIX.md`, `docs/workstreams/public-authoring-state-lanes-and-identity-fearless-refactor-v1/CLOSEOUT_AUDIT_2026-04-19.md`, and `docs/adr/0319-public-authoring-state-lanes-and-identity-contract-v1.md`.
- UiCx compat alias release retirement (closed narrow release-facing closeout lane that deleted the full `UiCx*` compatibility family from the published `fret` surface instead of carrying it into another release window): `docs/workstreams/uicx-compat-alias-release-retirement-v1/DESIGN.md`, `docs/workstreams/uicx-compat-alias-release-retirement-v1/CLOSEOUT_AUDIT_2026-04-20.md`, `docs/workstreams/uicx-compat-alias-release-retirement-v1/EVIDENCE_AND_GATES.md`, and `docs/workstreams/uicx-compat-alias-release-retirement-v1/WORKSTREAM.json`.
...
9. keep both `public-authoring-state-lanes-and-identity-fearless-refactor-v1` and
   `uicx-compat-alias-release-retirement-v1` closed as the current render-authoring closeout
   record; do not reopen old alias vocabulary or broader storage-model / mutation-owner debates
   without fresh cross-surface evidence,
...
- P0: Move **Docking UI** out of `crates/fret-ui` (policy-heavy, hard to optimize) into a dedicated component/app layer crate, while keeping runtime substrate clean.
  ...
  - Completed the retained-bridge migration path and removed `fret-ui/unstable-retained-bridge`; docking now uses public declarative entry points plus generic runtime mechanisms.
