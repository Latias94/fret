#!/usr/bin/env python3
"""
Headless renderer perf gate for a deterministic, cross-platform scene.

Why:
- Our WebGPU/Tint uniformity closure can force shaders into more conservative shapes.
- This gate keeps a lightweight, non-interactive guardrail that catches:
  - quad pipeline variant explosions (pipeline switches),
  - unintended draw-call growth,
  - basic renderer churn signals (bind group switches / bytes).

This is intentionally *not* a microbenchmark:
- Timing metrics are recorded for visibility, but the default thresholds focus on stable counters.
"""
...
def _to_ints(d: dict[str, str]) -> dict[str, int]:
    ...
    for k, v in d.items():
        if k.endswith("_ms"):
            # Keep ms metrics out of the threshold checker by default (record-only).
            continue
        out[k] = int(v)
    return out
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description=(
            "Run a deterministic headless renderer workload (svg atlas stress) and enforce "
            "stable counter thresholds (pipelines/draws/binds)."
        ),
    ...
    ap.add_argument(
        "--baseline",
        default="docs/workstreams/perf-baselines/svg-atlas-stress-headless.windows-local.v1.json",
    ...
    if not out_dir:
        out_dir = f"target/perf-gates/svg-atlas-stress-headless.{sha}.{time.strftime('%Y%m%d-%H%M%S')}"
    ...
    cmd = ["cargo", "run", "-q", "-p", "fret-svg-atlas-stress"]
    if bool(args.release):
        cmd.append("--release")
    cmd += ["--", "--headless", "--frames", str(int(args.frames))]
    ...
    print(f"[gate] svg-atlas-stress-headless -> {out_dir_path} (attempts={int(args.attempts)})")
    ...
    overall = {
        ...
        "suite": "svg-atlas-stress-headless",
