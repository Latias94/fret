param(
  ...
  [string]$OutDir = "",
  [string]$LaunchBin = "target/release/drop_shadow_demo",
...
function Usage {
  @"
Usage:
  pwsh -File tools/perf/diag_drop_shadow_v1_gate.ps1 `
    -Baseline <path> `
    [-OutDir <path>] `
    [-LaunchBin <path>] `
    [-TimeoutMs <n>] `
...
Notes:
  ...
  - Baselines are machine-dependent; generate one via:
      cargo run -p fretboard -- diag perf tools/diag-scripts/drop-shadow-v1-steady.json `
        ...
        --launch -- target/release/drop_shadow_demo
"@ | Write-Host
}

if ([string]::IsNullOrWhiteSpace($OutDir)) {
  $ts = [int](Get-Date -UFormat %s)
  $OutDir = "target/fret-diag-perf/drop-shadow-v1-steady.$ts"
}
...
Write-Host "[gate] drop-shadow-v1-steady -> $OutDir"
...
Write-Host "PASS (drop-shadow-v1-steady)"
