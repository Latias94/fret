#!/usr/bin/env python3
"""
Perf gate for the `liquid-glass-backdrop-warp-steady` suite (cross-platform, no jq/bash).
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Run the liquid-glass-backdrop-warp-steady perf suite and enforce perf baseline thresholds.",
    ...
    if not out_dir:
        out_dir = f"target/fret-diag-perf/liquid-glass-backdrop-warp-steady.{int(time.time())}"
    out_dir_path = _resolve_workspace_path(workspace_root, out_dir)
    out_dir_path.mkdir(parents=True, exist_ok=True)

    print(f"[gate] liquid-glass-backdrop-warp-steady -> {out_dir_path}")
    ...
    print("PASS (liquid-glass-backdrop-warp-steady)")
    return 0
