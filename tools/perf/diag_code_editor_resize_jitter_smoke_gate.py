#!/usr/bin/env python3
"""
Smoke gate for editor-grade code-editor rendering under resize jitter.

This is intentionally a *catastrophic regression* guard, not a calibrated baseline gate:

- Runs a code-editor focused diag perf script (UI gallery).
...
If you need calibrated thresholds, prefer the resize probes baseline gate:
  tools/perf/diag_resize_probes_gate.py --suite ui-code-editor-resize-probes
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--script",
        default="tools/diag-scripts/ui-gallery-code-editor-window-resize-drag-jitter-steady.json",
    )
    ...
    ap.add_argument("--max-top-total-us", type=int, default=350_000)  # 350ms
    ap.add_argument("--max-top-layout-us", type=int, default=250_000)  # 250ms
    ap.add_argument("--max-top-solve-us", type=int, default=200_000)  # 200ms
    ...
    if not out_dir:
        out_dir = f"target/fret-diag-code-editor-resize-jitter-smoke-{int(time.time())}"
    ...
    with stdout_path.open("wb") as out, stderr_path.open("wb") as err:
        rc = int(subprocess.run(cmd, cwd=str(workspace_root), stdout=out, stderr=err).returncode)
