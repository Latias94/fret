#!/usr/bin/env python3
"""
Perf gate for `liquid-glass-backdrop-warp-v2-steady` (cross-platform, no jq/bash).

Why this exists:
- v2 already has a checked-in baseline JSON.
- This script provides a single command to re-run the script and enforce baseline thresholds.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Run the liquid-glass-backdrop-warp-v2-steady perf script and enforce perf baseline thresholds.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ap.add_argument(
        "--baseline",
        required=True,
        help="Perf baseline JSON path (usually under docs/workstreams/perf-baselines/).",
    )
    ap.add_argument("--out-dir", default="")
    ap.add_argument("--launch-bin", default="target/release/liquid_glass_demo")
    ...
    if not out_dir:
        out_dir = f"target/fret-diag-perf/liquid-glass-backdrop-warp-v2-steady.{int(time.time())}"
    out_dir_path = _resolve_workspace_path(workspace_root, out_dir)
    out_dir_path.mkdir(parents=True, exist_ok=True)

    script_path = "tools/diag-scripts/liquid-glass-backdrop-warp-v2-steady.json"

    print(f"[gate] liquid-glass-backdrop-warp-v2-steady -> {out_dir_path}")
    print(f"[gate] script: {script_path}")
    print(f"[gate] baseline: {baseline}")
    print(f"[gate] launch-bin: {launch_bin}")

    build_rc = subprocess.run(
        ["cargo", "build", "-q", "-p", "fret-demo", "--release", "--bin", "liquid_glass_demo"],
        cwd=str(workspace_root),
    ).returncode
    if build_rc != 0:
        print("FAIL (failed to build liquid_glass_demo)", file=sys.stderr)
        return int(build_rc)
    ...
    print("PASS (liquid-glass-backdrop-warp-v2-steady)")
    return 0
