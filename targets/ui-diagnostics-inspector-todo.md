...
3. **Sharing**
   ...
   - Optional screenshot packaging for visual overlay debugging. (done: `FRET_DIAG_SCREENSHOTS=1` writes `target/fret-diag/screenshots/<bundle>/...` + `manifest.json`; `fretboard diag pack --include-screenshots` packs them; viewer can auto-select by manifest and render as an overlay background)
