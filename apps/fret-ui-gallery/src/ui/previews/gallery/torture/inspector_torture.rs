...
pub(in crate::ui) fn preview_inspector_torture(
    ...
    let selected_row_value = cx
        ...
        .flatten();
    let root_focus_id: Rc<Cell<Option<GlobalElementId>>> = Rc::new(Cell::new(None));
    let root_focus_id_for_rows = Rc::clone(&root_focus_id);
    ...
    let row = move |cx: &mut AppComponentCx<'_>, index: usize| {
        ...
        let row_focus_id = Rc::clone(&root_focus_id_for_rows);
        let row = cx.pressable(
            ...
            move |cx, st| {
                ...
                cx.pressable_add_on_activate_focus(Arc::new(move |host, _action_cx, _reason| {
    ...
    let root = list.attach_semantics(
