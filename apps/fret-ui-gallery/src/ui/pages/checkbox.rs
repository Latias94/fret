...
pub(super) fn preview_checkbox(cx: &mut UiCx<'_>) -> Vec<AnyElement> {
    ...
    let api_reference = doc_layout::notes_block([
        ...
        "`Checkbox::new(...)`, `Checkbox::new_optional(...)`, and `Checkbox::new_tristate(...)` cover the model-backed checked and mixed-state paths; the bool/optional-bool lanes accept the usual checked-state bridges while `Checkbox::from_checked(...)` / `from_checked_state(...)` plus `.action(...)` cover the default source-aligned snapshot/action path. `.on_click(...)` remains the lower-level command bridge when explicit command routing is genuinely needed.",
