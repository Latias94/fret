...
pub(super) fn preview_radio_group(cx: &mut AppComponentCx<'_>) -> Vec<AnyElement> {
    ...
    let required_disabled = snippets::required_disabled::render(cx);
    ...
    let required_disabled = DocSection::build(cx, "Required Disabled", required_disabled)
        .description(
            "Use `required(true)` on the group and item-level `disabled(true)` so grouped required semantics and disabled item action-state stay independent.",
        )
        .test_id_prefix("ui-gallery-radio-group-required-disabled")
        .code_rust_from_file_region(snippets::required_disabled::SOURCE, "example");
