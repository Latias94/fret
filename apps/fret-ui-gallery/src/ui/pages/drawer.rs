...
pub(super) fn preview_drawer(cx: &mut UiCx<'_>) -> Vec<AnyElement> {
    ...
    let api_reference = doc_layout::notes_block([
        ...
        "`DrawerContent::new([]).children(|cx| ...)` plus `DrawerHeader::new([]).children(|cx| ...)` / `DrawerFooter::new([]).children(|cx| ...)` is the default copyable content lane for upstream-like nested composition, while `children_raw(...)` remains the explicit pre-landed seam.",
        "`DrawerClose::from_scope().child(child)` is the closest Fret equivalent to upstream `DrawerClose asChild` for caller-owned close buttons, while `build(cx, child)` remains the immediate-landing escape hatch.",
        ...
        "`DrawerContent::build(...)` remains the sink-driven companion when a snippet genuinely wants `push_ui(...)` assembly instead of composable `children(|cx| ...)` sections.",
    ...
    let notes = doc_layout::notes_block([
        ...
        "The docs-path examples now share the same `Drawer::children([...])` root lane plus `DrawerContent::new([]).children(|cx| ...)` content lane, while `compose()` and `DrawerContent::build(...)` remain builder-first alternatives without pushing children API concerns into the mechanism layer.",
        "Docs-path footer close actions now consistently use `DrawerClose::from_scope().child(child)` so the copyable lane stays aligned with upstream `DrawerClose asChild` intent.",
        ...
        "`Demo` and `RTL` keep the official centered max-width inner content wrapper, while `Responsive Dialog` keeps the profile-form branch structure, so gallery visuals stay close to shadcn docs instead of only proving the raw mechanism works.",
    ...
    let demo = DocSection::build(cx, "Demo", demo)
        .description(
            "Official shadcn drawer demo with a centered max-width inner content wrapper, goal controls, and footer actions.",
        )
        .test_id_prefix("ui-gallery-drawer")
        .code_rust_from_file_region(snippets::demo::SOURCE, "example");
    let usage = DocSection::build(cx, "Usage", usage)
        .title_test_id("ui-gallery-section-usage-title")
        .description("Default copyable `children([...])` root lane with composable `children(|cx| ...)` content sections.")
