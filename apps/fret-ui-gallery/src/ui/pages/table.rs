...
pub(super) fn preview_table(cx: &mut UiCx<'_>) -> Vec<AnyElement> {
    ...
    let notes = doc_layout::notes_block([
        ...
        "If we later need exact checkbox-column spacing parity, that should still land as a `fret-ui-shadcn` recipe follow-up rather than a runtime contract change.",
    ]);
