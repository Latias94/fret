...
pub(super) fn preview_carousel(cx: &mut UiCx<'_>) -> Vec<AnyElement> {
    cx.keyed("ui_gallery.carousel_page", |cx| {
        ...
        let body = doc_layout::render_doc_page(
            cx,
            Some(
                "Preview mirrors the shadcn Carousel docs path first: Demo, About, Usage, Examples (Basic/Sizes/Spacing/Orientation), Options, API, Events, Plugins, RTL. After that, Gallery keeps Fret-only follow-ups explicit: `Fret Follow-ups`, `Compact Builder`, `Parts`, a dedicated `Loop` preview, engine/motion diagnostics, then `API Reference`.",
            ),
            vec![
                demo,
                about,
