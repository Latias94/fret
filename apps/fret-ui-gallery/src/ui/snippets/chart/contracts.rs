    ...
    ui::v_flex(|cx| {
        vec![
            shadcn::raw::typography::muted(
                "Chart UI contracts: static tooltip recipes, output-model auto wiring, label/item format hooks, custom-row composition via into_element_parts(...), config-key remapping via label_key/name_key, metadata-backed item formatting, and legend defaults from ChartConfig.",
            )
                .into_element(cx),
            tooltip(
                cx,
