...
pub fn render(cx: &mut UiCx<'_>) -> impl UiChild + use<> {
    ...
    let panel_body = ui::v_flex(move |cx| {
        ...
        let canvas_overlay_panel = ui_ai::WorkflowPanel::new([ui::h_flex(move |_cx| {
            vec![canvas_controls, canvas_toolbar]
        })
        .gap(Space::N3)
        .items_center()
        ...
        vec![
            ui::h_flex(move |_cx| vec![controls, toolbar])
                .gap(Space::N3)
                .items_center()
                .layout(LayoutRefinement::default().w_full().min_w_0())
                .into_element(cx),
