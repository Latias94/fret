...
fn square_image_id<H: UiHost>(cx: &mut ElementContext<'_, H>) -> Option<ImageId> {
...
fn square_image<H: UiHost>(
    ...
    let image_id = model_image_id.or_else(|| square_image_id(cx));
...
fn ratio_example<H: UiHost>(
    ...
    let frame = shadcn::AspectRatio::with_child(
        square_image(cx, demo_image, content_test_id).into_element(cx),
    )
    .ratio(ratio)
    .refine_style(
