...
pub fn render(cx: &mut AppComponentCx<'_>) -> impl UiChild + use<> {
    ui::h_flex(|cx| {
        let start_content = shadcn::PopoverContent::build(cx, |cx| {
        ...
        let center_content = shadcn::PopoverContent::build(cx, |cx| {
            [decl_text::text_paragraph(cx, "Aligned to center")]
        })
        .refine_layout(LayoutRefinement::default().w_px(Px(160.0)))
        .test_id("ui-gallery-popover-align-center-content");
        let end_content = shadcn::PopoverContent::build(cx, |cx| {
