...
impl UiGalleryDriver {
    ...
    pub(crate) fn select_gallery_page_in_models(
        ...
        let _ = app
            .models_mut()
            .update(state.workspace_workbench.window_layout(), |layout| {
                layout.active_pane = Some(Arc::from(UI_GALLERY_WORKSPACE_PANE_ID));
                if let Some(pane) = layout
                    .pane_tree
                    .find_pane_mut(UI_GALLERY_WORKSPACE_PANE_ID)
                {
                    pane.tabs.open_and_activate(page);
                }
            });
    }
    ...
    pub(crate) fn sync_selected_page_from_workspace_layout(
        app: &mut App,
        state: &mut UiGalleryWindowState,
        window: AppWindowId,
    ) -> bool {
        let Some(layout) = app
            .models()
            .get_cloned(state.workspace_workbench.window_layout())
        else {
            return false;
        };
        let Some((workspace_tabs, selected_page, _workspace_dirty_tabs)) =
