...
fn gallery_inspector_torture_uses_fixed_row_text_roles() {
    let normalized = assert_normalized_markers_present(
        ...
        &[
            ...
            "usefret_ui_kit::LayoutRefinement;",
...
#[test]
fn gallery_data_table_torture_can_disable_the_outer_content_scroll_shell() {
    let source = read_path(&manifest_path("src/ui/content.rs"));

    assert!(
        source.contains("disable_content_scroll_for_data_table_torture"),
        "data_table_torture should have an explicit content-scroll bypass gate",
    );
    assert!(
        source.contains("selected == PAGE_DATA_TABLE_TORTURE"),
        "data_table_torture should key the bypass gate off the torture page selection",
    );
    assert!(
        source.contains("|| disable_content_scroll_for_data_table_torture"),
...
#[test]
fn gallery_virtual_list_torture_can_disable_the_outer_content_scroll_shell() {
    let source = read_path(&manifest_path("src/ui/content.rs"));

    assert!(
        source.contains("disable_content_scroll_for_virtual_list_torture"),
        "virtual_list_torture should have an explicit content-scroll bypass gate",
    );
    assert!(
        source.contains("selected == PAGE_VIRTUAL_LIST_TORTURE"),
        "virtual_list_torture should key the bypass gate off the torture page selection",
    );
    assert!(
        source.contains("|| disable_content_scroll_for_virtual_list_torture"),
        "virtual_list_torture should join the bypass with the existing content-scroll guard",
    );
}
...
fn harness_virtual_list_torture_uses_fixed_row_text_roles() {
    let normalized = assert_normalized_markers_present(
        ...
        &[
            ...
            "structVirtualListRowActionStyle{",
            "fnvirtual_list_row_action_style(theme:&Theme)->VirtualListRowActionStyle",
            "letvariants=shadcn::button_variants(",
            "label_absolute_layout:fret_ui::element::LayoutStyle",
            "edit_absolute_layout:fret_ui::element::LayoutStyle",
            ".absolute().left(Space::N0).top(Space::N0).bottom(Space::N0).w_px(Px(label_width)).min_w_0()",
            ".absolute().right(Space::N0).top(Space::N0).bottom(Space::N0).w_px(Px(edit_width))",
            "fnvirtual_list_row_label_action<T,I>(cx:&mutAppComponentCx<'_>,style:&VirtualListRowActionStyle,label:T,test_id:I,on_activate:fret_ui::action::OnActivate,)->AnyElement",
            "cx.pressable(fret_ui::element::PressableProps{",
            "fret_ui_kit::declarative::text::text_button_label_fill(cx,label.clone())",
            "fnvirtual_list_row_label_action_absolute<T,I>(cx:&mutAppComponentCx<'_>,style:&VirtualListRowActionStyle,label:T,test_id:I,on_activate:fret_ui::action::OnActivate,)->AnyElement",
            "layout:style.label_absolute_layout",
            "fnvirtual_list_row_edit_action<T,I>(cx:&mutAppComponentCx<'_>,style:&VirtualListRowActionStyle,label:T,test_id:I,on_activate:fret_ui::action::OnActivate,)->AnyElement",
            "cx.pressable(fret_ui::element::PressableProps{",
            "fret_ui_kit::declarative::text::text_button_label(cx,label.clone())",
            "fnvirtual_list_row_edit_action_absolute<T,I>(cx:&mutAppComponentCx<'_>,style:&VirtualListRowActionStyle,label:T,test_id:I,on_activate:fret_ui::action::OnActivate,)->AnyElement",
            "layout:style.edit_absolute_layout",
            "virtual_list_row_label_action_absolute(",
            "virtual_list_row_edit_action_absolute(",
            "LayoutRefinement::default().relative().w_full().h_px(height_hint)",
            "letheader=(!minimal_harness).then(||{",
            "ifletSome(controls)=controls{out.push(controls);}",
            "ifletSome(editing_indicator)=editing_indicator{out.push(editing_indicator);}",
            "ifletSome(header)=header{children.push(header);}",
    ...
    for forbidden in [
        ...
        "fnvirtual_list_row_action_button<T,I>(",
        "fret_ui_kit::declarative::chrome::control_chrome_pressable_with_id_props(",
        "fnvirtual_list_row_label_action<T,I>(cx:&mutAppComponentCx<'_>,theme:&Theme,",
        "fnvirtual_list_row_edit_action<T,I>(cx:&mutAppComponentCx<'_>,theme:&Theme,",
        "fnvirtual_list_row_label_action_reserved<T,I>(",
        "label_reserved_layout:fret_ui::element::LayoutStyle",
        "shadcn::Button::new(format!(\"Row{index}\")).variant(shadcn::ButtonVariant::Ghost).size(shadcn::ButtonSize::Sm).test_id(format!(\"ui-gallery-virtual-list-row-{index}-label\")).on_activate(on_select_row.clone()).refine_layout(LayoutRefinement::default().flex_1().min_w_0()).into_element(cx)",
        "shadcn::Button::new(\"Edit\").variant(shadcn::ButtonVariant::Outline).size(shadcn::ButtonSize::Sm).test_id(format!(\"ui-gallery-virtual-list-row-{index}-edit\")).on_activate(on_select_row).into_element(cx)",
    ] {
        assert!(
            !normalized.contains(forbidden),
