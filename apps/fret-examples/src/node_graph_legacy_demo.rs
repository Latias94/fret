//! Legacy node graph demo (retained-bridge + compat retained canvas).
//!
//! This file intentionally stays as a reference implementation while the declarative-first
//! `node_graph_demo` evolves. Prefer the declarative demo for ecosystem authoring guidance.
...
impl NodeGraphDemoDriver {
    ...
    fn build_ui(app: &mut App, window: AppWindowId) -> NodeGraphDemoWindowState {
        ...
        let tuning = NodeGraphTuningOverlay::new(canvas_node, editor_config.clone(), style.clone())
...
pub fn run() -> anyhow::Result<()> {
    ...
    let diagnostics_enabled = std::env::var("FRET_DIAG")
        .ok()
        .is_some_and(|v| !v.trim().is_empty() && v.trim() != "0");
    let (mut view_value, editor_config) = if diagnostics_enabled {
        (
...
fn register_demo_commands(registry: &mut CommandRegistry) {
    ...
    registry.register(
        CommandId::new(CMD_TOGGLE_WIRE_HIGHLIGHT),
        CommandMeta::new("Toggle NodeGraph Wire Highlight")
            .with_category("Demo")
            .with_keywords(["wire", "highlight", "inner", "stroke"])
            .with_scope(CommandScope::App)
            .with_when(WhenExpr::parse("!focus.is_text_input").expect("valid when expr"))
            .with_default_keybindings([
                mac_cmd_shift(KeyCode::KeyH),
