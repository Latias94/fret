...
impl CustomEffectV2WebDriver {
    ...
    fn build_ui(app: &mut App, window: AppWindowId) -> CustomEffectV2WebWindowState {
        ...
        let controls = DemoControls {
            strength_px: CustomEffectV2ScalarControl::new(app.models_mut(), 14.0),
            max_sample_offset_px: CustomEffectV2ScalarControl::new(app.models_mut(), 18.0),
            tint_strength: CustomEffectV2ScalarControl::new(app.models_mut(), 0.8),
            blur_radius_px: CustomEffectV2ScalarControl::new(app.models_mut(), 12.0),
...
fn render(
    ...
    let root = declarative::RenderRootContext::new(&mut state.ui, app, services, window, bounds)
        .render_root("custom-effect-v2-web", |cx| {
            CustomEffectV2WebDriver::render_root(cx, binding.clone(), controls.clone())
        });
