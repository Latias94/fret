...
impl CustomEffectV2WebVariantControls for DemoControls {
    fn reset_variant_controls(&self, reset: &mut CustomEffectV2WebVariantReset<'_, '_>) -> bool {
        ...
        changed = reset.set_model(&self.strength_px, vec![14.0]) || changed;
        changed = reset.set_model(&self.max_sample_offset_px, vec![18.0]) || changed;
        changed = reset.set_model(&self.tint_strength, vec![0.8]) || changed;
        changed = reset.set_model(&self.blur_radius_px, vec![12.0]) || changed;
