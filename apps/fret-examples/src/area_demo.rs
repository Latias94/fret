...
fn create_window_state(
    _driver: &mut AreaDemoDriver,
    app: &mut App,
    window: AppWindowId,
) -> AreaDemoWindowState {
    AreaDemoDriver::build_ui(app, window)
}

fn hot_reload_window(
    _driver: &mut AreaDemoDriver,
    context: WinitHotReloadContext<'_, AreaDemoWindowState>,
) {
...
fn handle_event(
    _driver: &mut AreaDemoDriver,
    context: WinitEventContext<'_, AreaDemoWindowState>,
    event: &Event,
) {
...
fn render(_driver: &mut AreaDemoDriver, context: WinitRenderContext<'_, AreaDemoWindowState>) {
...
pub fn build_runner_config() -> WinitRunnerConfig {
    WinitRunnerConfig {
        main_window_title:
            "fret-demo area_demo (RMB drag zoom, Alt+LMB drag query, LMB double-click fit)"
                .to_string(),
...
fn build_fn_driver() -> FnDriver<AreaDemoDriver, AreaDemoWindowState> {
    FnDriver::new(
...
pub fn run() -> anyhow::Result<()> {
    ...
    crate::run_native_with_compat_driver(config, app, driver).context("run area_demo app")
}
