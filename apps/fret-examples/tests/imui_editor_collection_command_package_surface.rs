...
fn imui_editor_proof_demo_keeps_collection_command_package_app_owned_and_explicit() {
    ...
    for needle in [
        ...
        assert!(
            ...
            "collection browser input box-select session fixture owner should keep pointer/session fixtures explicit; missing `{needle}`"
        );
    }
    for needle in [
        "fn pointer_down(",
    ...
    for needle in [
        ...
        assert!(
            !browser_input_box_select_session_tests_fixtures_source.contains(needle),
            "collection browser input box-select session fixture owner should not take behavior tests or runtime event/model publication; unexpected `{needle}`"
        );
    }
    for needle in [
        "pub(super) struct ProofCollectionBrowserScopeInputModels",
    ...
    for needle in [
        ...
        assert!(
            ...
            "collection browser input context-menu tests owner should keep fixture imports and behavior coverage explicit; missing `{needle}`"
        );
    }
    for needle in [
        "pub(super) fn pointer_up(",
        ...
        assert!(
            ...
            "collection browser input context-menu tests fixture owner should keep PointerUpCx fixtures explicit; missing `{needle}`"
        );
    }
    for needle in [
        "fn pointer_up(",
    ...
    for needle in [
        ...
        assert!(
            !browser_input_context_menu_runtime_tests_source.contains(needle),
            "collection browser input context-menu tests owner should route PointerUpCx fixtures through context_menu/tests/fixtures.rs; unexpected `{needle}`"
        );
    }
    for needle in [
        "context_menu_anchor_prefers_window_position",
        ...
        assert!(
            !browser_input_context_menu_runtime_tests_fixtures_source.contains(needle),
            "collection browser input context-menu tests fixture owner should not take anchor behavior tests or runtime publication ownership; unexpected `{needle}`"
        );
    }
    for needle in [
        "pub(super) fn proof_collection_browser_scope_context_menu_anchor_from_up(",
