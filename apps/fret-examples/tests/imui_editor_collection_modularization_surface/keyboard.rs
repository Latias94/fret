pub(super) fn assert_keyboard_owner_split(keyboard_source: &str, keyboard_actions_source: &str) {
    ...
    for needle in [
        ...
        assert!(
            ...
            "the demo-local collection keyboard actions owner should route app-state mutation through ProofCollectionModelOwner; missing `{needle}`"
        );
    }
    for needle in [
        "cx.key_on_key_down_for(",
        ...
        assert!(
            !keyboard_actions_source.contains(needle),
            "the demo-local collection keyboard actions owner should not take key matching, snapshot reads, selection derivation policy, or raw model writes; unexpected `{needle}`"
        );
    }
}
