//! Renderer semantics: Straight vs Premultiplied alpha.
//!
//! This is intentionally a low-level example (SceneOps + image registration effects) rather than
//! a shadcn UI lesson.
//!
//! Visual expectation:
//! - The two cards should look visually identical.
//! - If premultiplied sources are handled incorrectly (double-multiplied), the right card will
//!   appear darker/washed out.
...
struct CompositingAlphaBasicsState {
...
fn handle_event(
    _driver: &mut (),
    context: WinitEventContext<'_, CompositingAlphaBasicsState>,
    event: &Event,
) {
    ...
    match event {
        ...
        Event::ImageRegisterFailed { token, message } => {
            if state.pending_straight == Some(*token) || state.pending_premul == Some(*token) {
                state.pending_straight = None;
                state.pending_premul = None;
                eprintln!("[compositing_alpha_basics] image register failed: {message}");
...
fn render(_driver: &mut (), context: WinitRenderContext<'_, CompositingAlphaBasicsState>) {
