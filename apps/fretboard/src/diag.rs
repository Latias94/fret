...
pub(crate) fn diag_cmd(args: Vec<String>) -> Result<(), String> {
    ...
    let mut check_retained_vlist_prefetch_reconciles_max: Option<u64> = None;
    ...
    while i < args.len() {
        ...
        match arg.as_str() {
            ...
            "--check-retained-vlist-reconcile-cache-reuse" => {
                i += 1;
                let Some(v) = args.get(i).cloned() else {
                    return Err(
                        "missing value for --check-retained-vlist-reconcile-cache-reuse"
                            .to_string(),
                    );
                };
            ...
            "--check-retained-vlist-prefetch-reconciles-max" => {
                i += 1;
                let Some(v) = args.get(i).cloned() else {
                    return Err(
                        "missing value for --check-retained-vlist-prefetch-reconciles-max"
                            .to_string(),
                    );
                };
                check_retained_vlist_prefetch_reconciles_max =
                    Some(v.parse::<u64>().map_err(|_| {
                        "invalid value for --check-retained-vlist-prefetch-reconciles-max"
                            .to_string()
                    })?);
                i += 1;
            }
            ...
            "--check-retained-vlist-attach-detach-min" => {
                i += 1;
                let Some(v) = args.get(i).cloned() else {
                    return Err(
                        "missing value for --check-retained-vlist-attach-detach-min".to_string()
                    );
                };
                check_retained_vlist_attach_detach_min = Some(v.parse::<u64>().map_err(|_| {
                    "invalid value for --check-retained-vlist-attach-detach-min".to_string()
                })?);
                i += 1;
            }
    ...
    match sub.as_str() {
        ...
        "run" => {
            ...
            if result.stage.as_deref() == Some("passed") {
                if check_stale_paint_test_id.is_some()
                    ...
                    || check_retained_vlist_prefetch_reconciles_max.is_some()
        ...
        "suite" => {
            ...
            if is_ui_gallery_data_table_retained_keep_alive_suite {
                if warmup_frames == 0 {
                    warmup_frames = 5;
                }
                for (key, value) in [
                    ...
                    ("FRET_UI_GALLERY_VIEW_CACHE_SHELL", "1"),
                    ("FRET_UI_GALLERY_DATA_TABLE_RETAINED", "1"),
                    ("FRET_UI_GALLERY_DATA_TABLE_KEEP_ALIVE", "256"),
                    ("FRET_DIAG_MAX_SEMANTICS_NODES", "10000"),
                    ("FRET_DIAG_SEMANTICS_TEST_IDS_ONLY", "1"),
                    ("FRET_DIAG_SCRIPT_AUTO_DUMP", "0"),
                ] {
            ...
            if is_ui_gallery_file_tree_torture_suite {
                ...
                check_retained_vlist_scroll_window_dirty_max =
                    check_retained_vlist_scroll_window_dirty_max.or(Some(0));
                check_wheel_scroll_test_id =
                    check_wheel_scroll_test_id.or(Some("ui-gallery-file-tree-node-0".to_string()));
                check_stale_paint_test_id =
                    check_stale_paint_test_id.or(Some("ui-gallery-file-tree-node-0".to_string()));
            }

            if is_ui_gallery_file_tree_torture_interactive_suite {
                if warmup_frames == 0 {
                    warmup_frames = 5;
                }
                for (k, v) in [
                ...
                check_retained_vlist_attach_detach_min =
                    check_retained_vlist_attach_detach_min.or(Some(1));
                check_retained_vlist_attach_detach_max =
                    check_retained_vlist_attach_detach_max.or(Some(512));
                check_retained_vlist_scroll_window_dirty_max =
                    check_retained_vlist_scroll_window_dirty_max.or(Some(0));
                check_wheel_scroll_test_id =
                    check_wheel_scroll_test_id.or(Some("ui-gallery-file-tree-node-0".to_string()));
            ...
            for src in scripts {
                ...
                let should_gate_retained_vlist_for_script = !(is_ui_gallery_tree_retained_suite
                    ...
                    || is_ui_gallery_retained_measured_suite
                ...
                let retained_vlist_prefetch_reconciles_max_for_script =
                    check_retained_vlist_prefetch_reconciles_max.filter(|_| {
                        should_gate_retained_vlist_for_script
                            && script_requires_retained_vlist_window_boundary_gate(&src)
                    });
                ...
                let wants_post_run_checks_for_script = check_stale_paint_test_id.is_some()
                    ...
                    || retained_vlist_prefetch_reconciles_max_for_script.is_some()
                ...
                if result.stage.as_deref() == Some("passed") && wants_post_run_checks_for_script {
                    ...
                    apply_post_run_checks(
                        ...
                        retained_vlist_prefetch_reconciles_max_for_script,
        ...
        "stats" => {
            ...
            if let Some(min) = check_retained_vlist_attach_detach_min
                && min > 0
            {
                check_bundle_for_retained_vlist_attach_detach_min(
                    bundle_path.as_path(),
                    min,
                    warmup_frames,
                )?;
...
fn apply_post_run_checks(
    ...
    check_retained_vlist_prefetch_reconciles_max: Option<u64>,
    ...
    if let Some(min) = check_retained_vlist_reconcile_cache_reuse_min
        && min > 0
    {
    ...
    if let Some(max) = check_retained_vlist_prefetch_reconciles_max {
        check_bundle_for_retained_vlist_prefetch_reconciles_max(bundle_path, max, warmup_frames)?;
    }
    if let Some(min) = check_retained_vlist_attach_detach_min
        && min > 0
    {
        check_bundle_for_retained_vlist_attach_detach_min(bundle_path, min, warmup_frames)?;
    }
    if let Some(max_delta) = check_retained_vlist_attach_detach_max {
...
fn check_bundle_for_retained_vlist_reconcile_cache_reuse_min(
    bundle_path: &Path,
    min_reconcile_reuse_frames: u64,
    warmup_frames: u64,
) -> Result<(), String> {
    let bytes = std::fs::read(bundle_path).map_err(|e| e.to_string())?;
    let bundle: serde_json::Value = serde_json::from_slice(&bytes).map_err(|e| e.to_string())?;
    check_bundle_for_retained_vlist_reconcile_cache_reuse_min_json(
        &bundle,
        bundle_path,
        min_reconcile_reuse_frames,
        warmup_frames,
    )
}

fn check_bundle_for_retained_vlist_reconcile_cache_reuse_min_json(
    bundle: &serde_json::Value,
    bundle_path: &Path,
    min_reconcile_reuse_frames: u64,
    warmup_frames: u64,
    ...
    let mut reconcile_frames: u64 = 0;
    let mut reconcile_reuse_frames: u64 = 0;
    let mut examined_snapshots: u64 = 0;
    let mut offenders: Vec<String> = Vec::new();
    ...
    for w in windows {
        ...
        for s in snaps {
            ...
            let any_reused = cache_roots
                .iter()
                .any(|r| r.get("reused").and_then(|v| v.as_bool()).unwrap_or(false));

            if any_reused {
                reconcile_reuse_frames = reconcile_reuse_frames.saturating_add(1);
            } else {
                let reuse_reason = cache_roots
                    .first()
                    .and_then(|r| r.get("reuse_reason"))
                    .and_then(|v| v.as_str())
                    .unwrap_or("<none>");
                offenders.push(format!(
                    "frame_id={frame_id} retained_reconciles={count} cache_roots={cache_roots_len} first_reuse_reason={reuse_reason}",
                    cache_roots_len = cache_roots.len(),
                ));
            }
        }
    }

    if reconcile_reuse_frames < min_reconcile_reuse_frames {
        let mut msg = String::new();
        msg.push_str("expected retained virtual-list reconcile to occur on cache-hit frames\n");
        msg.push_str(&format!("bundle: {}\n", bundle_path.display()));
        msg.push_str(&format!(
...
fn check_bundle_for_retained_vlist_prefetch_reconciles_max(
    bundle_path: &Path,
    max_prefetch_frames: u64,
    warmup_frames: u64,
) -> Result<(), String> {
    let bytes = std::fs::read(bundle_path).map_err(|e| e.to_string())?;
    let bundle: serde_json::Value = serde_json::from_slice(&bytes).map_err(|e| e.to_string())?;
    check_bundle_for_retained_vlist_prefetch_reconciles_max_json(
        &bundle,
        bundle_path,
        max_prefetch_frames,
        warmup_frames,
...
fn check_bundle_for_retained_vlist_prefetch_reconciles_max_json(
    bundle: &serde_json::Value,
    bundle_path: &Path,
    max_prefetch_frames: u64,
    warmup_frames: u64,
    ...
    if prefetch_frames > max_prefetch_frames {
        let mut msg = String::new();
        msg.push_str("retained virtual-list staged prefetch ran too frequently\n");
        msg.push_str(&format!("bundle: {}\n", bundle_path.display()));
        msg.push_str(&format!(
            "max_prefetch_frames={max_prefetch_frames} prefetch_frames={prefetch_frames} warmup_frames={warmup_frames} examined_snapshots={examined_snapshots}\n"
        ));
        return Err(msg);
    }
...
fn check_bundle_for_retained_vlist_attach_detach_min(
    bundle_path: &Path,
    min_delta: u64,
    warmup_frames: u64,
) -> Result<(), String> {
    let bytes = std::fs::read(bundle_path).map_err(|e| e.to_string())?;
    let bundle: serde_json::Value = serde_json::from_slice(&bytes).map_err(|e| e.to_string())?;
    check_bundle_for_retained_vlist_attach_detach_min_json(
        &bundle,
        bundle_path,
        min_delta,
        warmup_frames,
...
fn check_bundle_for_retained_vlist_attach_detach_min_json(
    bundle: &serde_json::Value,
    bundle_path: &Path,
    min_delta: u64,
    warmup_frames: u64,
    ...
    let mut reconcile_frames: u64 = 0;
    let mut examined_snapshots: u64 = 0;
    let mut best_delta: u64 = 0;
    let mut best_frame_id: u64 = 0;
    ...
    if reconcile_events == 0 {
        return Err(format!(
            "expected at least 1 retained virtual-list reconcile event (required for attach/detach min check), got 0 \
(warmup_frames={warmup_frames}, examined_snapshots={examined_snapshots}) bundle: {}",
    ...
    Err(format!(
        "retained virtual-list attach/detach delta never reached the configured minimum\n\
bundle: {}\n\
min_delta={min_delta} best_delta={best_delta} best_frame_id={best_frame_id} reconcile_events={reconcile_events} reconcile_frames={reconcile_frames} warmup_frames={warmup_frames} examined_snapshots={examined_snapshots}",
        bundle_path.display()
    ))
}
...
mod tests {
    ...
    #[test]
    fn check_bundle_for_retained_vlist_reconcile_cache_reuse_min_passes() {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                "window": 1,
                "snapshots": [{
                    ...
                    "debug": {
                        ...
                        "cache_roots": [
                            { "root_node": 100, "element": 200, "reused": true }
                        ]
                    }
                }]
            }]
        ...
        .expect("expected reconcile>=1 under cache-hit reuse");
    }

    #[test]
    fn check_bundle_for_retained_vlist_reconcile_cache_reuse_min_fails_when_reconcile_has_no_cache_reuse()
     {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                ...
                "snapshots": [{
                    ...
                    "debug": {
                        ...
                        "cache_roots": [
                            { "root_node": 100, "element": 200, "reused": false, "reuse_reason": "cache_key_mismatch" }
                        ]
                    }
                }]
            }]
        ...
        let err = check_bundle_for_retained_vlist_reconcile_cache_reuse_min_json(
            &bundle,
            Path::new("bundle.json"),
            1,
            0,
        )
        .expect_err("expected reconcile without cache-hit reuse to fail");
        assert!(
            err.contains("expected retained virtual-list reconcile to occur on cache-hit frames")
        );
    ...
    #[test]
    fn check_bundle_for_retained_vlist_attach_detach_min_passes() {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                "window": 1,
                "snapshots": [{
                    ...
                    "debug": {
                        "stats": {
                            "retained_virtual_list_reconciles": 1,
                            "retained_virtual_list_attached_items": 1,
                            "retained_virtual_list_detached_items": 0
                        }
                    }
                }]
            }]
    ...
    #[test]
    fn check_bundle_for_retained_vlist_attach_detach_min_fails_when_below_threshold() {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                "window": 1,
                "snapshots": [{
                    ...
                    "debug": {
                        "stats": {
                            "retained_virtual_list_reconciles": 1,
                            "retained_virtual_list_attached_items": 0,
                            "retained_virtual_list_detached_items": 0
                        }
                    }
                }]
        ...
        let err = check_bundle_for_retained_vlist_attach_detach_min_json(
            &bundle,
            Path::new("bundle.json"),
            1,
            0,
        )
        .expect_err("expected delta<1 to fail");
        assert!(err.contains("attach/detach delta never reached"));
        assert!(err.contains("min_delta=1"));
    }

    #[test]
    fn check_bundle_for_retained_vlist_attach_detach_min_fails_when_missing_reconciles() {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                "window": 1,
    ...
    #[test]
    fn check_bundle_for_retained_vlist_prefetch_reconciles_max_passes() {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                "window": 1,
                "snapshots": [
                    { "frame_id": 10, "debug": { "retained_virtual_list_reconciles": [{ "reconcile_kind": "prefetch" }] } },
                    { "frame_id": 11, "debug": { "retained_virtual_list_reconciles": [{ "reconcile_kind": "prefetch" }] } },
                    { "frame_id": 12, "debug": { "retained_virtual_list_reconciles": [{ "reconcile_kind": "escape" }] } }
                ]
            }]
        });
        ...
        .expect("expected prefetch_frames<=2");
    }

    #[test]
    fn check_bundle_for_retained_vlist_prefetch_reconciles_max_fails_when_exceeded() {
        let bundle = json!({
            "schema_version": 1,
            "windows": [{
                "window": 1,
                "snapshots": [
                    { "frame_id": 10, "debug": { "retained_virtual_list_reconciles": [{ "reconcile_kind": "prefetch" }] } },
                    { "frame_id": 11, "debug": { "retained_virtual_list_reconciles": [{ "reconcile_kind": "prefetch" }] } },
                    { "frame_id": 12, "debug": { "retained_virtual_list_reconciles": [{ "reconcile_kind": "prefetch" }] } }
                ]
            }]
        });

        let err = check_bundle_for_retained_vlist_prefetch_reconciles_max_json(
            &bundle,
            Path::new("bundle.json"),
            2,
            0,
        )
        .expect_err("expected prefetch_frames>2 to fail");
        assert!(err.contains("staged prefetch ran too frequently"));
        assert!(err.contains("prefetch_frames=3"));
    }
