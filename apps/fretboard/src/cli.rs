...
Usage:
  ...
  fretboard diag triage <base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> [--top <n>] [--sort <key>] [--warmup-frames <n>] [--json] [--out <path>]
    ...
    fretboard diag memory-summary [<base_or_session_out_dir>] [--within-session <id|latest|all>] [--top-sessions <n>] [--sort-key <key>] [--top <n>] [--vmmap-regions-sorted-top] [--vmmap-regions-sorted-agg] [--vmmap-regions-sorted-agg-top <n>] [--no-recursive] [--max-depth <n>] [--max-samples <n>] [--json] [--out <path>]
