...
Usage:
  ...
  fretboard diag list suites [--contains <needle>] [--all] [--top <n>] [--case-sensitive] [--json]
  ...
  fretboard diag doctor [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json>] [--dir <dir>] [--warmup-frames <n>] [--check|--check-all] [--fix|--fix-dry-run] [--fix-schema2] [--fix-bundle-json] [--fix-sidecars] [--json]
  fretboard diag doctor scripts [--max-examples <n>] [--strict] [--json]
  fretboard diag doctor campaigns [--strict] [--json]
  fretboard diag registry <check|write|print> [--path <path>] [--json]
  fretboard diag config doctor [--dir <dir>] [--env <KEY=VALUE>...] [--mode <launch|manual>] [--config-path <path>] [--show-env <set|all>] [--report-json] [--print-launch-policy]
  fretboard diag trace <base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> [--trace-out <path>]
  fretboard diag pack [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json>] [--dir <dir>] [--warmup-frames <n>] [--pack-out <path>] [--ai-packet] [--ai-only] [--include-all] [--include-root-artifacts] [--include-triage] [--include-screenshots] [--pack-schema2-only]
  fretboard diag triage <base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> [--top <n>] [--sort <key>] [--warmup-frames <n>] [--lite] [--metric <total|layout|paint>] [--json] [--out <path>]
    ...
    fretboard diag hotspots [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json>] [--warmup-frames <n>] [--hotspots-top <n>] [--max-depth <n>] [--min-bytes <n>] [--force] [--lite] [--metric <total|layout|paint>] [--json] [--out <path>]
    ...
    fretboard diag test-ids-index <base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> [--warmup-frames <n>] [--json]
    fretboard diag frames-index <base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> [--warmup-frames <n>] [--json]
    ...
    fretboard diag dock-graph <base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> [--json]
  ...
  fretboard diag query test-id [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json>] <pattern> [--warmup-frames <n>] [--mode <contains|prefix|glob>] [--top <n>] [--case-sensitive] [--json] [--out <path>]
  fretboard diag query snapshots [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.index.json|bundle.schema2.json>] [--warmup-frames <n>] [--window <id>] [--top <n>] [--include-warmup] [--include-missing-semantics] [--semantics-source <any|inline|table|none>] [--test-id <id>] [--step-index <n>] [--json] [--out <path>]
  fretboard diag query overlay-placement-trace [<base_or_session_out_dir|session_out_dir|script.result.json>] [--top <n>] [--kind <anchored_panel|placed_rect>] [--overlay-root-name <name>] [--anchor-test-id <id>] [--content-test-id <id>] [--preferred-side <top|bottom|left|right>] [--chosen-side <top|bottom|left|right>] [--flipped <true|false>] [--align <start|center|end>] [--sticky <partial|always>] [--json] [--out <path>]
  fretboard diag query scroll-extents-observation [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json>] [--warmup-frames <n>] [--window <id>] [--top <n>] [--all] [--deep-scan] [--timeline] [--json] [--out <path>]
  fretboard diag slice [<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json>] --test-id <test_id> [--warmup-frames <n>] [--frame-id <n>|--snapshot-seq <n>|--step-index <n>] [--window <id>] [--max-matches <n>] [--max-ancestors <n>] [--json] [--out <path>]
  fretboard diag inspect <on|off|toggle|status> [--consume-clicks <bool>]
  fretboard diag pick-arm
  fretboard diag pick [--timeout-ms <ms>] [--poll-ms <ms>]
  fretboard diag pick-script [--timeout-ms <ms>] [--poll-ms <ms>] [--pick-script-out <path>]
  fretboard diag pick-apply <script.json> [--timeout-ms <ms>] [--poll-ms <ms>] --ptr <json_pointer> [--out <path>]
  ...
  fretboard diag campaign list [--lane <lane>] [--tier <tier>] [--tag <tag>] [--platform <platform>] [--json]
  fretboard diag campaign show <campaign_id> [--json]
  fretboard diag campaign validate [<manifest.json>...] [--json]
  fretboard diag campaign share <campaign_or_batch_root|regression.summary.json> [--include-passed] [--top <n>] [--warmup-frames <n>] [--json]
  fretboard diag campaign run [<campaign_id>...] [--lane <lane>] [--tier <tier>] [--tag <tag>] [--platform <platform>] [--script-dir <dir>...] [--glob <pattern>...] [--prewarm-script <script.json>...] [--prelude-script <script.json>...] [--prelude-each-run] [--dir <dir>] [--timeout-ms <ms>] [--poll-ms <ms>] [--warmup-frames <n>] [--top <n>] [--json] [--pack] [--include-screenshots] [--max-test-ids <n>] [--all-test-ids] [--lint-eps-px <px>] [--no-suite-lint] [--check-pixels-changed <test_id>] [--check-pixels-unchanged <test_id>] [--env <KEY=VALUE>...] [--launch -- <cmd...>]
  ...
  fretboard diag stats (<base_or_session_out_dir|bundle_dir|bundle.json|bundle.schema2.json> | --diff <bundle_a> <bundle_b> | --stats-lite-checks-json) [--warmup-frames <n>] [--top <n>] [--sort <key>] [--verbose] [--json] [--check-stale-paint <test_id>] [--check-stale-paint-eps <px>] [--check-stale-scene <test_id>] [--check-stale-scene-eps <px>] [--check-idle-no-paint-min <n>] [--check-asset-load-missing-bundle-assets-max <n>] [--check-asset-load-stale-manifest-max <n>] [--check-asset-load-unsupported-file-max <n>] [--check-asset-load-unsupported-url-max <n>] [--check-asset-load-external-reference-unavailable-max <n>] [--check-asset-load-revision-changes-max <n>] [--check-bundled-font-baseline-source <none|bundled_profile>] [--check-asset-reload-epoch-min <n>] [--check-asset-reload-configured-backend <name|none|absent>] [--check-asset-reload-active-backend <name|none|absent>] [--check-asset-reload-fallback-reason <reason|none|absent>] [--check-pixels-changed <test_id>] [--check-pixels-unchanged <test_id>] [--check-semantics-changed-repainted] [--dump-semantics-changed-repainted-json] [--check-wheel-scroll <test_id>] [--check-wheel-scroll-hit-changes <test_id>] [--check-drag-cache-root-paint-only <test_id>] [--check-hover-layout|--check-hover-layout-max <n>] [--check-gc-sweep-liveness] [--check-notify-hotspot-file-max <file> <max>] [--check-view-cache-reuse-min <n>] [--check-view-cache-reuse-stable-min <n>] [--check-overlay-synthesis-min <n>] [--check-viewport-input-min <n>] [--check-dock-drag-min <n>] [--check-viewport-capture-min <n>] [--check-retained-vlist-reconcile-no-notify-min <n>] [--check-retained-vlist-attach-detach-max <n>] [--check-retained-vlist-keep-alive-reuse-min <n>]
  fretboard diag matrix ui-gallery [--dir <dir>] [--timeout-ms <ms>] [--poll-ms <ms>] [--warmup-frames <n>] [--compare-eps-px <px>] [--compare-ignore-bounds] [--compare-ignore-scene-fingerprint] [--check-view-cache-reuse-min <n>] [--check-view-cache-reuse-stable-min <n>] [--check-overlay-synthesis-min <n>] [--check-viewport-input-min <n>] [--env <KEY=VALUE>...] [--json] [--launch-high-priority] --launch -- <cmd...>
  ...
  fretboard diag perf <ui-gallery|script.json...> [--top <n>] [--sort <sort>] [--repeat <n>] [--warmup-frames <n>] [--prewarm-script <script.json>...] [--prelude-script <script.json>...] [--prelude-each-run] [--trace] [--check-perf-hints] [--check-perf-hints-min-severity <info|warn|error>] [--check-perf-hints-deny <codes>] [--perf-threshold-agg <max|p90|p95>] [--max-top-total-us <n>] [--max-top-layout-us <n>] [--max-top-solve-us <n>] [--max-frame-p95-total-us <n>] [--max-frame-p95-layout-us <n>] [--max-frame-p95-solve-us <n>] [--max-pointer-move-dispatch-us <n>] [--max-pointer-move-hit-test-us <n>] [--max-pointer-move-global-changes <n>] [--min-run-paint-cache-hit-test-only-replay-allowed-max <n>] [--max-run-paint-cache-hit-test-only-replay-rejected-key-mismatch-max <n>] [--perf-baseline <path>] [--perf-baseline-out <path>] [--perf-baseline-headroom-pct <n>] [--perf-baseline-seed-preset <path>...] [--perf-baseline-seed <scope@metric=max|p90|p95>...] [--timeout-ms <ms>] [--poll-ms <ms>] [--dir <dir>] [--json] [--check-pixels-changed <test_id>] [--check-pixels-unchanged <test_id>] [--env <KEY=VALUE>...] [--launch -- <cmd...>]
  fretboard diag perf-baseline-from-bundles <script.json> <bundle_artifact>... --perf-baseline-out <path> [--sort <sort>] [--perf-baseline-headroom-pct <n>] [--warmup-frames <n>] [--json]
...
Examples:
  ...
  fretboard diag list suites --contains ui-gallery
  fretboard diag resolve latest --dir target/fret-diag --json
  fretboard diag trace ./target/fret-diag/1234 --trace-out target/fret-diag-clap-smoke/trace.chrome.json
  fretboard diag pack --include-all --pack-schema2-only  # zips a schema2-first share artifact for the latest bundle
  fretboard diag triage ./target/fret-diag/1234 --lite --metric total  # prints a frames-index-backed triage summary
  fretboard diag hotspots ./target/fret-diag/1234 --lite --metric total --warmup-frames 4 --json
  fretboard diag bundle-v2 ./target/fret-diag/1234 --mode changed --json --out target/fret-diag-clap-smoke/bundle.schema2.changed.json
  fretboard diag ai-packet ./target/fret-diag/1234 --test-id ai-model-selector-root --packet-out target/ai.packet
  fretboard diag test-ids-index ./target/fret-diag/1234 --warmup-frames 4 --json
  fretboard diag frames-index ./target/fret-diag/1234 --warmup-frames 4 --json
  fretboard diag windows ./target/fret-diag/1234 --warmup-frames 4 --json
  fretboard diag dock-routing ./target/fret-diag/1234 --warmup-frames 4 --json
  fretboard diag dock-graph ./target/fret-diag/1234 --json
  ...
  fretboard diag suite --script-dir tools/diag-scripts/ui-gallery/data_table  # run every script under a directory (no positional suite required)
  ...
  fretboard diag stats ./target/fret-diag/1234 --check-asset-reload-epoch-min 1  # assert a bundle observed at least one shared asset reload invalidation
  fretboard diag stats --stats-lite-checks-json  # prints the stats-lite support matrix for the canonical check surface
  ...
  fretboard diag compare target/fret-diag/sessions/<session_a> target/fret-diag/sessions/<session_b> --footprint --json
  ...
  fretboard diag doctor --check target/fret-diag/sessions/<session_id> --warmup-frames 4
  fretboard diag doctor scripts --max-examples 10 --json
