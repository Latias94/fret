...
pub(super) fn todo_template_cargo_toml(package_name: &str, opts: ScaffoldOptions) -> String {
...
pub(super) fn hello_template_cargo_toml(package_name: &str, opts: ScaffoldOptions) -> String {
    let mut bootstrap_features: Vec<&str> = vec!["ui-app-driver", "diagnostics"];
    if opts.command_palette {
        bootstrap_features.push("ui-app-command-palette");
    }
