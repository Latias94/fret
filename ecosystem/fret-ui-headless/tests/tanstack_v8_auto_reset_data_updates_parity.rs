...
#[test]
fn tanstack_v8_auto_reset_data_updates_parity() {
    ...
    let fixture_path = manifest_dir
        ...
        .join("tanstack")
        .join("v8")
        .join("auto_reset_data_updates.json");
    let fixture: Fixture =
        serde_json::from_str(&std::fs::read_to_string(&fixture_path).expect("fixture file"))
            .expect("fixture json");

    assert_eq!(fixture.case_id, "auto_reset_data_updates");
    ...
    for snap in fixture.snapshots {
        ...
        // Auto reset behaviors also reset to `initialState`.
        ...
        let mut data_current = fixture.data.clone();

        // Initial render pass registers auto-reset behavior (no reset on first call).
        let mut auto_reset = TanStackAutoResetQueue::default();
        {
            let table_initial = Table::builder(&data_current)
        ...
        for action in &snap.actions {
            match action {
                FixtureAction::SetData { data } => {
                    data_current = data.clone();
                }
            }

            let table_post = Table::builder(&data_current)
                .columns(columns.clone())
                .get_row_key(|row, _idx, _parent| RowKey(row.id))
                .get_row_id(|row, _idx, _parent| RowId::new(row.id.to_string()))
                .initial_state(initial_state.clone())
        ...
        let table = Table::builder(&data_current)
            .columns(columns.clone())
            .get_row_key(|row, _idx, _parent| RowKey(row.id))
            .get_row_id(|row, _idx, _parent| RowId::new(row.id.to_string()))
            .state(state)
