...
pub(super) fn disclosure_header_element<H: UiHost>(
    ...
    cx.named("header", |cx| {
        ...
        let mut header = cx.pressable_with_id(props, move |cx, state, trigger_id| {
            ...
            let context_anchor_model = super::super::context_menu_anchor_model_for(cx, trigger_id);
            ...
            cx.pressable_on_activate(crate::on_activate(move |host, action_cx, _reason| {
                host.record_transient_event(action_cx, super::super::KEY_CLICKED);
                if has_children {
                    let _ = host
                        .models_mut()
            ...
            cx.pressable_on_pointer_up(Arc::new(move |host, acx, up| {
                if up.is_click && up.button == MouseButton::Right {
                    let _ = host
                        .update_model(&context_anchor_model, |value| *value = Some(up.position));
            ...
            trigger_response
                .set_core_clicked(cx.take_transient_for(trigger_id, super::super::KEY_CLICKED));
            trigger_response.set_secondary_clicked(
                cx.take_transient_for(trigger_id, super::super::KEY_SECONDARY_CLICKED),
            );
            trigger_response.set_double_clicked(
                cx.take_transient_for(trigger_id, super::super::KEY_DOUBLE_CLICKED),
            );
            trigger_response.set_context_menu_requested(
                cx.take_transient_for(trigger_id, super::super::KEY_CONTEXT_MENU_REQUESTED),
            );
            ...
            trigger_response.set_hover_blocked_by_active_item(
                super::super::hover_blocked_by_active_item_for(cx, trigger_id, &active_item_model),
            );
            super::super::sanitize_response_for_enabled(enabled, trigger_response);
