...
pub fn select_item_aligned_layout_from_elements<H: UiHost>(
    ...
    // Item-aligned positioning uses `offsetTop`-like measurements that must stay stable as the
    // internal viewport scrolls. Prefer layout bounds for scrolled descendants so wheel scrolling
    // cannot cause the overlay to "chase" the selected item.
    let listbox = cx
