...
fn viewport_width_at_least_init(width: Px, threshold: Px) -> bool {
    width.0 >= threshold.0
}

fn viewport_width_at_least_apply_hysteresis(
    width: Px,
    threshold: Px,
    hysteresis: ViewportQueryHysteresis,
    active: bool,
...
pub fn viewport_width_at_least<H: UiHost>(
    ...
    cx.scope(|cx| {
        ...
        cx.with_state(ViewportWidthAtLeastState::default, |st| {
            if !st.initialized {
                st.active = viewport_width_at_least_init(width, threshold);
                st.initialized = true;
            }

            st.active =
                viewport_width_at_least_apply_hysteresis(width, threshold, hysteresis, st.active);
            st.active
        })
    })
}
...
mod tests {
    ...
    fn viewport_width_at_least_hysteresis_transitions() {
        ...
        let mut active = viewport_width_at_least_init(Px(700.0), threshold);
        assert!(!active);

        active = viewport_width_at_least_apply_hysteresis(
            Px(768.0 + hysteresis.up.0 - 1.0),
            threshold,
            hysteresis,
            active,
