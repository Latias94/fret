...
#[track_caller]
pub fn drive_tween_f32_for_element<H: UiHost, K: Hash>(
    cx: &mut ElementContext<'_, H>,
    ...
    ("drive_tween_f32_for_element").hash(&mut hasher);
    key.hash(&mut hasher);
    ...
    let out = cx.with_state_for(element, TweenF32StateMap::default, |map| {
        let st = map
            .entries
            .entry(entry_key)
            .or_insert_with(TweenF32State::default);
        ...
        if st.animating && st.last_frame_id != frame_id {
            ...
            let value = tween_value_at(st.start, st.target, st.duration, st.ease, st.elapsed);
            let velocity = tween_velocity_at(st.start, st.target, st.duration, st.ease, st.elapsed);
            st.value = value;
            st.velocity = velocity;
...
mod tests {
    ...
    #[test]
    fn tween_for_element_advances_without_snapping_on_retarget() {
        let window = AppWindowId::default();
        let mut app = App::new();
        ...
        app.set_tick_id(TickId(1));
        app.set_frame_id(FrameId(2));
        let anchor = with_element_cx(&mut app, window, bounds(), "tween_for_element", |cx| {
            cx.keyed("anchor", |cx| cx.root_id())
        });

        let _ = with_element_cx(&mut app, window, bounds(), "tween_for_element", |cx| {
        ...
        let out = with_element_cx(&mut app, window, bounds(), "tween_for_element", |cx| {
            drive_tween_f32_for_element(
                cx,
                anchor,
                "value",
        ...
        assert!(
            ...
            "expected tween to advance but not snap; got value={}",
            out.value
        );
    ...
    #[test]
    fn tween_for_element_respects_cubic_bezier_ease() {
        let window = AppWindowId::default();
        let mut app = App::new();
