//! Material 3 filter chip (MVP).
//!
//! Outcome-oriented implementation:
//! - Token-driven shape/colors via `md.comp.filter-chip.*` (v30 sassvars subset).
...
impl FilterChip {
    ...
    pub fn style(mut self, style: FilterChipStyle) -> Self {
...
mod tests {
    ...
    #[test]
    fn filter_chip_labels_can_shrink_between_icon_slots() {
        let window = fret_core::AppWindowId::default();
        let mut app = App::new();
        let label = Arc::<str>::from(
            "A very long filter chip label that should shrink between the leading and trailing icons",
        );
        ...
        let el = fret_ui::elements::with_element_cx(
            ...
            "m3-filter-chip",
            |cx| {
                FilterChip::new(selected.clone(), label.clone())
        ...
        let label = find_text_by_content(&el, label.as_ref()).expect("filter chip label text");
