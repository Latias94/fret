...
mod tests {
    ...
    #[test]
    fn assist_chip_labels_can_shrink_beside_leading_icon() {
        let window = fret_core::AppWindowId::default();
        let mut app = App::new();
        let label = Arc::<str>::from(
            "A very long assist chip label that should shrink beside the leading icon",
        );
        ...
        let el = fret_ui::elements::with_element_cx(
            ...
            "m3-assist-chip",
            |cx| {
                AssistChip::new(label.clone())
                    .leading_icon(ids::ui::SEARCH)
                    .into_element(cx)
            },
        );

        let label = find_text_by_content(&el, label.as_ref()).expect("assist chip label text");
