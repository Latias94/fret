...
mod tests {
    ...
    #[test]
    fn navigation_rail_labels_can_shrink_within_item_slots() {
        let window = fret_core::AppWindowId::default();
        let mut app = App::new();
        let selected = Arc::<str>::from("inbox");
        let label = Arc::<str>::from(
            "A very long navigation rail label that should shrink within the item slot",
        );
        ...
        let el = fret_ui::elements::with_element_cx(
            ...
            "m3-navigation-rail",
            |cx| {
                NavigationRail::new(model.clone())
                    .items(vec![
                        NavigationRailItem::new(selected.clone(), label.clone(), ids::ui::SEARCH)
                            .badge_text("99+"),
                        NavigationRailItem::new("settings", "Settings", ids::ui::SEARCH),
                    ])
                    .into_element(cx)
            },
        );

        let label = find_text_by_content(&el, label.as_ref()).expect("navigation rail label text");
...
mod controllable_state_tests {
    ...
    #[test]
    fn navigation_rail_new_controllable_uses_controlled_value_when_provided() {
        ...
        with_element_cx(
            ...
            window,
            bounds(),
            "m3-navigation-rail-controlled",
            |cx| {
                let rail = NavigationRail::new_controllable(cx, Some(controlled.clone()), "search");
                assert_eq!(rail.value_model(), controlled);
            },
        );
    }

    #[test]
    fn navigation_rail_new_controllable_applies_default_value() {
        let window = AppWindowId::default();
        let mut app = App::new();
        ...
        with_element_cx(
            ...
            window,
            bounds(),
            "m3-navigation-rail-default",
            |cx| {
                let rail = NavigationRail::new_controllable(cx, None, "search");
                let value = cx
                    .watch_model(&rail.value_model())
                    .layout()
                    .cloned()
    ...
    #[test]
    fn navigation_rail_uncontrolled_multiple_instances_do_not_share_models() {
        let window = AppWindowId::default();
        let mut app = App::new();
        ...
        with_element_cx(
            ...
            window,
            bounds(),
            "m3-navigation-rail-uncontrolled",
            |cx| {
                let a = NavigationRail::uncontrolled(cx, "search");
                let b = NavigationRail::uncontrolled(cx, "settings");
