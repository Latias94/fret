        ...
        cx.scope(|cx| {
            cx.pressable_with_id_props(|cx, st, pressable_id| {
                ...
                cx.pressable_add_on_activate(Arc::new(move |host, action_cx, reason| {
                    ...
                    {
                        crate::foundation::action::dispatch_action(host, action_cx, reason, action);
                    }
...
mod tests {
    ...
    #[test]
    fn input_chip_labels_can_shrink_between_icon_slots() {
        let window = fret_core::AppWindowId::default();
        let mut app = App::new();
        let label = Arc::<str>::from(
            "A very long input chip label that should shrink between the leading and trailing icons",
        );
        let selected = app.models_mut().insert(false);

        let el =
            fret_ui::elements::with_element_cx(&mut app, window, bounds(), "m3-input-chip", |cx| {
                InputChip::new(selected.clone(), label.clone())
                    .leading_icon(ids::ui::SEARCH)
                    .trailing_icon(ids::ui::CLOSE)
                    .into_element(cx)
            });

        let label = find_text_by_content(&el, label.as_ref()).expect("input chip label text");
