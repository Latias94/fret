...
fn assert_semantic_touch_target(
    ...
    assert!(
        ...
        "{test_id}: expected width >= {min_width}, got {bounds:?}"
    );
    assert!(
        bounds.size.height.0 >= min_height,
        "{test_id}: expected height >= {min_height}, got {bounds:?}"
    );
}
...
fn navigable_material_rows_enforce_minimum_touch_target_at_runtime() {
    ...
    let tabs_selected = app.models_mut().insert(Arc::<str>::from("a"));
    render_one(&mut ui, &mut app, &mut services, window, |cx| {
        Tabs::new(tabs_selected)
    ...
    assert_semantic_touch_target(&ui, "m3-touch-tab-alpha", min, min);

    let nav_bar_selected = app.models_mut().insert(Arc::<str>::from("search"));
    render_one(&mut ui, &mut app, &mut services, window, |cx| {
        NavigationBar::new(nav_bar_selected)
    ...
    assert_semantic_touch_target(&ui, "m3-touch-nav-bar-search", min, min);

    let nav_rail_selected = app.models_mut().insert(Arc::<str>::from("search"));
    render_one(&mut ui, &mut app, &mut services, window, |cx| {
        NavigationRail::new(nav_rail_selected)
            ...
            .items(vec![
                ...
                NavigationRailItem::new("settings", "Settings", fret_icons::ids::ui::SETTINGS)
                    .test_id("m3-touch-nav-rail-settings"),
            ])
            .into_element(cx)
    });
    assert_semantic_touch_target(&ui, "m3-touch-nav-rail-search", min, min);

    let drawer_selected = app.models_mut().insert(Arc::<str>::from("inbox"));
    render_one(&mut ui, &mut app, &mut services, window, |cx| {
        NavigationDrawer::new(drawer_selected)
    ...
    assert_semantic_touch_target(&ui, "m3-touch-drawer-inbox", min, min);
    ...
    assert_semantic_touch_target(&ui, "m3-touch-menu-alpha", min, min);

    let list_selected = app.models_mut().insert(Arc::<str>::from("alpha"));
    render_one(&mut ui, &mut app, &mut services, window, |cx| {
    ...
    assert_semantic_touch_target(&ui, "m3-touch-list-alpha", min, min);
}
