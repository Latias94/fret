...
pub enum SceneSig {
    ...
    PushCompositeGroup,
    PopCompositeGroup,
    StrokeRRect(DrawOrder),
    ShadowRRect(DrawOrder),
    Quad(DrawOrder),
    VertexColorQuad(DrawOrder),
    ImageQuad(DrawOrder),
    VertexColorTriangle(DrawOrder),
    ImageTriangle(DrawOrder),
    Image(DrawOrder),
...
pub fn scene_signature(scene: &Scene) -> Vec<SceneSig> {
    scene
        ...
        .map(|op| match *op {
            ...
            SceneOp::PushClipPath { .. } => SceneSig::PushClipPath,
            ...
            SceneOp::PushBackdropSourceGroupV1 { .. } => SceneSig::PushBackdropSourceGroup,
            SceneOp::PopBackdropSourceGroup => SceneSig::PopBackdropSourceGroup,
            SceneOp::PushCompositeGroup { .. } => SceneSig::PushCompositeGroup,
            SceneOp::PopCompositeGroup => SceneSig::PopCompositeGroup,
            SceneOp::StrokeRRect { order, .. } => SceneSig::StrokeRRect(order),
            SceneOp::ShadowRRect { order, .. } => SceneSig::ShadowRRect(order),
            SceneOp::Quad { order, .. } => SceneSig::Quad(order),
            SceneOp::VertexColorQuad { order, .. } => SceneSig::VertexColorQuad(order),
            SceneOp::ImageQuad { order, .. } => SceneSig::ImageQuad(order),
            SceneOp::VertexColorTriangle { order, .. } => SceneSig::VertexColorTriangle(order),
            SceneOp::ImageTriangle { order, .. } => SceneSig::ImageTriangle(order),
    ...
    scene
        ...
        .filter_map(|op| match *op {
            SceneOp::Quad {
                ...
                order,
                rect: rect_sig(rect),
                background: paint_sig(background.paint),
