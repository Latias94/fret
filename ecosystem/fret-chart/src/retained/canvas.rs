...
use crate::slider_logic::{
    ...
    slider_drag_update_at_x, slider_drag_update_at_y, slider_norm, slider_window_after_delta,
};
...
impl ChartCanvas {
    pub fn new(spec: delinea::ChartSpec) -> Result<Self, ModelError> {
        ...
        Ok(Self::new_with_engine(ChartEngine::new(spec)?))
    }

    fn new_with_engine(engine: ChartEngine) -> Self {
        Self {
            engine,
    ...
    fn compute_layout(&self, bounds: Rect) -> ChartLayout {
        self.with_engine(|engine| {
            ...
            let active_grid = model
                .series_in_order()
                .find(|s| s.visible)
                .and_then(|primary| model.axes.get(&primary.x_axis).map(|a| a.grid));
    ...
    fn draw_legend<H: UiHost>(&mut self, cx: &mut PaintCx<'_, H>) {
        ...
        let series: Vec<delinea::engine::model::SeriesModel> = self.with_engine(|engine| {
            let model = engine.model();
            model.series_in_order().cloned().collect()
        });
        if series.is_empty() {
            return;
        }
        ...
        cx.scene.push(SceneOp::Quad {
            order: legend_order,
            rect: legend_rect,
            background: fret_core::Paint::Solid(self.style.legend_background).into(),

            border: Edges::all(self.style.legend_border_width),
            border_paint: fret_core::Paint::Solid(self.style.legend_border_color).into(),
        ...
        for (action, blob, metrics) in selector_blobs.into_iter() {
            ...
            if self.legend_selector_hover == Some(action) {
                cx.scene.push(SceneOp::Quad {
                    ...
                    rect,
                    background: fret_core::Paint::Solid(self.style.legend_hover_background).into(),
        ...
        for (i, (series_id, blob, metrics, visible)) in blobs.into_iter().enumerate() {
            ...
            cx.scene.push(SceneOp::Quad {
                ...
                background: fret_core::Paint::Solid(swatch).into(),
    ...
    fn draw_visual_map<H: UiHost>(&mut self, cx: &mut PaintCx<'_, H>) {
        ...
        for (i, layout) in tracks.into_iter().enumerate() {
            ...
            cx.scene.push(SceneOp::Quad {
                order,
                rect: track,
                background: fret_core::Paint::Solid(self.style.visual_map_track_color).into(),
    ...
    fn draw_axes<H: UiHost>(&mut self, cx: &mut PaintCx<'_, H>) {
        ...
        for (band, window) in &x_bands {
            ...
            cx.scene.push(SceneOp::Quad {
                ...
                background: fret_core::Paint::Solid(self.style.axis_line_color).into(),
                ...
                Self::axis_ticks_with_labels(&model, band.axis, *window, x_tick_count)
            {
                ...
                cx.scene.push(SceneOp::Quad {
                    ...
                    background: fret_core::Paint::Solid(self.style.axis_tick_color).into(),
                ...
                if cx.captured == Some(cx.node) {
                        ...
                        && Self::is_button_held(MouseButton::Left, *buttons)
                    {
                        ...
                        match drag.axis_kind {
                            SliderAxisKind::X => {
                                if let Some(update) = slider_drag_update_at_x(
                                    track,
                                    extent,
                                    drag.start_window,
                                    drag.start_pos.x.0,
                            ...
                            SliderAxisKind::Y => {
                                if let Some(update) = slider_drag_update_at_y(
                                    track,
                                    extent,
                                    drag.start_window,
                                    drag.start_pos.y.0,
                ...
                if *button == MouseButton::Left {
                    ...
                    match region {
                        AxisRegion::XAxis(axis) => {
                            ...
                            let (locked_min, locked_max) = self.axis_constraints(axis);
                            let permissions = SliderDragPermissions {
                                ...
                                && track.contains(*position)
                            {
                                ...
                                let Some(start) = slider_drag_start_at_x(
                                    track,
                                    extent,
                                    window,
                                    position.x.0,
                        ...
                        AxisRegion::YAxis(axis) => {
                                ...
                                && track.contains(*position)
                            {
                                ...
                                let Some(start) = slider_drag_start_at_y(
                                    track,
                                    extent,
                                    window,
                                    position.y.0,
    ...
    fn paint(&mut self, cx: &mut PaintCx<'_, H>) {
        ...
        if !self.cached_rect_scene_ops.try_replay_with(
            ...
            for cached in &self.cached_rects {
                ...
                ops.push(SceneOp::Quad {
                    ...
                    rect: cached.rect,
                    background: fret_core::Paint::Solid(fill_color).into(),
        ...
        if let Some(axis_pointer) = axis_pointer.as_ref() {
            ...
            if shadow {
                ...
                cx.scene.push(SceneOp::Quad {
                    ...
                    background: fret_core::Paint::Solid(self.style.crosshair_color).into(),
        ...
        if let Some(drag) = self.box_zoom_drag {
            ...
            if rect.size.width.0 >= 1.0 && rect.size.height.0 >= 1.0 {
                cx.scene.push(SceneOp::Quad {
                    ...
                    rect,
                    background: fret_core::Paint::Solid(self.style.selection_fill).into(),

                    border: Edges::all(self.style.selection_stroke_width),
                    border_paint: fret_core::Paint::Solid(self.style.selection_stroke).into(),
        ...
        if let Some(axis_pointer) = axis_pointer {
            ...
            cx.scene.push(SceneOp::Quad {
                ...
                background: fret_core::Paint::Solid(self.style.tooltip_background).into(),

                border: Edges::all(self.style.tooltip_border_width),
                border_paint: fret_core::Paint::Solid(self.style.tooltip_border_color).into(),
            ...
            for (i, line) in prepared_lines.into_iter().enumerate() {
                    ...
                    && let Some(series) = line.source_series
                {
                    ...
                    cx.scene.push(SceneOp::Quad {
                        ...
                        background: fret_core::Paint::Solid(self.series_color(series)).into(),
