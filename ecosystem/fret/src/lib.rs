...
pub mod component {
    ...
    pub mod ui_assets {
        ...
        pub fn svg_stats<H: fret_ui::UiHost>(
            cx: &mut fret_ui::ElementContext<'_, H>,
        ) -> SvgAssetStats {
            fret_ui_assets::ui::svg_stats_in(cx)
        }
...
mod authoring_surface_policy_tests {
    ...
    fn component_ui_assets_facade_keeps_snippet_assets_off_raw_extension_traits() {
        ...
        assert!(LIB_RS.contains("pub fn rgba8_image_state<H: fret_ui::UiHost>("));
        assert!(LIB_RS.contains("cx: &mut fret_ui::ElementContext<'_, H>"));
        assert!(LIB_RS.contains("fret_ui_assets::ui::use_image_source_state_in(cx, source)"));
        assert!(LIB_RS.contains(
            "fret_ui_assets::ui::use_image_source_state_from_asset_request_in(cx, request)"
        ));
