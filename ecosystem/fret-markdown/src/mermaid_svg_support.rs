...
fn mermaid_svg_entry<H: UiHost>(
    ...
    let handle = cx.use_query(key, policy, move |_token| {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| {
            ...
            let mut layout = merman::render::LayoutOptions::default();
            layout.text_measurer =
                Arc::new(merman::render::VendoredFontMetricsTextMeasurer::default());
            ...
            .map_err(|e| QueryError::permanent(format!("merman error: {e}")))?
            .ok_or_else(|| QueryError::permanent("merman: no diagram detected"))?;
    ...
    while let Some(lt_rel) = svg[i..].find('<') {
        ...
        if tag.starts_with("<foreignObject") {
            ...
            if width > 0.0 && height > 0.0 {
                ...
                if !lines.is_empty() {
                    overlays.push_str(r#"<g data-fret-markdown-foreignobject="fallback">"#);
