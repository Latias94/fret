...
#[test]
fn field_label_click_focuses_textarea_control() {
    ...
    let control_id = fret_ui_kit::primitives::control_registry::ControlId::from("textarea.message");
    ...
    let label_node = snap
        .nodes
        .iter()
        .find(|n| n.test_id.as_deref() == Some("textarea.label"))
        .map(|n| n.id)
        .expect("label node");
    let column_node = ui.children(root)[0];
    let control_host_node = ui.children(column_node)[1];
    let expected_focus = ui
        .first_focusable_descendant_including_declarative(&mut app, window, control_host_node)
        .expect("focusable textarea descendant");

    let label_center = center_of(ui.debug_node_bounds(label_node).expect("label bounds"));

    ui.dispatch_event(
        &mut app,
        &mut services,
        &fret_core::Event::Pointer(fret_core::PointerEvent::Down {
            pointer_id: fret_core::PointerId(9),
    ...
    assert_eq!(ui.focus(), Some(expected_focus));
}
