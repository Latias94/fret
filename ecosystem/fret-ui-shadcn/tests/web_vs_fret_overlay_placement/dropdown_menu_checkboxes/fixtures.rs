...
enum DropdownMenuCheckboxesRecipe {
    OverlayPlacement,
...
#[derive(Debug, Clone, Deserialize)]
struct DropdownMenuCheckboxesCase {
    id: String,
    web_name: String,
...
fn assert_dropdown_menu_checkboxes_menu_content_insets_match(web_name: &str) {
...
fn assert_dropdown_menu_checkboxes_menu_item_height_matches(web_name: &str) {
    let web = read_web_golden_open(web_name);
    let theme = web_theme(&web);
...
#[test]
fn web_vs_fret_dropdown_menu_checkboxes_cases_match_web_fixtures() {
    let raw = include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/tests/fixtures/overlay_placement_dropdown_menu_checkboxes_cases_v1.json"
    ));
    let suite: FixtureSuite<DropdownMenuCheckboxesCase> =
        serde_json::from_str(raw).expect("dropdown-menu-checkboxes fixture parse");
    assert_eq!(suite.schema_version, 1);
    assert!(!suite.cases.is_empty());

    for case in suite.cases {
        eprintln!("dropdown-menu-checkboxes case={}", case.id);
        match case.recipe {
            DropdownMenuCheckboxesRecipe::OverlayPlacement => {
                assert_overlay_placement_matches(
                    &case.web_name,
                    Some("menu"),
                    |cx, open| build_dropdown_menu_checkboxes_demo_overlay(cx, open),
                    SemanticsRole::Button,
                    Some("Open"),
                    SemanticsRole::Menu,
                );
            }
            DropdownMenuCheckboxesRecipe::CheckboxIndicatorSlotInset => {
                assert_dropdown_menu_checkboxes_indicator_slot_inset_matches_web_impl(
                    &case.web_name,
                );
            }
            DropdownMenuCheckboxesRecipe::MenuContentInsets => {
                assert_dropdown_menu_checkboxes_menu_content_insets_match(&case.web_name);
            }
            DropdownMenuCheckboxesRecipe::MenuItemHeight => {
                assert_dropdown_menu_checkboxes_menu_item_height_matches(&case.web_name);
            }
        }
    }
}
