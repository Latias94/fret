...
fn render_frame(
    ...
    let root = fret_ui::declarative::render_root(
        ...
        window,
        bounds,
        "select-escape-dismiss-focus-restore",
        root,
    );
    ui.set_root(root);
...
#[test]
fn select_escape_closes_and_restores_focus_to_trigger() {
    ...
    assert_eq!(
        app.models().get_copied(&open),
        Some(false),
        "expected select to close after Escape"
    );
    ...
    assert!(
        ...
        "expected focus to restore to the select trigger after Escape"
    );
}
