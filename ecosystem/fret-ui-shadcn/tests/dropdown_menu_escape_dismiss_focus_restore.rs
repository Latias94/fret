...
fn render_frame(
    ...
    let root = fret_ui::declarative::render_root(
        ...
        window,
        bounds,
        "dropdown-menu-escape-dismiss-focus-restore",
        root,
    );
    ui.set_root(root);
...
#[test]
fn dropdown_menu_escape_closes_and_restores_focus_to_trigger() {
    ...
    let trigger = find_by_test_id(&snap, "menu-trigger");
    click_at(
        &mut ui,
        &mut app,
        &mut services,
    ...
    assert_eq!(
        app.models().get_copied(&open),
        Some(false),
        "expected dropdown-menu to close after Escape"
    );
