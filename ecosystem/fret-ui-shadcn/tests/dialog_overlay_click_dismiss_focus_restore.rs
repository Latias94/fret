...
fn render_frame(
    ...
    let root = fret_ui::declarative::render_root(
        ...
        window,
        bounds,
        "dialog-overlay-click-dismiss-focus-restore",
        root,
    );
    ui.set_root(root);
...
#[test]
fn dialog_overlay_click_closes_and_restores_focus_to_trigger() {
    ...
    // Frame 2+: settle open overlays before clicking the overlay.
    ...
    // Click outside the centered dialog content (top-left corner).
    click_at(
        &mut ui,
        &mut app,
        &mut services,
    ...
    assert_eq!(
        app.models().get_copied(&open),
        Some(false),
        "expected dialog to close after overlay click"
    );
    ...
    assert!(
        ...
        "expected focus to restore to the trigger after overlay click"
    );
}
