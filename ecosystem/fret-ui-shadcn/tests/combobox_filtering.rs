...
mod input_events;
use input_events::{click_at, dispatch_key_press, dispatch_text_input};
...
#[test]
fn combobox_filter_text_then_enter_selects_first_match() {
    ...
    // Frame 2+: settle open overlays, then focus the search input.
    ...
    let snap = ui
        ...
        .cloned()
        .expect("expected semantics snapshot");
    let input = find_by_test_id(&snap, "combobox-test-input");
    click_at(
        &mut ui,
        &mut app,
        &mut services,
        Point::new(
            Px(input.bounds.origin.x.0 + 5.0),
            Px(input.bounds.origin.y.0 + 5.0),
        ),
    );
    timers.ingest_effects(&mut app);
    timers.fire_all(&mut ui, &mut app, &mut services);

    dispatch_text_input(&mut ui, &mut app, &mut services, "nu");
    timers.ingest_effects(&mut app);
    timers.fire_all(&mut ui, &mut app, &mut services);

    // Render a frame so filtering/highlight settles before accepting.
    let value_after_text = value.clone();
    let open_after_text = open.clone();
    render_frame(
        &mut ui,
        &mut app,
        &mut services,
    ...
    assert_eq!(
        ...
        Some("nuxt"),
        "expected filtering to highlight nuxt and Enter to select it"
    );
    assert_eq!(
        app.models().get_copied(&open),
        Some(false),
