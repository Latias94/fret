...
mod tests {
    ...
    fn dropdown_menu_item_select_closes_and_restores_trigger_focus() {
        ...
        // Open via keyboard and select first item with Enter.
        ui.dispatch_event(
            &mut app,
            &mut services,
            &Event::KeyDown {
    ...
    fn dropdown_menu_outside_press_closes_without_overriding_underlay_focus() {
        ...
        let underlay_bounds = ui.debug_node_bounds(underlay_node).expect("underlay bounds");
        assert!(
        ...
        assert!(
            hit_path.as_ref().is_some_and(|path| path.contains(&underlay_node)),
