...
mod tests {
    ...
    #[test]
    fn checkbox_aria_invalid_exposes_invalid_semantics() {
        let window = AppWindowId::default();
        let mut app = App::new();
        let mut ui: UiTree<App> = UiTree::new();
        ui.set_window(window);
        ...
        let root = fret_ui::declarative::render_root(
            ...
            window,
            bounds,
            "shadcn-checkbox-invalid-semantics",
            |cx| {
                vec![
                    Checkbox::from_checked(true)
                        .aria_invalid(true)
                        .a11y_label("Terms")
                        .test_id("invalid-checkbox")
                        .into_element(cx),
                ]
            },
        );
        ...
        let node = snap
            .nodes
            .iter()
            .find(|n| n.test_id.as_deref() == Some("invalid-checkbox"))
            .expect("checkbox semantics node");
        assert_eq!(node.role, fret_core::SemanticsRole::Checkbox);
