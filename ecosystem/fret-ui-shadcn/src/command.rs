...
impl CommandPalette {
    pub fn new(model: Model<String>, items: impl IntoIterator<Item = CommandItem>) -> Self {
        Self {
            ...
            should_filter: true,
            filter: None,
            // cmdk default: selection is internal and starts at the first enabled item.
            value: None,
            default_value: None,
    ...
    ///
    /// In cmdk, the input keeps focus while the listbox selection is tracked independently.
    /// Exposing this value as a `Model<Option<Arc<str>>>` makes that selection controllable and
    /// observable by recipes (Radix-aligned `useControllableState` outcome).
    ...
    pub fn into_element<H: UiHost>(self, cx: &mut ElementContext<'_, H>) -> AnyElement {
        ...
        cx.scope(|cx| {
            ...
            let active =
                controllable_state::use_controllable_model(cx, value, || default_value).model();
...
mod tests {
    ...
    fn render_frame_with_value(
        ...
        let root =
            fret_ui::declarative::render_root(ui, app, services, window, bounds, "cmdk", |cx| {
                vec![
                    CommandPalette::new(query, items)
                        .value(Some(value))
                        .into_element(cx),
                ]
            });
    ...
    fn cmdk_value_model_respects_controlled_selection() {
        ...
        let query = app.models_mut().insert(String::new());
        let value = app.models_mut().insert(Some(Arc::<str>::from("Beta")));
    ...
    fn cmdk_value_model_updates_on_arrow_navigation() {
        ...
        let query = app.models_mut().insert(String::new());
        let value = app.models_mut().insert(Some(Arc::<str>::from("Alpha")));
        ...
        let selected = app.models().get_cloned(&value).unwrap_or(None);
