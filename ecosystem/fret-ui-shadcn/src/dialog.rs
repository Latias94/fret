...
mod tests {
    ...
    #[test]
    fn dialog_description_children_preserve_shared_text_role_contracts() {
        let window = AppWindowId::default();
        let mut app = App::new();
        let bounds = Rect::new(
            Point::new(Px(0.0), Px(0.0)),
        ...
        let element = fret_ui::elements::with_element_cx(&mut app, window, bounds, "test", |cx| {
            DialogDescription::new_children([decl_text::text_compact_paragraph(
                cx,
        ...
        let text = find_text_element(&element, "Description body copy")
            .expect("expected DialogDescription role child text");
        let ElementKind::Text(props) = &text.kind else {
            panic!("expected text leaf");
        };
        assert!(props.style.is_none());
