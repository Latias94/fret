        ...
        move |cx| {
            ...
            if !show_resize_handle {
                ...
                let textarea = cx.text_area_with_id_props(move |cx, id| {
                    ...
                    // Text-entry controls need an active editing affordance on pointer focus as
                    // well as keyboard focus, so keep the shadcn border/ring transition keyed to
                    // focus rather than the window-scoped `focus-visible` heuristic alone.
...
mod tests {
    ...
    use fret_ui::element::{ElementKind, Length, TextAreaProps};
    ...
    fn textarea_focus_ring_tweens_in_and_out_like_a_transition() {
        ...
        assert!(
            always_paint_focus,
            "expected focused textarea to keep painting the ring while active"
        );
    ...
    fn find_text_area_props(el: &AnyElement) -> Option<&TextAreaProps> {
        if let ElementKind::TextArea(props) = &el.kind {
            return Some(props);
        }
        for child in &el.children {
            if let Some(found) = find_text_area_props(child) {
                return Some(found);
            }
        }
        None
    ...
    fn textarea_required_builder_sets_textarea_required_semantics() {
        ...
        let model = app.models_mut().insert(String::new());
        let el = elements::with_element_cx(&mut app, window, bounds, "textarea-required", |cx| {
            Textarea::new(model.clone())
                .a11y_label("Notes")
                .required(true)
                .resizable(false)
