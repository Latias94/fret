...
use fret_core::{
    AppWindowId, Edges, NodeId, PanelKey, Rect, SemanticsRole, Size, TextConstraints, TextOverflow,
    TextStyle, TextWrap,
};
use fret_ui::element::{AnyElement, ContainerProps, LayoutStyle, Length, ManagedSurfaceProps};
...
use super::types::{
    ...
    DockTabsDragPayload, HoverTarget, PreparedTabTitle,
};
use super::viewport::{
...
#[derive(Debug, Clone)]
struct DeclarativePressedTabClose {
...
#[derive(Debug, Clone)]
struct DeclarativePressedFloatingClose {
    pointer_id: fret_core::PointerId,
    floating: fret_core::DockNodeId,
}

#[derive(Debug, Clone)]
struct DeclarativeFloatingDrag {
    pointer_id: fret_core::PointerId,
    floating: fret_core::DockNodeId,
    grab_offset: fret_core::Point,
    start_rect: Rect,
    start: fret_core::Point,
...
#[derive(Debug, Clone)]
struct DeclarativeDividerDrag {
    pointer_id: fret_core::PointerId,
    handle: DividerDragState,
    min_px: Vec<fret_core::Px>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
struct DeclarativeFloatingHover {
    close: Option<fret_core::DockNodeId>,
    title_bar: Option<fret_core::DockNodeId>,
}

#[derive(Debug, Clone)]
struct DeclarativePendingDockDrag {
    pointer_id: fret_core::PointerId,
    start: fret_core::Point,
    panel: PanelKey,
    grab_offset: fret_core::Point,
...
#[derive(Debug, Clone)]
struct DeclarativePendingDockTabsDrag {
    pointer_id: fret_core::PointerId,
    start: fret_core::Point,
    tabs: fret_core::DockNodeId,
    grab_offset: fret_core::Point,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
struct DeclarativeTabHover {
    tab: Option<(fret_core::DockNodeId, usize)>,
    tab_close: bool,
    overflow_button: Option<fret_core::DockNodeId>,
}

#[derive(Default)]
struct DeclarativeDockInteractionService {
...
impl DeclarativeDockInteractionService {
    fn begin_tab_close(&mut self, window: AppWindowId, pressed: DeclarativePressedTabClose) {
    ...
    fn pressed_floating_close(&self, window: AppWindowId) -> Option<fret_core::DockNodeId> {
        self.pressed_floating_close
            .get(&window)
            .map(|pressed| pressed.floating)
    }

    fn begin_floating_drag(&mut self, window: AppWindowId, drag: DeclarativeFloatingDrag) {
    ...
    fn begin_divider_drag(&mut self, window: AppWindowId, drag: DeclarativeDividerDrag) {
        self.divider_drag
            .entry(window)
            .or_default()
    ...
    fn begin_viewport_capture(&mut self, window: AppWindowId, capture: ViewportCaptureState) {
    ...
    fn has_viewport_capture_for_window(&self, window: AppWindowId) -> bool {
        self.viewport_capture
            .get(&window)
            .is_some_and(|captures| !captures.is_empty())
    }

    fn tab_overflow_menu(&self, window: AppWindowId) -> Option<TabOverflowMenuState> {
        self.tab_overflow_menu.get(&window).cloned()
    }

    fn set_tab_overflow_menu(&mut self, window: AppWindowId, menu: Option<TabOverflowMenuState>) {
        match menu {
            Some(menu) => {
    ...
    fn tab_scroll_for(&self, window: AppWindowId) -> HashMap<fret_core::DockNodeId, fret_core::Px> {
    ...
    fn retain_tab_scroll_for_window(
        &mut self,
        window: AppWindowId,
    ...
    fn should_auto_scroll_tab_drag(
        &mut self,
        window: AppWindowId,
        tabs: fret_core::DockNodeId,
    ...
    fn tab_hover(&self, window: AppWindowId) -> DeclarativeTabHover {
        self.tab_hover.get(&window).copied().unwrap_or_default()
    }

    fn set_tab_hover(&mut self, window: AppWindowId, hover: DeclarativeTabHover) -> bool {
    ...
    fn floating_hover(&self, window: AppWindowId) -> DeclarativeFloatingHover {
        self.floating_hover
            .get(&window)
            .copied()
            .unwrap_or_default()
    ...
    fn set_floating_hover(&mut self, window: AppWindowId, hover: DeclarativeFloatingHover) -> bool {
...
fn collect_panels_for_window<H: UiHost>(app: &H, window: AppWindowId) -> Vec<(PanelKey, bool)> {
...
fn missing_panel_element<H: UiHost>(
    cx: &mut ElementContext<'_, H>,
    panel: &PanelKey,
) -> AnyElement {
...
fn bind_panel_children<H: UiHost>(
    app: &mut H,
    window: AppWindowId,
    panels: &[PanelKey],
    children: &[NodeId],
...
fn panel_nodes_for_window<H: UiHost>(app: &H, window: AppWindowId) -> HashMap<PanelKey, NodeId> {
...
fn prepare_declarative_tab_title(
    ...
    let reserve =
        fret_core::Px(super::consts::DOCK_TAB_CLOSE_SIZE.0 + super::consts::DOCK_TAB_CLOSE_GAP.0);
    let inner_max_w =
        fret_core::Px((super::consts::DOCK_TAB_MAX_W.0 - pad_x.0 * 2.0 - reserve.0).max(1.0));
    let style = TextStyle {
...
fn declarative_tab_detail_titles<H: UiHost>(
    app: &H,
...
fn prepare_declarative_tab_detail_paint(
...
fn declarative_tab_widths_for_layout<H: UiHost>(
    app: &H,
    window: AppWindowId,
...
fn declarative_tab_widths_from_prepared_titles<H: UiHost>(
    app: &H,
    theme: fret_ui::ThemeSnapshot,
    frame: &DockSpaceElementFrame,
...
fn declarative_tab_bar_geometry(
    theme: fret_ui::ThemeSnapshot,
    tab_widths: &HashMap<fret_core::DockNodeId, Arc<[fret_core::Px]>>,
    tabs: fret_core::DockNodeId,
    tab_bar: Rect,
    tab_count: usize,
) -> (super::tab_bar_geometry::TabBarGeometry, bool) {
    let strip_candidate = super::tab_overflow::tab_strip_rect_with_overflow_button(theme, tab_bar);
    ...
    let geom = tab_widths
        .get(&tabs)
        .filter(|w| w.len() == tab_count)
        .map(|w| super::tab_bar_geometry::TabBarGeometry::variable(tab_bar, w.clone()))
        .unwrap_or_else(|| super::tab_bar_geometry::TabBarGeometry::fixed(tab_bar, tab_count));
    (geom, false)
}

fn declarative_clamp_and_ensure_active_visible(
...
fn declarative_tab_scroll_for_frame<H: UiHost>(
    app: &H,
    window: AppWindowId,
    theme: fret_ui::ThemeSnapshot,
    layout_all: &HashMap<fret_core::DockNodeId, Rect>,
...
fn declarative_sync_tab_scroll_for_window<H: UiHost>(
    app: &mut H,
    window: AppWindowId,
    tab_scroll: &HashMap<fret_core::DockNodeId, fret_core::Px>,
