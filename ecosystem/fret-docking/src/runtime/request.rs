...
pub(super) fn handle_request_float_panel_to_new_window<H: UiHost>(
    app: &mut H,
    source_window: AppWindowId,
    panel: PanelKey,
    anchor: Option<WindowAnchor>,
) -> bool {
    handle_request_float_panel_to_new_window_with_dispatch(
        app,
        source_window,
        panel,
        anchor,
...
pub(super) fn queue_request_float_panel_to_new_window<H: UiHost>(
    app: &mut H,
    source_window: AppWindowId,
    panel: PanelKey,
    anchor: Option<WindowAnchor>,
...
pub(super) fn handle_request_float_tabs_to_new_window<H: UiHost>(
    app: &mut H,
    source_window: AppWindowId,
    ...
    handle_request_float_tabs_to_new_window_with_dispatch(
        app,
        source_window,
        source_tabs,
        panel,
...
pub(super) fn queue_request_float_tabs_to_new_window<H: UiHost>(
    app: &mut H,
    source_window: AppWindowId,
    source_tabs: DockNodeId,
    panel: PanelKey,
...
fn handle_request_float_panel_to_new_window_with_dispatch<H: UiHost>(
    app: &mut H,
    source_window: AppWindowId,
    panel: PanelKey,
    anchor: Option<WindowAnchor>,
...
fn handle_request_float_tabs_to_new_window_with_dispatch<H: UiHost>(
    app: &mut H,
    source_window: AppWindowId,
    source_tabs: DockNodeId,
    panel: PanelKey,
    ...
    let pointer_id = pointer_id.or_else(|| {
        app.find_drag_pointer_id(|d| {
            d.kind == fret_runtime::DRAG_KIND_DOCK_TABS
                && d.payload::<DockTabsDragPayload>()
                    .is_some_and(|p| p.source_tabs == source_tabs && p.tabs.contains(&panel))
        })
    });
    let should_emit = app.with_global_mut(DockTearOffMachine::default, |machine, _app| {
        machine.register_request(
