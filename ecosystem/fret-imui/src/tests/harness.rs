...
pub(super) use fret_ui::action::{DismissReason, DismissRequestCx, OnDismissRequest};
pub(super) use fret_ui::declarative::render_root;
pub(super) use fret_ui::element::Length;
pub(super) use fret_ui::tree::PointerOcclusion;
pub(super) use fret_ui::{ElementContext, GlobalElementId, UiTree};
pub(super) use fret_ui_kit::OverlayController;
pub(super) use fret_ui_kit::imui::UiWriterImUiFacadeExt;
pub(super) use fret_ui_kit::imui::{
...
pub(super) use fret_ui_kit::{OverlayPresence, OverlayRequest};

#[derive(Default)]
pub(super) struct FakeTextService {
...
pub(super) struct TestHost {
    ...
    pub(super) drags: HashMap<PointerId, DragSession>,
    pub(super) next_drag_session_id: u64,
    pub(super) tick_id: TickId,
    pub(super) frame_id: FrameId,
    pub(super) next_timer_token: u64,
    pub(super) next_clipboard_token: u64,
    pub(super) next_share_sheet_token: u64,
...
pub(super) fn advance_and_run_frame<R>(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    window: AppWindowId,
...
pub(super) fn render_imui_disabled_scope_overlay_scene(
...
pub(super) fn render_imui_disabled_scope_tooltip_hover_scene(
    cx: &mut ElementContext<'_, TestHost>,
...
pub(super) fn render_imui_popup_modal_barrier_hover_scene(
    cx: &mut ElementContext<'_, TestHost>,
    popup_id: &'static str,
    open_popup: bool,
...
pub(super) fn render_imui_shared_hover_delay_scene(
    cx: &mut ElementContext<'_, TestHost>,
...
pub(super) fn render_imui_active_item_blocks_hover_scene(
    cx: &mut ElementContext<'_, TestHost>,
...
pub(super) fn click_at_with_modifiers(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    at: Point,
...
pub(super) fn key_down_with_repeat(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    key: KeyCode,
...
pub(super) fn ctrl_modifiers() -> Modifiers {
    Modifiers {
        ctrl: true,
        ..Default::default()
    }
...
pub(super) fn ctrl_shortcut(key: KeyCode) -> KeyChord {
    KeyChord::new(key, ctrl_modifiers())
}

pub(super) fn key_down_ctrl(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    key: KeyCode,
...
pub(super) fn key_down_ctrl_repeat(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    key: KeyCode,
...
pub(super) fn pointer_up_at_with_is_click(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    at: Point,
...
pub(super) fn dispatch_all_timers(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
) -> usize {
...
pub(super) fn pending_nonrepeating_timer_tokens(app: &TestHost) -> Vec<TimerToken> {
    let mut pending: Vec<TimerToken> = Vec::new();
    for effect in &app.effects {
        if let Effect::SetTimer { token, repeat, .. } = effect
            && repeat.is_none()
...
pub(super) fn dispatch_timer_tokens(
    ui: &mut UiTree<TestHost>,
    app: &mut TestHost,
    services: &mut FakeTextService,
    tokens: &[TimerToken],
...
pub(super) fn hover_timer_token_for(
    kind: u64,
...
pub(super) fn first_child_point(ui: &UiTree<TestHost>, root: fret_core::NodeId) -> Point {
...
pub(super) fn bounds_for_test_id(ui: &UiTree<TestHost>, test_id: &str) -> Rect {
...
pub(super) fn ui_writer_imui_facade_ext_smoke<H: fret_ui::UiHost>(
...
pub(super) fn floating_window_nodes(
    ui: &UiTree<TestHost>,
...
pub(super) enum FloatingLayerOverlayVariant {
    Menu,
    Popover,
}

pub(super) fn window_behavior_options(behavior: FloatingWindowOptions) -> WindowOptions {
    WindowOptions::default().with_behavior(behavior)
}

pub(super) fn resizable_window_options(size: Size) -> WindowOptions {
    WindowOptions::default()
        .with_size(size)
        .with_resize(FloatingWindowResizeOptions::default())
}

pub(super) fn resizable_window_options_with_behavior(
    size: Size,
...
pub(super) fn open_window_options(open: &fret_runtime::Model<bool>) -> WindowOptions {
    WindowOptions::default().with_open(open)
}

pub(super) fn open_window_options_with_behavior(
...
pub(super) fn render_floating_layer_with_overlay(
    cx: &mut ElementContext<'_, TestHost>,
    open: fret_runtime::Model<bool>,
