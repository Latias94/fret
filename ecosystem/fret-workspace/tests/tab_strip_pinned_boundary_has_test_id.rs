...
fn render_frame(
    ...
    let root = fret_ui::declarative::render_root(
        ...
        move |cx| {
            ...
            // Include a focusable target to ensure tab strip doesn't depend on implicit focus.
...
#[test]
fn tab_strip_pinned_boundary_has_deterministic_test_id() {
    ...
    // Two frames so layout-derived ids/rects settle (tab strip caches tab rects).
    ...
    assert!(
        snap.nodes
            .iter()
            .any(|n| n.test_id.as_deref() == Some("tabstrip.drop_pinned_boundary")),
