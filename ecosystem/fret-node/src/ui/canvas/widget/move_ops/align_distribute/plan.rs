...
pub(super) fn plan_ops(
    ...
    let mut per_group_delta: std::collections::HashMap<crate::core::GroupId, CanvasPoint> =
    ...
    for node_id in nodes_sorted {
        ...
        if let Some(node_size) = node_size {
                ...
                && let Some(extent) = snapshot.interaction.node_extent
            {
                ...
                let mut rect_origin = node_rect_origin_from_anchor(to, node_size, node_origin);
