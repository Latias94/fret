use super::*;

pub(super) fn append_cross_ops(
    spec: &super::paint_grid_tiles::GridTileSpec,
    ops: &mut Vec<SceneOp>,
) {
    let s = spec.cross_size.max(0.0);
    if !(s.is_finite() && s > 0.0) {
        return;
    }

    let half = 0.5 * s;
    for ix in spec.x0..=spec.x1 {
        let x = ix as f32 * spec.spacing;
        let x_local = x - spec.tile_origin.x.0;
        for iy in spec.y0..=spec.y1 {
            let y = iy as f32 * spec.spacing;
            let y_local = y - spec.tile_origin.y.0;

            let is_major =
                ix.rem_euclid(spec.major_every) == 0 || iy.rem_euclid(spec.major_every) == 0;
            let color = if is_major {
                spec.major_color
            } else {
                spec.minor_color
            };

            ops.push(SceneOp::Quad {
                order: DrawOrder(1),
                rect: Rect::new(
                    Point::new(Px(x_local - 0.5 * spec.thickness.0), Px(y_local - half)),
                    Size::new(spec.thickness, Px(s)),
                ),
                background: fret_core::Paint::Solid(color).into(),
                border: Edges::all(Px(0.0)),
                border_paint: fret_core::Paint::TRANSPARENT.into(),
                corner_radii: Corners::all(Px(0.0)),
            });
            ops.push(SceneOp::Quad {
                order: DrawOrder(1),
                rect: Rect::new(
                    Point::new(Px(x_local - half), Px(y_local - 0.5 * spec.thickness.0)),
                    Size::new(Px(s), spec.thickness),
                ),
                background: fret_core::Paint::Solid(color).into(),
                border: Edges::all(Px(0.0)),
                border_paint: fret_core::Paint::TRANSPARENT.into(),
                corner_radii: Corners::all(Px(0.0)),
            });
        }
    }
}
