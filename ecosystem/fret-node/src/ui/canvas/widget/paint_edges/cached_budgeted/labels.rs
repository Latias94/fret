...
pub(super) fn paint_edge_labels_static_budgeted_cached<M: NodeGraphCanvasMiddleware, H: UiHost>(
    ...
    paint_edge_labels_static_budgeted(
        canvas,
        scene,
        services,
        scale_factor,
...
#[allow(clippy::too_many_arguments)]
pub(super) fn paint_edge_labels_static_budgeted<M: NodeGraphCanvasMiddleware>(
    canvas: &mut NodeGraphCanvasWith<M>,
    scene: &mut fret_core::Scene,
    services: &mut dyn fret_core::UiServices,
    scale_factor: f32,
    ...
    let pad_screen = canvas.style.paint.edge_label_padding;
    let corner_screen = canvas.style.paint.edge_label_corner_radius;
    let offset_screen = canvas.style.paint.edge_label_offset;
    let max_width_screen = canvas.style.paint.edge_label_max_width;
    let border_width_screen = canvas.style.paint.edge_label_border_width;

    let mut edge_text_style = canvas.style.paint.edge_label_text_style.clone();
    ...
    for edge in edges.iter().skip(next_edge) {
        ...
        let (prepared, skipped_by_budget) = canvas.paint_cache.text_blob_budgeted(
            services,
            label.clone(),
            &edge_text_style,
            constraints,
        ...
        let border_color = edge
            .hint
            .color
            .unwrap_or(canvas.style.paint.edge_label_border);
        let border_w = border_width_screen.max(0.0) / z;
        scene.push(SceneOp::Quad {
            order: DrawOrder(2),
            rect,
            background: fret_core::Paint::Solid(canvas.style.paint.edge_label_background).into(),
        ...
        scene.push(SceneOp::Text {
            ...
            text: blob,
            paint: (canvas.style.paint.edge_label_text).into(),
            outline: None,
            shadow: None,
        });
    }
